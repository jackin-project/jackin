...
`config.toml`, per-workspace files at `~/.config/jackin/workspaces/<name>.toml`, and `jackin.role.toml` are exceptions: all three are versioned schemas (`CURRENT_CONFIG_VERSION`, `CURRENT_WORKSPACE_VERSION`, `CURRENT_MANIFEST_VERSION` in `src/config/migrations.rs` and `src/manifest/migrations.rs`).

**When a version bump is required:** A PR that makes a **non-additive** change to the serde representation of `AppConfig`, `WorkspaceConfig`, `RoleManifest`, `HooksConfig`, or any other type whose serde representation lives in one of those three files must ship with five artifacts. Non-additive means renamed fields, removed fields, type changes, restructured tables, or anything else that would cause an existing file to fail deserialization against the new struct. The five artifacts are:
...
**When a version bump is NOT required:** Purely **additive** changes — adding a new `Option<T>` field with a `serde(default)` attribute, or any other change where old files deserialize unchanged against the new struct — do not require a version bump, migration step, or fixture. Reshape the change to be additive rather than bumping the version for a purely additive addition. Operator config and per-workspace files migrate automatically during `AppConfig::load_or_init` at startup; role manifests migrate via `jackin-validate --migrate <role-repo-path>`.
