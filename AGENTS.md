...
In rich-text prose, the product and project are always spelled `jackin❯`: lowercase letters followed immediately by the `❯` chevron. Plaintext-only environments may use `jackin>`. Compact logo or prompt surfaces may use `j❯`, with `j>` reserved for proven plaintext fallbacks. Do not write `Jackin`, `Jackin'`, `jackin'`, or bare `jackin` when referring to the brand, the product, or the project in normal text. Use the no-chevron spelling only for literal commands, binaries, crates, packages, environment variables, config keys, file paths, labels, selectors, URLs, and code identifiers, such as `jackin`, `jackin-capsule`, `JACKIN_DEBUG`, `~/.jackin/`, and `jackin.role.toml`. If the chevron makes a possessive or sentence awkward, rewrite the sentence instead of dropping it.
