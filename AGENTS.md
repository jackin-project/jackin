...
Rules that apply equally to humans live in the topic files. Agent-only rules are marked `(agent)` and concentrate in [`.github/AGENTS.md`](.github/AGENTS.md), [`BRANCHING.md`](BRANCHING.md), and [`PULL_REQUESTS.md`](PULL_REQUESTS.md).

> **Tool config files are symlinks.** Every `CLAUDE.md` in this repo is a symbolic link to the `AGENTS.md` beside it — never a copy, never an `@import`. Every directory that has an `AGENTS.md` has a `CLAUDE.md` symlink next to it. Recreate one with `ln -s AGENTS.md CLAUDE.md`. See [`RULES.md`](RULES.md).
...
Rust workspace specifics (module layout, lints, supply-chain hygiene) live in [`crates/AGENTS.md`](crates/AGENTS.md).
...
- **Documentation as the source of truth** — updating both the user-facing and contributor-facing docs surfaces in the same PR. The audience split is permanent; see [`docs/AGENTS.md`](docs/AGENTS.md).

Agent-only PR extras (merge authorization, base branch, force-push, CI-green-before-merge, squash format) live in [`.github/AGENTS.md`](.github/AGENTS.md).
...
Directory-scoped (apply in addition to this file, within their subtree):

- [`.github/AGENTS.md`](.github/AGENTS.md) — agent-only PR extras and GitHub Actions workflow authoring.
- [`docs/AGENTS.md`](docs/AGENTS.md) — docs-site stack, TypeScript rule, three-audience split, roadmap sidebar/overview discipline.
- [`crates/AGENTS.md`](crates/AGENTS.md) — Rust module layout, workspace lints, supply-chain hygiene.
- [`crates/jackin-tui-lookbook/AGENTS.md`](crates/jackin-tui-lookbook/AGENTS.md) — lookbook public-API-only rule.
- [`docker/construct/AGENTS.md`](docker/construct/AGENTS.md) — prefer official package-manager installs in the construct image.
