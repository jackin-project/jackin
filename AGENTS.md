...
Any PR that touches `AppConfig`, `WorkspaceConfig`, `RoleManifest`, `HooksConfig`, or any other type whose serde representation lives in one of those three files must ship with the version bump + migration step + schema-versions doc update before merge. A non-additive change (renamed field, removed field, type change, restructured table) without those three artifacts is incomplete; reviewers block merge until they appear or the change is reshaped to be additive (new optional field with a serde default).
