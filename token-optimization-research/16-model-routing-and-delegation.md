...
Method: deterministic samples piped through the free count_tokens harness (`python3 /tmp/ct.py <model> < sample`), one user message per call. Samples: first 2,000 B of `crates/jackin-core/src/agent.rs`; first 4,096 B and full 6,366 B of this repo's `AGENTS.md`; a fixed 528 B English paragraph; a 558 B JSON tool schema. Counts include a few tokens of message-wrapper overhead (ratios are marginally understated).
...
| AGENTS.md = 2,744 Fable tok (phase-0: 2,738) / 1,919 Sonnet-Haiku tok | local /tmp/ct.py on repo AGENTS.md (6,366 B) |
