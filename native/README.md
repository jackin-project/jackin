...
4. **App** — `JACKIN_APP_VERSION=… JACKIN_APP_BUILD=… ./scripts/build-usage-menu-bar-app.sh` produces a **arm64 (Apple Silicon)** `JackinUsageMenuBar.app` with no embedded dylib/framework/XCFramework, then ad-hoc signs.
...
| Artifact | `jackin-usage-menu-bar-<VERSION>-aarch64-apple-darwin.zip` + `.sha256` + `.bundle` + `.sbom.json` + GitHub attestation |
...
# final ZIP: native/dist/jackin-usage-menu-bar-0.6.0-aarch64-apple-darwin.zip
```
