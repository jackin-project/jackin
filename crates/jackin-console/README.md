...
| [`tui.rs`](src/tui.rs) · [`tui/`](src/tui) | view composition, product-owned chrome and input over canonical TermRock widgets and the shared jackin❯ operator-info facade; generic list state and neutral status/Debug-info rendering are not local components | — |
