...
fn resolve_instance_gate_rejects_unknown_and_ambiguous_targets() {
    ...
    assert!(config.resolve_instance("claude").is_err());
    // Unknown targets error at the gate.
    assert!(config.resolve_instance("codex-work").is_err());
    assert!(config.resolve_instance("codex").is_err());
    ...
    assert!(unmapped.resolve_instance("ghost").is_err());
}
