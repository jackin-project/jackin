...
fn render(
    ...
    let mut buf = Vec::new();
    bar.render(&mut buf, cols, tabs, active, states, &std::collections::HashSet::new(), None, false, None);
...
fn render_with_hover(
    ...
    let mut buf = Vec::new();
    bar.render(&mut buf, cols, tabs, active, states, &std::collections::HashSet::new(), hovered_tab, false, None);
    String::from_utf8_lossy(&buf).to_string()
}
