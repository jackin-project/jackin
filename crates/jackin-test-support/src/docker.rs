...
impl DockerApi for FakeDockerClient {
    ...
    async fn resolve_container_by_name(&self, name: &str) -> anyhow::Result<ContainerHandle> {
        ...
        if let Some(handle) = inspection.handle {
            return Ok(handle);
        }
        // CLI-driven role launches use `docker run` rather than
        // `DockerApi::create_container`; the injected FakeRunner cannot
        // mutate this fake daemon when that command succeeds. Treat a
        // missing default-queue result as that just-created role for tests,
        // while explicit inspect failures still fail closed.
        if matches!(inspection.state, ContainerState::NotFound) {
            return Ok(self.handle_for(name));
        }
        anyhow::bail!(
