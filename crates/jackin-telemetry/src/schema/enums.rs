...
bounded_values!(UiActionName { WorkspaceOpen => "workspace.open", WorkspaceSave => "workspace.save", WorkspaceLaunch => "workspace.launch", SettingsOpen => "settings.open", SettingsSave => "settings.save", DialogConfirm => "dialog.confirm", DialogCancel => "dialog.cancel", AgentSelect => "agent.select", AgentSpawn => "agent.spawn", TabSwitch => "tab.switch", TabRename => "tab.rename", TabClose => "tab.close", PaneSplit => "pane.split", PaneFocus => "pane.focus", PaneResize => "pane.resize", PaneZoom => "pane.zoom", PaneClear => "pane.clear", PaneClose => "pane.close", UsageRefresh => "usage.refresh", SessionDetach => "session.detach", FileExport => "file.export", ImageStage => "image.stage", LinkOpen => "link.open", AppExitRequest => "app.exit_request", ScreenBack => "screen.back", WorkspaceCreate => "workspace.create", WorkspaceDelete => "workspace.delete", InstancePurge => "instance.purge" });
