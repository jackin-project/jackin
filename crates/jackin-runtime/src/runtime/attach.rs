...
async fn start_or_reconnect_capsule_client_with_handle_with_lease(
    ...
    match inspect {
        ...
        ContainerState::Stopped { .. } | ContainerState::Created => {
            ...
            restart_stopped_dind_if_needed(
                paths,
                container_name,
                admission_lease,
                docker,
...
async fn restart_stopped_dind_if_needed(
    ...
    let Some(dind) = crate::runtime::cleanup::resolve_dind_handle_for_state(
        paths,
        container_name,
        docker,
    )
    ...
    match crate::runtime::backend::backend_for_state(paths, container_name) {
        crate::runtime::backend::InstanceBackend::Docker => {
            if start_first {
                ...
                let container = match known_container {
                    Some(container) => container.clone(),
                    None => docker.resolve_container_by_name(container_name).await?,
                };
