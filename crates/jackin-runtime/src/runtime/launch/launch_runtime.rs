...
pub(crate) async fn launch_role_runtime(
    ...
    #[cfg(test)]
    let run_role_result = {
        // Unit tests retain the command-runner observation seam so their
        // security-flag assertions remain deterministic. Production never
        // invokes `docker run`; it uses the typed create/start path below.
        let run_role = runner.run("docker", &run_args, None, &docker_run_opts);
        let audit = if let Some(progress) = steps.progress_mut() {
        ...
        if let Err(error) = audit {
            ...
            match created {
                Ok(container) => {
                    container_handle.borrow_mut().replace(container.clone());
                    docker
    ...
    if let Err(error) = run_role_result {
        let created_container = container_handle.borrow().clone();
        if let Some(container) = created_container.as_ref() {
            drop(docker.remove_container_by_id(container).await);
        }
        return Err(error);
    }
