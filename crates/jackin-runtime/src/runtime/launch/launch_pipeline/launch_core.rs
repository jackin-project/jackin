...
//!
//! This module holds `run_launch_core` + the `LaunchCore` context struct.
//! All items the core reaches from the launch parent are via `super::super::`
    ...
    R: CommandRunner,
{
    ...
    // captured lets that cross the seam (by value or ref)
    ...
    R: CommandRunner,
{
    ...
    if let Err(error) = super::super::write_instance_status(
        paths,
        &container_state,
        ...
        if let Some(run) = jackin_diagnostics::active_run() {
            if matches!(github_mode, jackin_config::GithubAuthMode::Ignore) {
                ...
                let token_breadcrumb = github_env_decls.get(gh_token_key).map_or_else(
                    ...
                    |value| {
                        super::super::auth_token_source_reference(
                            gh_token_key,
    ...
    let isolated_worktrees = materialized
        ...
        .collect();
    let mut launch_config = super::super::capsule_config(
        selector,
        &workspace.workdir,
    ...
    match backend {
        super::super::Backend::Docker => {}
        super::super::Backend::AppleContainer => {
            cleanup.run(docker).await;
            let mount_pairs = super::super::build_workspace_mount_pairs(&materialized);
    ...
    let ctx = super::super::LaunchContext {
        ...
        selected_image_refresh: selected_refresh_reason.map(|reason| {
            super::super::SelectedImageRefresh {
                role_git: &source.git,
                branch_override: opts.role_branch.as_deref(),
                reason,
            }
        ...
        sibling_auth_prewarm: super::super::SiblingAuthPrewarm {
