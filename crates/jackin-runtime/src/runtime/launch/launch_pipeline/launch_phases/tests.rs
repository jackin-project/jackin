...
#[tokio::test]
async fn grant_failure_cleanup_removes_adopted_sidecar_but_preserves_unbound_resources() {
    ...
    assert!(
        ...
        "without a role handle, cleanup must not remove a replacement's network; recorded: {recorded:?}"
    );
    assert!(
        !recorded
            .iter()
            .any(|c| c == "docker volume rm jk-role-certs"),
        "without a role handle, cleanup must not remove a replacement's volume; recorded: {recorded:?}"
    );
}
...
async fn post_start_failure_preserves_terminal_role_evidence_but_cleans_sidecars() {
    ...
    assert!(
        ...
        "unbound role identity must preserve the shared volume"
    );
    assert!(
        !recorded
            .iter()
            .any(|call| call == "docker network rm jk-failed-start-net"),
        "unbound role identity must preserve the shared network"
    );
}
...
async fn post_start_failure_cleans_live_role_and_private_socket() {
    ...
    cleanup
        .run_preserving_evidence_with_role_handle(
            &docker,
            &ContainerHandle::new("jk-live-start", "live-start-role-id").unwrap(),
        )
        .await;
