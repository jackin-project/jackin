    ...
    if result.is_err() || !keep {
        ...
        let remove_container = match dind_handle.as_ref() {
            ...
            None => Err(anyhow::anyhow!(
                "DinD identity unavailable; refusing name-based destructive cleanup for {dind}"
            )),
        };
...
async fn ensure_prewarm_state_identity(
    ...
    let Some(state) = read_prewarmed_dind_state(paths)
        ...
        return match inspection.handle {
            ...
            Some(actual) => anyhow::bail!(
                "prewarm container {} exists without a verified retained identity {}; refusing name-based prewarm cleanup",
                prewarm_dind,
                actual.id()
            ),
            None => anyhow::bail!(
    ...
    anyhow::ensure!(
        ...
        "retained DinD state is not a current kept identity; refusing name-based prewarm cleanup"
    );
    ...
    match inspection.handle {
        ...
        Some(actual) => anyhow::bail!(
            "retained DinD identity changed for {} (expected {}, found {}); refusing name-based prewarm cleanup",
            state.dind,
            expected.id(),
            actual.id()
        ),
...
mod tests {
    ...
    async fn retained_sidecar_rejects_same_name_replacement() {
        ...
        assert!(
            error
                .to_string()
                .contains("refusing name-based prewarm cleanup"),
