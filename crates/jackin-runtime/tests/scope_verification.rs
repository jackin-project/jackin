...
) -> (UsageCoordinator, Arc<GateExecutor>) {
...
fn s2_relay_allowlist_admits_exactly_abc() {
    ...
    let allowlist =
        UsageCapabilitySet::new([capability("acc-a"), capability("acc-b"), capability("acc-c")]);
...
fn s2_empty_scope_denies_everything() {
    ...
    for id in ["acc-a", "acc-b", "acc-c", "acc-d"] {
        ...
        assert_eq!(err.kind, UsageCoordinationErrorKind::Unauthorized, "{id} leaked");
    }
    let err = allowlist.resolve_surface("claude").unwrap_err();
    assert_eq!(err.kind, UsageCoordinationErrorKind::Unauthorized);
}
...
fn s2_stale_catalog_capability_is_revoked_for_direct_requests() {
    ...
    let sibling = coordinator.request_refresh(&capability("acc-a"), 0, true, NOW).unwrap();
...
fn s6_stale_cas_adopts_the_winner() {
    ...
    gate.wait_started(1);
    gate.release(1);
    let terminal = coordinator.join_generation(&cap, winner.generation, WAIT, NOW).unwrap();
    assert_eq!(terminal.phase, UsageRefreshPhase::Completed);
    ...
    let next = coordinator.request_refresh(&cap, winner.generation, true, NOW).unwrap();
    assert_eq!(next.generation, winner.generation + 1);
    gate.wait_started(2);
    gate.release(1);
    let terminal_next = coordinator.join_generation(&cap, next.generation, WAIT, NOW).unwrap();
    assert_eq!(terminal_next.phase, UsageRefreshPhase::Completed);
    let replay = coordinator.join_generation(&cap, winner.generation, WAIT, NOW).unwrap();
...
fn s6_rotation_fences_in_flight_and_discards_late_results() {
    ...
    gate.wait_started(1);
    coordinator.reconcile_catalog([entry("acc-a", "rev-2")], NOW).unwrap();
    ...
    let fresh = coordinator.request_refresh(&cap, cursor.generation, true, NOW).unwrap();
    assert_eq!(fresh.generation, stale.generation + 2);
    assert_eq!(fresh.phase, UsageRefreshPhase::Queued);
    gate.wait_started(2);
    gate.release(1);
    let terminal = coordinator.join_generation(&cap, fresh.generation, WAIT, NOW).unwrap();
    assert_eq!(terminal.phase, UsageRefreshPhase::Completed);
    assert!(terminal.snapshot.is_some(), "fresh generation lost its data");
...
fn s6_distinct_accounts_refresh_independently() {
    ...
    gate.wait_started(2);
    gate.release(2);
    let done_a = coordinator.join_generation(&a, gen_a.generation, WAIT, NOW).unwrap();
    let done_b = coordinator.join_generation(&b, gen_b.generation, WAIT, NOW).unwrap();
