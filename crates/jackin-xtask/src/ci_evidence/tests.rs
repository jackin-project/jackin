...
fn forged_success_classification_is_rejected() {
    ...
    assert!(error.to_string().contains("classification"), "error: {error:#}");
}
...
fn push_head_chain_gap_is_rejected_without_observed_head_fallback() {
    ...
    let error = validate_denominator("example/repo", &proof, &history, &push_heads, &proof.window)
        .unwrap_err();
    assert!(error.to_string().contains("coverage gap"), "error: {error:#}");
}
...
fn generated_evidence_workflows_are_complete_and_tamper_bound() {
    let root = docs::repo_root().unwrap();
    validate_workflow_contract(&root).unwrap();

    let directory = tempfile::tempdir().unwrap();
    let copy = directory.path();
    for relative in [WORKFLOW_CONTRACT_PATH, MISE_PATH, WORKFLOW_STATE_PATH] {
        let destination = copy.join(relative);
        fs::create_dir_all(destination.parent().unwrap()).unwrap();
        fs::copy(root.join(relative), destination).unwrap();
    }
    let state = fs::read_to_string(root.join(WORKFLOW_STATE_PATH)).unwrap();
    let outputs = state
        .split_once("[outputs]\n")
        .unwrap()
        .1
    ...
    for relative in outputs {
        ...
        let destination = copy.join(&relative);
        fs::create_dir_all(destination.parent().unwrap()).unwrap();
        if fs::symlink_metadata(&source).unwrap().file_type().is_symlink() {
    ...
    assert!(error.to_string().contains("generator identity"), "error: {error:#}");

    fs::copy(root.join(WORKFLOW_STATE_PATH), &state_path).unwrap();
    fs::OpenOptions::new()
        .append(true)
        ...
        .unwrap();
    let error = validate_workflow_contract(copy).unwrap_err();
    assert!(error.to_string().contains("output hash mismatch"), "error: {error:#}");
}
