...
pub(crate) fn run(args: TelemetryRegistryArgs) -> Result<()> {
    ...
    validate_error_ownership_census(&root, args.generate)?;
    validate_source_policy(&root)
}

const ERROR_OWNERSHIP_CENSUS: &str =
    "plans/unified-otel-observability/worksheets/011-error-ownership-census.md";

fn validate_error_ownership_census(root: &Path, generate: bool) -> Result<()> {
    let expected = generate_error_ownership_census(root)?;
    let path = root.join(ERROR_OWNERSHIP_CENSUS);
    if generate {
...
fn generate_error_ownership_census(root: &Path) -> Result<String> {
    ...
    let mut output = String::from(
        "# Generated error-ownership call-site census\n\nThis file is generated by `cargo xtask telemetry-registry --generate`. It inventories every production use of the generic propagated carrier, immediate consumed-error recorder, and typed configuration marker. The registry gate rejects line or ownership drift. Boundary-specific operation guards are tracked by the acceptance map; completing Plan 011 still requires reconciling every semantic `Err` boundary against these two inventories.\n\n| Site | Decision | Bounded owner |\n|---|---|---|\n",
    );
...
fn ownership_type(lines: &[&str]) -> String {
    for line in lines {
