...
fn print_budget(counts: &BTreeMap<PathBuf, usize>, budget: &Budget) {
    ...
    println!("# Regenerated by `cargo xtask lint files --print-budget`.");
    println!("# Numbers may only ever decrease; delete entries when a file drops under the cap.");
    println!("production_cap = {}", budget.production_cap);
    println!("test_cap = {}", budget.test_cap);
    println!();
    for (path, lines) in prod {
        println!("[[production]]");
        println!("path = \"{}\"", relative_for_print(path));
        println!("lines = {lines}");
        println!();
    }
...
fn relative_for_print(path: &Path) -> String {
