...
fn launch_configurations() -> std::collections::BTreeMap<String, crate::AgentConfiguration> {
...
fn validate_default_launch_list_reports_each_invalid_entry() {
    ...
    assert!(
        errors.iter().any(|error| error.contains("unknown agent configuration")),
        "{errors:?}"
    );
    assert!(
        errors.iter().any(|error| error.contains("duplicate launch configuration")),
        "{errors:?}"
    );
    assert!(
