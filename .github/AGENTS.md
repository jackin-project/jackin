...
- **Cargo-registry tools used across all jobs** (e.g. `cargo-nextest`): declare the workflow-facing short name in `mise.toml` with a pinned version and map it through `[tool_alias]` to the Cargo backend (`cargo-nextest = "0.9.136"` plus `cargo-nextest = "cargo:cargo-nextest"`). Keep `cargo-binstall` pinned and `settings.cargo.binstall = true` so mise uses compatible prebuilt Cargo binaries when available and source-build fallback when an upstream release has no runner-architecture asset. Tools needed by only one job (e.g. `cargo-zigbuild`, `cross`) can use `install_args: "cargo:<crate>"` instead.
