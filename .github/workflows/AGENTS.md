...
- Release-archive jobs use the shared `configure-sccache` action. Scope compiler caches by job family, runner OS/architecture, and target ABI. PRs are read-only; main/release runs may write. Bump its `sccache-vN` prefix to invalidate all compiler entries.
