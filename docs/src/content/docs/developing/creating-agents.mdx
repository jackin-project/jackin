...
An agent class can support more than one harness while keeping one shared Dockerfile:
...
The workspace chooses the default harness:
...
```bash
jackin load my-agent my-app --harness claude
```
...
The `--rebuild` flag forces a fresh image build, picking up Dockerfile changes and newer harness CLI downloads.
