...
With `JACKIN_CONSTRUCT_IMAGE` set, jackin validates role Dockerfiles against the canonical image name as usual (role repos always reference `projectjackin/construct:trixie`), but substitutes your local image at derived-build time so `docker build` uses `jackin-local/construct:trixie` as the actual base.
