...
3. **Runtime entrypoint** — the script that configures git, wires up GitHub if `gh` is already authenticated (it does not perform `gh auth login` — an unauthenticated `gh` is left alone with a notice), dispatches agent setup, and launches the selected agent
4. **Plugin installation** — any Claude plugins declared in the role manifest when launching the Claude agent
