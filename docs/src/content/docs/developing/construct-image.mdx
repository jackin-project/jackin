...
3. **Runtime entrypoint** — the script that configures git, wires up GitHub if `gh` is already authenticated (it does not perform `gh auth login` — an unauthenticated `gh` is left alone with a notice), installs plugins, and launches Claude
4. **Plugin installation** — any Claude plugins declared in the agent manifest

<Aside type="note">
Every load re-runs `docker build` on the derived Dockerfile. Docker's layer cache makes most subsequent loads fast, but there is no separate "skip build if image exists" shortcut. Passing `--rebuild` additionally invalidates the Claude installation layer so the build picks up a newer Claude version.
</Aside>
