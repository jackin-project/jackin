...
The construct source starts at [`docker/construct/Dockerfile`](https://github.com/jackin-project/jackin/blob/main/docker/construct/Dockerfile) in the jackin' repository. The declarative build definition lives in the repo-root `docker-bake.hcl`, and the supported command wrapper is the repo-root `Justfile`.
