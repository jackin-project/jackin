...
- `setup_once` is executed as a child process and gated by a marker file inside the container's writable layer. It runs again on container restart only if the previous run failed, but a freshly-created container (the typical case when you re-launch a role) starts with an empty writable layer and re-runs the hook. Treat it as "run once per container instance, not once per host." Use it for installs, downloads, and one-time config writes that are still cheap-enough to repeat for a brand-new container. Environment exports from this script do not reach the agent.
- `source` is sourced into the entrypoint shell on every container start. Use it for `export` statements, `PATH` changes, and other shell state that must be inherited by the launched agent. Keep it small; because it mutates the entrypoint shell, avoid installs, downloads, broad traps, and permanent directory changes. Use `return`, not `exit` — a sourced `exit` kills the entrypoint before the agent launches. Avoid embedding plaintext secrets: under `JACKIN_DEBUG=1` the entrypoint suspends `xtrace` around the dot-source so expanded values do not leak to the operator's terminal, but anything the script itself prints (or exports under a future trace path) is still visible. Mount secret files via the role repo and read them into env vars instead of inlining them.
