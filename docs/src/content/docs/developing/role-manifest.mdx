...
The optional `[harness]` table declares which AI harnesses this role can run. When omitted, jackin treats the manifest as Claude-only.

| Field | Required | Description |
|---|---|---|
| `supported` | Yes | Non-empty list of harness slugs: `"claude"` or `"codex"` |

Every harness listed in `supported` must have a matching table, even when that table is empty. A manifest with `supported = ["claude", "codex"]` needs both `[claude]` and `[codex]`.
...
The `[codex]` table is required when `"codex"` appears in `[harness].supported`.
