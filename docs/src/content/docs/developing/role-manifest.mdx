...
- `setup_once` is executed as a child process only until it succeeds once for that container. Use it for installs, downloads, and one-time config writes. Environment exports from this script do not reach the agent.
- `source` is sourced into the entrypoint shell on every container start. Use it for `export` statements, `PATH` changes, and other shell state that must be inherited by the launched agent. Keep it small; because it mutates the entrypoint shell, avoid installs, downloads, broad traps, and permanent directory changes.
