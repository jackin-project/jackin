...
jackin' enforces **strict parsing** — unknown fields are rejected with an error. This catches typos and prevents silent misconfiguration. Top-level `version` records the minimum manifest schema the file requires; older stamps remain valid until the manifest uses a newer field or enum value. Use `jackin role migrate <role-repo-path>` to update a local manifest before adopting newer schema features. See [Schema Versions](/reference/schema-versions/) for the full version history and migration matrix.
