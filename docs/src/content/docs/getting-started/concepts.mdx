...
An **agent** is an AI coding runtime (`Claude Code`, `Codex`, `Amp`, or `OpenCode` today) running inside an isolated `Docker` container. Each agent:
...
5. **Drop you inside** — `Claude Code` (or another supported runtime — `Codex`, `Amp`, or `OpenCode`) starts with full permissions inside that boundary.
