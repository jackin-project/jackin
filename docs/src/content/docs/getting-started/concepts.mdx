...
The visible result is the two-line jackin' chrome at the top of an attached terminal. The first line identifies the session and tabs; the second line shows the running instance identity and opens **Container info** when clicked. When the workspace's git checkout is on a non-default branch, the second line also surfaces the branch — first as `branch ⟳` while a background pull-request lookup runs, then as the branch name when no open PR is found, or as `PR #N: title` when one is. The branch surface stays hidden on the repo's default branch (`origin/HEAD`) and updates automatically when you `git checkout` from any pane. Clicking the `PR #N: title` form copies the pull-request URL to the outer terminal's clipboard. The rest of the terminal belongs to the focused agent or shell pane. Agent shortcuts, editors, full-screen TUIs, bracketed paste, mouse reporting, and normal terminal output still go to the agent unless the Capsule UI is explicitly active.
