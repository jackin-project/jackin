...
- Persists its state between sessions (Claude history, settings, plugins)

### Agent classes

An agent **class** is a template for creating agent containers. It's defined by a GitHub repository that contains a Dockerfile and a manifest file (`jackin.agent.toml`). For example:
