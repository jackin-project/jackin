...
Subsequent loads re-run `docker build` but reuse Docker's layer cache where nothing upstream changed, so repeated loads of the same agent are typically fast. The agent repo cache at `~/.jackin/agents/` is also reused. Passing `--rebuild` additionally invalidates the Claude installation layer so a newer Claude version is picked up.
