...
Those are different concerns, and combining them into one giant image brings back the same problem as a cluttered host machine: the agent can inspect too many tools, too many conventions, and too much implicit context.

Imagine one company with a shared monorepo:

- the frontend team uses a `frontend` agent class with Node, Playwright, design-system utilities, and UI-oriented Claude plugins
- the backend team uses a `backend` agent class with Rust or Go toolchains, database clients, and API-oriented plugins
