...
- `"chainargos/the-architect"` — matches only a specific agent
...
When multiple mount sources define mounts, they're combined in this order:
...
<Aside type="caution">
If two mounts target the same container destination, the later one wins. Be careful when combining workspace and load-time mounts that might overlap.
</Aside>
