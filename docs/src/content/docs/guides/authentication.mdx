...
description: Forward or provide harness credentials for agent containers
---
...
When a workspace or launch selects the `codex` harness, jackin requires `OPENAI_API_KEY` to be present in the resolved launch env before it builds or starts the container. There is no Codex equivalent of Claude's `sync` or `token` auth-forward modes in this slice.
