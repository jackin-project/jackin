...
Codex uses environment-only authentication in jackin V1. Set `OPENAI_API_KEY` through the same operator env system used for other secrets:
...
```toml
[env]
OPENAI_API_KEY = "op://Work/OpenAI/key"
```

When a workspace or launch selects the `codex` agent, jackin requires `OPENAI_API_KEY` to be present in the resolved launch env before it builds or starts the container. There is no Codex equivalent of Claude's `sync` or `token` auth-forward modes in this slice.
