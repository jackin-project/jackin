...
`JACKIN_AGENT` is scoped to an agent session, not the whole container. jackin'
passes the initial agent to the PID 1 multiplexer as an argv value, then sets
`JACKIN_AGENT` only when spawning `/jackin/runtime/entrypoint.sh` for a real
agent pane. Shell panes do not receive it.

Role identity, workspace workdir, supported agent slugs, and manifest model
overrides are written to `/jackin/run/agent.toml` for Capsule instead of being
duplicated as container-wide environment variables.
