...
- set a default agent such as `claude`, `codex`, `amp`, or `opencode`
...
For the exact safety policy (which branches count as safe, how the squash-merged-and-pruned case is detected, what the per-mount state file records), see [Foreground session finalization](/reference/architecture/#foreground-session-finalization) in the internals section.
...
# Clear default agent back to Claude fallback
