...
- set a default agent such as `claude` or `codex`
...
- `--agent` — default agent used by `jackin load` unless `--agent` is passed for that launch
...
# Change default agent
jackin workspace edit secure-api --agent codex

# Clear default agent back to Claude fallback
jackin workspace edit secure-api --clear-agent
```
