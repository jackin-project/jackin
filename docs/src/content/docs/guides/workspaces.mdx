...
- set a default harness such as `claude` or `codex`
...
- `--harness` — default harness used by `jackin load` unless `--harness` is passed for that launch
...
# Change default harness
jackin workspace edit secure-api --harness codex

# Clear default harness back to Claude fallback
jackin workspace edit secure-api --clear-harness
```
