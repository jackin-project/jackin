...
## What is an role repo?

An role repo is a GitHub repository that defines a specific role. It contains:

1. A **Dockerfile** — the agent's environment definition (tools, languages, SDKs)
2. A **manifest file** (`jackin.role.toml`) — metadata about the agent
...
Role repos follow the `jackin-{class-name}` naming pattern:
...
Role repos are cloned to `~/.jackin/agents/` on first load. Subsequent loads use the cached checkout. jackin' updates the repo on each load to keep the cache current.
...
<Aside type="caution">
Role repos only own their agent-specific environment layer. jackin' owns everything else: validating the contract, generating the derived Dockerfile, installing Claude Code, injecting the entrypoint, mounting workspaces, and wiring Docker-in-Docker.
</Aside>
## Example role repo
