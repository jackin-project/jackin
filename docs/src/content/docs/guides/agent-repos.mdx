...
Agent repos are cloned to `~/.jackin/agents/` on first load. Subsequent loads use the cached checkout. jackin updates the repo (pulls latest) each time you load an agent to ensure you're running the latest version.
