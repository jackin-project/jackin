...
The current derived image installs the agent runtimes (Claude Code, Codex, Amp, Kimi, OpenCode) from their upstream installers. That keeps the runtimes current, but it is less reproducible than pinning exact versions and checksums.
