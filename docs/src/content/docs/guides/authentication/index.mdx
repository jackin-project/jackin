...
`Claude Code` and `Codex` credentials forwarded into the agent container, with optional API-key or OAuth-token alternatives.
...
- **Per-agent dimension where it makes sense.** Agent forwarding has an agent dimension (`Claude` and `Codex` are configured separately because their credentials and runtime expectations differ). `GitHub CLI` forwarding doesn't — `gh` doesn't care which agent is invoking it, so the same `gh` login is shared by every agent in the container.
