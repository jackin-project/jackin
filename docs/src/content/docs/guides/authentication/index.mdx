...
`Claude Code`, `Codex`, and `Amp` credentials forwarded into the agent container, with optional API-key or OAuth-token alternatives.
...
- **Per-agent dimension where it makes sense.** Agent forwarding has an agent dimension (`Claude`, `Codex`, and `Amp` are configured separately because their credentials and runtime expectations differ). `GitHub CLI` forwarding doesn't — `gh` doesn't care which agent is invoking it, so the same `gh` login is shared by every agent in the container.
...
You should not need to hand-edit any configuration file. Operators who prefer the CLI for the global default can use `jackin config auth …`; everything more specific (per-workspace, per-(workspace × role × agent)) lives in the console's **Auth** tab and stays there.
...
Authentication forwarding is one of the places where jackin's repo-wide *[never mutate the host](/getting-started/design-principles/#never-mutate-the-host-machine-silently)* rule matters most. Forwarded credentials are read from your host but never written back; in-container token rotations stay inside the container; HTTPS rewrites and credential-helper wiring happen inside the container's own git config, never in your `~/.gitconfig`. See the design-principles page for the full rule and audit; this group's per-axis pages call out the rule again only where it shapes a specific feature.
