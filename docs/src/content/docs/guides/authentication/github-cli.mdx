...
<Aside type="note">
The dedicated `GitHub CLI` row in the console's **Auth** tab (and a `jackin config auth set --target github` CLI subcommand) are not yet wired — both are planned. Until they land, leave the global default at `sync` (the out-of-the-box behaviour) and rely on the launch-summary notice (below) to confirm the mode on every container start.
</Aside>
...
`gh` auth forwarding follows jackin's repo-wide
...
- [Agent Authentication](/guides/authentication/agents/) — parallel flow for `Claude` / `Codex` agent auth.
