...
description: Forward Claude Code and Codex credentials into agent containers
---
import { Aside } from '@astrojs/starlight/components'


This page covers the operator surface for forwarding `Claude Code` and `Codex` credentials into agent containers. For the broader pattern jackin' applies across every authentication axis (modes, scopes, defaults), see the [Authentication overview](/guides/authentication/).

`Claude Code` and `Codex` credentials are configured per agent at three scopes: global default, per workspace, per (workspace × role × agent). `sync` is the default at every scope — your host login is forwarded into the container on each launch.
...
<Aside type="caution">
`Codex` has no OAuth-token analog of `Claude`'s `CLAUDE_CODE_OAUTH_TOKEN`. Setting `auth_forward = "oauth_token"` for `Codex` is rejected. Use `api_key` (with `OPENAI_API_KEY`) or `sync` for `Codex`.
</Aside>
...
- A **mode picker** — `sync`, `api_key`, `oauth_token` (`Claude` only), `ignore`. `Codex` cells omit `oauth_token`.
...
Per-role overrides for `Claude` and `Codex` are added through the same tab. The footer of the editor surfaces the resolved mode and its provenance (global / workspace / role override) for whatever you have focused, so you can see what's effective without leaving the screen.
