...
| `~/.kimi-code/mcp.json` | Copied into the container's `/home/agent/.kimi-code/mcp.json` before Kimi starts. |
...
| Mode | Supported |
|---|---|
| `sync` (default) | Yes — forwards the Kimi config, OAuth credentials directory, device id, and MCP config |
