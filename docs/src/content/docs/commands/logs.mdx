---
title: jackin logs
description: "Show or follow the in-container multiplexer log"
---
...
Show, follow, or bundle the in-container multiplexer log for a running or recently-run container. The multiplexer writes one timestamped line per operator action (prefix key, pane split, tab close, agent spawn), PTY event (read EOF, child reaped), and PID-1 reap to a host-readable file under each container's state directory. The log is the primary input for bug reports about pane lifecycle, focus, or input routing.
...
Each line is timestamped in ISO-8601 UTC and prefixed with `[jackin-container]`. The multiplexer writes:

- **Action breadcrumbs** — every prefix-key chord, dialog action, pane spawn, split, zoom, close, tab close, and exited-session removal. These reconstruct what you pressed in what order.
...
Reader-task EOF and child-reaper events are both logged; the difference between them is load-bearing when triaging "agent exited but pane stayed open" reports — see [`reference/jackin-container`](/reference/jackin-container/) for the lifecycle model.
