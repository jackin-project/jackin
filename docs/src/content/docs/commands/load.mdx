...
8. Checks the local instance index for unfinished work in the same workspace or directory before creating a fresh runtime; matching-role restore candidates can be rebuilt from the prompt, and cross-role candidates are surfaced with `jackin hardline <instance>` guidance.
