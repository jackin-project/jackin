...
Every invocation gets a run diagnostics handle such as `jk-run-42f9aa`. Rich terminals show a full-screen launch progress surface with a sliding stage rail for jackin' stages (`identity`, `role`, `credentials`, `construct`, `derived image`, `workspace`, `network`, `sidecar`, `capsule`, `hardline`). Workspace repository polling from `git_pull_on_entry` is part of that surface; per-repo pull details go to diagnostics/debug evidence instead of printing over the launch cockpit. Non-rich terminals, `--no-tui`, and non-interactive runs use compact deterministic progress lines with the same stage vocabulary.

The full digital rain animation is a boundary marker: it appears when this launch enters jackin' work with no other jackin-managed role containers running, and the outro appears only when the foreground session exits and no role containers remain. Use `--no-rain` to disable that boundary animation without disabling launch progress.
...
| `--no-rain` | Disable the full boundary rain animation. Launch progress still renders normally. |
| `--no-tui` | Disable the rich launch progress TUI and use compact launch progress output. |
| `--debug` | Write detailed command, Docker, resolver, auth, and build traces to the run diagnostics artifact. The launch surface stays compact and does not stream raw logs. |
...
Failures in the launch surface show a short diagnosis, the failed stage, and the diagnostics run handle. Raw Docker output, build logs, stack traces, and debug traces are written only to diagnostics artifacts under jackin-owned state; they are not rendered in the operator-facing launch progress UI.
