...
5. **If the role supports multiple agents** and neither `--agent` nor a workspace `default_agent` is set, prompts to choose the agent runtime
...
<Aside type="note">
Host-side Docker work follows the operator's selected Docker daemon. If
`DOCKER_HOST` is set, jackin' uses it. Otherwise jackin' follows the
active Docker CLI context, so the image build, per-agent network, DinD
sidecar, and role container are created on the same Docker backend.
</Aside>
<Aside type="note">
