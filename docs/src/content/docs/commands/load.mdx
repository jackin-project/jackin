...
8. Checks the local instance index for unfinished work in the same workspace or directory before creating a fresh runtime; matching-role restore candidates can be rebuilt from the prompt, and related role instances can be recovered or rebuilt in place from the saved manifest source/ref.
