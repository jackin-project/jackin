...
| `--agent <NAME>` | Override the launch agent (`claude`, `codex`, or `amp`). When omitted on a multi-agent role with no `default_agent` on the workspace, an interactive prompt asks. |
...
8. Checks the local instance index for unfinished work in the same workspace or directory before creating a fresh runtime; matching-role restore candidates can be rebuilt from the prompt, related running/stopped role containers can be recovered in place, and missing related instances are surfaced with `jackin hardline <instance>` rebuild guidance.
