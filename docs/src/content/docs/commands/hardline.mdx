...
Click the container name in the identity strip to open **Container info**. The modal shows the container ID, role, active pane's agent, current workdir, git branch when the workdir is a repository, and the GitHub pull request URL when `gh pr view` can resolve one for the current branch. Press `Enter` on the modal to copy the container ID.
