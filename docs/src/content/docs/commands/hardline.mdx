...
When the workspace workdir is on a branch with an open GitHub pull request, the multiplexer shows a bottom pull-request bar. The left side shows the PR number and title; the right side shows the container ID. While `gh` is still resolving the branch's pull request, the left side says `Resolving PR · <branch>` so the pending lookup is visible. On a non-`main` / non-`master` branch without an open PR, the same line shows the branch name instead. Hovering either clickable side changes the pointer and slightly lifts the color, matching the top tab strip. Capsule polls the local Git branch several times per second so branch switches surface immediately, while GitHub PR and CI metadata stays cached per branch and refreshes on a conservative cadence so `gh` is not polled aggressively.
...
Click the PR or branch text on the left side of the bar to open **GitHub context**. The modal shows the branch, GitHub pull request URL, pull request title, and best-effort CI status from `gh pr checks` when a pull request exists. The bottom footer hints that `Enter` copies the GitHub URL when a pull request is available, the GitHub URL value is emphasized as the copy target, and clicking that value performs the same copy action.
