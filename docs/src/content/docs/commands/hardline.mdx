...
The multiplexer always reserves the bottom row for the workspace and container status bar. The left side starts with the current workspace name and the right side shows the container ID. When the workspace workdir is on a branch with an open GitHub pull request, the left side adds the PR number and title. While `gh` is still resolving the branch's pull request, the left side says `Resolving PR · <branch>` so the pending lookup is visible. On a non-`main` / non-`master` branch without an open PR, the same line shows the branch name instead. On `main`, `master`, or a directory without a recognizable branch, the row remains visible but only shows the workspace and container. Hovering either clickable side changes the pointer and slightly lifts the color, matching the top tab strip. Capsule polls the local Git branch several times per second so branch switches surface immediately, while GitHub PR and CI metadata stays cached per branch and refreshes on a conservative cadence so `gh` is not polled aggressively.

Click the container ID on the right side of that bar to open **Container info**. The modal shows the full container ID, role, active pane's agent, and current workdir. The bottom footer hints that `Enter` copies the container ID, the Container ID value is emphasized as the copy target, and clicking that value performs the same copy action.

Click the PR or branch text on the left side of the bar to open **GitHub context**. The modal shows the branch, GitHub pull request URL, pull request title, and best-effort CI status from `gh pr checks` when a pull request exists. The workspace-only text on the default branch is not a GitHub-context target. The bottom footer hints that `Enter` copies the GitHub URL when a pull request is available, the GitHub URL value is emphasized as the copy target, and clicking that value performs the same copy action.
