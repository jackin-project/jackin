...
When the workspace workdir is on a branch with an open GitHub pull request, the multiplexer shows a bottom pull-request bar. The left side shows the PR number and title; the right side shows the container ID. On a non-`main` / non-`master` branch without an open PR, the same line shows the branch name instead. Hovering either clickable side changes the pointer and slightly lifts the color, matching the top tab strip. Capsule polls the local Git branch every second so branch switches surface quickly, while GitHub PR and CI metadata stays cached per branch and refreshes on a conservative cadence so `gh` is not polled aggressively.
