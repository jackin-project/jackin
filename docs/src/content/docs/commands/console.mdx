...
- **Auth** — `Claude Code` and `Codex` (and, when wired, `GitHub CLI`) authentication forwarding modes at workspace and per-role scope. See [Authentication](/guides/authentication/) for the shared pattern across every auth axis.
