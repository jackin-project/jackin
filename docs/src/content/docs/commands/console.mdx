...
- **See active and recoverable instances.** Workspace and current-directory detail panes show an Instances panel when jackin-managed local state exists for that scope, with instance ID, status, agent runtime, and role.
...
- **Auth** — `Claude Code`, `Codex`, `Amp`, and `GitHub CLI` authentication forwarding modes at workspace and per-role scope. See [Authentication](/guides/authentication/) for the shared pattern across every auth axis.
