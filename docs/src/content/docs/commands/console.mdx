...
`jackin console` is the simplified, intuitive TUI for everyday workspace launches. [`jackin load`](/commands/load/) is the full-surface CLI — together with its neighbours (`hardline`, `eject`, `exile`, `purge`, `archive`, `workspace`, `config`) it exposes every flag and capability jackin' offers.
