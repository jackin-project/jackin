...
The console is always the full experience and has no flags to scale it back: a short digital-rain intro with the jackin' logo plays when you open it (entering the construct), launches show the rich progress cockpit, and a closing rain + logo plays when the last container exits (leaving the construct). When other containers are still running, exiting instead shows a compact summary of which roles are working on which projects, so you can reconnect. `--debug` writes detailed evidence to the run diagnostics artifact instead of turning the screen into raw logs. (The lower-level [`jackin load`](/commands/load/) keeps `--no-rain` / `--no-tui` for scripted, non-interactive launches.)
