...
Enable or disable automatic `Co-authored-by` trailer injection for agent commits. When enabled, jackin installs a `prepare-commit-msg` hook inside every container at launch. The hook appends the correct `Co-authored-by` trailer for the running agent on every non-amend, non-merge commit, deduplicating by agent email address so the trailer appears at most once.
