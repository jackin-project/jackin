...
Enable or disable automatic trailer injection for agent commits. When enabled, jackin installs a `prepare-commit-msg` hook inside every container at launch. The hook appends two trailers to every non-amend, non-merge, non-squash commit, deduplicating each so it appears at most once:

1. **`Co-authored-by`** — identifies the AI agent that produced the commit.
2. **`Signed-off-by`** — DCO sign-off read from `git config user.name` and `git config user.email` inside the container.

Supported agents and their `Co-authored-by` trailers:
...
The `Signed-off-by` trailer uses whatever identity `git config user.name` and `git config user.email` resolve to inside the container at commit time (forwarded from the host by jackin). If either is unset, the DCO trailer is skipped.

This setting can also be toggled in `jackin console` via the **General** tab in the Settings panel (`S` from the workspace list).

```bash
jackin config git coauthor-trailer enable
# → auto_coauthor_trailer: enabled

jackin config git coauthor-trailer disable
# → auto_coauthor_trailer: disabled
```
