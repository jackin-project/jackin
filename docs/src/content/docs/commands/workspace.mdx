...
```bash
jackin workspace claude-token setup <WORKSPACE> (--vault <VAULT> | --interactive) [OPTIONS]
```
...
| `--interactive` / `-i`   | Open a TUI drill-down (Account → Vault → Item) to choose where the token lands. Mutually exclusive with `--vault` and `--reuse`. Within the picker, select an existing item to write the token in-place, select `[ + New item ]` to create a fresh 1Password item, or select `[ + New field ]` on an existing item to append a new field. |
...
# TUI picker — choose vault and item interactively
...
See [Claude OAuth-token authentication](/guides/authentication/agents/claude-code/#oauth_token-claude-code-only) for the operator-level concept; [Claude Token Orchestrator](/reference/claude-token-orchestrator/) for the under-the-hood mechanics (contributor reference).
