...
| `--role <ROLE>`          | Wire the token for a specific role override instead of all roles in the workspace. Omit to wire the workspace-level slot. The interactive flow prompts for the scope when `--role` is absent. |
