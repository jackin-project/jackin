...
| `--default-agent <claude\|codex\|amp>` | Default agent for this workspace |
...
| `--default-agent <claude\|codex\|amp>` | Set default agent |
...
| `doctor` | Resolve the wired token via `op` and report its SHA-256 prefix so the operator can confirm the canonical slot points at a real, readable item. |
...
```bash
jackin workspace claude-token setup <WORKSPACE> --vault <VAULT> [OPTIONS]
```

| Option | Description |
|---|---|
| `--vault <VAULT>`        | 1Password vault name or UUID where the new item lands. Required unless `--reuse` is passed. |
| `--item-name <NAME>`     | Override the auto-generated item title. `{ws}` substitutes the workspace name. Default: `jackin · {ws} · claude-token`. |
...
jackin workspace claude-token setup my-app --reuse "op://Personal/jackin · my-app · claude-token/token"
```

### `workspace claude-token rotate`

```bash
jackin workspace claude-token rotate <WORKSPACE> [OPTIONS]
```
...
op_account       work@example.com
op_ref           Personal/jackin · my-app · claude-token/token
token sha256     a3f9c2b81e74… (12 hex prefix; matches stored value)
```

See [Claude OAuth-token authentication](/guides/authentication/agents/#oauth_token-claude-only) for the operator-level concept; [Claude Token Orchestrator](/reference/claude-token-orchestrator/) for the under-the-hood mechanics (contributor reference).
