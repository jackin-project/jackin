...
- **Identity strip** — container / role details and the clickable container-info entry point. The modal shows the full container ID, role, active pane's agent, current workdir, current git branch when the workdir is a repository, and a best-effort GitHub pull request URL from `gh pr view` when one exists for that branch.
