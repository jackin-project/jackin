...
One socket file exists per running instance on the host. The first byte selects the channel:
...
The control channel currently exposes `Status`, which returns a typed `SessionList`.
...
- `alacritty_terminal` is a terminal-emulator core, not a small compositor helper. It would couple jackin to a heavier internal grid API without replacing the jackin-specific chrome rules.

| Approach | Performance model | Correctness risk | Integration / maintenance | License / reuse |
|---|---|---|---|---|
| Internal Zellij-style row cache over `vt100` | Scan visible rows for dirty panes; emit only changed rows plus that pane's chrome | Low: keeps the existing parser and compositor coordinate rules | Small local API; coupled to jackin pane geometry by design | Apache-compatible dependencies; reusable inside `jackin-container`, not a separate crate boundary |
| `vt100::Screen::contents_diff` | Emits a diff between two whole-screen origins | Medium: pane offsets, dimming, borders, overlays, and selection would need wrapping around every byte stream | Low dependency cost but awkward at jackin call sites | Apache-compatible; reusable only for full-origin surfaces |
...
- `pointer` over clickable jackin chrome such as tabs, the menu button, the container-info entry point, and clickable dialog rows such as the Container ID copy target.
- `text` over pane bodies where jackin's own mouse selection is available because the focused program did not request mouse reporting.
...
| <RepoFile path="crates/jackin-container/src/runtime_setup.rs" /> | Implements `jackin-container runtime-setup`, the Rust replacement for deterministic entrypoint shell setup |
