...
- [Console agent session control](/reference/roadmap/console-agent-session-control/) — instance discovery, console workspace tree-view with expandable instance rows and session pane, `hardline --shell`, tmux-backed primary session, secondary agent sessions via `hardline --new`, and console `a`/`x`/`N`/`X`/`P` instance keybindings shipped; Phase 4 (live session reconciliation, agent runtime status, resource panel integration) remains open
- [jackin-container: in-container multiplexer server](/reference/roadmap/jackin-container-binary/) — Phases 1–2 shipped: improved bash supervisor with cleanup; `jackin-container` Rust binary as PID 1 with zombie reaping; structured session inventory over Unix socket. Phase 3 (in-container PTY multiplexer replacing tmux) is being rewritten — the first attempt failed on alt-screen, prefix-key collision with `0x0A`, mouse passthrough, and exit-on-last-session; the rewrite uses the `vt100` crate, a tmux-style prefix-key state machine, a persistent server, binary hot-path framing, and zellij as the architectural reference, with explicit tmux-setting parity (extended keys, focus events, OSC passthrough, zero escape delay, mouse motion) and Ghostty compatibility (kitty keyboard protocol, kitty graphics, BSU/ESU, OSC 52). Phase 4 (daemon and desktop bridge) remains open
