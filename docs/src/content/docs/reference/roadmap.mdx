...
- [Unique container identity and restore](/reference/roadmap/unique-container-identity-restore/) — new launches use DNS-safe unique container identities, per-instance manifests with last attach outcomes, a rebuildable instance index, legacy isolation-state manifest backfill, Docker-unavailable diagnostics, deleted-container writable-layer loss reporting, metadata-rich load/hardline recovery prompts, instance-ID targeting for hardline/eject/purge, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref, stopped/missing-DinD hardline recovery for existing crashed role containers, a console active-instance panel backed by local state, and Java Testcontainers DinD smoke coverage; roadmap tracks full hardline recovery actions, richer console recovery controls, and stronger recovery guarantees
