...
import EarlyDevelopmentNotice from '../../../components/EarlyDevelopmentNotice.astro'
...
- **[Migrate the docs site to Fumadocs](/reference/roadmap/fumadocs-migration/)** — swap the docs rendering engine from Astro + Starlight to Fumadocs on TanStack Start (Vite), built with Bun and statically prerendered to GitHub Pages, with zero visible change: same phosphor-green identity, light/dark palette, fonts, landing page, always-dark code blocks, and custom chrome. Adds an AI-friendly surface (`llms.txt`, per-page Markdown, copy-for-LLM actions) and moves all OG/social/icon generation to Takumi. Research is complete (verified token system, static Orama search, Takumi OG, AI/LLM features, component mapping, audited inventory); the page lays out the parity checklist, gotchas, and a phased plan (status: open — research complete, design proposal and phased plan)
