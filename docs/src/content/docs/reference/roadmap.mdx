...
- **[Agent workflow orchestration](/reference/roadmap/agent-workflow-orchestration/)** — deterministic, visible multi-agent workflows that turn a roadmap page, GitHub issue, or local spec into an isolated jackin' run, live Capsule sessions, GitHub-visible progress, a reviewable PR, review/fix loops, and a final operator verification gate. Captures adversarial research on Conductor, Contrabass, agtx, Forge MCP, MCO/Hive, Optio, Vibe Kanban, Sandcastle, Handler.dev, Helmor/Orca, OpenClaw Code Agent, Ruah/Bernstein, Switchboard, and GitHub Agentic Workflows. The current recommendation is to validate the shared state spine first — run records, GitHub reporter, manual phase controls, and intervention events — before building a full native `roadmap-to-pr` runner or adopting an external orchestrator (status: open — research and design proposal)
