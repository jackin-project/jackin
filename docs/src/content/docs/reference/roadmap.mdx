...
- [jackin-container: in-container multiplexer server](/reference/roadmap/jackin-container-binary/) — Phases 1–3 shipped: `jackin-container` Rust binary as PID 1 with zombie reaping; structured session inventory over Unix socket; in-container PTY multiplexer built on the `vt100` crate with a Zellij-style dirty-row renderer, tmux-style prefix-key model (`Ctrl+B` opt-in), persistent server that exits cleanly on last-session-end, binary tag+length attach framing, single-client takeover, mode-state restore on focus swap, OSC 52 / 9 / 2 / 8 passthrough, and top chrome with a brand pill, tab strip, and identity row. Phase 4 (host daemon integration and Desktop Agent Hub bridge) remains open
