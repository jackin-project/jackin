...
- [Unique container identity and restore](/reference/roadmap/unique-container-identity-restore/) — new launches use DNS-safe unique container identities, per-instance manifests, a rebuildable instance index, legacy isolation-state manifest backfill, Docker-unavailable diagnostics, deleted-container writable-layer loss reporting, instance-ID targeting for hardline/eject/purge, bare `hardline` lookup for indexed workspace/ad-hoc instances, exact `hardline` rebuild for missing recoverable instances, and Java Testcontainers DinD smoke coverage; roadmap tracks the hardline recovery picker, console recovery UI, and stronger recovery guarantees
