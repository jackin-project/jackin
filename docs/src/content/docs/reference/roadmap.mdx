...
- **[Global mount visibility and editing](/reference/roadmap/global-mount-visibility/)** — show applicable global mounts in workspace surfaces, keep them visually separate from saved workspace mounts, and add a dedicated global-config TUI page for viewing and editing `jackin config mount` entries (status: open — design proposal)
