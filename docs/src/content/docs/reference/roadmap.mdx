...
- [Config versioning and migration framework](/reference/roadmap/config-versioning-migration/) — shipped per-file schema gates for config, workspace files, and role manifests, plus automatic config/workspace migration, desktop role manifest migration through `jackin role migrate`, and CI migration through `jackin-validate --migrate`; roadmap tracks deferred `--pr` automation and the Renovate-style auto-migration GitHub Action
- [Console agent session control](/reference/roadmap/console-agent-session-control/) — instance discovery and the console Instances panel (with recover/reconnect, new session, inspect, and guarded purge actions) shipped with unique container identity and restore; shell entry, reconnectable tmux-backed primary session, and stop-one-without-stopping-all secondary sessions remain in Phase 2+
...
- **[Herdr research](/reference/roadmap/herdr-research/)** — research of Herdr, a Rust terminal multiplexer for AI agents (AGPL-3.0), which shares Jackin's multi-agent workspace and observability problem space but uses a bare-host process model rather than container isolation. Covers the concepts to research and potentially reimplement independently: two-stage done state, workspace-level status roll-up, notification suppression when the operator is already looking, sound escalation opt-in, PTY inference heuristic, and blocking wait semantics on the daemon socket. AGPL-3.0 prohibits embedding; all reimplementation must be written from scratch (status: open — research captured, no implementation committed)
