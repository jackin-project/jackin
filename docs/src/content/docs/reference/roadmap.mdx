...
- [Docker API migration](/reference/roadmap/bollard-migration/) — Phase 1 shipped: `DockerApi` trait with `BollardDockerClient`, all lifecycle/cleanup/inspect/exec/image-inspect/pull operations migrated to bollard, `ContainerState` extended to all 9 Docker states, CLI stderr string-matching eliminated; `docker build` and interactive `docker run -it` remain deferred to Phase 2
