...
- [Alternative provider support (Z.AI / GLM Coding Plan)](/reference/roadmap/alternative-provider-support/) — Phases 1–3 shipped: `ZAI_API_KEY` env injection mechanic, in-container `Dialog::ProviderPicker` after agent selection, and console inline provider picker after `N`-key agent selection with workspace-aware key resolution; Phase 4 (auth tab `AuthKind::Zai` row) remains open
