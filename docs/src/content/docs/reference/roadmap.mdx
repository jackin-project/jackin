...
- [Unique container identity and restore](/reference/roadmap/unique-container-identity-restore/) — new launches use DNS-safe unique container identities, per-instance manifests with last attach outcomes, a rebuildable instance index, legacy isolation-state manifest backfill, purged lifecycle tombstones, Docker-unavailable diagnostics, deleted-container writable-layer loss reporting, metadata-rich load/hardline recovery prompts, instance-ID targeting for hardline/eject/purge, plain-purge refusal while matching Docker resources still exist, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref, stopped/missing-DinD hardline recovery for existing crashed role containers, interactive moved-directory recovery for exact ad-hoc hardline restores, read-only `hardline --inspect` state reports, `hardline --new` foreground agent spawning inside running instances, a directory-aware hardline action picker, console active-instance recovery controls backed by local state, and Java Testcontainers DinD smoke coverage; roadmap tracks richer prompts across existing agent sessions and stronger snapshot/archive guarantees
