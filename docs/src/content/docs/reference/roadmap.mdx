...
- **[Agent workflow orchestration](/reference/roadmap/agent-workflow-orchestration/)** — deterministic, visible multi-agent workflows that turn a roadmap page, GitHub issue, or local spec into an isolated jackin' run, live Capsule sessions, GitHub-visible progress, a reviewable PR, Claude/Codex review/fix loops, and a final operator verification gate. Captures research on Conductor, Contrabass, agtx, Forge MCP, MCO/Hive, Bernstein, GitHub Agent HQ, Claude Code Review, Gemini/Antigravity, and Grok Build while keeping jackin' as the execution substrate until an external orchestrator clearly becomes the right dependency (status: open — research and design proposal)
