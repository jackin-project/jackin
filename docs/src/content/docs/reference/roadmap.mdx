...
- [Role authoring CLI](/reference/roadmap/role-authoring-cli/) — shipped desktop `jackin role validate`, `jackin role migrate`, and `jackin role create`; roadmap keeps standalone `jackin-validate` as the CI/Renovate-style role maintenance binary and tracks bulk cached-role migration plus richer scaffolding policy
