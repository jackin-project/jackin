...
- [Unique container identity and restore](/reference/roadmap/unique-container-identity-restore/) — new launches use DNS-safe unique container identities, per-instance manifests with last attach outcomes, a rebuildable instance index, legacy isolation-state manifest backfill, purged lifecycle tombstones, Docker-unavailable diagnostics, deleted-container writable-layer loss reporting, metadata-rich load/hardline recovery prompts, in-place recovery/rebuild for related instances using saved manifest source/ref, instance-ID targeting for hardline/eject/purge, plain-purge refusal while matching Docker resources still exist, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref and durable per-instance agent home, stopped/missing-DinD hardline recovery for existing crashed role containers, interactive moved-directory recovery for exact ad-hoc hardline restores with current-directory, browser, or manual path selection, read-only `hardline --inspect` state reports, `hardline --new` foreground agent spawning inside running instances, a directory-aware hardline action picker, exact-selector prompts when multiple live agent processes are detected, console active-instance recovery controls backed by local state, and Java Testcontainers DinD smoke coverage; roadmap tracks exact per-session TTY reattach beyond Docker's attachable container TTY and keeps container writable-layer mutations explicitly out of scope
