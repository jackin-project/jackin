...
- [Unique container identity and restore](/reference/roadmap/unique-container-identity-restore/) — new launches use DNS-safe unique container identities, per-instance manifests, a rebuildable instance index, legacy isolation-state manifest backfill, and bare `hardline` lookup for indexed workspace/ad-hoc instances; roadmap tracks the hardline recovery picker, console recovery UI, and stronger recovery guarantees
