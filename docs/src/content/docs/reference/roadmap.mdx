...
- **[Workspace Claude token setup](/reference/roadmap/workspace-claude-token-setup/)** — guided `jackin workspace claude-token setup` orchestrator that PTY-captures the upstream `claude setup-token` output, writes the result to 1Password, wires the workspace config, and validates end-to-end (status: in progress — orchestrator + CLI shipped; TUI integration and expiry banner remain)
