...
- **[Alternative provider support (Z.AI / GLM Coding Plan)](/reference/roadmap/alternative-provider-support/)** — per-tab API backend selection for Claude Code and future agents: configure `ZAI_API_KEY` once in the global or workspace auth tab (same `AuthKind` pattern as GitHub CLI), then choose `Anthropic` or `Z.AI` when spawning any Claude tab in the console or the in-container multiplexer; provider suffix shown in tab strip only when multiple providers are configured; process-level env injection keeps Anthropic and Z.AI tabs isolated in the same container (status: open — design complete, implementation planned)
