...
- [Multi-runtime support for Codex and Amp](/reference/roadmap/multi-runtime-support/) — `Claude Code`, `Codex`, and `Amp` all shipped as built-in runtimes; full feature parity across `claude` / `codex` / `amp` still in flight
...
- [Workspace Claude token setup](/reference/roadmap/workspace-claude-token-setup/) — `jackin workspace claude-token setup|rotate|revoke|doctor` orchestrator with PTY-captured tokens, 1Password write, post-write SHA validation, the in-container OAuth-token onboarding skeleton, and the launch-banner expiry countdown all shipped; TUI `[G] Generate token` action, Apple Keychain backend, and bulk `--all` migration flag remain. Under-the-hood mechanics at [Claude Token Orchestrator](/reference/claude-token-orchestrator/)
...
<Aside type="note">
`Claude Code`, `Codex`, and `Amp` are all built-in runtimes today (see Partially implemented above). The remaining work to reach full `claude` / `codex` / `amp` feature parity — runtime-specific auth, config, state, and entrypoint behaviour — continues under the same roadmap item.
</Aside>
- **[Multi-runtime support for Codex and Amp](/reference/roadmap/multi-runtime-support/)** — finish deeper runtime parity beyond basic launch/auth support. Phase 1 focuses on secure local usability, not full Claude-feature parity.
