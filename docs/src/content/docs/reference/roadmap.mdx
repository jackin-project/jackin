...
- [Config versioning and migration framework](/reference/roadmap/config-versioning-migration/) — shipped per-file `version = "v1alpha1"` gates for config, workspace files, and role manifests, plus automatic config/workspace migration and explicit role manifest `--migrate`; roadmap tracks deferred `--pr` automation
