...
- **[Codebase readability program](/reference/roadmap/codebase-readability/)** — multi-phase internal-quality work covering documentation/setup, behavioral specs, DRY deduplication, module-contract refactors, and test coverage. Re-analyzed May 2026 against the current codebase (~91K lines of production Rust, 13 files over 1000 lines, pervasive TUI/auth/Docker duplication). Includes the [unified TUI scrollbars](/reference/roadmap/unified-tui-scrollbars/) refactor plan for moving scrollbar geometry, clamping, drag mapping, and launch-progress overscroll fixes into one shared implementation. The full leaf list lives in the program doc and in the sidebar under **Reference → Roadmap → Codebase health**.
