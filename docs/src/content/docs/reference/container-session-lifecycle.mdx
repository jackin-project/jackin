...
2. The launcher checks that PID 1 did not exit before attach. If it exited, the launcher captures the last 40 lines of container logs and surfaces an actionable error.
3. The launcher runs `docker exec -it <container> /usr/local/bin/jackin-capsule`. The client connects to the daemon socket and blocks the operator terminal for the duration of the attach.
