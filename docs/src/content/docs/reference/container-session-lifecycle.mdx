...
Every jackin container runs the Jackin Capsule (`jackin-capsule`) as PID 1. The daemon owns the PTY sessions, renders the in-container multiplexer, serves `/run/jackin/jackin.sock`, and exits when the last live session ends. The host attaches with a separate `docker exec -it <container> /usr/local/bin/jackin-capsule` client. See [Jackin Capsule](/reference/jackin-capsule/) for the control-plane design and wire protocol.
...
1. `docker run -d ... <image> <agent>` starts the container detached. The image entrypoint is `/usr/local/bin/jackin-capsule`; the trailing `<agent>` argv tells PID 1 which initial agent tab to spawn. It is not exported as a container environment variable.
...
3. jackin runs `docker exec -it <container> /usr/local/bin/jackin-capsule`. The client connects to the daemon socket and blocks the operator terminal for the duration of the attach.

Implementation: <RepoFile path="src/runtime/launch.rs" /> (`launch_role_runtime`, `diagnose_premature_exit`) and <RepoFile path="crates/jackin-capsule/src/main.rs" /> (`resolve_initial_agent`).

## Reconnecting (`jackin hardline`)

`jackin hardline` inspects the target container and chooses the reconnect path:

- **Container is running**: attach with `docker exec -it <container> /usr/local/bin/jackin-capsule`.
- **Container is running and the operator requested a new agent session**: run `docker exec --workdir <workdir> -it <container> /usr/local/bin/jackin-capsule new <agent>`. The agent slug travels as argv; `JACKIN_AGENT` is set later inside the spawned PTY.
- **Container is running and the operator requested a shell**: run `docker exec -it <container> /usr/local/bin/jackin-capsule new`. No agent slug means a shell pane.
...
```bash
/usr/local/bin/jackin-capsule status
```

The command opens the Capsule control channel, sends a typed `Status` request, and prints a line-oriented summary that older host paths parse into `AgentSession` records. Host console previews use the same socket with a direct JSON `Snapshot` request. If the container is not running, inventory is reported as `sessions:not_running` without contacting Docker.

Implementation: <RepoFile path="src/runtime/attach.rs" /> (`inspect_agent_sessions`) and <RepoFile path="crates/jackin-capsule/src/client.rs" /> (`run_status`).
