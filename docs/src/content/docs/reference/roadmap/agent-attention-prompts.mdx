...
1. **An MCP server jackin' auto-registers in every agent session.** Each agent runtime jackin' supports (Claude Code, Codex; Amp forthcoming) integrates MCP servers via its own config. jackin' already wires `tirith` and `shellfirm` for the Claude runtime via <RepoFile path="crates/jackin-container/src/runtime_setup.rs" />; the equivalent registration for Codex / Amp is part of this proposal's scope. The new `jackin-attention` MCP server adds two tools the agent can call:
