...
- **Agent-state event stream.** `jackin-container` emits effective state transitions over the per-container control channel. The daemon subscribes to `blocked`, `done`, `idle`, `working`, and stuck-diagnostic events instead of scraping PTY output from the host.
- **Optional `jackin-attention` MCP server.** Static binary baked into the construct image. Registered automatically in each agent's MCP config at container launch — extending the registration pattern <RepoFile path="crates/jackin-container/src/runtime_setup.rs" /> uses today for the Claude runtime's `tirith` / `shellfirm` to every supported runtime. When the agent adds attention context, the MCP server pushes metadata over the same per-container control channel. It does not replace the state authority.
...
- <RepoFile path="crates/jackin-container/src/runtime_setup.rs" /> — current MCP-registration pattern for `tirith` / `shellfirm`. The `jackin-attention` MCP server registers through the same code path.
