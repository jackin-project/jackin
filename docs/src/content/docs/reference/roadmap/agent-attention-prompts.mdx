...
1. **An MCP server jackin' auto-registers in every agent session.** Each agent runtime jackin' supports today (Claude Code, Codex; Amp soon) integrates MCP servers via its own config; jackin' already does this for `tirith` and `shellfirm`. The proposed `jackin-attention` MCP server adds two tools the agent can call:
...
- **`jackin-attention` MCP server.** Static binary baked into the construct image. Registered automatically in each agent's MCP config at container launch (parallel to today's `tirith` / `shellfirm` registration in <RepoFile path="docker/runtime/entrypoint.sh" />). When the agent calls `attention.waiting`, the MCP server pushes the event over the existing per-container control channel (the same bind-mount or pipe used for daemon ↔ container coordination — see [jackin daemon](/reference/roadmap/jackin-daemon/) for the channel design).
...
The configuration knobs (managed through `jackin console` → workspace editor → a new **Notifications** tab, parallel to the existing Auth / Mounts / Environments tabs):
...
1. `jackin-attention` MCP server, baked into construct, auto-registered for every agent runtime jackin' supports today.
...
- [Universal Orchestrator Program → Live agent observability](/reference/roadmap/multicode-inspired-features/) — adjacent work on surfacing what the agent is *doing* in real time. Attention prompts close the loop the other direction (when the agent stops doing things and waits).
