...
**Status**: In progress — daemon bridge foundation landed
...
The daemon work now provides the transport this feature needs:

- The host daemon owns macOS Notification Center dispatch.
- Role containers receive `JACKIN_DAEMON_SOCKET=/jackin/daemon.sock`.
- The construct image includes `jackin-notify`, a hook-friendly helper:

```sh
jackin-notify --title "agent waiting" --body "Need operator input" --urgency normal
```

This is intentionally lower-level than the final MCP server. It lets role hooks and agent-authored scripts send attention pings immediately while the MCP contract and runtime auto-registration are completed.
...
- **The jackin' daemon dispatches the host-side notification.** Per the [jackin' daemon](/reference/roadmap/jackin-daemon/) plug-in surface, this feature is one adapter that listens for state/attention events from any running container and translates them to OS-native notifications:
...
- **Bridge helper while the MCP/event path matures.** Until the MCP server and event stream land, `jackin-notify` exposes the same host-notification path for hooks and scripts over the daemon socket mounted at `/jackin/daemon.sock`. This validates the container-to-host notification transport without making it the final state source.
- **Click-to-focus adapter.** The notification carries an action that, when activated, runs a host-side command to focus the right terminal tab. The first Desktop slice should implement Ghostty only, matching the [jackin' Desktop Agent Hub](/reference/roadmap/jackin-desktop-agent-hub/) v1 launch contract. Other terminal emulators get a notification without click focus until their adapters are deliberately added.
...
1. Consume effective `blocked` and derived `done` transitions from the [agent runtime status authority](/reference/roadmap/agent-runtime-status/).
...
3. Daemon adapter listening on the per-container control channel, dispatching macOS Notification Center notifications. The base adapter has landed through `jackin daemon notify` and `jackin-notify`; high urgency requests the native `Glass` notification sound.
...
6. Linux `notify-send` adapter later, after the macOS flow is validated.
...
- [jackin' daemon](/reference/roadmap/jackin-daemon/) — host-side process this feature plugs into. The base notification bridge exists; the real Capsule-backed event stream remains open.
...
- [jackin' Desktop Agent Hub](/reference/roadmap/jackin-desktop-agent-hub/) — native macOS surface that should display attention/ready-for-review state and owns the first Ghostty click-to-focus UX.
- [Agent runtime status authority](/reference/roadmap/agent-runtime-status/) — canonical source for `blocked`, derived `done`, `idle`, `working`, `unknown`, and stuck diagnostics. Attention prompts consume this stream; they do not infer state independently.
- [Agent Orchestrator Research Program → Live agent observability](/reference/roadmap/agent-orchestrator-research/) — adjacent work on surfacing what the agent is *doing* in real time. Attention prompts close the loop the other direction (when the agent stops doing things and waits).
- <RepoFile path="crates/jackin-capsule/src/runtime_setup.rs" /> — current MCP-registration pattern for `tirith` / `shellfirm`. The `jackin-attention` MCP server registers through the same code path.
