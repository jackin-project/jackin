...
This requires a `WorkspaceConfig` schema change: a new `RegistryConfig` struct under the `registry` key. Because `WorkspaceConfig` is a versioned schema, the change requires:
...
The registry starts when the first role instance in a workspace launches (if `registry.enabled = true`). It stays running across role starts and stops — killing it between sessions defeats the cache. It stops when the workspace is explicitly destroyed via `jackin eject` or `jackin prune`, or when the operator runs the future `jackin workspace registry stop` subcommand (Phase 2).
...
`ensure_workspace_registry(workspace: &str, config: &RegistryConfig)` performs:
...
- `jackin workspace registry start|stop|status` subcommands — explicit lifecycle management independent of role launches
...
- <RepoFile path="src/config/mod.rs" /> — `RegistryConfig` struct, `WorkspaceConfig` extension
