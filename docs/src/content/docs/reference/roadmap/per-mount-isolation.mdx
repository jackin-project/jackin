...
At `load` time, materialization happens **after** the real container name is claimed and the per-container state directory exists. `resolve_load_workspace` should stay a config/path resolver; it does not know which random instance ID this launch will be assigned. The runtime should introduce a narrow post-resolution step:
...
- <RepoFile path="src/runtime/cleanup.rs" /> — `purge_container_state` houses the running-agent guard (uses `list_agent_names`); `Command::Purge` dispatches through here
- <RepoFile path="src/config/mod.rs" /> — serialization of the `isolation` field
...
- <RepoFile path="src/isolation/mod.rs" />, <RepoFile path="src/isolation/materialize.rs" />, <RepoFile path="src/isolation/state.rs" />, <RepoFile path="src/isolation/branch.rs" />, <RepoFile path="src/isolation/cleanup.rs" />, <RepoFile path="src/isolation/finalize.rs" /> — `isolation` directory module covering materialization, `isolation.json`, mount-destination slugging, and host-side git shell-outs
...
- **Isolation-mode flips between loads do not fire drift detection.** Changing a mount's `isolation` value in TOML between loads silently takes effect on the next launch without warning the operator. Source-`src` drift on a materialized `dst` is guarded, but the `isolation` field itself is not. See the matching comment + test in <RepoFile path="src/config/workspaces.rs" />.
