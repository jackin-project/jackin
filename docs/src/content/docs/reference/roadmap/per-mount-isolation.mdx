...
- Clean-exit cleanup deletes clean worktrees automatically when no commits would be lost, including clean branches whose local commits are already reachable from their upstream/remote tracking branch.
...
1. Interaction with `hardline` offline lockdown — `worktree` is local-only, so this should be fine, but worth verifying.
2. Dirty-tree definition — should `--force` acknowledge untracked files too, or only tracked/staged changes?
3. Branch reuse — confirm that first materialization branches from host `HEAD`, while later loads reuse the existing worktree/branch without resetting or rebasing it.
