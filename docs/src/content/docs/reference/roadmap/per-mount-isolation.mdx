...
- `src/runtime/launch.rs` — `load`/`launch` path where materialization hooks in and foreground finalization begins
- `src/runtime/attach.rs` — `hardline` attach/restart path; should call the shared foreground finalizer after attach returns
