...
- **`clone`** — jackin' runs `git clone --local <src> <per-agent-copy>`, using local object hardlinks for disk efficiency while giving the agent a fully self-contained `.git/` with **independent refs**. After cloning, jackin' reads the host repo's `origin` URL and rewrites the per-agent clone's `origin` to match — only `origin` is copied (other host remotes such as `upstream` or `fork` are intentionally not propagated). SCP / `ssh://` **GitHub** URLs are normalized to `https://` so the container's `gh` credential helper can authenticate without an SSH key (non-GitHub URLs pass through unchanged); embedded credentials in an HTTP(S) origin (e.g. a host-side PAT baked into `https://x-access-token:<pat>@github.com/...`) are stripped before the rewrite so the per-container `.git/config` cannot leak the operator's token to a clone-mode agent. `git push origin <branch>` from inside the container therefore reaches GitHub directly — the agent's branches show up on the operator's GitHub remote, not on the host's `git branch -a`, until the operator pulls them. If the host repo legitimately has no `origin` remote (fresh `git init`, never pushed), jackin' leaves the clone's `origin` at the bind-mount path; the agent has no upstream until the operator adds one. Other failures reading the host's `origin` (permission denied, corrupt config) abort the launch rather than silently misroute pushes. The concrete operator situations clone mode targets:
...
**Hooks inherit from the host repo.** Worktrees share `.git/hooks/` with the parent — operators who rely on pre-commit/pre-push to keep commits clean get that for free inside agent containers. This is also a trust risk: repo-local hooks, `core.hooksPath`, and shared git config can execute host-authored policy inside the agent workflow and can drift while long-running agents are active. Clone mode is the safer default for untrusted agents because its `.git` directory is independent. A future hook-approval flow should make hook source, hash, install path, and drift visible before an agent is allowed to run commit/push workflows.
...
- `jackin load --force` dirty-host acknowledgement for non-interactive worktree materialization; `jackin console` gets an interactive confirmation path.
