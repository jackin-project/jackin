...
- **Normal clean exit** — when the attached agent process exits cleanly and Docker reports `exit_code = 0` without OOM, jackin' tears down Docker resources and attempts safe isolated-worktree cleanup. Safe cleanup removes an isolated worktree only when it has no uncommitted changes and no branch-only commits that would be lost. If cleanup is unsafe and stdin is interactive, jackin' asks what to do before deleting anything:
  ...
  Returning to the agent means restarting the just-exited container before teardown, attaching again, and retrying safe cleanup after the agent exits again. This is a launch-time cleanup loop, not normal `hardline` behavior; `hardline` may continue to reject clean-exited containers after the original `load` process has ended. Force delete uses the same destructive cleanup path as `purge` for the selected isolated worktree. If stdin is non-interactive, jackin' preserves the worktree, prints its path plus the reason, and tells the operator to inspect with `jackin cd <container>` or discard with `jackin purge <container>`. This hooks into the existing `ContainerState::Stopped { exit_code: 0, oom_killed: false }` branch in `src/runtime/launch.rs`, right next to runtime resource cleanup. Non-interactive warning shape:

  ```text
  [jackin] preserved isolated worktree for jackin-the-architect:
           /.../.jackin/data/jackin-the-architect/isolated/workspace-jackin
           reason: worktree has uncommitted changes
           run `jackin cd the-architect` to inspect or `jackin purge the-architect` to discard
  ```
- **Detached or crashed sessions** — if the container is still running after attach returns, or it stopped with a non-zero/OOM state, jackin' preserves both the container recovery path and isolated worktrees for `hardline`/manual inspection.
...
- `load` materializes, clean exit safely removes throwaway isolated worktrees when no work would be lost, unsafe clean-exit cleanup asks interactive operators whether to return to the agent, preserve, or force-delete, `eject` leaves worktrees alone, and `purge` force-deletes everything (container, DinD sidecar, `isolated/`, worktree, local branch).
