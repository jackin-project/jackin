...
`isolation.json` is required because purge cannot rely on the current TOML. Operators may change mount isolation, change mount sources, or remove the workspace entirely between `eject` and `purge`; cleanup must still know which host repo/worktree/branch were created for that container. Each record should include at least the workspace name, mount `dst`, original resolved `src`, isolation mode, materialized worktree path, scratch branch, and container name.
...
- Runtime source-drift guard: `load` errors when a saved workspace mount reuses a materialized isolated `dst` with a different `src`; CLI/TUI workspace edits should detect the same drift before saving and ask whether to delete preserved state or cancel.
