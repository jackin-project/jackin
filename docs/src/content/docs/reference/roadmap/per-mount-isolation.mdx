...
- **Normal clean exit** — when the attached agent process exits cleanly and Docker reports `exit_code = 0` without OOM, jackin' tears down Docker resources and attempts safe isolated-worktree cleanup. Safe cleanup removes an isolated worktree only when it has no uncommitted changes and no branch-only commits that would be lost. If cleanup is unsafe, jackin' preserves the worktree, prints its path plus the reason, and tells the operator to inspect with `jackin cd <container>` or discard with `jackin purge <container>`. This hooks into the existing `ContainerState::Stopped { exit_code: 0, oom_killed: false }` branch in `src/runtime/launch.rs`, right next to runtime resource cleanup. Warning shape:
...
- `load` materializes, clean exit safely removes throwaway isolated worktrees when no work would be lost, `eject` leaves worktrees alone, and `purge` force-deletes everything (container, DinD sidecar, `isolated/`, worktree, local branch).
