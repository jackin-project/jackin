...
- **Convenience navigation.** `jackin cd <container>` should not pretend it can mutate the parent shell process. The portable V1 behavior is to print the host worktree path for the container's primary isolated mount; docs can provide a shell helper (`cd "$(jackin cd <container>)"`) if operators want direct navigation. If a container has multiple isolated mounts, `jackin cd <container> <dst>` can disambiguate.
...
- `jackin cd <container> [dst]` prints the host path for an isolated worktree.
