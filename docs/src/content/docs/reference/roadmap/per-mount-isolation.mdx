...
4. Source changes after materialization — if a mount keeps the same `dst` but changes `src`, should `load` hard-error until purge to avoid hiding old WIP?
5. Branch reuse — confirm that first materialization branches from host `HEAD`, while later loads reuse the existing worktree/branch without resetting or rebasing it.
