...
- **Convenience navigation.** `jackin cd <container>` should not pretend it can mutate the parent shell process. The portable V1 behavior is to print the host worktree path for the container's primary isolated mount, which composes cleanly with shell tools:

  ```sh
  cd "$(jackin cd the-architect)"
  code "$(jackin cd the-architect)"
  git -C "$(jackin cd the-architect)" status
  ```

  Docs can provide a shell helper for operators who want direct parent-shell navigation:
  ...
  For an interactive inspection workflow, `jackin cd <container> [dst] --shell` spawns a child shell with its current directory set to the isolated worktree path. Exiting that shell returns the operator to the original parent shell and original directory. If a container has multiple isolated mounts, the optional `dst` argument disambiguates.
...
- `jackin cd <container> [dst]` prints the host path for an isolated worktree; `--shell` opens a child shell in that path.
...
- New `src/cli/cd.rs` (or similar) — `jackin cd <container> [dst] [--shell]` command
