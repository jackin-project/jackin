...
- **Wire format.** JSON-line request/response, similar to how `gh` CLI talks to its keyring helper. Avoids gRPC dependency footprint at this scale.
...
Rough order of operations, to be confirmed during the design pass:
...
3. **Phase 2 — caffeinate migration.** Move keep-awake reconciler from per-command into the daemon. Smallest migration target with clearest existing behaviour to compare against.
4. **Phase 3 — live auth sync.** Add the per-axis watcher adapters. First reactive feature.
5. **Phase 4+ — container event watcher and beyond.** Each new reactive feature is an adapter against the daemon's stable plug-in surface.
