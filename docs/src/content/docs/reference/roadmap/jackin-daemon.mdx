...
- **Keep-awake `caffeinate`.** Today's per-command reconciler (`src/runtime/caffeinate.rs`) becomes a daemon-owned watcher: the daemon subscribes to "any role from a keep-awake workspace running" via Docker events and holds the caffeinate lock. CLI commands no longer reconcile.
