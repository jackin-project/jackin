...
- `src/config.rs` — new auth mode and config persistence
- `src/cli.rs` — help text and examples
- `src/lib.rs` — `config auth` command handling
- `src/instance.rs` — auth provisioning semantics for the new mode
- `src/runtime.rs` — runtime env injection and launch-time warnings
...
- `src/config.rs` — auth mode definitions and config resolution
- `src/cli.rs` — `jackin config auth` help text
- `src/lib.rs` — auth config command handlers
- `src/instance.rs` — credential forwarding and state preparation
- `src/runtime.rs` — launch-time env injection and notices
