...
- `copy` is convenient for first launch but drifts over time
- `sync` repairs startup drift but does not help once multiple live sessions continue running for hours
- `ignore` is explicit but throws away the main convenience benefit jackin wants to provide
...
Choose a hybrid plan:

1. Keep `copy`, `sync`, and `ignore` for backward compatibility.
2. Add a new token-based mode built around `CLAUDE_CODE_OAUTH_TOKEN`.
3. Treat token-based auth as the recommended mode for long-lived or concurrent jackin sessions.
4. Improve docs and warnings around `copy` and `sync` so operators understand their limits.

In other words:

- `copy` remains the lowest-friction first-run mode
- `sync` remains the simplest "follow host state on launch" mode
- token mode becomes the durable mode for serious day-to-day usage
...
- `src/config/mod.rs` — `AuthForwardMode` enum, persistence types
- `src/config/agents.rs` — auth mode resolution
- `src/cli/config.rs` — `jackin config auth` help text
- `src/app/mod.rs` — auth config command handlers
- `src/instance/auth.rs` — credential forwarding and state preparation
- `src/runtime/launch.rs` — launch-time env injection and notices
- `docs/src/content/docs/guides/authentication.mdx` — current forwarding model and troubleshooting
- `docs/src/content/docs/commands/config.mdx` — current operator auth commands
- `docs/src/content/docs/reference/configuration.mdx` — persisted config schema
