...
- [smolvm SDK overview](https://smolmachines.com/sdk/) - current SDK shape, language bindings, server requirement, and machine API surface.
...
- The public SDK docs currently present TypeScript/JavaScript and Python SDKs that talk to a smolvm server. A direct Rust embedding API for jackin' must be verified rather than assumed.
...
6. How stable are the CLI, public SDK/server, and any Rust-consumable lower-level API, and which one fits jackin' release/distribution needs?
