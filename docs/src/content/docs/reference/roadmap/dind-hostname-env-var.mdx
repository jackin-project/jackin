...
- **Discoverability**: Agent authors must know `DOCKER_HOST` exists and understand its format. A `JACKIN_*` env var is self-documenting alongside `JACKIN`
...
- <RepoFile path="src/runtime/launch.rs" /> — where `DOCKER_HOST` is constructed and passed to the container
- <RepoFile path="docker/runtime/entrypoint.sh" /> — runtime setup (no changes needed)
- <RepoFile path="src/env_model.rs" /> — reserved-runtime-env list including `JACKIN` (and precedent for additional `JACKIN_*` reservations)
