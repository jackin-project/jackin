...
**Status**: Partially implemented — Phases 1–3 shipped, Phase 4 (host daemon integration and Desktop Agent Hub bridge) remains open. Phase 3 landed an in-container PTY multiplexer built on the `vt100` crate with a tmux-style prefix-key model (`Ctrl+B` opt-in via `JACKIN_PREFIX`), a persistent server that exits cleanly when the last session ends, binary tag+length attach framing on the hot path, single-client takeover, mode-state restore on focus swap, OSC passthrough (52/9/2/8) gated to the focused pane, and a row-0 brand-pill + tab strip status bar.

The shipped-Phase-3 design and operator-visible behaviour now live in:

- [jackin-container — Internals](/reference/jackin-container/) (architecture, lifecycle, distribution, input routing)
- [Multiplexer design rules](/reference/multiplexer-design-rules/) (passthrough contract, verification checklist)
...
The remainder of this page is the original Phase 1–3 design rationale, kept for the design-history record. The canonical "what jackin-container does today" pages are the three links above.
...
The hand-rolled emulator in `crates/jackin-container/src/terminal.rs` (deleted in the Phase 3 rewrite — replaced by `session.rs`) is **deleted**. Replaced by [`vt100`](https://docs.rs/vt100/) — a maintained Rust crate explicitly designed for "tmux-like" use cases. It gives the rewrite all of the following for free:
