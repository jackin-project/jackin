...
- Implement the [agent runtime status authority](/reference/roadmap/agent-runtime-status/): raw `working` / `blocked` / `idle` / `unknown`, derived `done`, foreground-process detection, semantic runtime reports, visible-screen signals, stale-report arbitration, and stuck diagnostics. Silence alone is not a blocker signal.
...
- **[Agent runtime status authority](/reference/roadmap/agent-runtime-status/)** — the `agent-state-changed` event stream from Phase 3 is the delivery mechanism for blocked/working/done/idle/stuck indicators in the console and hardline.
