...
2. If no initial agent argv is provided *and* `JACKIN_SUPPORTED_AGENTS` lists at least one agent, the daemon can come up empty — row 0 brand bar, no tabs, no panes — and render a centred picker hint listing each agent in `JACKIN_SUPPORTED_AGENTS` plus `Shell`. The first prefix-action or click chooses the first tab.
...
- The `JACKIN_SUPPORTED_AGENTS` env-var contract between the derived image and the daemon, plus the initial-agent argv contract from the host launcher into PID 1.
