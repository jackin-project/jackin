...
- `src/runtime/launch.rs` - current launch orchestration
- `src/runtime/attach.rs` - attach and hardline behavior
- `src/runtime/cleanup.rs` - eject, purge, orphan GC
- `src/docker.rs` - Docker CLI execution model
- `src/instance/naming.rs` - persisted state and Docker-style naming assumptions
- `src/instance/auth.rs`, `src/instance/plugins.rs` - state preparation that assumes container semantics
- `src/workspace/resolve.rs`, `src/workspace/mounts.rs` - workspace mount resolution
- `src/config/mod.rs` - future config surface for sandbox mode
