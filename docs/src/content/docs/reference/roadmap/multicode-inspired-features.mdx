...
5. **The security docs are blunt about remaining risk.** Workspace changes are
...
| Task source abstraction | Manual workspaces in original; issue queue in fork | *Gap — see [Task source abstraction](/reference/roadmap/task-source-abstraction/)* |
...
| Credential source plurality | Backends such as env and command | Partial — `op://` and `${env.VAR}` shipped; *see [Credential source pattern](/reference/roadmap/credential-source-pattern/)* |
...
| Recovery | Pre-session snapshots, restore, formal backup invariants | Persistent VM state; `sbx rm` cleanup | [Session snapshot and rollback](/reference/roadmap/session-snapshot-rollback/) |
...
|---|---|---|
| [Session contract and explain mode](/reference/roadmap/session-contract-explain-mode/) | Hazmat session contract / `hazmat explain`; Docker Sandboxes security model | Workspace resolution; auth strategy |
...
| Item | Primary inspiration | Depends on |
|---|---|---|
| [Network egress policy](/reference/roadmap/network-egress-policy/) | Docker Sandboxes host proxy; Hazmat `pf`/DNS hardening; Claude devcontainer allowlist | Session contract; selectable sandbox backends |
...
1. **Fleet quick wins**: workspace description, archive, handler system,
   resource limits, and ephemeral mounts can land as independent PRs.
2. **Session contract first for containment work**: do not build network,
   snapshot, or integration behavior until jackin can preview the fully
   resolved launch boundary and host-side effects.
