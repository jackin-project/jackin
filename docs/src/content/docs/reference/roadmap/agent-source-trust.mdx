...
- <RepoFile path="src/config/mod.rs" /> — `AgentSource` (with `trusted` field)
- <RepoFile path="src/config/agents.rs" /> — `trust_agent()`, `resolve_agent_source()`
- <RepoFile path="src/runtime/launch.rs" /> — `confirm_agent_trust()`, trust gate in `load_agent_with()`
- <RepoFile path="src/selector.rs" /> — namespace/name parsing
