...
jackin's first-cut credential-forwarding flow injects auth tokens into agent containers via `docker run -e KEY=VALUE`. This applies uniformly to:
...
| Container filesystem (`hosts.yml` written under Sync mode) | yes |
...
- [Credential proxy](/reference/roadmap.mdx) (existing roadmap line) — earlier idea about proxy-based credential injection; the host-bridge / `secret.request` flow is the operator-mediated answer to the same problem.
