...
Status: partially implemented. Manual `jackin archive <instance>` exports an ejected instance's `~/.jackin/data/<container>/` tree to a tar file under `~/.jackin/archive/` or an explicit operator-chosen path, and refuses while role or DinD containers still exist. Automatic archive lifecycle, unarchive, indexed archive listing, and destructive snapshot/rollback flows remain open.
...
- Expand archive/export support with unarchive, listing, removal, and console visibility for preserved instances.
