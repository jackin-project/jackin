...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests with last attach outcomes, the rebuildable instance index, first-pass missing-container detection, Docker-unavailable diagnostics, explicit deleted-container writable-layer loss reporting, metadata-rich load/hardline recovery prompts, cross-role load blocking/guidance for unfinished workspace instances, instance-ID targeting for hardline/eject/purge/archive, purged lifecycle tombstones, plain-purge refusal while matching Docker resources still exist, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref, stopped/missing-DinD hardline recovery for existing crashed role containers, interactive moved-directory recovery for exact ad-hoc hardline restores, read-only `hardline --inspect` state reports with live agent-session inventory, `hardline --new` foreground agent spawning inside running instances, manual tar export for ejected instance state, a directory-aware hardline action picker with live session counts, console active-instance recovery controls backed by local state, Java Testcontainers DinD smoke coverage, and legacy isolation-state manifest backfill have shipped; exact per-session reattach prompts and stronger snapshot/rollback guarantees remain open (Infrastructure)
...
Status: partially implemented. Manual `jackin archive <instance>` exports an ejected instance's `~/.jackin/data/<container>/` tree to a tar file under `~/.jackin/archive/` or an explicit operator-chosen path, refuses while role or DinD containers still exist, lists local archives with `jackin archive --list`, and removes local archive files with `jackin archive <instance> --remove`. Automatic archive lifecycle, unarchive, persistent archive index metadata, console archive visibility, and destructive snapshot/rollback flows remain open.
...
- Expand archive/export support with unarchive, persistent index metadata, and console visibility for preserved instances.
