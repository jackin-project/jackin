...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests with last attach outcomes, the rebuildable instance index, first-pass missing-container detection, Docker-unavailable diagnostics, explicit deleted-container writable-layer loss reporting, metadata-rich load/hardline recovery prompts, cross-role load blocking/guidance plus in-place recovery for related running/stopped instances, instance-ID targeting for hardline/eject/purge, purged lifecycle tombstones, plain-purge refusal while matching Docker resources still exist, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref and per-instance durable agent home, stopped/missing-DinD hardline recovery for existing crashed role containers, interactive moved-directory recovery for exact ad-hoc hardline restores with current-directory or entered-path selection, read-only `hardline --inspect` state reports with live agent-session inventory, `hardline --new` foreground agent spawning inside running instances, a directory-aware hardline action picker with live session counts, exact-selector action prompts when multiple live agent processes are detected, console active-instance recovery controls backed by local state, Java Testcontainers DinD smoke coverage, and legacy isolation-state manifest backfill have shipped; exact per-session TTY reattach beyond Docker's attachable container TTY, visual moved-path browser recovery, missing-related-instance in-place rebuild routing, and future persistent lifecycle storage remain open. Container writable-layer recovery is explicitly out of scope (Infrastructure)
...
- **State directory exists but host workdir moved**: resolve by workspace config first. Exact ad-hoc `hardline` restores can prompt to use the current directory or an entered moved project path and mount it at the original in-container workdir; future restore pickers should add a visual path browser and inspect-only path.
...
- Keep restore scoped to host-backed jackin-managed state and make every missing-container diagnostic explicit about the deleted writable-layer boundary.
