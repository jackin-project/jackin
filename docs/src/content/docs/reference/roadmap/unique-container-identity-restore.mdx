...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests with last attach outcomes, the rebuildable instance index, first-pass missing-container detection, Docker-unavailable diagnostics, explicit deleted-container writable-layer loss reporting, instance-ID targeting for hardline/eject/purge, bare `hardline` lookup for indexed workspace/ad-hoc instances, exact `hardline` rebuild for missing recoverable instances, Java Testcontainers DinD smoke coverage, and legacy isolation-state manifest backfill have shipped; recovery pickers, console recovery UI, and stronger restore guarantees remain open (Infrastructure)
