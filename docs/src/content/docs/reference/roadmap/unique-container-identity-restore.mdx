...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests with last attach outcomes, the rebuildable instance index, first-pass missing-container detection, Docker-unavailable diagnostics, explicit deleted-container writable-layer loss reporting, instance-ID targeting for hardline/eject/purge, bare `hardline` lookup for indexed workspace/ad-hoc instances, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref, Java Testcontainers DinD smoke coverage, and legacy isolation-state manifest backfill have shipped; recovery pickers, console recovery UI, and stronger restore guarantees remain open (Infrastructure)
...
Status: partially implemented for CLI targeting and exact missing-instance rebuilds. Bare `jackin hardline` can resolve indexed workspace/ad-hoc instances from the current directory, exact `hardline` targets rebuild recoverable missing instances through the launch restore path with the recorded role source/ref, and explicit `hardline`, `eject`, and `purge` selectors can target an indexed instance ID. Interactive recovery pickers, agent-session selection, and console panels remain open.
