...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests, the rebuildable instance index, first-pass missing-container detection, and legacy isolation-state manifest backfill have shipped; hardline recovery UI and stronger restore guarantees remain open (Infrastructure)
