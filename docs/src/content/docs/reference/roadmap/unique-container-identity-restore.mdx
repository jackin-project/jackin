...
- **State directory exists but host workdir moved**: resolve by workspace config first. Exact ad-hoc `hardline` restores can prompt to use the current directory as the moved project path and mount it at the original in-container workdir; future restore pickers should add a richer path browser and inspect-only path.
