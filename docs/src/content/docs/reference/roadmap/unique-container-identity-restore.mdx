...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests, the rebuildable instance index, first-pass missing-container detection, bare `hardline` lookup for indexed workspace/ad-hoc instances, and legacy isolation-state manifest backfill have shipped; hardline recovery UI and stronger restore guarantees remain open (Infrastructure)
...
Status: partially implemented. New launches write `.jackin/instance.json` with lifecycle status and Docker resource names, maintain a rebuildable `~/.jackin/data/instances.json` lookup index, backfill legacy isolation-only state directories during index rebuild, `jackin load` detects matching unfinished state whose Docker container is missing before allocating a fresh name, and bare `jackin hardline` can resolve indexed workspace/ad-hoc instances from the current directory. Full interactive recovery pickers remain open.
