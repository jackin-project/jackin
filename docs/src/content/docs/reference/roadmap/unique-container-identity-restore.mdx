...
- **Role container exists but DIND is missing/stopped**: hardline restarts a stopped existing DIND sidecar and waits for TLS readiness before restarting the role container. Missing sidecar/network rebuild remains open; if the role container has important writable-layer state, surface that it cannot be safely rehydrated without the original container.
