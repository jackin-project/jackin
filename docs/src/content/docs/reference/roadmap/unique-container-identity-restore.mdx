...
**Status**: Resolved — DNS-safe unique names, per-instance manifests with last attach outcomes, the rebuildable instance index, missing-container detection, Docker-unavailable diagnostics, explicit deleted-container writable-layer loss reporting, metadata-rich load/hardline recovery prompts, cross-role load blocking/guidance plus in-place recovery/rebuild for related instances using saved manifest source/ref, instance-ID targeting for hardline/eject/purge, purged lifecycle tombstones, plain-purge refusal while matching Docker resources still exist, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref and per-instance durable agent home, stopped/missing-DinD hardline recovery for existing crashed role containers, interactive moved-directory recovery for exact ad-hoc hardline restores with current-directory, browser, or manual path selection, read-only `hardline --inspect` state reports with live agent-session inventory, `hardline --new` foreground agent spawning inside running instances, a directory-aware hardline action picker with live session counts, exact-selector action prompts when multiple live agent processes are detected, console active-instance recovery controls backed by local state, and Java Testcontainers DinD smoke coverage have shipped. Container writable-layer recovery is explicitly out of scope. Reconnectable named agent sessions belong to [Console agent session control](/reference/roadmap/console-agent-session-control/), and lifecycle history beyond the manifest/index belongs to [Persistent storage layer](/reference/roadmap/persistent-storage-layer/). (Infrastructure)
...
Jackin's restore promise should be precise: if work was written to jackin-managed local storage, jackin can guide the operator back to it even when Docker containers are deleted. Jackin intentionally does not recover writes that existed only in a deleted container's writable layer. Packages installed ad hoc inside the container, global files edited outside mounted paths, DinD images, and other container-layer mutations are disposable by design; put durable tools in the role Dockerfile or durable data under a mounted path.
...
- **Role container exists but DIND is missing/stopped**: hardline restarts a stopped existing DIND sidecar or recreates the derived jackin network and DIND sidecar, then waits for TLS readiness before restarting the role container. If the role container has unmounted writable-layer state, surface that it remains tied to that existing Docker container and is not part of jackin's rebuild promise.
- **Docker containers are missing but state directory exists with durable agent home or worktree/clone records**: recreate a new role container using the same `container_base` and remount the stored agent home plus worktree/clone paths. Do not regenerate a different base name for a restore, because persisted home state, isolation paths, and scratch branches are keyed by the original container name.
...
- **Docker containers are missing and the instance had unpersisted container-layer changes**: tell the operator those container-layer changes are gone and will not be restored, including ad-hoc package installs, global files outside mounted paths, and DinD images. This is an explicit product boundary, not something this roadmap item should recover.
- **State directory exists but host workdir moved**: resolve by workspace config first. Exact ad-hoc `hardline` restores can prompt to use the current directory, browse for the moved project directory, or enter another path manually and mount it at the original in-container workdir.
- **State directory exists but role source changed**: exact hardline restore defaults to the stored role source/ref when available. A future picker should offer a fresh launch path if the operator wants the latest trusted role instead.

Do not introduce a separate `jackin restore` command. `hardline` is the CLI verb for reconnecting to existing work. It is directory-aware in the same way `load` is: running `jackin hardline` from inside a configured workspace or ad-hoc project directory looks up matching unfinished/running instances and prompts when more than one exists. If the selected container has multiple detected agent processes, exact selectors prompt for a safe action rather than blindly attaching. Reconnectable per-session attach requires a named session backend and is tracked by [Console agent session control](/reference/roadmap/console-agent-session-control/). If no matching instance exists, `hardline` fails with a clear "nothing to hardline" message or points the operator at the normal fresh `load` path.
...
- **Manual Docker prune**: cert volumes may disappear while role containers remain stopped. Recreate DIND/certs/network when safe; otherwise explain which Docker-owned resources are gone and that jackin will restore only host-backed state.
...
- **Non-git mounts**: clone/worktree restore does not cover them unless they are already mounted from the host or stored under jackin-managed per-instance state. Container-layer copies are disposable.
...
Status: implemented. Every `jackin load` invocation claims a DNS-safe random instance base (`jackin-[<workspace>-]<agent-role>-<id>`), and the DinD e2e smoke runs Java Testcontainers from inside the launched role.

- Slugging, truncation, ID generation, and DNS-label validation live in <RepoFile path="src/instance/naming.rs" />.
- `hardline`, `eject`, and `purge` resolve operands by instance ID or container base; class-family scans use `class_family_matches` against the random-instance-id shape.
- DIND, network, cert volume, `DOCKER_HOST`, `JACKIN_DIND_HOSTNAME`, `DOCKER_TLS_SAN`, `NO_PROXY`, and `TESTCONTAINERS_HOST_OVERRIDE` are all derived from the instance base.

### Phase 2 — Instance manifest and restore detection

Status: implemented. New launches write `.jackin/instance.json` with lifecycle status, last attach outcome, and Docker resource names, maintain a rebuildable `~/.jackin/data/instances.json` lookup index, retain `purged` tombstones when state is intentionally discarded, `jackin load` detects matching unfinished state whose Docker container is missing before allocating a fresh name, distinguishes missing containers from Docker-unavailable inspect failures, reports that deleted container writable-layer changes are unrecoverable and will not be restored, shows restore choices with instance ID/status/agent/role/mount state, blocks non-interactive fresh loads when unfinished instances for other roles or agents exist in the same workspace/directory, can recover or rebuild a related role directly from the interactive `load` prompt using the saved manifest source/ref, and `jackin hardline`/`eject`/`purge` can resolve indexed instances by ID.
...
Status: implemented for identity/restore scope. Bare `jackin hardline` can resolve indexed workspace/ad-hoc instances from the current directory and shows candidate prompts with workspace, role, agent runtime, lifecycle status, Docker state, live agent-session count, and mount-state summary. When the target came from the current directory rather than an explicit selector, hardline prompts for reconnect/recover, new foreground agent session, inspect, or cancel. Exact selectors also prompt for the safe action when multiple live agent processes are detected. Exact `hardline` targets rebuild recoverable missing instances through the launch restore path with the recorded role source/ref and per-instance durable agent home, exact ad-hoc hardline restores prompt before using the current directory, browsing for the moved project directory, or entering another moved path manually, stopped/missing DIND sidecars are restored before crashed role containers, explicit `hardline`, `eject`, and `purge` selectors can target an indexed instance ID, plain `purge` refuses local-state deletion while matching Docker resources still exist, `jackin hardline --inspect` prints read-only manifest/session/Docker/DinD/network/mount state without attaching, `jackin hardline --new [--agent <agent>]` starts another foreground agent process inside a selected running instance, and the console shows local active/recoverable instances for the highlighted workspace or current directory with recover, new-session, inspect, and guarded-purge actions.

Exact reconnect to a secondary foreground session is intentionally not part of this item: raw `docker exec -it` sessions cannot be reattached after the terminal disappears. [Console agent session control](/reference/roadmap/console-agent-session-control/) owns the named session backend that will make exact per-session attach/reconnect possible.
...
- Routine restore/operator actions do not require copying raw Docker container names; hardline, inspect, new foreground agent, eject, purge, and console recovery flows are exposed through jackin-owned CLI/TUI surfaces. Shell entry and reconnectable secondary sessions are tracked under [Console agent session control](/reference/roadmap/console-agent-session-control/).
- The system clearly reports that container-writable-layer changes are not restored when Docker containers were deleted, including ad-hoc package installs, global files outside mounted paths, and DinD images.
...
This feature writes only under jackin-managed state directories and creates/removes jackin-managed Docker resources during normal launch, restore, and purge flows. It must not mutate host repositories except through the existing explicit per-mount isolation and finalization flows. Restore remains limited to host-backed state; Docker writable-layer reconstruction is out of scope.
...
- <RepoFile path="src/app/mod.rs" /> — console and CLI dispatch for hardline, inspect, new-session, and guarded-purge instance actions.
- <RepoFile path="src/console/manager/input/list.rs" /> — workspace/current-directory instance shortcuts.
- <RepoFile path="src/console/manager/render/list.rs" /> — workspace/current-directory detail panes, including local active-instance visibility and action hints.
...
- <RepoFile path="tests/dind_e2e.rs" /> — DIND env/connectivity coverage plus Java Testcontainers child-container smoke coverage.
