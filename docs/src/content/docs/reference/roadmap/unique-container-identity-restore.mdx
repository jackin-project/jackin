...
Status: implemented for new launches. Legacy exact-name operations still understand deterministic role names while new `jackin load` invocations claim DNS-safe random instance bases, and the DinD e2e smoke now runs Java Testcontainers from inside the launched role.
