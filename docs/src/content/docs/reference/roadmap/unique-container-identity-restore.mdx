...
Status: partially implemented for CLI targeting and exact missing-instance rebuilds. Bare `jackin hardline` can resolve indexed workspace/ad-hoc instances from the current directory and shows candidate prompts with workspace, role, agent runtime, lifecycle status, Docker state, and mount-state summary. When the target came from the current directory rather than an explicit selector, hardline prompts for reconnect/recover, new foreground agent session, inspect, or cancel. Exact `hardline` targets rebuild recoverable missing instances through the launch restore path with the recorded role source/ref, exact ad-hoc hardline restores prompt before using the current directory as a moved project path, stopped/missing DIND sidecars are restored before crashed role containers, explicit `hardline`, `eject`, and `purge` selectors can target an indexed instance ID, plain `purge` refuses local-state deletion while matching Docker resources still exist, `jackin hardline --inspect` prints read-only manifest/Docker/DinD/network/mount state without attaching, `jackin hardline --new [--agent <agent>]` starts another foreground agent process inside a selected running instance, and the console shows local active/recoverable instances for the highlighted workspace or current directory with recover, new-session, inspect, and guarded-purge actions. Richer prompts across existing agent sessions remain open.

- Expand `jackin hardline` with richer prompts across existing agent sessions.
