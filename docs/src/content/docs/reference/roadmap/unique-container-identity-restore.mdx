...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests, the rebuildable instance index, first-pass missing-container detection, explicit deleted-container writable-layer loss reporting, instance-ID targeting for hardline/eject/purge, bare `hardline` lookup for indexed workspace/ad-hoc instances, and legacy isolation-state manifest backfill have shipped; hardline recovery UI and stronger restore guarantees remain open (Infrastructure)
