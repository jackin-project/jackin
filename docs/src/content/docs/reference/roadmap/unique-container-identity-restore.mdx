...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests, the rebuildable instance index, first-pass missing-container detection, Docker-unavailable diagnostics, explicit deleted-container writable-layer loss reporting, instance-ID targeting for hardline/eject/purge, bare `hardline` lookup for indexed workspace/ad-hoc instances, Java Testcontainers DinD smoke coverage, and legacy isolation-state manifest backfill have shipped; hardline recovery UI and stronger restore guarantees remain open (Infrastructure)
...
Status: partially implemented for CLI targeting. Bare `jackin hardline` can resolve indexed workspace/ad-hoc instances from the current directory, and explicit `hardline`, `eject`, and `purge` selectors can target an indexed instance ID. Interactive recovery pickers, agent-session selection, and console panels remain open.
