...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests with last attach outcomes, the rebuildable instance index, first-pass missing-container detection, Docker-unavailable diagnostics, explicit deleted-container writable-layer loss reporting, metadata-rich load/hardline recovery prompts, cross-role load blocking/guidance for unfinished workspace instances, instance-ID targeting for hardline/eject/purge, purged lifecycle tombstones, plain-purge refusal while matching Docker resources still exist, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref and per-instance durable agent home, stopped/missing-DinD hardline recovery for existing crashed role containers, interactive moved-directory recovery for exact ad-hoc hardline restores with current-directory or entered-path selection, read-only `hardline --inspect` state reports with live agent-session inventory, `hardline --new` foreground agent spawning inside running instances, a directory-aware hardline action picker with live session counts, console active-instance recovery controls backed by local state, Java Testcontainers DinD smoke coverage, and legacy isolation-state manifest backfill have shipped; exact per-session reattach prompts, visual moved-path browser recovery, and future persistent lifecycle storage remain open. Container writable-layer recovery is explicitly out of scope (Infrastructure)
