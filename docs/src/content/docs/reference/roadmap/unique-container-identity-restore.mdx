...
Status: partially implemented for CLI targeting and exact missing-instance rebuilds. Bare `jackin hardline` can resolve indexed workspace/ad-hoc instances from the current directory and shows candidate prompts with workspace, role, agent runtime, lifecycle status, Docker state, and mount-state summary. Exact `hardline` targets rebuild recoverable missing instances through the launch restore path with the recorded role source/ref, stopped/missing DIND sidecars are restored before crashed role containers, explicit `hardline`, `eject`, and `purge` selectors can target an indexed instance ID, plain `purge` refuses local-state deletion while matching Docker resources still exist, `jackin hardline --inspect` prints read-only manifest/Docker/DinD/network/mount state without attaching, and the console shows local active/recoverable instances for the highlighted workspace or current directory. Full hardline recovery actions, agent-session selection, and richer console controls remain open.
