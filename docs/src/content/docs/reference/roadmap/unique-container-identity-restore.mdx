...
**Status**: Partially implemented — DNS-safe unique names, per-instance manifests with last attach outcomes, the rebuildable instance index, first-pass missing-container detection, Docker-unavailable diagnostics, explicit deleted-container writable-layer loss reporting, instance-ID targeting for hardline/eject/purge, bare `hardline` lookup plus metadata-rich prompts for indexed workspace/ad-hoc instances, exact `hardline` rebuild for missing recoverable instances using the recorded role source/ref, stopped/missing-DinD hardline recovery for existing crashed role containers, a console active-instance panel backed by local state, Java Testcontainers DinD smoke coverage, and legacy isolation-state manifest backfill have shipped; full hardline recovery actions, richer console recovery controls, and stronger restore guarantees remain open (Infrastructure)
