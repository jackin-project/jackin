...
Extend the instance manifest from [Unique container identity and restore](/reference/roadmap/unique-container-identity-restore/) with session records, or store them in the future persistent storage layer. The manifest already carries `instance_id`, `container_base`, `workspace_name`, `workdir`, `role_key`, `agent_runtime`, and `status`. Session records add a `sessions` array alongside the existing top-level fields.
