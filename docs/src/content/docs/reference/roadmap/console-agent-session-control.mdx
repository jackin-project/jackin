...
**Status**: Partially implemented — Phase 1 instance discovery and console Instances panel shipped with unique container identity and restore; shell entry and reconnectable tmux-backed session substrate are Phase 2 (open — design proposal, [Agent Orchestrator Research Program](/reference/roadmap/agent-orchestrator-research/))
...
- Add `tmux` to the construct image (<RepoFile path="docker/construct/Dockerfile" />, `apt-get install tmux`).
...
- Change `load_role_with` in <RepoFile path="src/runtime/launch.rs" /> to start the container detached (`-d`) with the supervisor as CMD, then immediately run `docker exec -it <container> tmux new-session -s jackin-primary /jackin/runtime/entrypoint.sh [agent-args]` to give the operator the agent TTY.
- Change `hardline_agent` in <RepoFile path="src/runtime/attach.rs" /> to attach via `docker exec -it <container> tmux attach-session -t <session-name>` instead of `docker attach`.
...
- Update `inspect_agent_sessions` in <RepoFile path="src/runtime/attach.rs" /> to query `tmux list-sessions` inside the container instead of parsing `docker top` output.
