...
**Status**: Partially implemented — Phase 1 (runtime foundation), Phase 2 (Codex baseline), and Phase 3 (Amp baseline) shipped; Phase 4 (operator UX) partially shipped (`--agent` selector, last-agent memory, console agent picker); Phase 5 (parity work) not started
...
- <RepoFile path="src/derived_image.rs" /> installs Claude plugins in the generated image layer.
- <RepoFile path="docker/runtime/entrypoint.sh" /> registers Claude MCP servers, then launches `claude`.
