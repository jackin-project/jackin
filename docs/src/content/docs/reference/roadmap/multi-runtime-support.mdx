...
<Aside type="note">
The foundation slice shipped under **harness** terminology. This roadmap preserves the original "runtime" wording for historical context; current code and docs use `harness` for the AI CLI selected at launch.
</Aside>

## Problem

jackin' already solves several operator problems in a runtime-independent way: roles as Dockerfiles, named workspaces, scoped mounts, isolated container lifecycles, and per-agent persisted state. But the actual runtime contract is still Claude-specific end to end.
...
- The main value of jackin' for local development is not Claude itself. It is the isolation, mount scoping, reusable roles, and multi-agent workflow.
...
- <RepoFile path="src/manifest/mod.rs" /> requires a `[claude]` table.
- <RepoFile path="src/derived_image.rs" /> appends `curl -fsSL https://claude.ai/install.sh | bash` and bakes in a Claude entrypoint.
- <RepoFile path="docker/runtime/entrypoint.sh" /> installs Claude plugins, registers Claude MCP servers, then launches `claude`.
- <RepoFile path="src/instance/auth.rs" /> provisions only Claude auth state (`.claude.json`, `.claude/.credentials.json`).
- <RepoFile path="src/instance/plugins.rs" /> serializes only Claude marketplace/plugin data.
- <RepoFile path="src/runtime/launch.rs" /> mounts only Claude state files and prints Claude-specific auth diagnostics.
- <RepoFile path="src/runtime/image.rs" /> and <RepoFile path="src/version_check.rs" /> only know how to detect and refresh Claude versions.

### Existing org role repos
...
The important observation is that both role repos already separate shared toolchain concerns from runtime concerns fairly well:

- the Dockerfile defines the toolchain
- identity and env prompts are generic
- only the `[claude]` table is truly Claude-specific

That argues for keeping one shared role repo per role, not creating separate `*-codex` or `*-amp` repo forks.
...
- one role repo defines the environment layer
...
The first successful version should let an operator do this with the same role:
...
- <RepoFile path="src/manifest/mod.rs" />
- <RepoFile path="src/manifest/validate.rs" />
- <RepoFile path="src/derived_image.rs" />
- <RepoFile path="docker/runtime/entrypoint.sh" />
- <RepoFile path="src/instance/mod.rs" />
- <RepoFile path="src/instance/auth.rs" />
- <RepoFile path="src/instance/plugins.rs" />
- <RepoFile path="src/runtime/launch.rs" />
- <RepoFile path="src/runtime/image.rs" />
- <RepoFile path="src/version_check.rs" />
- <RepoFile path="src/runtime/naming.rs" />
- <RepoFile path="src/app/context.rs" />
...
- generated Dockerfiles in <RepoFile path="src/derived_image.rs" />
- entrypoint and runtime helper scripts under `docker/runtime/`
- mount destinations in <RepoFile path="src/runtime/launch.rs" />
- docs examples and built-in mount defaults such as Gradle caches
- first-party role repos that occasionally reference `/home/claude`
...
- generic plugin or skill installation in `jackin.role.toml`
...
- Which existing Claude-only env names should be generalized immediately, especially `JACKIN`?
...
- <RepoFile path="src/runtime/image.rs" />
- <RepoFile path="src/version_check.rs" />
- <RepoFile path="docs/src/content/docs/developing/role-manifest.mdx" />
- <RepoFile path="docs/src/content/docs/developing/creating-roles.mdx" />
- <RepoFile path="docs/src/content/docs/guides/authentication.mdx" />
- <RepoFile path="docs/src/content/docs/reference/architecture.mdx" />
