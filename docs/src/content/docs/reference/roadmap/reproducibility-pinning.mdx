...
- `src/config/agents.rs` — `resolve_agent_source()`; `AgentSource` (defined in `src/config/mod.rs`) would need version/commit fields
- `src/runtime/repo_cache.rs` — `resolve_agent_repo`, repo checkout logic; calls into config layer to resolve source
- `src/manifest/mod.rs` — version/provenance metadata
