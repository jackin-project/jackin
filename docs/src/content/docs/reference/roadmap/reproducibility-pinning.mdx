...
Per the project's pre-release schema rule (see <RepoFile path="AGENTS.md" />), non-additive changes to a serde-bearing config struct require a version bump and a migration registry entry. Purely additive changes — new `Option<T>` fields with `serde(default)` — do not. The `commit` and `version` fields proposed here are additive `Option<String>` with defaults, so no version bump is required. If the implementation later reshapes the change to be non-additive (e.g. making the fields required, renaming them, or restructuring the table), the version bump, migration step, `from-v1alpha2/` fixture, and re-baked existing fixtures become required at that point.
...
- A timeline entry on the schema-versions reference page only if the change becomes non-additive and requires a version bump.
