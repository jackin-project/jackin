...
The binary is named `jackin-capsule` and lives at `/jackin/runtime/jackin-capsule` inside every Capsule-managed role container. It replaces <RepoFile path="docker/runtime/supervisor.sh" /> as PID 1.
...
- The derived image build pipeline that downloads or builds `jackin-capsule` and installs it at `/jackin/runtime/jackin-capsule`. <RepoFile path="src/bin/build_jackin_capsule.rs" /> stays as-is.
