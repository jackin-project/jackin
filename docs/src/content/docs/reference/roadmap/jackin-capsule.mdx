...
- Top chrome: optional compact quota glyph for the focused account, e.g. `Claude 36% left`, `Codex stale`, or `Amp login`. This should be short enough not to crowd tabs.
...
This keeps the multiplexer useful while the operator is focused inside an agent: before starting an expensive run, they can check whether the focused account still has enough subscription headroom and how much the current workspace has already burned.
