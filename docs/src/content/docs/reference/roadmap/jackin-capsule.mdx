...
- **Phase 3d — Resize, mouse, terminal compatibility, status bar polish.** Add the client-side `SIGWINCH` listener and `resize` frames. Fix the mouse passthrough no-op. Wire focus events, OSC/APC passthrough rules, BSU/ESU, kitty-keyboard pass-through, and OSC 52 routing per "Terminal compatibility: tmux setting parity" and "Ghostty compatibility" above. Reassert a multiplexer-owned outer terminal title from the workspace and branch / pull request context. Fix the status bar tab-label width drift. Keep command-palette shortcuts active without rendering a top-bar hint.
