...
Phase 4 should add a read-only usage overlay fed by the host daemon's [Token & Cost Telemetry](/reference/roadmap/token-cost-telemetry/) cache. The Capsule does not poll providers and does not persist credentials. It asks the daemon for the focused pane's account quota and workspace/session spend, then renders the result inside the multiplexer.

Default shape:

- Top chrome: compact always-visible quota glyph for the focused account, e.g. `Claude 80%`, `Codex 12%`, `Codex stale`, or `Amp login`. This should be short enough not to crowd tabs and should prioritize warning states over decorative detail.
- Prefix `u` / palette `Usage`: modal dialog showing the focused agent's provider, account label, quota window, remaining percentage, reset countdown, source confidence, cache freshness, current session tokens/cost, workspace tokens/cost, and burn-rate projection when available.
...
This keeps the multiplexer useful while the operator is focused inside an agent: before starting an expensive run, they can see the account headroom in the chrome, then open the dialog only when they need reset timing, confidence, or workspace spend detail.
