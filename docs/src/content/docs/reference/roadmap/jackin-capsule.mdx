...
**jackin' equivalent.** The client sets the outer terminal to "any-event tracking + SGR mouse" (`\x1b[?1003h\x1b[?1006h`) on attach. The multiplexer parses each mouse event: top-chrome events hit-test against the tab strip / identity row, pane-border events drive resize, and pane-content events are re-encoded relative to the focused pane's rect origin and written to that pane's PTY only when the pane opted into mouse reporting. Wheel events in mouse-disabled normal-screen panes drive jackin' scrollback when vt100 has retained history. Normal-screen transcript TUIs that keep their own history get cursor-key fallback and scroll chrome from the same focused-pane screen heuristics whether the pane was spawned as an agent or shell. Wheel events in mouse-disabled alternate-screen panes use cursor-key fallback so full-screen programs keep owning their interaction surface. Mouse mode and encoding are tracked with `vt100::Screen::mouse_protocol_mode()` and `mouse_protocol_encoding()`, so SGR, default xterm/X10, and UTF-8 mouse modes are distinct on the PTY side.
