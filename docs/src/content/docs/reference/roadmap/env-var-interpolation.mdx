...
- `src/manifest/mod.rs` — `EnvVarDecl` schema
- `src/manifest/validate.rs` — env var declaration parsing and interpolation validation
- `src/env_model.rs` — `extract_interpolation_refs`, `topological_env_order`, reserved-env list
- `src/env_resolver.rs` — launch-time env var resolution with `${env.VAR}` interpolation
- `src/config/mod.rs` — operator config types (`AppConfig`, `AgentSource`)
- `src/workspace/mod.rs` — workspace-level overrides (`WorkspaceConfig`)
