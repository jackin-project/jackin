...
- `used.unit == limit.unit` whenever both are `Some` (renderers compare amounts).
- `limit.is_some() ⇒ used.is_some()` (knowing the cap without the current consumption is not a real provider state; the converse is fine — `used` alone is meaningful).
- `last_error.is_some() ⇔ status ∈ { UsageSnapshotStatus::Error, UsageSnapshotStatus::Stale }` (and only when the stale state came from a failed refresh).
...
`UsageAmount` packs `{ amount, unit }` so SQL stores it as `<field>_amount INTEGER` + `<field>_unit TEXT` column pairs. `window: UsageWindow` serializes as `window_kind TEXT`. `UsageRefreshError` serializes to `last_error TEXT` as JSON. `account_key_hash` is a write-time hash of the provider-stable account identity (OAuth `sub`, provider user id, or salted hash of the canonical login email — **not** the editable `account_label` display string); it lives in the SQL row but not in the value struct, so consumers cannot stamp inconsistent hashes and a label rename does not produce a duplicate snapshot row.
