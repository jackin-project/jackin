...
- **Capsule multiplexer quota signal.** Inside `jackin-capsule`, the top chrome shows a compact always-visible quota signal for the focused agent, e.g. `Claude 80%`, `Codex 12%`, `Amp login`, or `stale`. Prefix `u` (or a palette command) opens a read-only dialog with account availability, reset countdown, today/session/workspace tokens, burn rate, and cache freshness. Detailed numbers belong in the dialog; the chrome signal exists so the operator can see “safe / low / unavailable” without leaving the agent.
...
- **Always visible:** macOS status item, Desktop running-agent rows, `jackin console` workspace/agent rows, and Capsule top chrome all show a compact quota signal for the active or most constrained account.
...
- Compact quota badges on Desktop running-agent rows, `jackin console` workspace/agent rows, macOS status item, and Capsule top chrome.
