...
Per-instance DB rows mirror the value fields; the SQL row adds a surrogate `id INTEGER PRIMARY KEY` and stores `at` as `occurred_at INTEGER` (epoch seconds). `instance` is implicit in the DB file path and is not stored per-row.
...
pub struct AccountUsageSnapshot {
    ...
    pub used: Option<UsageAmount>,        // {amount, unit} pair
    pub limit: Option<UsageAmount>,       // {amount, unit} pair
...
`confidence` answers "how much do we trust this number?" — provenance. `status` answers "is this snapshot useful right now?" — lifecycle. The two axes are intentionally separate; a snapshot can be `provider_authoritative` + `stale` (we have a trusted number that's past TTL) or `local_estimate` + `fresh` (best-effort number we just computed).

`UsageAmount` packs `{ amount: u64, unit: &str }` (e.g. `tokens`, `messages`, `requests`, `usd_micros`) so SQL stores it as `<field>_amount INTEGER` + `<field>_unit TEXT` column pairs. `UsageRefreshError` carries `{ kind: ErrorKind, at: SystemTime, message: String }` and serializes to `last_error TEXT` as JSON. `account_key_hash` is a write-time hash of `(provider, account_label-after-normalization)`; it lives in the SQL row but not in the value struct (the daemon derives it on write so consumers cannot stamp inconsistent hashes).
...
Claude and Codex ship first; Amp/Kimi/OpenCode rows render with `unsupported`, `needs_login`, or local-estimate state until their source contracts are proven.
...
- `usage.accounts.refresh { provider?, account? }`: schedules a refresh and returns the last cached snapshot immediately; does not block the UI on provider network calls.
...
7. **Capsule focused signal.** Once the daemon can map Capsule `session_id` to provider/account, add the always-visible compact quota signal to the status chrome and the read-only modal behind prefix `u`. Rendering contract lives in [jackin' Capsule → Usage and quota overlay](/reference/roadmap/jackin-capsule/#usage-and-quota-overlay).
...
- **Confidence and status visible:** every rendered number carries two labels — confidence (`provider-authoritative`, `local estimate`, `fallback`) for provenance, and status (`fresh`, `stale`, `needs login`, `unsupported`, `error`) for lifecycle. `Kimi unsupported` beats a missing badge. A `provider-authoritative` + `stale` snapshot is not healthier than a `local estimate` + `fresh` one; render both axes.
...
- **macOS status bar.** Most-constrained active account by default: provider glyph plus remaining percent or `stale`/`login` state. Menu expands to all accounts with reset timers, source labels, and repair actions.
...
- **Capsule multiplexer.** Focused-tab provider only — see [jackin' Capsule → Usage and quota overlay](/reference/roadmap/jackin-capsule/#usage-and-quota-overlay) for the rendering contract (status-line layout, prefix `u`, modal contents, daemon-disconnected and needs-login fallbacks).
...
Each adapter consumes the runtime's structured output (same input stream as the [agent runtime status](/reference/roadmap/agent-runtime-status/) and [tag protocol](/reference/roadmap/agent-tag-protocol/) parsers), resolves source priorities from the matrix above, and emits `UsageSample` for per-message attribution plus `AccountUsageSnapshot` for account quota. The adapter knows its runtime; the consumer doesn't.

Hidden constraints not captured in the matrix:

- **Claude:** stream-json chunks are cumulative — dedup by message/request identity before emitting `UsageSample` or token totals double-count.
...
- Capsule multiplexer quota signal — Capsule's rendering contract lives in [jackin' Capsule → Usage and quota overlay](/reference/roadmap/jackin-capsule/#usage-and-quota-overlay).
