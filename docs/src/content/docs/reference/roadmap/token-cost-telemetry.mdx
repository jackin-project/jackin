...
- **macOS status bar.** Shows the most constrained active account by default: provider glyph/name plus remaining percentage or “stale/login” state. The menu expands to all accounts with reset timers, confidence/source labels, and repair actions. Operators can choose a compact mode, but the default must make the quota risk visible without opening a browser.
- **`jackin console` account screen.** A new Accounts / Usage view lists every known account, provider, source, current availability, reset time, last refresh, and which workspaces currently use it. It also links to workspace spend summaries and exposes authentication repair actions.
- **Capsule multiplexer quota drawer.** Inside `jackin-capsule`, prefix `u` (or a palette command) opens a read-only dialog for the focused agent: account availability, reset countdown, today/session/workspace tokens, burn rate, and cache freshness. The top chrome can show a compact quota glyph for the active account, but detailed numbers belong in the dialog to avoid noisy chrome.
...
- Capsule multiplexer quota dialog for the focused agent.
