...
Per-instance DB rows mirror these fields directly; `instance` is implicit in the DB file path and is not stored per-row.
...
- **Source and freshness visible:** label every number by source (`provider-authoritative`, `local estimate`, `stale`, `needs login`, `unsupported`). `Kimi unsupported` beats a missing badge. A stale provider-authoritative number is not healthier than a fresh local estimate.
...
The contract must answer, for the focused agent: which Claude/Codex/Amp/Kimi/OpenCode accounts jackin' knows about, which one this agent is using, how much quota remains, when it resets, how much this workspace/session burned today/this week/lifetime, and whether the displayed limit is provider-authoritative, locally estimated, stale, or unavailable.
