...
6. **Console and Desktop surfaces.** Add compact badges, then account/detail panels. Badges must carry both axes from the Surface contract — confidence (`provider_authoritative` / `local_estimate` / `fallback`) and status (`fresh` / `stale` / `needs_login` / `unsupported` / `error`) — before shipping percentage-only displays.
...
- **Confidence and status visible:** every rendered number carries two labels — confidence (`provider_authoritative`, `local_estimate`, `fallback`) for provenance, and status (`fresh`, `stale`, `needs_login`, `unsupported`, `error`) for lifecycle. `Kimi unsupported` beats a missing badge. A `provider_authoritative` + `stale` snapshot is not healthier than a `local_estimate` + `fresh` one; render both axes.
