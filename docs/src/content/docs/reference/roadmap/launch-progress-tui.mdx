...
**Status**: Partially implemented — core runtime/TUI shipped in PR #452; remaining work is hardening and richer diagnostics tooling

PR #452 implemented the shared launch progress spine: run IDs for every command, JSONL run diagnostics, boundary rain gating, structured launch stages, compact/rich/test renderers, console-triggered launch reuse, `--no-rain`, `--no-tui`, and `JACKIN_NO_MOTION=1`. The first implementation intentionally does not add a `jackin diagnostics` command. Remaining follow-up work is to harden the rich renderer with broader snapshot coverage, expose a diagnostics viewer once the JSONL shape settles, and add deeper startup parallelism beyond the initial independent preflight overlap.
...
The existing no-intro concept should become an explicit no-rain control. The user-facing flag should be `--no-rain`; internally, avoid carrying `no_intro` naming forward. `--no-rain` disables boundary rain only. It does not disable the rich launch TUI.
...
| Compact renderer | TTY without rich support or explicit `--no-tui` mode | Same events as short line output. `--no-tui` disables the rich launch renderer, not boundary rain. |
...
- `--no-tui` disables the rich launch progress TUI and keeps compact output; `--no-rain` disables boundary rain only.
