...
The footer acts as the stable status/control strip. During normal launch it can show the log retrieval command and any active non-destructive key. During fatal failure it shows only `Enter Close` plus the log retrieval command. Do not put key hints inside the main panel or inside error dialogs.
