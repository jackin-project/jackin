...
- **Older-stamped manifest uses a newer feature**: jackin refuses to use the role and emits a feature-specific error with `jackin role migrate <role-repo-path>`. For example, `kimi` and `opencode` require `v1alpha3`; a `v1alpha2` manifest that declares either agent must be migrated before launch.
