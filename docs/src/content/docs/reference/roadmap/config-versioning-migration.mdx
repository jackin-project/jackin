...
The `--pr` mode runs from an operator's machine. The remaining gap is the third-party role-repo case: the role author has not noticed the schema bump, has not run `jackin-validate --migrate`, and the manifest fails validation in CI until someone opens a PR by hand. The fix is a Renovate-style automation that watches role repos and opens a migration PR when their `jackin.role.toml` falls behind the current schema.

The action extends [`validate-agent-action`](https://github.com/jackin-project/validate-agent-action) with a `migrate` mode (rather than introducing a new repo — role authors install one action). Two operating modes mirror Renovate's deployment shapes:

1. **Self-hosted GitHub App** ("Mend Renovate" equivalent). Install the jackin GitHub App on the org or per-repo. The app holds `pull_requests:write` + `contents:write` on the role repos it watches and runs the migration on a cron from a central runner. Identity for the PR is the app bot, parallel to `renovate[bot]`. Operators do not manage tokens.

2. **Workflow-in-repo with PAT or `GITHUB_TOKEN`** ("self-hosted Renovate" / `renovate-bot/renovate-action` equivalent). Role authors add a workflow to their own repo that calls the action on a schedule (`on: schedule: - cron: "0 6 * * *"`) and on `workflow_dispatch`. The workflow uses `secrets.GITHUB_TOKEN` (which can open PRs from the same repo) or an operator-supplied PAT for cross-repo cases. Operators manage the token themselves but skip the app registration.

Both modes share the same internal flow:

1. Check out the role repo at the default branch.
2. Run `jackin-validate <repo>` to detect whether the manifest is already current.
3. If `--migrate` would change the file, switch to a deterministic branch name (e.g. `jackin/schema-migration/v1alpha1-to-v1alpha2`) and run `jackin-validate --migrate <repo>`.
...
6. Close the PR (and delete the branch) once the role manifest is current — guards against zombie PRs after the maintainer hand-merges or hand-migrates.

Open design questions for the Phase 4 implementation PR:

- **App vs PAT default in docs.** Renovate's default-recommended path is the hosted app; jackin should pick one as the documented default and call out the other as fallback.
...
- **Branch lifecycle.** When a role author rebases, force-pushes, or hand-edits the migration PR, the action must detect drift and either rebase its own commit or stop (Renovate calls this "rebase strategy" — it has multiple modes; jackin should pick one).
- **Conflict handling.** If a role author has already partially migrated, the action's migration step may produce a no-op diff. The action should detect that case and close the PR rather than open an empty one.

This phase is **not implemented in the current branch.** It lands in a separate PR after Phase 1/2 stabilize. Tracked here so the design is reviewable before code lands.
