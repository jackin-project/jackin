...
Before this work, neither class had a `version` field. Jackin could not distinguish "file was never created" from "file predates the current schema" from "file was written by a newer binary this older binary cannot read."
...
The `jackin validate --migrate <role>` subcommand applies the manifest migration chain to the local role clone and writes the result back. The operator can then inspect the diff, commit, and push.
...
- [Configuration File](/reference/configuration/) — internals reference; `version` and migration behavior are documented here once Phase 1 ships.
