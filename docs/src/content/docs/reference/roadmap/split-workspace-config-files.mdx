...
The questions below were settled at ship time and are kept as historical context; the shipped behaviour matches the proposed answers (filename validation, empty-dir handling, and the `.toml` extension filter all live in <RepoFile path="src/config/persist.rs" />).
...
- <RepoFile path="src/config/persist.rs" /> — `AppConfig::load_or_init`, `load_workspace_files`, and the `workspaces/<name>.toml` writer.
- <RepoFile path="src/config/editor.rs" /> — `ConfigEditor::save` (per-workspace writes).
