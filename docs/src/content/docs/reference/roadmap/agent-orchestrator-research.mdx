...
| [`ogulcancelik/herdr`](https://github.com/ogulcancelik/herdr) | Two-stage done state (Done vs Idle), workspace-level status roll-up, notification suppression when tab is focused, sound-escalation opt-in, PTY inference heuristic for zero-config status detection, blocking `wait` semantics on the socket API | AGPL-3.0 (embedding conflicts with Jackin's Apache 2.0 license); PTY heuristics degrade when the observed terminal is `docker attach` rather than the agent process; no container isolation, mount model, or auth forwarding | See [Herdr research](/reference/roadmap/herdr-research/) |
