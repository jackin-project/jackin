...
| Host-native worktrees as isolation unit | **No** | The core isolation model is the reason jackin uses containers. Host-native agents run as the host user; jackin's hard rule against host-side mutations blocks this as a default |
| Same-process agent and IDE surface | **No** | Orca runs agents directly on the host machine. Jackin's container model is the reason to exist |
...
The workflow of marking up diff lines with comments and sending the batch to the agent as structured feedback is pure UX — it does not depend on where the agent runs. Jackin's [operator handler system](/reference/roadmap/operator-handler-system/) and the console's session control surface are the right home for this. A future operator action could batch annotated diff hunks into a follow-up agent prompt inside the same container.

### Worktree-session visual table

Orca's main view is a table of worktrees with live agent status. Jackin's console workspace tree is the analog. The gap is status richness: Orca shows active/interrupted/blocked; jackin shows limited instance state. This is exactly the [agent runtime status](/reference/roadmap/agent-runtime-status/) gap.
...
Several of these gaps — subagent visibility, permission-stall detection — are exactly what jackin's container model can solve cleanly because the container boundary gives jackin a uniform observation point for agent lifecycle events.

## Why not copy Orca directly

Orca's core bet is that worktrees are sufficient isolation. That works for a developer who trusts their agents and wants parallel throughput on one machine. It does not work for jackin's model where:

- The operator's host machine must not be mutated silently (hard rule in <RepoFile path="AGENTS.md" />)
...
3. **Subagent visibility.** How does Orca's hidden-tmux-session problem map to jackin's container model? Jackin can observe container process trees; is that sufficient to detect and surface subagent sessions?
...
5. **Self-hosted relay.** Orca's mobile-companion relay is a separate infrastructure concern. Does jackin's daemon design (see [jackin daemon](/reference/roadmap/jackin-daemon/)) make self-hosted relay a natural Phase 2 feature or an unrelated problem?
...
| Subagent visibility | [jackin-container: in-container supervisor binary](/reference/roadmap/jackin-container-binary/) |
