...
- `src/config/mod.rs` — config format would need secret references
- `src/runtime/launch.rs` — launch-time resolution
- `src/workspace/mod.rs` — workspace config integration (`WorkspaceConfig`)
