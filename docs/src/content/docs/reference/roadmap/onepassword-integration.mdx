...
1. **1Password CLI passthrough**: Install or expose the `op` CLI in roles and let operators authenticate inside the container. Simple and flexible, but pushes too much setup onto each agent session.
...
- <RepoFile path="src/config/mod.rs" /> — config format would need secret references
- <RepoFile path="src/runtime/launch.rs" /> — launch-time resolution
- <RepoFile path="src/workspace/mod.rs" /> — workspace config integration (`WorkspaceConfig`)
