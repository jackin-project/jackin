...
The risk is that `j❯` alone is quiet as a static billboard. The implementation should pair it with a stronger standalone logo treatment for web and README surfaces, then make sure that standalone logo visibly reduces to the same `j❯` prompt in the terminal.
...
The exact website composition is still open. The invariant is that the large visual mark, the README hero, the favicon, and the shell prompt must be recognizably the same object at different levels of detail.
...
| **C. Hardline connector** | `j` shaped as a cable/line feeding into `❯` | README hero, app icon, sticker | Ties the Matrix vocabulary to a concrete connection image | Could become generic cable/plug tech branding if not constrained |
...
- The proposed rebrand retires the trailing-apostrophe wordmark. The implementation PR must update the agent-only spelling rule in <RepoFile path="AGENTS.md" /> at the same time it updates public docs and assets, so the repository does not mix old and new brand rules indefinitely.
...
The shared Clap `--help` banner should use frozen brand rain, not animation. The rain field should be built from the brand vocabulary (`j`, `a`, `c`, `k`, `i`, `n`, `❯`, and the box/block drawing glyphs the CLI already uses) and should frame the `jackin❯` / `by tailrocks` lockup without overwhelming the command list. This is the static equivalent of the interactive digital rain already used in TUI launch/exit moments: recognizably jackin❯, but still copyable terminal text.
...
Website and docs work should replace the old apostrophe wordmark with the proposed wordmark and tokenized brand colors in one coordinated PR. The current docs title override in <RepoFile path="docs/src/components/overrides/SiteTitle.astro" /> and theme tokens in <RepoFile path="docs/src/styles/docs-theme.css" /> are likely first-touch files.
...
The existing public assets under <RepoFile path="docs/public/favicon.svg" />, <RepoFile path="docs/public/apple-touch-icon.png" />, <RepoFile path="docs/public/icon-192.png" />, <RepoFile path="docs/public/icon-512.png" />, <RepoFile path="docs/public/og-image.png" />, and <RepoFile path="docs/public/og-image-github.png" /> should be regenerated together once the spec is accepted.
...
After the rebrand lands, prose should use the new wordmark where rich text reliably renders `❯`; plaintext-only contexts may use `jackin>`. Until that implementation PR updates the repository's agent rules, contributors should continue following the current spelling rule in <RepoFile path="AGENTS.md" /> outside this roadmap proposal.
...
2. Choose the system: decide which direction owns the standalone website/icon mark, which owns the prompt, and which owns environmental treatments such as frozen rain.
3. Freeze the final spec: confirm the mark, chevron character, colors, typography, prohibited treatments, prompt shape, byline lockup, and asset list.
4. Update repository rules and published docs together: agent spelling policy, README, docs navigation, landing page, public docs references, and any user-facing pages that describe the product name or terminal output.
5. Run the global brand text sweep: find every prose occurrence of `jackin'`, `Jackin`, `Jackin'`, and `JACKIN` and replace product/project mentions with `jackin❯`. Use `jackin>` only after confirming the target surface cannot render `❯`. Do not rewrite literal commands, code identifiers, paths, package names, env vars, URLs, or examples where the operator must type `jackin`.
6. Replace docs/site visual assets: Starlight title, landing hero, README hero banner, favicons, app/PWA icons, Open Graph card, and any social-card generation code.
7. Update CLI and terminal output: frozen `--help` banner, version/splash treatment, jackin-owned status prefixes, Capsule chrome, prompt setup inside role containers, and tests for ANSI color output where practical.
8. Update package/distribution surfaces: Homebrew metadata, release artifacts, GitHub social preview, and any future desktop/mobile icon source files.
9. Sweep old assets and screenshots: remove or replace apostrophe-era images, stale docs snippets, and vendor-facing material. Do not leave both brands active except inside historical changelog/release context after the first release.

## Host-Side Effects

Prompt and shell branding are subject to the host-mutation rule in <RepoFile path="AGENTS.md" />. The implementation must not silently edit the operator's host shell files, dotfiles, terminal profile, Git config, repository metadata, or GitHub CLI config. Prompt changes belong inside role containers, jackin-owned runtime scaffolding, or an explicitly surfaced operator opt-in.
...
- <RepoFile path="AGENTS.md" /> — current agent-only spelling rule that must change with the rebrand implementation
- <RepoFile path="README.md" /> — public repository entry point
- <RepoFile path="docs/src/pages/index.astro" /> — landing page
- <RepoFile path="docs/src/components/overrides/SiteTitle.astro" /> — docs header wordmark
- <RepoFile path="docs/src/styles/docs-theme.css" /> — docs color and font tokens
- <RepoFile path="src/tui/output.rs" /> — CLI output helpers and terminal title
- <RepoFile path="src/tui/animation.rs" /> — current TUI logo animation
- <RepoFile path="docker/runtime/entrypoint.sh" /> — role-container shell/runtime setup surface
