...
**Status**: Partially implemented — CLI setup / rotate / revoke / doctor, 1Password write + validation, expiry banner, and host-side PTY OAuth-token onboarding (driven through `claude setup-token` from <RepoFile path="src/host_claude.rs" />) are shipped; canonical `oauth_token` auth slot, TUI generate action, Apple Keychain backend, pre-launch validity probe, and bulk migration remain
