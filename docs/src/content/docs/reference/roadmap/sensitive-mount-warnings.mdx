...
- <RepoFile path="src/workspace/sensitive.rs" /> — sensitive-mount detection (`find_sensitive_mounts`, `confirm_sensitive_mounts`)
- <RepoFile path="src/workspace/mounts.rs" /> — mount validation
- <RepoFile path="src/tui/prompt.rs" /> — `prompt_choice`, `require_interactive_stdin` helpers
