...
| [Rootless DinD](/reference/roadmap/rootless-dind/) | Both are hardening steps that improve the DinD security posture without requiring microVM. Rootless DinD reduces the sidecar's host privilege; process sandboxing reduces per-operation access inside the container. |
