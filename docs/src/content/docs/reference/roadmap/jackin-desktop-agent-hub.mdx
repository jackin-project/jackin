...
now add launch-only Ghostty actions through `workspace/launch` and reconnect
actions through `desktop/open`. Account cards and attention actions remain in
...
- Treat the daemon branch as editable, not fixed. If a required Desktop
  contract belongs in the daemon branch, update that branch directly instead of
  working around a weak daemon API in later Desktop branches.
...
- PR 1: Add the simple native window shell with `NavigationSplitView`.
  Started with a shared-model `WindowGroup`, sidebar destinations, and basic
  Pull Requests, Projects, Workspaces, Running Agents, Accounts, and Settings
  panes.
- PR 2: Add the inbox-style Pull Requests view with organization/project
  filters.
  Started with shared filters for organization, project/repository, and
  draft/open state over the daemon-backed PR list.
- PR 3: Add the Projects view for recently active jackin' repositories.
  Started with repository grouping from the daemon-backed PR list and an
  organization filter.
- PR 4: Add the Workspaces view for saved workspace visibility and launch-only
  actions.
  Started with saved workspace rows, running counts, and Ghostty launch buttons
  through `workspace/launch`.
- PR 5: Add the Running Agents view with per-workspace agent counts and Ghostty
  open/hardline actions.
  Started with daemon-backed session rows and Ghostty hardline buttons through
  `desktop/open`.
