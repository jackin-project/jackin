...
The data model and confidence-labeling rules (`provider-authoritative`, `local estimate`, `stale`, `needs login`, `unsupported`) are the telemetry roadmap's contract; Desktop renders them.
