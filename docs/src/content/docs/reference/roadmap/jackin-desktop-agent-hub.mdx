...
- The proposal is intentionally layered on the roadmap items that motivated it: [jackin' daemon](/reference/roadmap/jackin-daemon/), [Live bidirectional auth sync](/reference/roadmap/live-auth-sync/), [Agent attention prompts](/reference/roadmap/agent-attention-prompts/), and [Host bridge — secrets and approved host actions](/reference/roadmap/host-bridge/). Those documents define the backend responsibilities Desktop should surface rather than reimplement.
...
- OpenUsage is a generic AI subscription tracker. jackin' Desktop should not become a catalog of every AI provider; it should start with the runtimes jackin' launches and accounts the operator explicitly wires into jackin'.
- A public plugin marketplace is premature. jackin' should first prove the typed adapter shape for `claude`, `codex`, `amp`, `kimi`, and `opencode`.
- Provider refresh must not silently write to host-side auth/config state. Any repair action that changes the host has to be explicit and surfaced under the host-side effects rule.
...
- Tokemon — https://github.com/richyparr/tokemon and https://www.tokemon.ai/
- Token Tracker — https://github.com/mm7894215/TokenTracker and https://www.tokentracker.cc/
- Brim — https://getbrim.tech/
- MeterBar — https://github.com/shipshitdev/meterbar.app and https://meterbar.app/
- Control Tower — https://github.com/krishcdbry/ControlTower and https://control-tower.dev/

These are better comparisons than CodexBar for the specific “is the number accurate and understandable?” problem.

Useful ideas:

- Tokemon's source hierarchy: provider/OAuth quota endpoint first for subscription limits; local JSONL logs only as fallback or project-cost attribution.
- Token Tracker's local-first normalization: hooks and passive readers normalize many agent CLIs into a shared token/cost/project schema, plus machine-readable status output.
- Brim's trust posture: narrow scope, native rings for 5-hour and weekly windows, explicit refresh/freshness, and clear separation between local logs and provider quota.
- MeterBar's simple account dashboard and CLI JSON surface.
- Control Tower's multi-account, quiet-hours, usage-history, and alert concepts, with maturity treated as lower confidence.

Product rule:

- Desktop must label quota data by confidence: **provider-authoritative**, **local estimate**, **stale**, **needs login**, or **unsupported**.
- The status item should never show a bare percentage without provider, window, and freshness available in the popover.
- Provider scraping is a fallback, not the default. The daemon owns account connectors and authentication repair states.
...
- Optional text: `3` active sessions, `2 wait` when attention is required, or the most constrained active account when quota display is enabled.
- Quota display mode should be first-class, not hidden: `Claude 36%`, `Codex 18%`, `Amp login`, or a compact ring/gauge icon. The popover must explain the provider, account, quota window, reset time, freshness, and source before the operator trusts the number.
...
6. Accounts: Claude Enterprise, Claude Personal, Codex Personal, Amp Personal.
   Account rows should follow the [Token & Cost Telemetry](/reference/roadmap/token-cost-telemetry/) account-snapshot model: quota window, reset time, source, stale/error state, confidence label, and repair action. Tokemon, Token Tracker, Brim, MeterBar, CodexBar, and OpenUsage are comparison references for these rows; the jackin' daemon remains the data source.
...
The daemon should cache account and usage data once on the host and fan it out to the Desktop app, `jackin console`, CLI commands, and Capsule. Running role containers should not each poll Claude/Codex/Amp independently. If an agent needs to read the latest state from inside a container, Capsule should expose a read-only control-channel response or a daemon-materialized file under `/jackin/run/usage/` with freshness metadata. The host cache is the write authority; container files are views.
