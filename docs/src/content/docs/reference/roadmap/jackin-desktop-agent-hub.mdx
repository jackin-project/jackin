...
routing. The current implementation is intentionally consolidated in
[PR #317](https://github.com/jackin-project/jackin/pull/317), not split across a
stack of smaller pull requests.
...
jackin'
  ...
  Active workspaces
    jackin-project/jackin
      ...
      Open PR #299 on GitHub
      Open PR #299 in DiffsHub
...
The first window slice now adds this `NavigationSplitView` shell and wires the
sidebar destinations to the same daemon-backed menu model. The Pull Requests
and Projects panes now include first-pass organization, project, and draft/open
filters over daemon-provided PR state. The Workspaces and Running Agents panes
must become the operator's live answer to: "Which workspaces exist, how many
instances are running, which branch is each instance changing, and which pull
request should I review?" Account cards and attention actions remain in focused
follow-up work after the consolidated PR establishes that live workspace /
instance / PR spine.
...
This program is being delivered through
[PR #317](https://github.com/jackin-project/jackin/pull/317) on branch
`jackin/desktop-attention-routing`. The consolidated PR is the review unit for
the daemon, macOS Desktop shell, menu bar foundation, GitHub PR surfaces,
account-status foundation, Ghostty/browser actions, notification bridge, and
attention-route contract.

The sections below are subsystem checkpoints inside PR #317, not instructions
to open sequential pull requests. Agents working this goal should update PR
#317 directly and should not create stacked follow-up branches unless the
operator explicitly changes the delivery plan.

Delivery rules:

- Keep all Desktop Agent Hub implementation work on
  `jackin/desktop-attention-routing` for PR #317.
...
- Follow the repo's existing PR and commit standards for PR #317:
...
- PR #317 must include focused tests or an explicit reason tests are not
  meaningful for a slice. UI work should include the strongest practical local
  verification for the current repo shape.
- Treat the daemon/Desktop/API/docs contract as editable inside PR #317. If a
...
- Keep the PR description current: what #317 adds, what it intentionally
