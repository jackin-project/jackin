...
**Status**: In progress — daemon-backed menu bar foundation landed; live Capsule state, full Desktop window, quota telemetry, and attention routing remain open.
...
The first window slice now adds this `NavigationSplitView` shell and wires the sidebar destinations to the same daemon-backed menu model. The Pull Requests and Projects panes now include first-pass organization, project, and draft/open filters over daemon-provided PR state. The Workspaces and Running Agents panes now show saved workspace visibility, per-workspace running counts, running instance repository/branch metadata, linked PR details when available, and separate GitHub/DiffsHub review actions. Account cards and attention actions remain in focused follow-up work after the consolidated implementation establishes that live workspace / instance / PR spine.
...
- Keep the consolidated PR reviewable through focused commits, clear subsystem
...
- Keep the PR description current: what the consolidated implementation adds, what it intentionally
...
- Add the Projects view for recently active jackin' repositories using
  repository grouping from the daemon-backed PR list and an organization filter.
