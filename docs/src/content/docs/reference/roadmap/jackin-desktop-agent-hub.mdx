...
The data model and confidence + status labels (confidence: `provider_authoritative` / `local_estimate` / `fallback`; status: `fresh` / `stale` / `needs_login` / `unsupported` / `error`) are the telemetry roadmap's contract; Desktop renders them.
