...
- `desktop/macos/` contains the first native macOS SwiftUI/AppKit package:
  ...
  `workspace/list`, and `session/list` so the menu can show daemon health,
  saved workspaces, and running-agent counts. This is intentionally a menu bar
  skeleton, not the full Desktop window or PR menu.
