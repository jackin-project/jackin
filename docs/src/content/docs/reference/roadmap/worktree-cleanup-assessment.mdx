...
Implemented. `assess_cleanup` in <RepoFile path="src/isolation/finalize.rs" /> now enumerates every local branch in the worktree (instead of hardcoding `record.scratch_branch`) and applies the per-branch policy table from the **Proposed Fix** section below; `[gone]` upstream is treated as Safe so squash-merged-and-pruned branches no longer trip the prompt. The prompt also takes a new `PreservedReason` discriminator (`Dirty` vs `Unpushed`) so the wording matches what is actually at risk — operators no longer see "still has uncommitted changes" alongside a clean tree. The non-interactive `[jackin] preserved isolated worktree …` warning gets the same per-reason phrasing.

The sections below preserve the original analysis.
...
The "Foreground session finalization" bullet in [Per-mount isolation](/reference/roadmap/per-mount-isolation/) says:
...
- [Per-mount isolation](/reference/roadmap/per-mount-isolation/) — V1 design doc; the "Foreground session finalization" bullet's safety contract is the one this fix corrects.
- [Architecture](/reference/architecture/) — operator-facing explanation of clean-exit cleanup; user-facing wording changes from Piece 2 should land here too.
