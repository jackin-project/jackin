...
Saved workspaces live one-per-file under `~/.config/jackin/workspaces/` rather than as `[workspaces.<name>]` tables inside a single `config.toml`. The layout works directly with **dotfile managers** ([chezmoi](https://www.chezmoi.io/), [yadm](https://yadm.io/), [GNU stow](https://www.gnu.org/software/stow/), or a plain git repository tracking `~/.config/jackin/`). Operators commonly want to:
