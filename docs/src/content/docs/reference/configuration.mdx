...
Saved workspaces live one-per-file under `~/.config/jackin/workspaces/` rather than as `[workspaces.<name>]` tables inside a single `config.toml`. The layout works directly with **dotfile managers** ([chezmoi](https://www.chezmoi.io/), [yadm](https://yadm.io/), or a plain git repository tracking `~/.config/jackin/`). Operators commonly want to:
