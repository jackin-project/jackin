...
`auth_forward` is configured per agent (`claude`, `codex`, `amp`, `opencode`) at three
...
`[amp]`, or `[opencode]` block is rejected at parse time.
...
same rule as `[claude]` / `[codex]` / `[opencode]`. Optional GHE-targeted env vars
...
| `default_agent` | Optional workspace default agent (`"claude"`, `"codex"`, `"amp"`, or `"opencode"`). Omit to default to Claude. |
