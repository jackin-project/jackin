...
Saved workspaces live one-per-file under `~/.config/jackin/workspaces/` rather than as `[workspaces.<name>]` tables inside a single `config.toml`. The split is deliberate so the layout works with **dotfile managers** ([chezmoi](https://www.chezmoi.io/), [yadm](https://yadm.io/), [GNU stow](https://www.gnu.org/software/stow/), or a plain git repository tracking `~/.config/jackin/`). Operators commonly want to:

- **Version-control the global `config.toml`** (auth defaults, agent settings) while keeping `workspaces/<name>.toml` files private or per-machine.
...
- **Selectively include workspaces** — share team-relevant ones by tracking a subset of files, leave the rest out via `.gitignore`.
...
The schema-versioning framework supports this directly: each file carries its own `version = "v1alpha1"` stamp, so a dotfile-managed setup that is partially old (some workspaces stamped, some not) migrates per-file on first load. See [Schema Versions](/reference/schema-versions/) for the full version history and the legacy-to-`v1alpha1` migration matrix.
