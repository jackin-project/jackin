...
When `auto_coauthor_trailer` is `true`, jackin installs a `prepare-commit-msg` hook inside every role container on launch. The hook reads which agent is running and appends the correct `Co-authored-by` trailer to every new commit message automatically. Amend operations are skipped so the trailer is not added twice.
...
The `[git]` table is omitted from `config.toml` when all fields are at their defaults (the table is not written unless the operator sets it). See [Schema Versions](/reference/schema-versions/#v1alpha3) for the field-level reference.
