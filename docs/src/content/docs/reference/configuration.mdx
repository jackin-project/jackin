...
Top-level `version` is the config schema version. jackin' writes `version = "v1alpha4"` today. Older config files with no `version` are treated as legacy and migrated automatically on startup before parsing. See [Schema Versions](/reference/schema-versions/) for the full version history and migration matrix.
