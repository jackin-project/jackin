...
When `coauthor_trailer` is `true`, jackin installs a shared `prepare-commit-msg` hook inside the role container the first time a session needs it. The hook reads which agent is running and appends the correct `Co-authored-by` trailer to every new commit message automatically. Amend and squash operations are skipped so consolidated messages keep their existing trailer block.
