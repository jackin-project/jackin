...
| `mounts[].isolation` | Per-mount isolation mode: `shared` (default) or `worktree`. `clone` is reserved in the enum vocabulary but rejected at validation time until implemented. Omitted/`shared` values are not written back on save. Global mounts (under `[docker.mounts]` or `[[mounts]]`) reject this field at parse time. See [Per-mount isolation](../guides/workspaces.mdx#per-mount-isolation). |
