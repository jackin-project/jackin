...
- **Session tabs** — one tab per session. Active tab is bold with a warm amber background, separate from the brand green. Inactive tabs are grey. Mouse-clickable tabs lift their background on hover. Tabs show a state glyph for the session roll-up. The blocked attention glyph is latched: incidental PTY output does not clear it, and explicit operator keyboard input to that pane does.
- **Hidden command-palette hint** — the palette shortcut remains active, but Capsule no longer spends top-row columns on a visible menu hint. Row 0 is reserved for the brand pill and session tabs.

When the workspace workdir's current branch has an open GitHub pull request, Capsule also reserves the bottom terminal row for a white pull-request bar. The left side shows `PR #<number> · <title>` and the right side shows the container ID. On a non-`main` / non-`master` branch without an open PR, the same row shows `Branch · <branch>` on the left and the container ID on the right. The left side opens a GitHub context modal with branch, URL, title, best-effort `gh pr checks` status, and an `Enter` hint for copying the PR URL when a pull request exists. The GitHub URL value is the emphasized clickable copy target. The right side opens Container info with the full container ID, role, active pane's agent, current workdir, and an `Enter` hint for copying the container ID. The Container ID value is the emphasized clickable copy target. All click targets switch to a pointer cursor on supported terminals and lift their color while hovered. The daemon polls local Git branch state every second in a background task, so switching branches updates the bar before GitHub metadata is refreshed. `gh` PR/check lookups are cached per branch and refreshed no more than once per minute; switching to `main` or `master` clears the visible context, resizes panes back into the freed row, and removes the bar on the next redraw.

Every Capsule dialog renders through the same full-frame overlay path. That path redraws the background with strongly darkened pane colors, paints the branch context bar if present, then paints the active dialog and replaces the bottom row with the dialog's footer hint. No dialog appears without the dim cue, and modal key hints always live at the bottom of the screen instead of inside or next to the dialog.
