...
When the workspace workdir's current branch has an open GitHub pull request, Capsule also reserves the bottom terminal row for a white pull-request bar. The left side shows `PR #<number> · <title>` and the right side shows the container ID. On a non-`main` / non-`master` branch without an open PR, the same row shows `Branch · <branch>` on the left and the container ID on the right. The left side opens a GitHub context modal with branch, URL, title, best-effort `gh pr checks` status, and an `Enter` hint for copying the PR URL when a pull request exists. The right side opens Container info with the full container ID, role, active pane's agent, current workdir, and an `Enter` hint for copying the container ID. Both click targets switch to a pointer cursor on supported terminals and lift their color while hovered. The daemon caches the last resolved branch/PR pair and refreshes it periodically; switching to `main` or `master` clears the cache, resizes panes back into the freed row, and removes the bar on the next redraw.

Every Capsule dialog renders through the same full-frame overlay path. That path redraws the background in its dimmed state, paints the branch context bar if present, then paints the active dialog and footer hint last so no dialog appears without the dim cue and no footer hint is hidden behind the bottom bar.
...
- `pointer` over clickable chrome owned by jackin', such as tabs, the bottom-row GitHub and container-info entry points, and clickable dialog rows such as the Container ID copy target.
...
Clickable Capsule chrome also repaints on hover when passive mouse motion identifies a known target. Tabs, the bottom GitHub context target, and the bottom container-info target use a slightly brighter background or link color so clickability is visible even when pointer-shape escapes are unsupported.
