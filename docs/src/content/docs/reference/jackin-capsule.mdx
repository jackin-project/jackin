...
| Interactive attach | `docker exec -it <container> /usr/local/bin/jackin-capsule` | Starts a second `jackin-capsule` process in attach client mode. That client connects to the PID 1 daemon over `/jackin/run/jackin.sock` and bridges the operator terminal. |
