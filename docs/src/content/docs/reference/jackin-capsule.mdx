...
- **Wheel input** is routed by pane ownership. Mouse-enabled programs receive re-encoded pane-local SGR or xterm mouse events. Primary-screen panes that did not request mouse reporting use jackin' scrollback, including agent panes such as Codex that render a transcript in the primary grid. Alternate-screen panes without mouse reporting receive cursor-key fallback, matching terminal emulator alternate-screen scroll behavior.
