...
- **Identity strip** — container / role details and the clickable container-info entry point. The modal opens immediately with loading placeholders for git metadata, then shows the full container ID, role, active pane's agent, current workdir, current git branch when the workdir is a repository, and best-effort open GitHub pull request metadata from `gh pr list --head` when one exists for that branch.

When the workspace workdir's current branch has an open GitHub pull request, Capsule also reserves the bottom terminal row for a white pull-request bar. The left side shows `PR #<number> · <title>` and the right side shows the PR URL wrapped in an OSC 8 hyperlink. On a non-`main` / non-`master` branch without an open PR, the same row shows `Branch · <branch>` without a link. The daemon caches the last resolved branch/PR pair and refreshes it periodically; switching to `main` or `master` clears the cache, resizes panes back into the freed row, and removes the bar on the next redraw.
