...
Every Capsule dialog renders through the same full-frame overlay path. That path redraws the full multiplexer background in backdrop colors: pane bodies, pane borders, active-pane focus chrome, the top status bar, and the bottom session context bar are all dimmed before the active dialog is painted. No focused-pane green border or scrollbar remains visible behind a modal; focus returns to pane chrome only after the dialog closes. The dialog footer hint renders above the bottom status row with one blank spacer row between them, so modal key hints stay in the bottom chrome area instead of inside or next to the dialog.
