...
When the workspace workdir's current branch has an open GitHub pull request, Capsule also reserves the bottom terminal row for a white pull-request bar. The left side shows `PR #<number> · <title>` and the right side shows the container ID. On a non-`main` / non-`master` branch without an open PR, the same row shows `Branch · <branch>` on the left and the container ID on the right. The left side opens a GitHub context modal with branch, URL, title, and best-effort `gh pr checks` status. The right side opens Container info with the full container ID, role, active pane's agent, and current workdir. Both click targets switch to a pointer cursor on supported terminals and lift their color while hovered. The daemon caches the last resolved branch/PR pair and refreshes it periodically; switching to `main` or `master` clears the cache, resizes panes back into the freed row, and removes the bar on the next redraw.
