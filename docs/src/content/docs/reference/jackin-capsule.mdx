...
Every Capsule dialog renders through the same full-frame overlay path. That path redraws the background with strongly darkened pane colors, paints the branch context bar if present, then paints the active dialog and the footer hint directly below it so no dialog appears without the dim cue and no footer hint is hidden behind the bottom bar.
