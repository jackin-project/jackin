...
- **Menu button** — right side of row 1. Compressed to just the hamburger glyph (`☰`) so it always fits regardless of tab count or terminal width. Clicking opens the command palette; the keyboard equivalent is `Ctrl+\` (override via `JACKIN_PALETTE_KEY`). Switches to a bright-green highlight background while the optional tmux-style prefix gesture is mid-way through.
- **Branch / pull-request slot** — left of the identity label on row 2. Hidden when the workspace's git checkout is on the repo default branch (resolved from `origin/HEAD`); otherwise renders `branch ⟳` while the PR lookup runs, then `branch` (no open PR) or `PR #N: title` (open PR). Clicking the `PR #N: title` form copies the URL via OSC 52. The slot is the first chrome surface dropped when the terminal is too narrow to fit it next to the identity label, since the same data is reachable from the container-info modal.
...
The branch / pull-request slot is driven by a `notify` watcher subscribed to the workspace's `.git/` directory. Events whose path basename is `HEAD` queue a `WorkdirBranchChanged` event onto the daemon channel; the handler bumps a monotonic request-id and spawns a background thread that resolves the current branch with `git branch --show-current`, the base branch with `git symbolic-ref --short refs/remotes/origin/HEAD`, and (when off-base) the open PR with `gh pr list --head <branch> --state open --limit 1 --json url,title,number`. The request-id pattern lets a fast `git checkout` followed by a slow `gh pr list` discard the stale result instead of landing it on the wrong branch.
...
Key press arrives
  │
  ├─ Row 0 mouse click → tab switch (or menu-button click → command palette)
  ├─ Row 1 mouse click → container info on the identity label, branch slot click copies the PR URL via OSC 52
