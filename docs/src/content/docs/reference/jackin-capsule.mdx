...
- **Terminal output** is read by a per-session PTY reader, parsed into that pane's `vt100::Screen`, composed with Capsule chrome, and sent back to the attached client as raw ANSI bytes in `Output` frames.
...
On Docker Desktop, the host cannot open the Linux VM's Unix socket path directly even though the parent directory is bind-mounted. In that case <RepoFile path="src/runtime/snapshot.rs" /> falls back to `docker exec ... /jackin/runtime/jackin-capsule snapshot`; the in-container client still speaks the same control protocol to the same daemon socket, then prints the JSON response for the host to read.
...
- **Menu button** — the right side of row 0 keeps a clean clickable `☰ Menu` button. When prefix mode is enabled and awaiting the next key, the button changes to `prefix…` so the operator can see that Capsule has consumed the prefix byte and is waiting for the command key.

Capsule always reserves the bottom terminal row for the white workspace and container status bar. The left side starts with the workspace name; the right side shows the container ID. When the workspace workdir's current branch has an open GitHub pull request, the left side adds `PR #<number> · <title>`. While `gh` is still resolving an open pull request for a branch, the left side adds `Resolving PR · <branch>` instead of pretending there is no pull request. On a non-`main` / non-`master` branch without an open PR, the same row adds `Branch · <branch>` on the left. On `main`, `master`, or a directory without a recognizable branch, the bar stays visible with only the workspace and container. The PR or branch portion opens a GitHub context modal with branch, URL, title, best-effort `gh pr checks` status, and an `Enter` hint for copying the PR URL when a pull request exists. The workspace-only label is not a GitHub-context target. The GitHub URL value is the emphasized clickable copy target. The right side opens Container info with the full container ID, role, active pane's agent, current workdir, and an `Enter` hint for copying the container ID. The Container ID value is the emphasized clickable copy target. All click targets switch to a pointer cursor on supported terminals and lift their color while hovered. The daemon polls local Git branch state several times per second in a background task, so switching branches updates the bar before GitHub metadata is refreshed. `gh` PR/check lookups are cached per branch and refreshed no more than once per minute; switching to `main` or `master` clears the branch/PR detail while keeping the always-on workspace and container row.

Every Capsule dialog renders through the same full-frame overlay path. That path redraws the background with strongly darkened pane colors, paints the always-on bottom workspace/container status bar, then paints the active dialog and renders the dialog's footer hint above the bottom status row with one blank spacer row between them. No dialog appears without the dim cue, and modal key hints stay in the bottom chrome area instead of inside or next to the dialog.
...
Key press arrives
  ...
  ├─ Bottom context-row click → GitHub context on the left, container info on the right
...
The attach client enables SGR any-event mouse tracking on the outer terminal. When the daemon sees passive pointer motion and the active attach client advertises a known OSC 22-capable identity (`xterm-ghostty`, `xterm-kitty`, Foot, or iTerm via `TERM` / `TERM_PROGRAM`), it emits OSC 22 pointer-shape updates:

- `pointer` over clickable chrome owned by jackin', such as tabs, the bottom-row GitHub and container-info entry points, and clickable dialog values such as the Container ID and GitHub URL copy targets.
...
Clickable Capsule chrome also repaints on hover when passive mouse motion identifies a known target. Tabs, the bottom GitHub context target, the bottom container-info target, and dialog copy values use a slightly brighter background or link color so clickability is visible even when pointer-shape escapes are unsupported.
