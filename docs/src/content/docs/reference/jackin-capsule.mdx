...
- **Wheel input** is routed by pane ownership. Mouse-enabled programs receive re-encoded pane-local SGR or xterm mouse events. Primary-screen panes that did not request mouse reporting use jackin' scrollback when vt100 has retained history. When that history is empty but the primary grid looks like a pane-owned transcript UI, such as Codex in an agent tab or Codex launched from a shell tab, the pane shows scroll chrome and receives cursor-key wheel fallback. Alternate-screen panes without mouse reporting receive cursor-key fallback, matching terminal emulator alternate-screen scroll behavior.
