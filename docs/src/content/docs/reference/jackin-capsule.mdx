...
- **Wheel input** is routed by pane ownership. Mouse-enabled programs receive re-encoded pane-local SGR or xterm mouse events. Primary-screen panes that did not request mouse reporting use jackin' scrollback when vt100 has retained history; primary-screen agent panes such as Codex fall back to cursor-key wheel emulation when that history is empty. Alternate-screen panes without mouse reporting receive cursor-key fallback, matching terminal emulator alternate-screen scroll behavior.
