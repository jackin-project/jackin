...
- **Migrations are a hard PR-review rule.** Every change to the serde representation of one of the three versioned files must bump the corresponding `CURRENT_*_VERSION`, add a migration step, and update this page. Because all structs use `#[serde(deny_unknown_fields)]`, previous jackin binaries cannot deserialize files that contain unknown fields. The version stamp is the gate that lets an old binary detect the mismatch and print "upgrade jackin" instead of crashing with a cryptic serde error. The full rule lives in <RepoFile path="AGENTS.md" /> and <RepoFile path="PULL_REQUESTS.md" />.
...
**Summary**: restamps existing `v1alpha2` files as `v1alpha3` and moves `version = "v1alpha3"` to the first line. This sequential migration also expands the agent surface to include the new `kimi` agent.

**Before** (v1alpha2 config file — excerpt):
...
| `default_agent` | string | No | `legacy` | `claude`, `codex`, `amp`, or `kimi`; omitted defaults to Claude |
...
| `agents` | string array | No | `legacy` | `claude`, `codex`, `amp`, and/or `kimi`; omitted means Claude-only |
...
| `[kimi].model` | string | No | `v1alpha3` | Kimi model override |
