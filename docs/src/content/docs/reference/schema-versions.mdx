...
Each entry below documents one migration the current binary still applies. The fixture link points at the canonical before/after pair the test suite walks through the live migration code on every CI run (see <RepoFile path="tests/migration_fixtures.rs" />), so a regression breaks here before it breaks on a delayed operator's machine. Newest version first; entries get appended (not edited) when a `CURRENT_*_VERSION` is bumped.
...
The full before/after for each file kind lives in the linked fixture directories, which the migration test harness reads directly.
