...
|---|---|---|---|
| `~/.config/jackin/config.toml` | `v1alpha5` | jackin | Automatic on startup |
| `~/.config/jackin/workspaces/<name>.toml` | `v1alpha4` | jackin | Automatic on startup |
| `jackin.role.toml` | `v1alpha4` | Role repository | Desktop: `jackin role migrate <role-repo-path>`; CI/automation: `jackin-validate --migrate <role-repo-path>` |
...
**Summary**: introduces the `[git]` table with `auto_coauthor_trailer` (boolean, default `false`). When enabled, jackin installs a `prepare-commit-msg` hook inside agent containers via `core.hooksPath` that appends two trailers to every non-amend, non-squash, non-merge commit: a `Co-authored-by` line identifying the running agent, and a `Signed-off-by` DCO line read from the container's `git config user.name` / `user.email`. Both are deduplicated. The migration is a no-op restamp.
...
| Manifest (`jackin.role.toml`) | `v1alpha3` | Desktop: `jackin role migrate <role-repo-path>`; CI/automation: `jackin-validate --migrate <role-repo-path>` | <RepoFile path="tests/fixtures/migrations/manifest/from-v1alpha3/meta.toml" /> |
...
| `[git].auto_coauthor_trailer` | bool | No | `v1alpha5` | Install `prepare-commit-msg` hook that appends `Co-authored-by` (agent) and `Signed-off-by` (DCO) trailers; omitted when `false` |
