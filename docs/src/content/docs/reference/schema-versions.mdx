...
Jackin' is built to evolve fast. Breaking changes to schemas are expected — that is how the project explores ideas without dragging compatibility shims forward. The trade-off is that operators and role authors should never have to hand-edit a TOML file to recover from an upgrade. Versioning resolves the tension:

- **Each schema file carries its own `version` stamp.** When jackin reads the file, it compares the on-disk version against the binary's `CURRENT_*_VERSION` and applies any pending migration steps before parsing — operators see a working setup after every upgrade, and the project keeps moving fast on the schema.
...
- **Migrations are a hard PR-review rule.** Every change to one of the three versioned files must bump the corresponding `CURRENT_*_VERSION`, add a migration step, and update this page. The full rule lives in `AGENTS.md` and `PULL_REQUESTS.md`.
...
| Version | File kind | Status | Summary |
|---|---|---|---|
| `legacy` | Config, workspace, manifest | Supported for migration | No top-level `version` field |
| `v1alpha1` | Config | Current | Adds `version = "v1alpha1"`; migrates legacy monolithic workspace tables into split workspace files |
| `v1alpha1` | Workspace | Current | Adds `version = "v1alpha1"` to per-workspace files |
| `v1alpha1` | Manifest | Current | Adds required `version = "v1alpha1"` to `jackin.role.toml` |
...
| `[hooks].setup_once` | string | No | `legacy` | Relative path to a one-time install script gated by a marker on the container writable layer |
