...
- **Role-owned files migrate explicitly.** Role authors run `jackin role migrate <role-repo-path>` to rewrite a `jackin.role.toml` against the latest schema, then commit and push the result. Jackin never writes back into a role repo on its own — that would silently mutate someone else's source tree. The standalone `jackin-validate --migrate <role-repo-path>` binary remains available for GitHub workflows.
...
| `jackin.role.toml` | `v1alpha3` | Role repository | Explicit `jackin role migrate <role-repo-path>` |
...
|---|---|---|---|
| Manifest (`jackin.role.toml`) | `v1alpha2` | Explicit `jackin role migrate <role-repo-path>` | <RepoFile path="tests/fixtures/migrations/manifest/from-v1alpha2/meta.toml" /> |
...
| Manifest (`jackin.role.toml`) | `v1alpha1` | Explicit `jackin role migrate <role-repo-path>` | <RepoFile path="tests/fixtures/migrations/manifest/from-v1alpha1/meta.toml" /> |
...
| Manifest (`jackin.role.toml`) | `legacy` | Explicit `jackin role migrate <role-repo-path>` | <RepoFile path="tests/fixtures/migrations/manifest/from-legacy/meta.toml" /> |
...
`jackin role migrate <role-repo-path>` to rewrite a local copy.
