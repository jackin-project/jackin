...
- **Migrations are a hard PR-review rule.** Every *non-additive* change to one of the three versioned files — renamed fields, removed fields, type changes, restructured tables — must bump the corresponding `CURRENT_*_VERSION`, add a migration step, and update this page. Purely additive changes (new `Option<T>` fields with `serde(default)`) do not require a version bump because old files deserialize unchanged. The full rule lives in <RepoFile path="AGENTS.md" /> and <RepoFile path="PULL_REQUESTS.md" />.
...
| `[kimi].auth_forward` | string | No | `v1alpha2` | `sync`, `api_key`, or `ignore` |
...
| `[kimi].auth_forward` | string | No | `v1alpha2` | Workspace Kimi auth override |
...
| `[roles.<role>.kimi].auth_forward` | string | No | `v1alpha2` | Per-workspace, per-role Kimi auth override |
...
| `[kimi].model` | string | No | `v1alpha2` | Kimi model override |
