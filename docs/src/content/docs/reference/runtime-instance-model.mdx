...
| Agent runtime | One supported CLI runtime: `claude`, `codex`, `amp`, or `opencode`. |
...
Implementation lives in <RepoFile path="src/runtime/launch.rs" /> (`resolve_restore_candidate`, related restore helpers, name claiming, and launch orchestration).
...
|---|---|
| Running | Attach to the live primary foreground session, or start another foreground session when `--new` is supplied. |
...
`hardline --inspect` is read-only and should report manifest state even when Docker is unavailable. When Docker can inspect a running role container, it also reports live agent-session inventory from `tmux list-sessions` inside the running container.

`hardline --new` creates a named secondary tmux session (`jackin-<agent>-<id>`) inside the running container. Secondary sessions are independently reconnectable — a second `jackin hardline --new` or console `a` from the same instance reattaches to any live session or creates a fresh one. `hardline --shell` opens an ephemeral one-shot zsh session without a tmux session.

Implementation lives in <RepoFile path="src/runtime/attach.rs" /> and the hardline dispatch paths in <RepoFile path="src/app/mod.rs" />.
...
- reconnect/recover (`r`)
- new agent session (`a`/`A`) — starts a secondary tmux session inside the running container
- ephemeral shell (`x`/`X`) — opens a one-shot zsh session without a tmux session
- read-only inspect (`i`)
- guarded purge (`p`)
...
Implementation lives in <RepoFile path="src/console/manager/state.rs" />, <RepoFile path="src/console/manager/render/list.rs" />, and <RepoFile path="src/console/manager/input/list.rs" />.
...
Implementation lives in <RepoFile path="src/runtime/cleanup.rs" />.

## Source References

| Area | Source |
|---|---|
| DNS-safe names and instance IDs | <RepoFile path="src/instance/naming.rs" /> |
| Docker labels and derived resource names | <RepoFile path="src/runtime/naming.rs" /> |
| Instance manifest and rebuildable index | <RepoFile path="src/instance/manifest.rs" /> |
| Launch, restore discovery, name claiming | <RepoFile path="src/runtime/launch.rs" /> |
| Attach, inspect, session inventory | <RepoFile path="src/runtime/attach.rs" /> |
| Eject, purge, orphan cleanup | <RepoFile path="src/runtime/cleanup.rs" /> |
| Workspace/materialized mount state | <RepoFile path="src/isolation/materialize.rs" /> and <RepoFile path="src/isolation/state.rs" /> |
| Foreground finalizer | <RepoFile path="src/isolation/finalize.rs" /> |
| Console instance rendering/actions | <RepoFile path="src/console/manager/render/list.rs" /> and <RepoFile path="src/console/manager/input/list.rs" /> |
| DinD/Testcontainers smoke coverage | <RepoFile path="tests/dind_e2e.rs" /> |
