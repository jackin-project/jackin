...
`hardline --new` starts another foreground agent process inside a running indexed instance. Today this is not a reconnectable named secondary session; [Console agent session control](/reference/roadmap/console-agent-session-control/) owns the tmux-backed session substrate that will make secondary sessions independently reconnectable.
