...
`OpWriteRunner::item_field_set(item_id, vault_id, field_label, value, section)` writes a new token value into a specific field of an existing 1Password item without touching any other fields or metadata. It follows the same never-on-argv contract as `item_create` via a two-step GET → piped-template-EDIT sequence:
...
2. **Modify in-process** — the `fields` array is scanned for a field whose `label` or `id` matches `field_label`. If found, its `value` is replaced and its `type` is set to `CONCEALED`. If not found, a new field object (`{ id, label, type: "CONCEALED", value }`) is appended. When a `section` is supplied, its slug is added to the item's top-level `sections` array (if absent) and the field's `section` is pointed at it.
...
4. **Reference extraction** — the JSON returned by `op item edit` is scanned for the target field by label (case-insensitive) or id; the resulting `op://` reference is built from UUIDs (vault `id`, item `id`, field `id`) so it stays stable across renames.
...
`OpCli::with_account(Some(id))` pins every subprocess invocation to `op --account <id>`. The orchestrator constructs one `OpCli` per run via the `op_cli_for(config, workspace, explicit) -> OpCli` helper which folds the rule "explicit `--op-account` flag wins over the account recorded on the workspace's stored `op://` reference (`OpRef::account`)". The same helper is used by every `run_*` entry point, so account resolution lives in one place. `item_create` / `item_field_set` stamp the run's pinned account onto the `OpRef` they return, so the wired env value records the account it was created under; reads later rebind to that per-ref account via `OpRunner::read_with_account`.
