...
`OpWriteRunner::item_field_set(item_id, vault_id, field_label, value)` writes a new token value into a specific field of an existing 1Password item without touching any other fields or metadata. It follows the same never-on-argv contract as `item_create` via a two-step GET → stdin-EDIT sequence:
...
2. **Modify in-process** — the `fields` array is scanned for a field whose `label` or `id` matches `field_label`. If found, its `value` is replaced and its `type` is set to `CONCEALED`. If not found, a new field object (`{ id, label, type: "CONCEALED", value }`) is appended.
3. **stdin-EDIT** — the modified JSON is serialised and piped to `op item edit - --vault <vault> --format json`. The `-` positional arg tells `op` to read the full item template from stdin; `op` identifies which item to update from the `id` field inside the JSON body. The token value therefore never appears in `argv` or `/proc/<pid>/cmdline`.
