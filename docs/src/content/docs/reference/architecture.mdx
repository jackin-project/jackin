...
8. **Start the agent container detached** (`docker run -d ... <image> <agent>`) with `jackin-capsule` as PID 1. The trailing `<agent>` argv selects only the initial tab; it is not a container-level environment variable.
9. **Attach the operator terminal** via `docker exec -it <container> /usr/local/bin/jackin-capsule`. The client connects to the PID 1 daemon over `/jackin/run/jackin.sock` and renders the in-container multiplexer.
...
Implementation: <RepoFile path="crates/jackin-capsule/src/daemon.rs" />, <RepoFile path="src/runtime/launch.rs" /> (`launch_role_runtime`), <RepoFile path="src/runtime/attach.rs" /> (`reconnect_or_create_session`, `spawn_agent_session`, `spawn_shell_session`).
...
| Terminal feature advertisement | Pane PTYs run with `TERM=xterm-256color`; host-specific terminfo is mounted separately when the outer terminal needs it. |
...
Implementation: <RepoFile path="crates/jackin-capsule/src/client.rs" />, <RepoFile path="crates/jackin-capsule/src/input.rs" />, <RepoFile path="crates/jackin-capsule/src/session.rs" />, and <RepoFile path="crates/jackin-capsule/src/daemon.rs" />.
