...
4. **Generate a derived build context** — copy the repo, inject agent assets (entrypoint + pre-launch hook), and render a derived Dockerfile
...
│   └── jackin-agent-smith/
│       ├── claude/                # Claude-only state (when launched under Claude)
│       │   ├── state/             # Claude Code session history, context
│       │   │   └── .credentials.json  # OAuth tokens (forwarded or container-local)
│       │   ├── account.json       # Claude Code account metadata and preferences
│       │   └── plugins.json       # Installed Claude plugins list
│       ├── codex/                 # Codex-only state (when launched under Codex)
│       │   ├── config.toml        # Codex config generated at launch
