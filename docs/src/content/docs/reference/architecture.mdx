...
For host-side Docker API calls, jackin' honors `DOCKER_HOST` when the operator
sets it explicitly. Otherwise it inspects the active Docker CLI context and
connects to that context's Docker host, so API calls such as network creation
land on the same daemon that later `docker run` commands use.
