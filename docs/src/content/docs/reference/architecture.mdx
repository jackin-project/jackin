...
That means an role defines the tools and conventions, while jackin' injects the agent-specific pieces such as Claude, Codex, Kimi, or OpenCode installation, the entrypoint, and UID/GID remapping.
...
8. **Start the agent container detached** (`docker run -d`) with `supervisor.sh` as the entrypoint override — a bash wait loop that becomes PID 1 and keeps the container alive while agent sessions run via `docker exec`
9. **Start the first agent session** via `docker exec -e TMUX= -it <container> tmux new-session -e JACKIN_AGENT=<agent> -s jackin-<agent>-<id> -- /jackin/runtime/entrypoint.sh [model-flags]`. `TMUX=` prevents nested-session warnings when the operator's terminal is itself inside tmux. `JACKIN_AGENT` is passed via `tmux new-session -e` rather than as a container-level env var so each session can specify its own agent runtime independently.

The detached-start + exec split decouples the container's lifetime from the foreground session: closing the terminal tab detaches from the tmux session inside the container, but the supervisor (PID 1) and the tmux server keep running. `jackin hardline` reattaches to the most recently used session or creates a new one when no sessions are running. Cleanup distinguishes three post-session states:
...
`jackin hardline` inspects the target container. If it's running, it attaches to the most recently used tmux session via `docker exec ... tmux attach-session`, or creates a fresh `jackin-<agent>-<id>` session when none are running. If it's stopped with a non-zero exit (crash / OOM), it restores the required DinD sidecar/network/certs when needed, restarts the role container, and attaches. If the role container itself is missing but indexed recoverable state exists, hardline can rebuild the runtime around jackin-managed local state. Clean exits (exit code 0) are treated as completed sessions — use `jackin load` to start a fresh instance.

### Container supervisor and session model

Every role container runs <RepoFile path="docker/runtime/supervisor.sh" /> as PID 1. The supervisor is a bash wait loop (`while true; do sleep 3600 & wait $! || true; done`) that traps SIGTERM/SIGINT and exits cleanly on `docker stop`. Its sole job is to keep the container alive while agent sessions run via `docker exec`.

Sessions are tmux sessions inside the running container. All agent sessions are equal — there is no "primary" or "secondary" distinction.

| Session type | tmux name | Created by | Reconnectable |
|---|---|---|---|
| Agent session | `jackin-<agent>-<id>` | `jackin load`, `jackin hardline`, or console `a`/`A` | Yes — `jackin hardline` attaches to the most recent session or creates a new one |
| Shell | (no session) | `hardline --shell` or console `x`/`X` | No — ephemeral `docker exec /bin/zsh` |

When an agent session exits cleanly (the operator types `/exit` or the agent finishes), the tmux session is removed but the container and supervisor keep running. The container only stops when the operator explicitly stops it, or when `jackin load` teardown runs after the foreground session ends cleanly (exit code 0).

Agent model flags (`--model`) are passed as arguments to the tmux session exec, not as CMD args to `docker run`. This means model overrides from `jackin load --model` apply to the initial session only. Secondary sessions started via `hardline --new` use the role's default agent configuration unless the operator selects a different runtime.

Session state is not yet reconciled against the live container on console refresh (Phase 4 of the session control roadmap). The `sessions` field in the instance manifest is written but not populated until reconciliation lands.

Implementation: <RepoFile path="docker/runtime/supervisor.sh" />, <RepoFile path="src/runtime/launch.rs" /> (`launch_role_runtime`), <RepoFile path="src/runtime/attach.rs" /> (`reconnect_or_create_session`, `spawn_agent_session`, `spawn_shell_session`).
