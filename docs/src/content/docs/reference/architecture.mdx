...
| `derived image` | Generate the role-specific `DerivedDockerfile`, assemble the temporary Docker build context, include runtime assets such as `jackin-capsule`, run `docker build`, and capture Docker stdout/stderr to a plain run sidecar log on failure. |
...
│       │   └── mcp.json           # Kimi MCP config (synced from host ~/.kimi-code/mcp.json under auth_forward=sync)
