...
The file lives next to the existing `.claude/` and `.jackin/` per-container state and is the source of truth for `purge` and `jackin cd` after the operator has changed (or removed) the workspace config. Each record contains at least:
...
| `branch` | Scratch branch name (`jackin/scratch/<branch-instance>`) |
...
- **Stopped, exit code 0 (clean)** → attempt safe isolated-worktree cleanup. A worktree is safe to delete only when it has no uncommitted changes and no branch-only commits that would be lost (either `HEAD == base_commit`, or every local commit is reachable from the upstream tracking branch). Clean+pushed worktrees are auto-deleted; clean+unpushed ones are preserved or prompted.
- **Clean exit with unsafe cleanup**, interactive stdin → ask the operator whether to return to the agent (restart + re-attach), preserve recovery state, or force-delete.
- **Clean exit with unsafe cleanup**, non-interactive → preserve the recovery state and print the worktree path plus how to return, inspect, or discard.
