...
2. **Validate the repo contract** — require `jackin.agent.toml` and a valid Dockerfile path
3. **Generate a derived build context** — copy the repo, inject runtime assets, and render a derived Dockerfile
4. **Build the image on the host Docker engine** — reusing host-side Docker cache where possible
5. **Create a per-agent Docker network**
6. **Start a privileged `docker:dind` sidecar**
7. **Start the agent container** — mounts, env vars, labels, and `DOCKER_HOST` all point at the sidecar
8. **Run Claude Code with full permissions inside that boundary**
