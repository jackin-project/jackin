...
The role repo owns the environment layer. jackin' owns the derived harness layer.

That means an role defines the tools and conventions, while jackin' injects the harness-specific pieces such as Claude or Codex installation, the entrypoint, and UID/GID remapping.
...
4. **Generate a derived build context** — copy the repo, inject harness assets (entrypoint + pre-launch hook), and render a derived Dockerfile
...
9. **Attach the foreground session** with `docker attach --detach-keys= --sig-proxy=false`, then **run the selected harness with full permissions inside that boundary**
...
The file lives next to the per-container harness state and `.jackin/` metadata and is the source of truth for `purge` after the operator has changed (or removed) the workspace config. Each record contains at least:
...
│       ├── config.toml            # Codex config generated when launching Codex
...
That means jackin' is biased toward **reproducible roles** rather than **mutable long-lived sandboxes**.
