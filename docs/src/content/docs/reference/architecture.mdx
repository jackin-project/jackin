...
9. **Start the first agent session** via `docker exec -e TMUX= -it <container> tmux new-session -s jackin-<agent>-<id> -- /jackin/runtime/entrypoint.sh [model-flags]`. `TMUX=` prevents nested-session warnings when the operator's terminal is itself inside tmux.
