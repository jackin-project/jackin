...
Full digital rain is gated by jackin-managed role-container discovery. It runs on first-container entry and last-container exit only; discovery failure skips the animation and records the reason in diagnostics instead of blocking launch.
