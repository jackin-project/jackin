...
8. **Start the agent container detached** (`docker run -d ... <image> <agent>`) with `jackin-container` as PID 1. The trailing `<agent>` argv selects only the initial tab; it is not a container-level environment variable.
9. **Attach the operator terminal** via `docker exec -it <container> /usr/local/bin/jackin-container`. The client connects to the PID 1 daemon over `/run/jackin/jackin.sock` and renders the in-container multiplexer.
...
`JACKIN_AGENT` is set per agent PTY by <RepoFile path="crates/jackin-container/src/session.rs" />, immediately before `/jackin/runtime/entrypoint.sh` runs. The daemon's own process does not carry `JACKIN_AGENT`, so shell panes and future mixed-agent sessions do not inherit a misleading container-wide agent value.
...
Implementation: <RepoFile path="crates/jackin-container/src/daemon.rs" />, <RepoFile path="src/runtime/launch.rs" /> (`launch_role_runtime`), <RepoFile path="src/runtime/attach.rs" /> (`reconnect_or_create_session`, `spawn_agent_session`, `spawn_shell_session`).
