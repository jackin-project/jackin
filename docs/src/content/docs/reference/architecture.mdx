...
The agent repo owns the environment layer. jackin' owns the derived harness layer.

That means an agent class defines the tools and conventions, while jackin' injects the harness-specific pieces such as Claude or Codex installation, the entrypoint, and UID/GID remapping.
