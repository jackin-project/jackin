...
9. **Attach the operator terminal** via `docker exec -it <container> /usr/local/bin/jackin-capsule`. The client connects to the PID 1 daemon over `/run/jackin/jackin.sock` and renders the in-container multiplexer.
