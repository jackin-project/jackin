...
9. **Attach the primary agent session** via `docker exec -e TMUX= -it <container> tmux new-session -A -s jackin-primary -- /jackin/runtime/entrypoint.sh [model-flags]`. The `-A` flag attaches if `jackin-primary` already exists or creates it fresh. `TMUX=` prevents nested-session warnings when the operator's terminal is itself inside tmux.

The detached-start + exec split decouples the container's lifetime from the foreground session: closing the terminal tab detaches from the tmux session inside the container, but the supervisor (PID 1) and the tmux server keep running. `jackin hardline` reattaches to the same `jackin-primary` session or creates a new one. Cleanup distinguishes three post-session states:
...
Sessions are tmux sessions inside the running container. Each session type has a distinct naming convention and lifetime:

| Session type | tmux name | Created by | Reconnectable |
|---|---|---|---|
| Primary | `jackin-primary` | `jackin load` or `jackin hardline` | Yes — `jackin hardline` uses `tmux new-session -A` which attaches-or-creates |
| Secondary agent | `jackin-<agent>-<id>` | `hardline --new` or console `a`/`A` | Yes — another `hardline --new` or `a` from the same instance |
| Shell | (no session) | `hardline --shell` or console `x`/`X` | No — ephemeral `docker exec /bin/zsh` |

When an agent session exits cleanly (the operator types `/exit` or the agent finishes), the tmux session is removed but the container and supervisor keep running. The container only stops when the operator explicitly stops it, or when `jackin load` teardown runs after the primary session ends cleanly (exit code 0).
...
Implementation: <RepoFile path="docker/runtime/supervisor.sh" />, <RepoFile path="src/runtime/launch.rs" /> (`launch_role_runtime`), <RepoFile path="src/runtime/attach.rs" /> (`attach_primary_session`, `spawn_agent_session`, `spawn_shell_session`).
