...
The reusable host widget is `<RepoFile path="src/console/widgets/confirm.rs" />` (`ConfirmState` + `render`). In-container surfaces that cannot link the host crate (different binary, separate process) must mirror the layout in their own renderer — the in-container daemon's `render_confirm_action` in <RepoFile path="crates/jackin-container/src/dialog.rs" /> is the canonical copy and any other in-container confirm must call it (or a sibling helper with the same layout output) rather than rolling a third shape.
...
Two implementation shapes carry this contract today:

1. **`Vec<Dialog>` stack** (canonical, used by `crates/jackin-container/src/dialog.rs`). `dialog_top()` is "is a dialog open"; entries land via `dialog_push`; back-nav arms call `dialog_pop_one`; terminal-action arms call `dialog_clear`.
2. **Per-flow return-path stash** (legacy, used by host console for the auth flow — `pending_auth_form_return`, `pending_env_key`, `pending_picker_target`). The parent's state is stashed in an `EditorState` field before the child modal mounts, and the child's cancel arm rebuilds the parent from the stash. Works for shallow chains but does not compose past two levels and is easy to forget when adding a new transition.

The bare `Option<Modal>` slot in <RepoFile path="src/console/manager/state.rs" /> is *not* a third valid shape — it cannot represent a parent under a child, so any new modal→modal transition added to a code path that still uses `Option<Modal>` regresses this rule by construction. New code MUST use one of the two shapes above; the eventual direction for the host console is to converge on the `Vec<Modal>` stack and retire the stash slots.

Known compliance gaps as of writing — see <RepoFile path="src/console/manager/input/editor.rs" /> Modal handlers where `editor.modal = None` is set on `Cancel` while a parent modal exists in the operator's mental chain:

- `Modal::RoleOverridePicker` cancel (line ~1040) — Esc from the role-override picker should restore the parent `ScopePicker` rather than dump to the Secrets tab.
- `Modal::ScopePicker` → `Modal::TextInput` / `Modal::RoleOverridePicker` (line ~1117) — TextInput / role picker cancel currently sets `modal = None`; should restore the `ScopePicker`.
- `Modal::AuthRolePicker` cancel (line ~1209) — Esc on the role picker dumps to the Auth tab; should restore whatever opened it (sentinel-add row).
- `Modal::SourcePicker` cancel (line ~1162) and the `Modal::OpPicker` / nested `Modal::TextInput` paths chained from it should walk back one step rather than clearing both.
- `FileBrowser → MountDstChoice → TextInput` chain inside the editor mount-add flow (Editor TextInput line ~1695, MountDstChoice line ~1801) — none of the three currently restores its parent.

Treat new Esc-back regressions on any surface as bugs; fix the immediate flow with the appropriate shape (stack push/pop for in-container, stash-and-restore for host editor where the stack refactor has not landed yet).
