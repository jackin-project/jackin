...
The shared `op_picker` Create mode and the plain-prompt CLI path are both shipped. The remaining unbuilt piece is the **console-side "generate token" action** — running the `claude setup-token` OAuth mint from inside the console and writing the result through the extended `op_picker`. That needs suspend/resume-around-PTY plumbing in the console event loop so the OAuth capture can run without tearing down the console; it is tracked as the [Workspace Claude Token Setup](/reference/roadmap/workspace-claude-token-setup/) generate-token action.
