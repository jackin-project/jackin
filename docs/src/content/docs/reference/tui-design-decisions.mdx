...
The rich launch surface is built from the same chrome as every other top-level screen (see *Brand chrome is shared across every top-level screen*): the shared ` jackin' ` pill header (labelled `loading`) on top, a single phosphor-bordered body box titled `Loading <role> into <target>`, and a white bottom **status bar** rendered through the shared `status_bar` widget (<RepoFile path="src/console/widgets/status_bar.rs" />) — the same white band the in-container multiplexer paints at the bottom. The status bar shows the current activity on the left (black, bold) and the container's short instance id as a blue link chip on the right (falling back to the diagnostics run handle until the container is named). The white-band background and the blue link colour live in `jackin-tui` so the host bar and the multiplexer bar cannot drift. Making the instance-id chip clickable to open the same `ContainerInfo` dialog the multiplexer shows is deferred until the loading screen gains mouse handling. The body box holds a compact identity block (agent, workdir, any mounts whose host source differs from the container path with same-path mounts omitted, optional image and container) above a **sliding stage rail**: the ten stages render as markers (`●` done, `◉` active, `○` queued, `◌` skipped, `✕` failed) joined by a connector track that fills phosphor-green left→right as stages complete. The fill is eased toward its target each frame so a completed stage flows the green sweep forward smoothly instead of snapping. The active marker and its label pulse gently (white ↔ phosphor-green); a `just-completed · active · next` label row names where the launch came from, where it is, and where it is going; and the active stage's live detail renders below with a stable-width animated ellipsis so long Docker builds, workspace repository polling, and sidecar waits never read as frozen. The surface shows a diagnostics run handle only — it must never stream raw Docker logs, build output, per-repo pull output, stack traces, or debug traces; those belong in the JSONL run diagnostics artifact. A fatal failure reuses the shared red-border `error_popup` widget (`Enter`/`O` to dismiss); animated accents freeze while the popup owns the screen so no live cue moves behind the modal.
...
Full digital rain is a session-boundary animation, not a per-container animation. It runs before launch only when no jackin-managed role containers are active, and after the foreground session only when no role containers remain. `--no-rain` disables that boundary ritual. `--no-tui` disables only the rich renderer and keeps compact progress. `JACKIN_NO_MOTION=1` keeps the same layout while suppressing animated accents.
...
Hover-lift backgrounds (`TAB_BG_*_HOVER`) exist in the shared palette for the multiplexer's pointer feedback. The host console tab strips do not yet wire hover because the console subscribes only to button/drag/scroll mouse modes (`?1000h`/`?1002h`), not any-event motion (`?1003h`) — enabling motion tracking is a separate decision, deliberately deferred to avoid touchpad-motion flooding.
