...
### Why this rule exists — canonical counter-example

<RepoFile path="src/console/widgets/token_store_picker/mod.rs" /> is the canonical example of what must never happen. It duplicates the Account → Vault → Item navigation state machine from <RepoFile path="src/console/widgets/op_picker/mod.rs" /> — 1067 lines of closely parallel code, 100+ matched patterns. Every bug fix to `op_picker` navigation, filtering, R-key refresh, or section display must be manually reproduced in `token_store_picker` or the two implementations drift. The correct shape was to extend `op_picker` with a mode parameter that enables the extra steps (`[ + New item ]` sentinel, field-label input, `TokenStoreSelection` commit variants) rather than maintaining a second drill-down widget. Consolidating these is tracked in the [Workspace Claude Token Setup](/reference/roadmap/workspace-claude-token-setup/) roadmap item.
...
|---|---|---|---|
| Op picker (1Password drill-down) | <RepoFile path="src/console/widgets/op_picker/mod.rs" /> | Account → Vault → Item → Field 4-stage drill-down with filter, R-key refresh, collapsible section headers, background loading, and `op://` reference commit | Any flow that needs to select a 1Password field reference |
...
This is **the reference example** for how to design any multi-step TUI flow in jackin'. Every screen reuses a component from the catalog above; the flow adds **zero new widgets** and only **extends** `op_picker`. Read this whenever you design a new flow and ask "which existing component is each of my screens?" before writing any code.

The flow configures a Claude OAuth token for a workspace, launched entirely from inside `jackin console`. The operator never leaves the console and never runs a CLI command.

**Entry point — the Edit auth dialog.** The Auth tab's "Edit auth" dialog currently shows `Mode: oauth_token`, `CLAUDE_CODE_OAUTH_TOKEN required`, and a hint: *"tip: run jackin workspace claude-token setup &lt;workspace&gt; --vault &lt;vault&gt; from your shell."* That hint is the design defect: it pushes the operator out to the CLI. It must be replaced by an in-dialog action that starts the wizard below. The flow begins from this dialog, not from a CLI flag.

The screens, in order, each mapped to the component it reuses:

| Step | Screen | Component reused | Notes |
|---|---|---|---|
| 1 | **Scope**: `All roles` / `Specific role` | Scope picker (`scope_picker.rs`) | Identical two-button dialog to the "New environment variable" scope step. |
| 2 *(if "Specific role")* | **Role**: filter + role list | Role picker (`role_picker.rs`) | Same `RolePickerState` the env-var flow uses. Skipped entirely for "All roles". |
| 3 | **Source**: `Plain text` / `1Password` | Source picker (`source_picker.rs`) | Same two-button shape. Titled `Source for CLAUDE_CODE_OAUTH_TOKEN`. |
| 4a *(if "Plain text")* | **Credential** input box | Text input box (`text_input.rs`) | The reusable input box. **For the Claude token flow the value comes from the Claude OAuth capture, not typed here** — but the box is the canonical reference component for every other flow that needs a typed value. |
| 4b *(if "1Password")* | **Account → Vault → Item → Field** drill-down | `op_picker` (extended) | The existing `op_picker`, reused verbatim for the four drill-down screens; extended only with the creation sentinels below. |

**Step 4b in detail — extending `op_picker`, not copying it.** The 1Password branch reuses the existing `op_picker` Account, Vault, Item, and Field screens exactly as they render today. The only additions are creation sentinels, each following the `+ Add X` creation-sentinel pattern (a `+ ` row at the bottom of the list):

- **Item stage** — a `+ New item` sentinel below the item list. Selecting it opens the **text input box** to name the new item, then the captured token is written into it.
- **Field stage** — a `+ New field` sentinel and a `+ New section` sentinel below the field list. Selecting either opens the **text input box** to name the new field or section.
- Selecting an **existing field** writes the token into that field in-place.
...
**Implementation contract.** These sentinels extend `op_picker` via a mode parameter (e.g. `OpPickerMode::TokenStore`) that turns the creation rows on. They MUST NOT be implemented by forking `op_picker` into a second module. The sentinel selections produce `TokenStoreSelection::NewItem` / `TokenStoreSelection::ReplaceItem` commit variants instead of the plain `op://` reference string. The current <RepoFile path="src/console/widgets/token_store_picker/mod.rs" /> — a 1067-line copy of the `op_picker` state machine — is exactly what this rule forbids and is tracked for removal in [Workspace Claude Token Setup](/reference/roadmap/workspace-claude-token-setup/).

### Why the flow must start in the TUI, not the CLI

The CLI `--interactive` flag spins up a standalone alt-screen TUI session inside a running terminal, captures the operator's selection, then returns. This approach:

- Cannot compose with the console's existing event loop (it opens a second raw-mode terminal via `token_store_dialog.rs`).
- Loses all the console's context (workspace state, auth tab focus, operator's in-progress edits).
- Requires the operator to leave the console, run a CLI command, then return — breaking their flow.

The Auth tab is already open when the operator wants to configure a token. The wizard flow opens directly from there, using the console's own modal stack, and the operator never leaves the console. The CLI path (`jackin workspace claude-token setup --vault … --item-name …`) remains valid for scripted or automated workflows; it is not the interactive path.
