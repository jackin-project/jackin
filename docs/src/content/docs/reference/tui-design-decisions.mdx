...
1. **Mouse click handler (`update_scroll_focus`)** — for blocks that store focus state (e.g. `scroll_focused: bool` on individual state structs), the handler should also check scrollability before setting `scroll_focused = true`, so the state field accurately reflects whether the block is interactive. If the state field is `true` but `render_scrollable_block` shows no green border because content fits, there is no visible inconsistency — but the state is misleading for keyboard dispatch. For scrollable blocks whose focus is driven by `tab_content_scroll_focused` (the workspace editor non-Mounts tabs), the check is already built into the mouse handler.
