...
The rich launch surface follows console chrome rules: phosphor palette, tight bordered geometry, footer-only hints, and a single fatal-failure action (`Enter Close`). The main panel shows product-level stage status as a sliding stage rail, with completed stages compressing left, the active stage in focus, and queued stages staged to the right. Waiting states keep a small animated pulse so long Docker builds, workspace repository polling, and sidecar waits do not read as frozen. The surface shows a diagnostics run handle only. It must never stream raw Docker logs, build output, per-repo pull output, stack traces, or debug traces; those belong in the JSONL run diagnostics artifact.
