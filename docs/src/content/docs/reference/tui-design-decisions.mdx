...
Modal dialogs that open from a parent modal (`Menu → New tab` → AgentPicker; `Menu → Split pane` → SplitDirectionPicker → AgentPicker; `Menu → Close` → CloseTargetPicker → ConfirmClose) MUST push onto a dialog stack rather than replace the parent. Esc pops one frame so the operator returns to the previous dialog instead of being dumped back to the pane. Terminal actions (an agent picked, a destructive action confirmed, a copy-to-clipboard fired) clear the whole stack so the operator drops onto the focused pane in one step. Mouse-click outside the box pops one frame, matching Esc.

Two pieces follow from the rule:

- The dialog state lives on a `Vec<Dialog>` (top of stack = visible). `dialog_top()` is the canonical "is a dialog open" check; new entries land via `dialog_push`; the back-nav arm calls `dialog_pop_one`; terminal-action arms call `dialog_clear`. The bare `Option<Dialog>` shape pre-dates this rule and is no longer correct for any surface that supports sub-dialogs.
- A sub-dialog's "title" string must read as a step inside its parent's flow — `Split: ← Left` for the agent picker opened from the SplitDirectionPicker, not `Agent picker` again. The operator's mental model is the breadcrumb chain; the title is the part that surfaces it.
