...
The left pane scroll uses the shared `jackin_tui::scroll` clamping path after each wheel/key mutation, then `render_scrollable_block` clamps defensively again on every frame.
