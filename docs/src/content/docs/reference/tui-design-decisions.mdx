...
| Scope picker | <RepoFile path="src/console/widgets/scope_picker.rs" /> | Two-button horizontal strip for binary choices ("All roles" / "Specific role", "Plain text" / "1Password") | Any two-option exclusive-choice prompt |
| Source picker | <RepoFile path="src/console/widgets/source_picker.rs" /> | Two-button horizontal strip for value-source selection | Choosing between input sources |
...
| Panel rain | <RepoFile path="src/console/widgets/panel_rain.rs" /> | Background rain animation for visual ambiance | Background decoration |
...
**Text input**: `TextInputState` in <RepoFile path="src/console/manager/state.rs" /> (host console) or its equivalent in the capsule dialog layer. Reuse for any single-line text entry box.

**Two-button choice pattern** (`scope_picker.rs`, `source_picker.rs`): when a step in a flow requires the operator to pick one of exactly two options, use or model after this pattern. It produces a consistent visual shape the operator recognises immediately.
...
This is the intended shape for interactive Claude token configuration launched from the Auth tab in the jackin' console. It demonstrates how to compose existing components into a multi-step flow with zero new widgets.

**Entry point**: The Auth tab shows `oauth_token` mode with an "Edit auth" action. The operator opens the auth form. If no token is configured yet, the form surfaces a "Set up token" action (replacing the current hint that sends the operator to the CLI).

**Step 1 — scope choice** (`scope_picker.rs`): "All roles" / "Specific role". Reuses the two-button horizontal strip that the "New environment variable" dialog already uses for scope selection. No new component needed.

**Step 2 (conditional) — role picker** (`role_picker.rs`): If the operator chose "Specific role", show the filter-and-select role list. Reuses the same `RolePickerState` the env-var flow uses. If "All roles" was chosen, this step is skipped.

**Step 3 — source choice** (`scope_picker.rs` or `source_picker.rs`): "Plain text" / "1Password". Same two-button pattern. No new component needed.

**Step 4a — plain text input** (`TextInputState`): If the operator chose "Plain text", show a single-line text input box. Reuses the `TextInputState` the credential input flow already uses. The value is written to config on commit.

**Step 4b — 1Password field picker** (extended `op_picker`): If the operator chose "1Password", open the existing `op_picker` Account → Vault → Item → Field drill-down. For this flow, `op_picker` needs two additional sentinel rows in the Item and Field stages:
- `[ + New item ]` — creates a new 1Password item to receive the token.
- `[ + New field ]` / `[ + New section ]` — creates a new field inside an existing item.

These sentinels extend `op_picker` via a mode parameter (e.g. `OpPickerMode::TokenStore`). They must **not** be implemented by forking `op_picker` into a second module. The sentinel rows produce a `TokenStoreSelection::NewItem` or `TokenStoreSelection::ReplaceItem` commit variant instead of the normal `op://` reference string.

**What this flow replaces**: The `--interactive` CLI flag added in the `feature/interactive-token-store-picker` PR is a stopgap. The TUI flow above is the correct operator-facing shape. The `token_store_picker` widget and `token_store_dialog.rs` standalone event loop are technical debt created because the TUI flow was not built first — they are candidates for removal once the Auth-tab flow lands. The roadmap item [Workspace Claude Token Setup](/reference/roadmap/workspace-claude-token-setup/) tracks the consolidation.
...
- Cannot compose with the console's existing event loop (it opens a second raw-mode terminal).
