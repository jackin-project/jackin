...
This is **the reference example** for how to design any multi-step TUI flow in jackin'. Every screen reuses a component from the catalog above; the flow adds **zero new widgets** and only **extends** `op_picker` — and that extension (`OpPickerMode::Create`) is already shipped. Read this whenever you design a new flow and ask "which existing component is each of my screens?" before writing any code.

The flow configures a Claude OAuth token for a workspace, launched entirely from inside `jackin console`. The operator never leaves the console and never runs a CLI command. The drill-down and creation widgets are shipped; wiring this flow to the Auth tab — running the OAuth mint from inside the console — is the remaining work, tracked as the [Workspace Claude Token Setup](/reference/roadmap/workspace-claude-token-setup/) generate-token action.
...
| 4b *(if "1Password")* | **Account → Vault → Item → Field** drill-down | `op_picker` (Create mode) | The existing `op_picker`, reused verbatim for the four drill-down screens; its shipped `OpPickerMode::Create` adds the creation sentinels below. |
...
- **Console wizard flow.** The rich, multi-step wizard above belongs in `jackin console`, opened from the Auth tab — already on screen when the operator wants to configure a token — using the console's own modal stack so the operator never leaves the console and keeps all of its context (workspace state, auth-tab focus, in-progress edits). Every screen reuses a shared component: `scope_picker`, `role_picker`, `source_picker`, `text_input`, and `op_picker` in Create mode.

The shared `op_picker` Create mode, the plain-prompt CLI path, and the console-side **generate-token action** are all shipped. On the workspace-level Claude `oauth_token` auth form, `G` opens `op_picker` in Create mode; on commit the `run_console` loop suspends the terminal (leaves raw-mode + alt-screen, mirroring `TerminalGuard`), runs the `claude setup-token` OAuth mint + 1Password write via `token_setup::run_setup`, then resumes — the one place in the console that hands the real terminal to a child process. Role-level generate and a plain-text generate target remain unwired (the action is gated to the workspace-level Claude `oauth_token` slot); see [Workspace Claude Token Setup](/reference/roadmap/workspace-claude-token-setup/).
