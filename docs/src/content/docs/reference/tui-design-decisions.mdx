...
- Uses the shared fixed-prefix line renderer (`render_line_with_fixed_prefix_scroll`) so the disclosure/cursor prefix stays visible while only the workspace label scrolls.
...
- Selected and hovered rows must paint their background across the full visible row after horizontal scroll. Do not let generic line scrolling clip the highlighted prefix or leave unpainted cells inside the row.
