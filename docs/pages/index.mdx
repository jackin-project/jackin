...
<HomePage.Root>
  <HomePage.Logo />
  <HomePage.Tagline>Isolate AI coding agents in Docker containers. You're the Operator. They're already inside.</HomePage.Tagline>
  <HomePage.Buttons>
...
- **Isolated by default** — Each agent runs in its own Docker container. It can only see directories you explicitly mount and tools baked into its image.
- **Full autonomy inside** — Agents run with `--dangerously-skip-permissions` inside the container boundary. Claude is unrestricted there, but the container still limits its world.
- **Operator control** — You control the blast radius: which folders agents read/write, whether mounts are read-only, and which Docker network they use.
- **Scale with confidence** — Run multiple agents simultaneously, each in their own isolated environment with separate state and networking.
