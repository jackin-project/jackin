...
- **Credential files inside the container.** Credentials from an assigned profile account are readable by the agent after forwarding. API-key and OAuth-token accounts also provide credentials to the selected process. Docker profiles do not replace workspace account authorization.
