...
The account registry replaces the old per-agent `auth_forward` and `sync_source_dir` policy tables. Global config uses schema `v1alpha11`; workspace files use `v1alpha9`.
