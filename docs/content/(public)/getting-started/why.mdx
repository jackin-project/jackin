...
| **The agent inside** | One of six launch-supported branches (`Claude Code`, `Codex`, `Amp`, `Kimi`, `OpenCode`, or `Grok Build`); the agent parser/catalog recognizes 12 slugs, while `Antigravity`, `Gemini`, `Cursor`, `Muse`, `omp`, and `Hermes` are not wired to an entrypoint branch yet |
...
<Aside type="note">
jackin❯ is currently a proof of concept. The agent parser/catalog recognizes 12 built-in agent slugs: `claude`, `codex`, `amp`, `kimi`, `opencode`, `grok`, `antigravity`, `gemini`, `cursor`, `muse`, `omp`, and `hermes`. The container entrypoint currently supports six launch branches — `claude`, `codex`, `amp`, `kimi`, `opencode`, and `grok` — while the other six remain parser/catalog entries rather than supported container launch branches. The isolation model is runtime-agnostic by design, but the implementation contract is still moving quickly while the project is pre-stable.
</Aside>
