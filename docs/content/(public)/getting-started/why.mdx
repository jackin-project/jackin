...
<Aside type="note">
jackin❯ is currently a proof of concept. The built-in agent catalog currently contains 12 agents: `Claude Code`, `Codex`, `Amp`, `Kimi`, `OpenCode`, `Grok Build`, `Antigravity`, `Gemini`, `Cursor`, `Muse`, `omp`, and `Hermes`. Six high-autonomy launch branches currently cover `claude`, `codex`, `amp`, `kimi`, `opencode`, and `grok`; full feature parity across all 12 is tracked on the [Roadmap](/roadmap/). The isolation model is runtime-agnostic by design, but the implementation contract is still moving quickly while the project is pre-stable.
</Aside>
