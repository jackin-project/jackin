...
Without `--stdin` or `--secret-ref`, key/token input uses a masked prompt. Supported providers are `anthropic`, `openai`, `amp`, `xai`, `opencode`, `moonshot`, `zai`, `minimax`, `google`, `cursor`, `meta`, and `openrouter`. Profile agents are `claude`, `codex`, `amp`, `kimi`, `opencode`, `grok`, `antigravity`, `gemini`, `cursor`, `muse`, `omp`, and `hermes`. `omp` and `hermes` have no native provider: their accounts always need an explicit `--provider`.
