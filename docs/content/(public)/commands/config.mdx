...
Agent credentials are managed with `jackin account` and `jackin workspace account`.
The old `config auth` policy command has been removed. See [Agent Accounts](/guides/authentication/agents/) for discovery, profile folders, provider keys, and workspace authorization.
