...
Kimi profile accounts authenticate using their selected host profile directory (default discovery: `~/.kimi-code`). jackin❯ forwards `~/.kimi-code/config.toml`, files under `~/.kimi-code/credentials/`, and `~/.kimi-code/device_id` into the container before Kimi starts. jackin❯ launches Kimi with `--yolo` by default; `model` is appended as `kimi --model <model>` when set. Moonshot API-key accounts supply `KIMI_API_KEY`; standalone OAuth-token accounts are Claude-specific.
...
Provider routing and provider-specific models belong to named accounts. Set the API account's `model` when adding or editing the account, then assign that account to the workspace agent. The selected account model overrides the role's agent model; an explicit launch `--model` overrides both.

Role `[<agent>.providers.<provider>]` tables are removed and rejected. Keep `[claude].model`, `[codex].model`, or `[opencode].model` only for role defaults.
...
- `setup_once` is executed as a child process and gated by a marker file in jackin❯ per-instance runtime state. It runs again only if the previous run failed or the instance state was purged. Treat it as "run once per jackin❯ instance, not once per process restart" — under session isolation the marker is session-scoped, so it runs once per session. Use it for installs, downloads, and one-time config writes that are still safe to repeat after state is intentionally discarded. Environment exports from this script do not reach the agent.
...
Hooks that need to persist files (markers, caches, helper binaries) must write under `JACKIN_HOOK_STATE_DIR`, which the entrypoint exports before any hook runs. It resolves to `$JACKIN_SESSION_STATE_DIR/hook-state` under session isolation and falls back to `/jackin/state/hook-state` otherwise. Do not write directly under capsule-wide `/jackin/state` — Landlock-confined agent sessions cannot write there and the write fails with `EACCES`. Namespace your files under a role-specific subdirectory and fall back to `/jackin/state/hook-state/<role>` when the variable is unset:

```bash title="hooks/setup-once.sh"
state_dir="${JACKIN_HOOK_STATE_DIR:-/jackin/state/hook-state}/my-role"
mkdir -p "$state_dir"
```
