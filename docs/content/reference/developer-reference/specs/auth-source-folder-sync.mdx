...
Amp and OpenCode profile credentials may also carry explicit `xdg_roots`
(`data`, `config`, and `cache`). The data and config roots provide the selected
profile state; the configured cache root is mounted directly as the host-side
source for that instance's unique `/home/agent/.cache/<agent>[-<slot>]` target.
The capsule exports that target as `XDG_CACHE_HOME`, so the PTY-session cache
does not replace the declared root. Overlapping configured cache roots and
overlapping private container mount paths are rejected.
