...
The rich account editor belongs in `jackin console`. It reuses masked text entry, the folder browser, and secret-reference selection. Workspace account controls stage allowed IDs and agent/role selections until the workspace save commits.

`jackin account add` uses plain prompts or standard input for secret values, and supports explicit secret references for scripting. It never launches another alternate-screen UI. The retired workspace token-generation wizard is no longer an active account setup path.
