...
## Capsule terminal model pipeline (termpane)

The `jackin-capsule` render is **all Ratatui**. termpane is the per-pane terminal **model**, not a separate render pipeline:
...
2. **Per-pane terminal model (`termpane`).** Each pane's body comes from the agent's PTY. termpane parses the agent's escape bytes and maintains a cell grid for that pane — the `vte → DamageGrid → GridSnapshot` model in the [termpane](https://github.com/tailrocks/termpane) crate (a pinned git dependency). That model is then *rendered* by `PaneBodyWidget`, a Ratatui `Widget` that paints the `GridSnapshot`'s cells into the same Ratatui buffer as everything else.
...
- **Own the grid.** `DamageGrid` ([termpane src/grid.rs](https://github.com/tailrocks/termpane/blob/v0.6.4/src/grid.rs)) is a ring-backed `RowStore` terminal grid owned by termpane; live capsule sessions share one daemon `RowArena` for primary, alternate, and scrollback row buffers. It records dirty rows at mutation time (`DirtyTracker`). It is the capsule's sole terminal model: screen cells, alternate screen, scrollback, cursor, mouse/focus/bracketed-paste/kitty-keyboard mode state, and the typed OSC/CSI passthrough stream all live here. The capsule previously used a forked `vt100::Parser`; that dependency has been removed from production, test, fuzz, benchmark, and source-policy paths.
...
- **Correctness gate: conformance replay harness.** `tests/conformance.rs` in termpane feeds identical byte streams to `DamageGrid` in one chunk and byte-by-byte, then asserts identical final grids and geometry/style/cursor invariants. Any behavior change to `DamageGrid` must keep the harness green.
