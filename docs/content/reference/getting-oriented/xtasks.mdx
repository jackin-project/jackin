...
`cargo xtask pr body --base <ref>` assembles a PR body from the diff. It prints a classified change digest (rust / docs / capsule / schema, plus the changed-file list) to **stderr**, and the body skeleton — <RepoFile path="docs/PULL_REQUEST_TEMPLATE.md">docs/PULL_REQUEST_TEMPLATE.md</RepoFile> with the verify-locally `###` blocks that do not apply to the diff removed — to **stdout**. The split lets you redirect the body to a file (`cargo xtask pr body > body.md`) while reading the digest in the terminal. Block selection: Checkout always; Static checks / Rust tests / User smoke on a Rust change; Schema migration smoke on a versioned-schema touch; Docs checks / Documentation on a `docs/**` change; jackin-capsule smoke on a `crates/jackin-capsule/` change. The prose sections stay as template placeholders for the author to fill.
