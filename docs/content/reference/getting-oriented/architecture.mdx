...
│       ├── claude/                # Claude auth handoff (provisioned when Claude is the active agent)
│       │   ├── credentials.json   # Forwarded OAuth credentials under a profile account
│       │   └── account.json       # Forwarded Claude Code account metadata under a profile account
│       ├── codex/                 # Codex auth handoff (provisioned when Codex is the active agent)
│       │   └── auth.json          # ChatGPT OAuth tokens (synced from host ~/.codex/auth.json under a profile account)
│       ├── amp/                   # Amp auth handoff (provisioned when Amp is the active agent)
│       │   └── secrets.json       # Amp API key or OAuth token (synced from host ~/.local/share/amp/secrets.json under a profile account)
│       ├── kimi-code/             # Kimi auth handoff (provisioned when Kimi is the active agent)
│       │   ├── config.toml        # Kimi config (synced from host ~/.kimi-code/config.toml under a profile account)
│       │   ├── credentials/       # Kimi credentials (synced from host ~/.kimi-code/credentials under a profile account)
│       │   └── device_id          # Kimi device id (synced from host ~/.kimi-code/device_id under a profile account)
│       ├── opencode/              # OpenCode auth handoff (provisioned when OpenCode is the active agent)
│       │   └── auth.json          # OpenCode provider credentials (synced from host ~/.local/share/opencode/auth.json under a profile account)
...
The project targets **Rust edition 2024** with the newest pinned stable Rust, currently **1.97** (see `rust-version` in <RepoFile path="Cargo.toml">Cargo.toml</RepoFile> and the exact channel in <RepoFile path="rust-toolchain.toml">rust-toolchain.toml</RepoFile>), and `unsafe_code = "forbid"` at the crate root. Older compilers are unsupported. Clippy lints are configured with `correctness` and `suspicious` set to `deny` and `complexity`/`style`/`perf`/`pedantic`/`nursery` set to `warn` — see <RepoFile path="Cargo.toml">Cargo.toml</RepoFile> for the full lint table.
...
Agent runtimes use a sealed adapter registry. Named accounts own provider compatibility, credential resolution, and per-agent routing.
...
- `state_paths()` — `AgentStatePaths { credential_dir, credential_file, folder_env_var }` — the per-agent credential location on the host and the environment hint for the operator. Used by account discovery and profile-folder provisioning.
- `parse_version(output)` — parses `--version` stdout to extract a semver string, replacing five per-agent branches.

**Named accounts** (<RepoFile path="crates/jackin-config/src/accounts.rs">crates/jackin-config/src/accounts.rs</RepoFile>) define provider identity, agent compatibility, profile folders, API keys, OAuth tokens, and account bindings. Workspace agent assignments resolve a named account before launch; the capsule cannot select a different provider from ambient credentials.

<RepoFile path="crates/jackin-runtime/src/runtime/launch/account_config.rs">crates/jackin-runtime/src/runtime/launch/account_config.rs</RepoFile> materializes selected-account Codex and OpenCode routing. <RepoFile path="crates/jackin-env/src/accounts.rs">crates/jackin-env/src/accounts.rs</RepoFile> resolves protected credentials per agent. <RepoFile path="crates/jackin-capsule/src/session.rs">crates/jackin-capsule/src/session.rs</RepoFile> strips inherited account variables and injects only that agent's credentials.

The protocol `Provider` enum retains display labels. It no longer owns availability, endpoint generation, or credential lookup. Provider models belong to accounts; role schema `v1alpha7` rejects the removed per-provider model tables.

**Sealed runtime trait.** `AgentRuntime` remains sealed so extending the built-in agent set requires changing the registry and reviewing its runtime contract.
