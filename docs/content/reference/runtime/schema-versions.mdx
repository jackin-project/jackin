...
| `~/.config/jackin/workspaces/<name>.toml` | `v1alpha9` | jackin❯ | Automatic on startup |
...
**Summary**: stamps an initialized `[bootstrap]` sentinel on upgrades so
pre-existing installations are not treated as fresh installs. An installer
marker with `fresh_install = true` remains untouched.
...
|---|---|---|---|---|
| `version` | string | Yes | `v1alpha1` | Current value: `v1alpha11` |
...
| `[bootstrap].version` | integer | Yes | `v1alpha11` | Bootstrap sentinel schema version; currently `1` |
| `[bootstrap].fresh_install` | bool | No | `v1alpha11` | Whether an installer-created config still needs its first scan |
