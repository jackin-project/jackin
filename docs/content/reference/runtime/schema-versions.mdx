...
fingerprint in `account_scan_exclusions`. Multi-provider profile credentials
also persist the exact `source_selector.entry` and optional
`source_selector.profile` fields for the selected source-store identity.
Explicit account scans skip that source after reload; direct account
registration clears the matching exclusion.
...
| `[accounts.<id>]` | table | No | `v1alpha10` | One reusable account record |
| `[accounts.<id>].enabled` | bool | No | `v1alpha10` | Whether the account may authenticate launches; defaults to `true` |
| `[accounts.<id>].name` | string | Yes | `v1alpha10` | Human-readable account display name |
| `[accounts.<id>].provider` | string | Yes | `v1alpha10` | Canonical provider slugs: `anthropic`, `openai`, `amp`, `xai`, `opencode`, `moonshot`, `zai`, `minimax`, `google`, `cursor`, `meta`, or `openrouter` (`openrouter` is for multi-provider clients only) |
| `[accounts.<id>].credential` | table | Yes | `v1alpha10` | Credential source configuration |
| `[accounts.<id>.credential].type` | string | Yes | `v1alpha10` | `profile`, `api_key`, or `oauth_token` |
| `[accounts.<id>.credential].agent` | string | Conditional | `v1alpha10` | Target agent for `profile` and `oauth_token` credentials; canonical agent slugs: `claude`, `codex`, `amp`, `kimi`, `opencode`, `grok`, `antigravity`, `gemini`, `cursor`, `muse`, `omp`, or `hermes` |
| `[accounts.<id>.credential].directory` | string | Conditional | `v1alpha10` | Host directory containing a `profile` credential |
| `[accounts.<id>.credential].value` | string | Conditional | `v1alpha10` | Secret reference or literal for `api_key` and `oauth_token` credentials |
...
| `[roles.<role>]` | table | No | `legacy` | Role source configuration |
...
| `workspaces` | table | No | `legacy` | In-memory/legacy embedded workspace map; migrated into workspace files |
| `role_repo_refresh_ttl_seconds` | integer | No | `legacy` | Launch-time role-repo refresh cache window; omitted uses the built-in 60-second default, while `0` forces a fetch on every launch |
...
| `default_agent` | string | No | `legacy` | `claude`, `codex`, `amp`, `kimi`, `opencode`, `grok`, `antigravity`, `gemini`, `cursor`, `muse`, `omp`, or `hermes`; omitted lets launch resolution use the role manifest or prompt |
...
| `[roles.<role>.github].auth_forward` | string | No | `legacy` | `sync`, `token`, or `ignore` |
...
| `[github].auth_forward` | string | No | `legacy` | Workspace GitHub auth override: `sync`, `token`, or `ignore` |
