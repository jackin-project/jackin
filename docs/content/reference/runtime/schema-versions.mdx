...
Named accounts replace per-agent authentication-policy tables. Global configuration owns account credentials; workspaces own allowed IDs and agent selections, with optional role selections. Existing agent policy tables are rejected before writing migration output because implicit credential grants cannot be converted into explicit authorization safely.
...
| Field | Type | Required | Since | Notes |
|---|---|---|---|---|
| `version` | string | Yes | `v1alpha1` | Current value: `v1alpha8` |
