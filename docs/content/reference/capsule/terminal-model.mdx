...
jackin❯ Capsule uses [`termpane`](https://github.com/tailrocks/termpane) as its owned terminal model (a pinned git dependency, currently tag `v0.1.0`). This page records why the old `vt100` dependency was retired and what `termpane` owns.
...
## What `termpane` Does Differently

| Concern | Old `vt100` path | `termpane` path |
|---|---|---|
...
| Scrollback | external screen plus Capsule-side bookkeeping | primary, alternate, and scrollback rows in termpane row stores |
...
- `termpane` owns the virtual-terminal profile: display width, DSR replies, reported colors, SGR support, DECRQM mode-2027 stance, and OSC policy are stable for the session.
...
`termpane` is the only terminal model in Capsule:
...
|---|---|
| `aa0e87` | real Capsule direct-grid-patch p99 timing and bytes per changed cell |
| `048a0e` | focused-path zero-allocation proof |
| `46eab7` | historical direct-`GridPatch` wire output matched theoretical minimum with 0.00% overhead, helping quantify the cost accepted when ADR-005 retired that tier |
| `0bb409` | headless 16- and 32-pane scaling measurements |

## Why Not Adopt A Different Emulator

The evaluated alternatives each helped clarify the design, but none replaced the need for `termpane`. The full survey from the pre-build audit:

| Candidate | Parser | Damage | Packed cells | Passthrough | Diff-to-escape | Verdict |
|---|---|---|---|---|---|---|
| `vt100` (fork) | own SM | none | `String`/cell | `Callbacks` (forked) | yes (unused by us) | retired — wrong shape |
| `vte` alone | yes | n/a | n/a | n/a | n/a | parser only — always depend |
| `alacritty_terminal` | `vte` | yes, line-level | yes, packed | GPU-shaped | none | grid + damage fit; no multiplexer API |
| `termwiz` / `wezterm-term` | own | yes (`Change`) | partial | yes | yes (unused for us) | too broad; unstable public API |
| Rio / `copa` | own | unknown | unknown | unknown | n/a | low adoption; not evaluated |
| `avt` | own | none | n/a | none | none | snapshot-only; not a grid |
| libvterm | C library | yes | yes | yes | yes | ruled out — non-Rust |
| Ghostty / Alacritty (full) | `vte` / own | yes | yes, packed | partial | GPU only | GPU-first; wrong output stage |
| Zellij `grid` | own | yes, per-row | yes | partial | own | closest design; not published |

**Key finding:** the "fast engines have no diff-to-escape" argument that originally dismissed `alacritty_terminal` is a strawman for jackin❯ — the Capsule owns its emit. What the model needs from a grid is **damage + packed cells**, where `alacritty_terminal` is strong and `vt100` was the weakest option.

- `alacritty_terminal` has a fast grid and damage ideas, but it is built for GPU terminal rendering and does not provide jackin❯ re-emitting multiplexer API.
...
The result is intentionally hybrid: depend on `vte` for the parser, borrow proven grid/damage ideas from mature terminals and multiplexers, and keep jackin❯'s capsule-specific model in a small Rust crate ([termpane](https://github.com/tailrocks/termpane)) that lives in its own repository.

## Borrow and re-implement ledger

`termpane` depends on one parser (`vte`) and re-implements grid/damage ideas from mature terminals with attribution. Every borrow site carries an inline attribution comment in source naming the project and license.

| Source | What termpane takes | How | License | Attribution |
|---|---|---|---|---|
| `vte` | VT/ANSI parser state machine | depend — never rebuild | MIT | `vte` crate (doy) |
| Alacritty ring-`Storage` | ring-backed row storage for primary, alternate, and scrollback rows | re-implement / reference | Apache-2.0/MIT | Alacritty project |
...
- `termpane` owns terminal state; Capsule code should not recreate a second terminal model beside it.
...
- Full-frame visual behavior stays under Ratatui and the TUI architecture rules; `termpane` is the model, not a parallel chrome renderer.
