...
3. **Dependency review beyond advisories.** `cargo deny`/`cargo audit` catch known-bad dependencies; they say nothing about unreviewed code. `crates/AGENTS.md` (removed in #956; consolidated into root AGENTS.md) records a deliberate cargo-vet/cargo-crev no-adopt while jackin❯ is solo-maintainer; this item is that decision's named revisit trigger. Trial [cargo-vet](https://mozilla.github.io/cargo-vet/) as an advisory lane in exemptions-heavy mode, importing shared audits so the marginal cost of an explicit audit record per new/updated dependency stays near zero. Promote to a gate only if audit-import coverage proves the burden negligible, and update the AGENTS entry when the trial starts.
