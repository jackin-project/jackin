...
- It pairs cleanly with [agent runtime
  status](/roadmap/agent-runtime-status/) and [tag
  protocol](/roadmap/agent-tag-protocol/): the captured tags
...
- It uses the same handler-resolution machinery as the [operator handler
  system](/roadmap/operator-handler-system/) — different config
...
jackin❯ existing reserved keystrokes (the navigation set in the
...
- `{repo}`, `{issue}`, `{pr}`, `{link}` — most-recent value from the
  [tag protocol](/roadmap/agent-tag-protocol/) index
...
- **Where does the tool's stdout go?** A modal that overlays the
  ...
  panel](/roadmap/console-resource-panel/) infrastructure.
...
- <RepoFile path="crates/jackin-config/src/app_config.rs">crates/jackin-config/src/app_config.rs</RepoFile> — `[[tool]]` parsing
- <RepoFile path="crates/jackin/src/console/tui.rs">crates/jackin/src/console/tui.rs</RepoFile> — keybinding
...
- [Agent tag protocol](/roadmap/agent-tag-protocol/) — source
