...
4. **Failure and loop control.** A per-worker budget cap and a maximum re-delegation count so the planner cannot spin cheap workers indefinitely. This pairs with the budget-enforcement idea tracked in the [brainstorm backlog](/roadmap/brainstorm-backlog/).
