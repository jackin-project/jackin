...
[agent tag protocol](/roadmap/agent-tag-protocol/), the operator
...
- It's the most operator-visible payoff of the [tag
  protocol](/roadmap/agent-tag-protocol/) — without live status,
...
- It's the lowest-risk way to introduce a third-party network dependency
  (Octocrab) into jackin❯ runtime — the surface is narrow (read-only
...
Persist via the [persistent storage layer](/roadmap/persistent-storage-layer/).

### Auth source

Reuses the [credential source pattern](/roadmap/credential-source-pattern/):
...
[`web` handler](/roadmap/operator-handler-system/).
...
- Operator-host PAT via the existing credential resolution
  ...
  source pattern](/roadmap/credential-source-pattern/) lands.
...
- **Per-instance cache vs operator-global.** Per-instance matches
  jackin❯ existing data layout but means a link tracked by two
...
- **Token scope guidance.** What's the minimum-privilege token the docs
  ...
  need `repo`. multicode docs are clear on this; jackin❯ docs should
  match.
...
- <RepoFile path="crates/jackin/src/console/tui.rs">crates/jackin/src/console/tui.rs</RepoFile> — links panel
