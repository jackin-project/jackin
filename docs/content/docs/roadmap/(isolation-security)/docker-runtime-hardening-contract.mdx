...
- DinD is tier-aware: `none`, `rootless`, and `privileged` are selectable; rootless uses `docker:dind-rootless`, fails closed where cgroup v2 is required, and is the recommended explicit grant for Docker workflows on cgroup v2.
...
- <RepoFile path="plans/051-rootless-dind-default-research.md" /> — completed rootless DinD default decision.
