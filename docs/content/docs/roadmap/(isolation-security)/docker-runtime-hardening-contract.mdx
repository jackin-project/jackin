...
**Status**: Partially implemented — Docker profile enforcement is wired and `standard` is the compiled default; credential env-only posture and rootless-default research remain
...
1. Research whether rootless DinD can become the standard default after the same workload evidence exists.
2. Keep the `standard` matrix green as new built-in roles and agent workflows are added.
...
2. **Rootless default research.** Rootless DinD is selectable and passed the cgroup-v2 matrix cell as an explicit grant, but making it the standard default remains research until the supported-host evidence proves it should be granted automatically. Sysbox is dropped unless rootless proves insufficient.
...
- <RepoFile path="plans/049-standard-compatibility-matrix.md" /> — completed default-flip evidence gate.
- <RepoFile path="plans/050-standard-default-flip.md" /> — completed compiled-default flip.
