...
- OTLP Capsule egress is jackin❯-owned infrastructure only in `allowlist` or `open` mode: every normalized per-signal endpoint host is derived and admitted without exposing host exporter headers to the container. `network=none` always wins and produces host-only telemetry with explicit degraded-coverage health; observability never weakens an isolation grant.
