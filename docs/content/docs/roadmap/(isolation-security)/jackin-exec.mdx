...
**Remaining work:** live smoke of the picker/render/keystroke path through `jackin load`; hardening `host.sock` so a direct in-container caller cannot bypass the picker approval; and temp-file materialization for key-type credentials (writing an SSH private key to `/jackin/run/secrets/<uuid>` + `SSH_KEY_FILE`, deleting after exit).
