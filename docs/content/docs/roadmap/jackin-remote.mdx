...
- The Kubernetes platform vision in the [roadmap
  vision](/roadmap/#vision) addresses this long-term, but is
  far away. A simpler SSH-bridge sits naturally between local-only and
  full Kubernetes.
- It demonstrates that jackin❯ role + workspace + queue model
...
The fundamental architecture choice for jackin❯ version:
...
Every synced path needs an explicit direction and owner. `sync_up` means the
local host is authoritative, `sync_bidi` means conflict behavior is part of
the contract, and remote-only jackin state stays remote unless the operator
requests a recovery copy. Credentials are never included in broad sync rules;
they use the credential source pattern and per-invocation grants instead.
...
- **Status bus over SSH.** [Agent runtime
  status](/roadmap/agent-runtime-status/) events need to
...
- <RepoFile path="crates/jackin/src/cli/role.rs">crates/jackin/src/cli/role.rs</RepoFile> — `--remote` flag plumbing
...
- [Roadmap vision](/roadmap/#vision) — the long-term K8s
