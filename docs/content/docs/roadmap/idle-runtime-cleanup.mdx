...
- It generalizes naturally to anything an role wants done while
...
Generalize the concept: roles declare what to run when idle, the
operator decides whether to enable it, jackin❯ runtime supervisor fires
...
status](/roadmap/agent-runtime-status/) bus. When an instance
...
- **Default delay/interval values.** multicode uses 300s/900s. Are
  those sensible jackin defaults, or should roles be more
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs">crates/jackin-runtime/src/runtime/launch.rs</RepoFile> — wires supervisor into
  instance lifecycle
- The persistent-storage module — cleanup history table
- <RepoFile path="crates/jackin/src/console/tui.rs">crates/jackin/src/console/tui.rs</RepoFile> — last-cleanup
  display
