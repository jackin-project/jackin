...
1. **Pin the binary.** Add a SHA-pinned `rvben/rumdl@<sha>` step to <RepoFile path=".github/workflows/docs.yml">.github/workflows/docs.yml</RepoFile>, scheduled between `Checkout repository` and `Check source repository links`. The local pre-commit equivalent would be a new `bun run check:md` script calling the same binary via `mise`.
...
- <RepoFile path="docs/package.json">docs/package.json</RepoFile> — would gain a new `check:md` script
