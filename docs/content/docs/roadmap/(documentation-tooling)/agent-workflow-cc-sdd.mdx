...
**Status**: Open — iterating. The roadmap item `.mdx` is the living change-spec (`## Problem`/`## Design`/`## Tasks` while open, retiring into canonical docs on ship); the in-repo `jackin-dev` helper backs the human PR workflow. No external spec tool was adopted. This is documentation/workflow tooling, not Rust code-health debt.
