...
**Status**: Open — design proposal (Phase 2 — Live operator surface, [Agent Orchestrator Research Program](/roadmap/agent-orchestrator-research/))
...
| F8 | Trivial | <RepoFile path="crates/jackin-isolation/src/finalize.rs">crates/jackin-isolation/src/finalize.rs</RepoFile> cites a `worktree-cleanup-assessment.mdx` doc that does not exist; fold the policy table into this item or an internals page and fix the reference. |
...
- [Déjà Vu](/roadmap/deja-vu/) — the backstop for "never lose valuable context": even when a session is unrecoverable, the cross-agent conversation archive survives instance teardown, so the operator can still re-read what was discussed and decided.
