...
**Status**: Open — design proposal (Phase 5, [Agent Orchestrator Research Program](/roadmap/agent-orchestrator-research/))
...
- It's runtime-agnostic by design — the same skill markdown bind-mounts
  ...
  the runtime adapter.
- It complements (doesn't replace) the custom plugin marketplace story
  (now shipped — see the [role manifest reference](/developing/role-manifest/)):
...
- It gives the [tag protocol](/roadmap/agent-tag-protocol/)
...
- It also supports workflow and playbook skills such as "independent
  fix", "address review feedback", or "prepare a PR", which are often more
  valuable to a queue-driven operator than stack syntax hints.
...
- New repo `jackin-project/jackin-skills` ships a published bundle
  including the [tag protocol](/roadmap/agent-tag-protocol/)
  emission skills.
...
- Skill marketplace integration (browse/install via TUI). The
  shipped custom plugin marketplace support (see the
  [role manifest reference](/developing/role-manifest/)) handles
  published skills; this mount is for local/team ones.
...
- **Should workspace skills mount into Claude as "plugins" or
  ...
  [multi-runtime](/roadmap/multi-runtime-support/) work
...
- Custom plugin marketplace (shipped) — published-skill distribution;
  complementary to this leaf. See the
  [role manifest reference](/developing/role-manifest/)
