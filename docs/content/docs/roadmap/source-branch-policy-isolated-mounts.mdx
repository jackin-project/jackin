...
- <RepoFile path="crates/jackin-isolation/src/materialize.rs" /> — resolve the policy before `worktree add` / `clone` materialization and use the resolved commit/ref without mutating the host checkout.
- <RepoFile path="crates/jackin-isolation/src/state.rs" /> — confirm the existing `base_commit` remains sufficient; add the resolved policy/ref only if cleanup or diagnostics needs it.
