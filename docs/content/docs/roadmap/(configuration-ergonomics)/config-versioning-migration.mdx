...
The main config file and workspace files are versioned independently. A workspace-only schema change bumps `CURRENT_WORKSPACE_VERSION` (currently `v1alpha8`); a global config-only change bumps `CURRENT_CONFIG_VERSION` (currently `v1alpha8`). The example above shows a `config.toml` snippet; a per-workspace file under `~/.config/jackin/workspaces/<name>.toml` would carry `version = "v1alpha8"` as well.
...
- **Older-stamped manifest uses a newer feature**: jackin refuses to use the role and emits a feature-specific error with `jackin role migrate <role-repo-path>`. For example, `opencode` and `kimi` require manifest versions newer than the legacy baseline; an older-stamped manifest that declares either agent must be migrated before launch.
...
The `jackin role migrate <role-repo-path>` command applies the manifest migration chain to a desktop local role clone and writes the result back. The operator can then inspect the diff, commit, and push. CI, validation workflows, and Renovate-style automation are expected to use the standalone role-focused migrator binary when that deferred automation lands; verify the final binary name before documenting or scripting it.
...
`--pr` runs from an operator's machine, so a role repo whose author has not run `jackin role migrate` stays broken in CI until someone opens a PR by hand. A scheduled action closes that gap by watching role repos and opening migration PRs upstream automatically. That action should run the small role-focused migrator binary, not the full `jackin` operator binary; confirm the binary name before implementation.
