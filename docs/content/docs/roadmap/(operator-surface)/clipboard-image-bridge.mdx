...
7. **Live-validate launch-failure detail handling** from a fresh throwaway failed launch: bounded redacted detail held by the active launch process, hover-selected vs fallback detail action, and a useful unavailable/expired state after that process exits. Command-output sidecars and diagnostics JSONL reveal are transitional telemetry-log surfaces removed by [Unified OpenTelemetry observability](/roadmap/unified-otel-observability/); this bridge must not preserve a file reveal path for either one.
