...
7. **Live-validate launch-failure detail handling** from a fresh throwaway failed launch: non-telemetry command-output reveal, hover-selected vs fallback `R`, and safe reveal-path reporting. The diagnostics JSONL reveal is transitional and is removed by [Unified OpenTelemetry observability](/roadmap/unified-otel-observability/), so it is not a completion dependency for this bridge.
