...
- [x] Delivery is approved as one future `jackin❯` implementation branch/PR, with TermRock created if absent, inherited donor commits preserved unchanged, and every new signed/buildable bootstrap commit pushed directly to its `main` without TermRock PRs.
