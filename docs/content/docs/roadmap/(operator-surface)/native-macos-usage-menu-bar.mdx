...
**Status**: **jackin❯ Desktop v1 UI complete** (identity, logomark, status-item modes, glance popover, Usage window, Settings format pickers, Capsule-parity Rust strings via UniFFI). Host-global cache, Tahoe design, arm64 static PR gate, release **validate** + cask PR automation ship. **Only open residual is activation ops**: first notarized ZIP after Apple secrets in `release-macos` → operator-merged first cask → plan 004 production proof (see `native/README.md` Paths A–C).
