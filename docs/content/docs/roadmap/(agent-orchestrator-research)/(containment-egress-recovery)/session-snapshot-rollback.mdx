...
**Status**: Partially implemented — diagnostic `Snapshot` / `StatusCapture` calls exist; reversible filesystem snapshot and rollback semantics remain open (Containment track, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))
...
- Integrate with [container credential exposure](/reference/research/security/credential-exposure/container-credential-exposure/) so secret handling stays explicit.
