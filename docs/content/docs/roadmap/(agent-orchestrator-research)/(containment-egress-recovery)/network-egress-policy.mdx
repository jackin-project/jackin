...
**Status**: Partially implemented — Docker grant-based allowlists, launch-contract rendering, and the fail-closed Capsule guest firewall exist; remaining work is the operator-facing egress contract, DinD inner-container coverage, non-Docker parity, and decision logs (Containment track, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))
...
### Phase 2 — DinD guest enforcement parity

- Extend the shipped guest enforcement beyond the agent container to the DinD sidecar and any agent-created inner-container path.
- Include tests that show forbidden domains/private addresses fail from inside the agent session and from DinD-created containers.
- Keep the contract honest when a path is only partially enforced.

### Phase 3 — MicroVM host-side proxy research

- Fold the existing Docker Sandboxes comparison in [selectable sandbox backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) into a concrete proxy design.
- Decide whether the proxy belongs to the [host bridge](/roadmap/host-bridge/), the sandbox backend, or a separate daemon adapter.
