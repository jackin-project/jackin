...
**Status**: Partially implemented — Docker grants, launch-contract rendering, and Capsule firewall substrate exist; remaining work is the operator-facing egress contract, fail-closed enforcement policy, non-Docker parity, and decision logs (Containment track, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

jackin❯ already has Docker network/security primitives and a Capsule firewall substrate: Docker security grants, profile validation, launch-contract rendering, and in-container hooks that can host egress enforcement. What is missing is not a raw place to put firewall code; it is the explicit operator contract that says which egress is allowed, whether enforcement is host/guest/partial, and when launch must fail closed.
