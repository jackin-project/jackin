...
[Unified OpenTelemetry observability](/roadmap/unified-otel-observability/) owns the telemetry interaction with this policy. In `allowlist` or `open` mode, launch derives every normalized per-signal OTLP endpoint host and treats it as jackin❯ infrastructure egress; a host-loopback Collector address is rewritten per endpoint to the container host gateway. `network=none` admits no OTLP host and continues with host-only telemetry plus a safe coverage event. Only routing is propagated: host exporter headers, client credentials, and arbitrary Resource attributes never cross into the Capsule.
