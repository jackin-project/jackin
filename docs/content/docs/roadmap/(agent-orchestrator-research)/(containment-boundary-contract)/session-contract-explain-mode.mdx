...
**Status**: Partially implemented — Docker profile session-contract output and `status explain` surfaces exist; remaining work is one backend-neutral resolved contract object plus richer pre-launch explain output across mounts, credentials, side effects, and recovery behavior (Containment track, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))
