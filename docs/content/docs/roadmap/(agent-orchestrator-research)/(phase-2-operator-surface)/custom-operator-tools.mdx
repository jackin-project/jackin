...
**Status**: Open — `jackin-exec`, credential picker, and MCP execution primitives exist; the operator-configured `[[tool]]` surface remains a design proposal (Phase 2, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

jackin❯ already has pieces that custom tools should reuse: in-container `jackin-exec`, the credential picker, and MCP execution plumbing. Remaining work is the operator-configured `[[tool]]` surface: reserved-key validation, substitutions from workspace/session/tag state, console rendering, output capture, and clear host-vs-container execution semantics.
...
- <RepoFile path="crates/jackin/src/config.rs">crates/jackin/src/config.rs</RepoFile> — `[[tool]]` parsing
- <RepoFile path="crates/jackin/src/console/tui.rs">crates/jackin/src/console/tui.rs</RepoFile> — keybinding registration and modal rendering
...
- [Operator handler system](/roadmap/operator-handler-system/) — shared command-resolution machinery
