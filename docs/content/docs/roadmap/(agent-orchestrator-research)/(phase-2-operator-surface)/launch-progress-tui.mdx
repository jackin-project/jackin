...
`jackin diagnostics validate` emits all three signal markers and requests a force-flush, but positive per-signal backend delivery proof remains pending on the unified roadmap. History and comparison are backend responsibilities; jackin❯ does not provide local telemetry readers or a backend-independent trace viewer.
