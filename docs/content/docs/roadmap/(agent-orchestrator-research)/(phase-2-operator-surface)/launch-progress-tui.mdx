...
- Preserve useful launch evidence through the OTLP-only transition. The launch surface continues to show the run id and current-run stage/timing state, while traces and cross-run comparison live in the configured backend; failure UI may expose exporter health but cannot reveal a local telemetry artifact.
...
- Launch displays the run id needed for backend correlation and keeps current-run stage/timing evidence visible without depending on a local telemetry artifact; exporter-disabled/failure states are explicit and do not offer a nonexistent reveal action.
