...
- Per-session state indicators now consume the [Agent Runtime Status](/roadmap/agent-runtime-status/) authority; remaining polish is tied to live reconciliation below.
- Per-instance CPU/RAM usage in the console resource panel (requires integration with the [Console Resource Panel](/roadmap/console-resource-panel/) roadmap item).
