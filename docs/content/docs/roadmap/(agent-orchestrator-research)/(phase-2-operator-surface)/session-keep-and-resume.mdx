...
**Status**: Partially implemented — shipped baseline now covers dirty/unsafe exit assessment, dirty keep/delete/return action wiring, `keep_awake`, launch recipe pins for role SHA/base metadata, stopped-container `docker start` reconnect paths, rich restore candidate inspection/deletion, explicit purge/prune helpers, terminal-status pruning, orphan lock reaping, and prune-time stale-`active` reconciliation. Remaining work is kept-clean resume, per-workspace cleanup policy, faithful deleted-container recreate/rebuild tiers, resume UX framed as normal continuity, launch-time/live index reconciliation, full socket-dir/sidecar rematerialization for all restore tiers, and concise operator docs (Phase 2 — Live operator surface, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

Current exit plumbing already assesses dirty/unsafe isolated work, offers dirty-session keep/delete/return decisions, marks preserved dirty/unpushed state as restorable, can restart stopped containers from `hardline`/launch paths, stores partial launch-recipe pins, and has purge/prune machinery that removes terminal instance state and orphan locks. Remaining work is the full durable resume model: a keep outcome for clean sessions, per-workspace cleanup policy, faithful recreate/rebuild tiers for deleted containers/images, socket-dir and sidecar rematerialization across restore, launch-time/live index reconciliation, and an operator surface that treats resume as normal session continuity instead of forensic recovery.
...
| F12 | Partial | `prune_instances` reconciles stale `active` rows whose container is missing to `crashed`; the same reconciliation still needs to run in launch/console flows before restore decisions. |
| F13 | Shipped baseline | `start_container` is now called by hardline attach and launch restore paths for stopped containers. |
| F14 | Partial | The manifest now stores `role_git_sha`, `base_image_ref`, `base_image_digest`, and supported agents, and Tier 3 plumbing references the pinned role SHA; full replay of mount/env/run recipe and base-image digest pinning remains open. |
| F15 | Partial | Stopped-container restore exists, but full Tier 1 durability still needs explicit recipe-derived `agent.toml` rematerialization and DinD sidecar health sequencing wherever the socket dir was removed. |
