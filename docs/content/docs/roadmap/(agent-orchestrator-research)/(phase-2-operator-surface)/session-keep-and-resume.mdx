...
**Status**: Partially implemented — dirty/unsafe exit assessment, keep/delete action wiring, `keep_awake`, launch recipe pinning, stopped-container `docker start` restore, basic restore candidates, and clean terminal purge exist; kept-clean resume, full durable resume continuity, recreate/rebuild tiers, operator resume UX, and live index reconciliation remain open (Phase 2 — Live operator surface, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

Current exit plumbing already assesses dirty/unsafe exit states and carries keep/delete action wiring for preserved work. Launch recipe pinning, stopped-container start restore, and clean terminal purge are now in place. Remaining work is the full durable resume ladder and operator-facing continuity model: kept-clean outcomes, recreate/rebuild tiers, socket-dir and sidecar rematerialization hardening where not yet proven, live index reconciliation, and an operator surface that treats resume as normal session continuity instead of forensic recovery.
...
- **Phase 0 — Exit-dialog hardening.** Restore a plain-text degrade when the terminal cannot render the rich surface, `clog!` the swallowed popup failure, and de-flicker the double alt-screen enter (F7).
