...
**Status**: Partially implemented — dirty/unsafe exit assessment, keep/delete action wiring, `keep_awake`, and basic restore candidates exist; durable resume continuity, kept-clean sessions, launch recipes, and `docker start` tiers remain open (Phase 2 — Live operator surface, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

Current exit plumbing already assesses dirty/unsafe exit states and carries keep/delete action wiring for preserved work. Remaining work is the durable resume ladder and full instance reuse: kept-clean outcomes, launch-time recipe snapshots, `docker start` with socket-dir rematerialization, recreate/rebuild tiers, and an operator surface that treats resume as normal session continuity instead of forensic recovery.
