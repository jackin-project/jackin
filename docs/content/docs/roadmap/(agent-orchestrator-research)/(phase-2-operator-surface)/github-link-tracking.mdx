...
**Status**: Partially implemented — focused Capsule PR context and CI URL actions exist; durable cross-instance GitHub link tracking/cache/status remains open (Phase 2, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

Capsule already ships PR context for the focused container: it runs `gh pr list`, derives PR title and check URL, renders the bottom-bar dialog, updates the terminal title, and invalidates stale context when `HEAD` changes. Remaining work is durable link tracking: SQLite/cache state, a fetcher cadence, and console-wide live status across all instances rather than only the focused Capsule bar.
