...
**Status**: Partially implemented — Docker `[docker]` security/resource grants, validation, launch-contract output, and Docker flag translation exist; remaining work is nicer role/agent-facing names, one-shot operator overrides, OOM surfacing, and non-Docker translation (Phase 1, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

jackin❯ now has Docker security/resource fields (`memory`, `memory_reservation`, `cpus`, `pids`, `nofile`) and validation/translation for the Docker profile path. Remaining work is to normalize that substrate into a nicer role/agent-facing contract, one-shot operator overrides, OOM status, and future non-Docker backend translation.
...
Docker exposes the right primitives (`--memory`, `--cpus`, `--ulimit nofile=N`, plus `--memory-reservation` for soft limits). jackin❯ now has Docker-profile plumbing for those primitives; this item tracks the remaining role/agent-facing contract and cross-backend translation.
...
- **Cross-backend resource translation is the right home for this design.** Docker, Apple `container`, and the planned [selectable sandbox backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) each express limits differently — a declarative layer means each backend translates once.
...
The shipped low-level substrate lives in the role manifest's `[docker]` grants. The remaining design question is whether to keep exposing those Docker-shaped names directly or add a friendlier backend-neutral layer. The right level for defaults is still the **role manifest**, because limits scale with the toolchain. Operator/workspace overrides remain useful for one launch on a smaller machine.

### Possible backend-neutral config
...
- **Apple container** (when [selectable backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) ships): direct per-allocation limits.
...
- Decide whether a backend-neutral `[runtime.limits]` block is still needed on top of the shipped `[docker]` grant fields, or whether docs/aliases are enough.
- Add `--memory-max` / `--memory-high` / `--cpus` / `--nofile` flags on `jackin load` for one-shot overrides if operator demand confirms the need.
- Translate the same logical limits to Apple Container and any later sandbox backend.
...
- OOM surfacing: preserve state, report the kill reason in status/diagnostics, and notify once the operator surface can consume it.

## Defer

- Per-workspace default overrides. Manifest-level plus one-shot CLI is enough until a concrete workspace policy need appears.
...
- **OOM preserve_state interaction with worktree cleanup** (the shipped per-branch safety policy described in [Per-mount isolation](/guides/workspaces/#per-mount-isolation)). An OOM-killed instance should always preserve. The cleanup helper already handles non-zero exits; OOM is a special case of that. Confirm the existing helper sees OOM as non-zero.

## Related Files

- <RepoFile path="crates/jackin-manifest/src/manifest.rs">crates/jackin-manifest/src/manifest.rs</RepoFile> — `[runtime.limits]` schema
- <RepoFile path="crates/jackin-manifest/src/validate.rs">crates/jackin-manifest/src/validate.rs</RepoFile> — limit value validation
...
- <RepoFile path="crates/jackin/src/cli/role.rs">crates/jackin/src/cli/role.rs</RepoFile> — `--memory-max` etc. flags
...
- [Console resource panel](/roadmap/console-resource-panel/) — consumer of the runtime side (current usage vs configured limit)
