...
**Status**: Partially implemented — Docker security/resource grants and validation exist; remaining work is role/agent-facing limits, operator overrides, OOM surfacing, and non-Docker translation (Phase 1, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

jackin❯ now has Docker security/resource fields (`memory`, `memory_reservation`, `cpus`) and validation/translation for the Docker profile path. Remaining work is to normalize that substrate into a role/agent-facing contract and translate it consistently across role manifests, operator overrides, launch-contract output, and future non-Docker backends.
