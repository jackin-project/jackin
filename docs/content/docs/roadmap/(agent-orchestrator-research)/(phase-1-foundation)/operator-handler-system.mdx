...
- Multiple downstream items in this program need it: [GitHub link
  tracking](/roadmap/github-link-tracking/) opens issue/PR URLs,
  [agent tag protocol](/roadmap/agent-tag-protocol/) opens repo
  paths, [custom operator tools](/roadmap/custom-operator-tools/)
...
multicode also exposes a `[compare]` block in the current reference
...
- <RepoFile path="crates/jackin/src/cli/role.rs">crates/jackin/src/cli/role.rs</RepoFile> — adds `jackin open` / `jackin diff`
...
- <RepoFile path="crates/jackin-config/src/resolve.rs">crates/jackin-config/src/resolve.rs</RepoFile> — primary-mount resolution
...
- [Agent tag protocol](/roadmap/agent-tag-protocol/) — emitted
