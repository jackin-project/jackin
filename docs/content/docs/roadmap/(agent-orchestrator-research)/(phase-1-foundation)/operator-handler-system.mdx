...
**Status**: Partially implemented — host affordance primitives for safe URL open, file export, reveal/open, and host action framing exist; remaining work is the named configurable handler layer (Phase 1, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

Capsule already ships host affordances for the most common cases: safe URL opening, verified file export, and queued host open/reveal actions for exported files. Remaining work is the configurable handler layer: named operator handlers, platform defaults, substitutions, and CLI/console routing that reuse those affordances instead of adding one-off launchers.
...
- <RepoFile path="crates/jackin/src/cli/role.rs">crates/jackin/src/cli/role.rs</RepoFile> — adds `jackin open` / `jackin diff` / `jackin web` subcommands
- <RepoFile path="crates/jackin/src/console/tui.rs">crates/jackin/src/console/tui.rs</RepoFile> — keybindings
- <RepoFile path="crates/jackin-config/src/resolve.rs">crates/jackin-config/src/resolve.rs</RepoFile> — primary-mount resolution for the handler argument

## See Also

- [GitHub link tracking](/roadmap/github-link-tracking/) — consumes `web` handler
