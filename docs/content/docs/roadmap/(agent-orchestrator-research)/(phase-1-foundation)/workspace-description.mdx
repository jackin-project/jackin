...
**Status**: Open — remaining consistent surface work (Phase 1, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Current state

Code inspection found no persisted workspace `description` field on `WorkspaceConfig`; current `description` hits are Cargo/package descriptions, TUI helper text, MCP tool descriptions, and diagnostics labels. So this remains open, but it is small consistency work across the existing workspace schema, CLI, `jackin console`, and roadmap/program references rather than a new subsystem.
...
- jackin❯ `workspace list` and `workspace show` reveal mounts and agent bindings but not *intent* — operators end up encoding intent in the workspace *name*, which is awkward.
- This is the smallest feature in the [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/) that demonstrably improves day-to-day operator UX.
...
The `--clear-description` flag matches the existing `--clear-default-role` shape in <RepoFile path="crates/jackin/src/cli/workspace.rs">crates/jackin/src/cli/workspace.rs</RepoFile>.
...
- <RepoFile path="crates/jackin/src/console/tui.rs">crates/jackin/src/console/tui.rs</RepoFile> — workspace picker rendering
