...
2. **Runtime resolution.** In <RepoFile path="crates/jackin-runtime/src/isolation/materialize.rs" />, resolve the policy (`host_head` / `current_branch` / `default` / `named` / `ask`) to a concrete ref before `worktree add` or `clone`, without ever mutating the host checkout (no implicit `switch`/`checkout`/`pull`/`fetch`).
