...
Shipped baseline: Capsule exits cleanly when the last session ends so the host cleanup path can tear down the role container, sidecar, cert volume, and network. The runtime also has agent status reporting (`Working`, `Blocked`, `Done`, `Idle`, `Unknown`) and can query a running container's session inventory through the Capsule control socket. Remaining work is idle policy, not basic last-session cleanup: role-declared idle hooks, operator opt-in, time-based trigger/re-arm, restart/recycle semantics, cleanup history, and operator-visible cleanup status.
...
The implementation watches the workspace status (their equivalent of [agent runtime status](/roadmap/agent-runtime-status/)) and fires when status has been `Idle` for the configured delay.
...
3. Record the cleanup in persistent storage, likely a dedicated `cleanup_history` table unless the future tool-history store already exists by implementation time.
...
- Capsule exits when the last live session exits; clean exits skip stale pane snapshots, remove the session, drain, and let host cleanup run.
- Reconnect finalization tears down resources after clean stop or cleaned still-running sessions by marking the instance `CleanExited` and calling `eject_role`.
- Instance manifests already record backend resources for Docker role container, DinD sidecar, network, and cert volume.
- Agent runtime status wire types and Capsule state arbitration are implemented; panes carry `agent_status_report` when populated.
- Host attach code can inspect running containers by executing `jackin-capsule status`, but there is no idle-hook supervisor yet.

## Remaining Scope (V1)

- Schema-bumped idle block on `jackin.role.toml`.
- Operator opt-in via `enable_idle_hooks` per role.
- Idle detection from Capsule agent status reports.
...
- Cleanup history written to persistent storage.
...
- New module (e.g. `crates/jackin-runtime/src/runtime/idle.rs`) — supervisor
- <RepoFile path="crates/jackin-core/src/manifest.rs">crates/jackin-core/src/manifest.rs</RepoFile> and <RepoFile path="crates/jackin-manifest/src/manifest.rs">crates/jackin-manifest/src/manifest.rs</RepoFile> — idle schema and version validation
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs">crates/jackin-runtime/src/runtime/launch.rs</RepoFile> — wires supervisor into instance lifecycle
- <RepoFile path="crates/jackin-runtime/src/runtime/attach.rs">crates/jackin-runtime/src/runtime/attach.rs</RepoFile> — existing status/session inventory query path to build on
- A future persistent-storage module — cleanup history table
- <RepoFile path="crates/jackin/src/console/tui.rs">crates/jackin/src/console/tui.rs</RepoFile> — last-cleanup display
...
- [Console resource panel](/roadmap/console-resource-panel/) — visibility surface
