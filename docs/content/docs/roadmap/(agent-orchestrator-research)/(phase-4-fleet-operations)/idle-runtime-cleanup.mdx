...
Last-session cleanup is shipped: Capsule exits cleanly when the last session ends so the host cleanup path can tear down the role container, sidecar, cert volume, and network. Remaining work is idle policy, not basic last-session cleanup: time-based retention, resumable-session protection, daemon reconciliation, and operator-visible cleanup rules.
...
Generalize the concept: roles declare what to run when idle, the operator decides whether to enable it, jackin❯ runtime supervisor fires the hook based on observed status.
...
The supervisor watches the [agent runtime status](/roadmap/agent-runtime-status/) bus. When an instance has been `Idle` for `delay_seconds` continuously:
...
- <RepoFile path="crates/jackin-manifest/src/manifest.rs">crates/jackin-manifest/src/manifest.rs</RepoFile> — `[runtime.idle]` schema
