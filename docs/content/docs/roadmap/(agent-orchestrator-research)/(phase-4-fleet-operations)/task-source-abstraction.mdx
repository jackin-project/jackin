...
No task-source abstraction has shipped yet. Source search found no `TaskSource`, `TaskOutcome`, `[task_sources]`, `github_issues`, `file_glob`, `stdin_pipe`, `queue feed`, `WorkItemSource`, `WorkflowRun`, or autonomous queue implementation in the Rust crates. Current shipped adjacent state is limited to Capsule/usage persistence: the Capsule-local Turso account-usage cache and host `jackin usage <instance> accounts --sync-host-cache` flow documented in the [persistent storage layer](/reference/research/agent-orchestration/memory/persistent-storage-layer/). The broader storage layer, task table, workflow run ledger, GitHub reporter, and autonomous dispatch queue remain planned.
...
- <RepoFile path="crates/jackin/src/config.rs">crates/jackin/src/config.rs</RepoFile> and <RepoFile path="crates/jackin/src/workspace.rs">crates/jackin/src/workspace.rs</RepoFile> currently expose workspace/config schema but no `task_sources` parsing or workspace task-source assignment.
- <RepoFile path="crates/jackin/src/app.rs">crates/jackin/src/app.rs</RepoFile>, <RepoFile path="crates/jackin/src/cli/usage/store.rs">crates/jackin/src/cli/usage/store.rs</RepoFile>, <RepoFile path="crates/jackin-runtime/src/instance.rs">crates/jackin-runtime/src/instance.rs</RepoFile>, and <RepoFile path="crates/jackin-protocol/src/control.rs">crates/jackin-protocol/src/control.rs</RepoFile> show the current CLI/runtime/protocol surfaces; source search across those crates found no queue-feed command, workflow-run model, task-source trait, or issue-polling loop.
- <RepoFile path="crates/jackin-capsule/src/telemetry_store.rs">crates/jackin-capsule/src/telemetry_store.rs</RepoFile> and <RepoFile path="crates/jackin/src/cli/usage/store.rs">crates/jackin/src/cli/usage/store.rs</RepoFile> implement only the `account_usage_snapshots` storage subset, matching the persistent-storage page's partial implementation note.
- [Agent workflow orchestration](/reference/research/agent-orchestration/workflow-systems/agent-workflow-orchestration/) still names `WorkItemSource` and a durable run ledger as future design, and explicitly says the next low-regret implementation should be run records plus GitHub reporter rather than full automation.
...
- New module or crate-internal area — task identity, policy, source trait, and V1 implementations.
- Persistent storage module — queued task and workflow/run tables once that layer lands.
- <RepoFile path="crates/jackin/src/config.rs">crates/jackin/src/config.rs</RepoFile> — future `[task_sources]` parsing.
- <RepoFile path="crates/jackin/src/workspace.rs">crates/jackin/src/workspace.rs</RepoFile> — future workspace task-source assignment.
