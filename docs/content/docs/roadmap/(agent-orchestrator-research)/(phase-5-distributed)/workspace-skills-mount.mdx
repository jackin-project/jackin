...
**Status**: Open — design proposal (Phase 5, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

Claude plugin and marketplace install paths already ship through the role manifest/runtime setup. Remaining work is the cross-runtime workspace skills mount: local/team skill directories mounted read-only into Claude/Codex/Amp-compatible destinations, with conflict rules against published plugins and no implicit marketplace behavior.
...
- It complements (doesn't replace) the custom plugin marketplace story (now shipped — see the [role manifest reference](/developing/role-manifest/)): marketplaces distribute *reusable* skills published broadly; the workspace skills mount handles *local* / *team-specific* skills the operator doesn't want to publish.
- It gives the [tag protocol](/roadmap/agent-tag-protocol/) a natural distribution channel: jackin❯ can ship a skill bundle that roles pull in via this mount, and operators see the protocol "for free" without having to install plugins per role.
...
- Config — [`config.toml` `[isolation] add-skills-from`](https://github.com/graemerocher/multicode/blob/main/config.toml)
- Skills directory — [`workspace-skills/`](https://github.com/graemerocher/multicode/tree/main/workspace-skills) (real examples — `git-commit-coauthorship`, `independent-fix`, `java-install`, `machine-readable-*`, `micronaut-projects-guide`)
...
- New repo `jackin-project/jackin-skills` ships a published bundle including the [tag protocol](/roadmap/agent-tag-protocol/) emission skills.

## Defer

- Skill version pinning. Skills are filesystem paths; pin the path, not a version. If versioning becomes important, revisit.
- Skill marketplace integration (browse/install via TUI). The shipped custom plugin marketplace support (see the [role manifest reference](/developing/role-manifest/)) handles published skills; this mount is for local/team ones.
...
- **Should workspace skills mount into Claude as "plugins" or "skills"?** Claude Code's plugin format is heavier; "skills" via Claude's skills feature is closer to the multicode shape. Recommended: *use Claude skills*; the [multi-runtime](/roadmap/multi-runtime-support/) work needs to confirm Claude's skills plumbing matches.
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs">crates/jackin-runtime/src/runtime/launch.rs</RepoFile> — bind-mount plumbing
...
- New docs page: `docs/content/docs/(public)/guides/skills.mdx`

## See Also

- [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/)
- [Multi-runtime support](/roadmap/multi-runtime-support/) — per-runtime mount destinations
- [Agent tag protocol](/roadmap/agent-tag-protocol/) — primary distribution use case for skill bundles
- Custom plugin marketplace (shipped) — published-skill distribution; complementary to this leaf. See the [role manifest reference](/developing/role-manifest/)
