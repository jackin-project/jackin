...
**Status**: Partially implemented — env values, `op://` references, auth modes, provider keys, and `jackin-exec` mediation exist; one shared typed credential-source abstraction remains open (Phase 5, [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/))

## Problem

This is a unification task, not a greenfield credential feature. jackin❯ already has auth forwarding, env/provider-key handling, credential picker flows, and `jackin-exec` command mediation. Remaining work is one source-reference pattern that all of those mechanisms can share across env vars, provider tokens, 1Password/keychain references, MCP tools, and future daemon adapters.

jackin❯ already has several credential-resolution paths, documented in [Environment Variables](/guides/environment-variables/) and [Authentication](/guides/authentication/). They do not yet share one typed credential-source abstraction.
...
- Preserve the shipped literal/env/`op` behavior in the current env and `jackin-exec` surfaces, then add the missing shared backends: `command`, OS secret store, and `file`.
