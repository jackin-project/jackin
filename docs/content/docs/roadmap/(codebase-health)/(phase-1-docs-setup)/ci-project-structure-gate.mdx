...
<RepoFile path="PROJECT_STRUCTURE.md">PROJECT_STRUCTURE.md</RepoFile> has drifted from the actual module layout because there is no automated check. PRs that add new `.rs` module files can merge without mentioning them in the navigation map.

## Implementation

Add a step to the `check` job in <RepoFile path=".github/workflows/ci.yml">.github/workflows/ci.yml</RepoFile>:
...
- **Greps for basename** (e.g., `op_picker` for <RepoFile path="crates/jackin-console/src/tui/op_picker.rs">crates/jackin-console/src/tui/op_picker.rs</RepoFile>) — PROJECT_STRUCTURE.md uses prose descriptions containing the module name.
...
3. Add the YAML step above to <RepoFile path=".github/workflows/ci.yml">.github/workflows/ci.yml</RepoFile> after the `cargo fmt --check` step.
