...
<RepoFile path="crates/jackin/tests/manager_flow.rs">crates/jackin/tests/manager_flow.rs</RepoFile> (1,528 lines) and <RepoFile path="crates/jackin/tests/dind_e2e.rs">crates/jackin/tests/dind_e2e.rs</RepoFile> (1,669 lines) remain large, and test helpers (mock runners, config builders, workspace builders) are still duplicated across multiple test files rather than consolidated into a shared crate.
