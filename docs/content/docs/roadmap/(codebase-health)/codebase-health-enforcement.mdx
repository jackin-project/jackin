...
**Status**: Partially implemented — the original exception-ledger burn-down is complete, but the PR #664 merge-readiness audit found behavior and CI-wiring blockers that must be fixed before this item can be treated as shipped. The immediate checklist lives in <RepoFile path="plans/codebase-health-remaining.md">plans/codebase-health-remaining.md</RepoFile>; this page tracks the durable follow-up work that remains after that PR-local checklist closes.

## Merge-Readiness Audit

PR #664 is a structure-only refactor. Before it can merge, the audit checklist must restore or prove all pre-refactor behavior that was weakened during extraction:

- Restore the `jackin prewarm` `clap` constraints that were dropped when flags moved into nested structs.
- Make launch failure dialogs visible above lower-priority overlays, dismissible through the shared modal path, and safe for long diagnostics.
- Wire the file-size, schema, and strict codebase-health gates into required CI aggregation and path filters.
- Make file-size and test-layout ratchets fail stale or no-longer-needed exception rows.
- Keep roadmap and PR text honest until the required gates and regression tests are in place.
...
- Test-placement audit found no PR-introduced merge blocker: `cargo run -p jackin-xtask --locked -- lint tests` reports zero scanned violations, the strict lint umbrella reports zero test-layout violations, and a diff-only scan found no added `#[test]`, `#[tokio::test]`, or `#[rstest]` functions outside dedicated `tests.rs` or `tests/` files. A repo-wide grep did find one pre-existing stricter-layout gap, <RepoFile path="crates/jackin-tui-lookbook/src/keymap_smoke.rs">crates/jackin-tui-lookbook/src/keymap_smoke.rs</RepoFile>, which has direct `#[test]` functions and should move under a sibling `tests.rs` or be covered by a widened test-layout gate in a later cleanup.
...
- Dependency-direction gate: strict mode exists and passes locally; the PR #664 merge checklist must wire the full codebase-health gate set into required CI before this item can close.
