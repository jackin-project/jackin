...
**Status**: Partially implemented — `jackin-dev pr sync/path/env/status/explain/clean` exists for local PR bundles; remaining work is console-native review mode, per-project recipes, docs preview automation, companion role pinning, and generic verification command execution.
