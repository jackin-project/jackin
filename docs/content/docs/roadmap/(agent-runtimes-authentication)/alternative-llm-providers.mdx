...
**Status**: Partially implemented — Phases 1–4 shipped on `main` (catalog + Claude Code + Codex MiniMax + Codex OpenAI default + OpenCode + operator Auth-tab rows for MiniMax/Z.AI and the existing Kimi runtime key, provider picker that collapses the agent's native auth, and per-provider model overrides in role manifests via `[<agent>.providers.<id>]`); Codex GLM and Kimi cells remain deferred pending upstream Responses-API endpoint.
...
Operators now have shipped alternative-provider paths for several provider/runtime cells, including MiniMax, Kimi, GLM/Z.AI, and OpenCode/Codex/Claude surfaces where the upstream protocol works. The remaining work is narrower: provider parity edges, provider picker/auth UX polish, update/availability detection, config generation drift, and deferred Codex cells where upstream still lacks a Responses-compatible endpoint.

This item tracks that provider catalog and parity work across **MiniMax**, **Kimi**, and **GLM/Z.AI**, across **Claude Code**, **Codex**, and **OpenCode**.
...
This item keeps **only the cells that are clearly documented, testable, and work end-to-end**. Cells blocked by an upstream protocol gap are explicitly out of scope until that gap closes — jackin❯ does not half-build a routing path that cannot route.

| Provider | Claude Code | Codex | OpenCode |
|---|---|---|---|
| **Anthropic** (native) | ✅ shipped | n/a | ✅ shipped |
| **OpenAI** (native) | n/a | ✅ shipped — Codex's default auth; no `wire_api` redirect needed | n/a |
| **MiniMax** | ✅ shipped | ✅ shipped (`wire_api = "responses"`) | ✅ shipped |
| **Z.AI / GLM** | ✅ shipped | ✖ deferred — Chat-only, blocked on current Codex | ✅ shipped |
| **Kimi** | ✅ shipped | ✖ deferred — Chat-only, no Responses endpoint | ✅ shipped |
...
5. **Per-provider quota surfacing.** Subscriptions carry 5-hour / weekly / 7-day quota windows. Whether jackin❯ surfaces remaining quota is a separate concern, likely deferred to the [Desktop Agent Hub](/reference/research/desktop/agent-hub/jackin-desktop-agent-hub/) account-state work.
...
- <RepoFile path="crates/jackin-capsule/src/runtime_setup.rs">crates/jackin-capsule/src/runtime_setup.rs</RepoFile> — in-container agent setup, Codex `config.toml` generation, and OpenCode provider JSON generation
- <RepoFile path="docker/runtime/entrypoint.sh">docker/runtime/entrypoint.sh</RepoFile> — default OpenCode permission fallback and runtime dispatch
...
- [Reliable Claude authentication strategy](/reference/research/authentication/claude-auth-strategy/) — the parallel work for Anthropic's own auth
