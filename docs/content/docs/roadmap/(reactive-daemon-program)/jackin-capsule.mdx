...
This section is design history/spec context from the Phase 3 rewrite, not a live defect list. The shipped implementation differs where the status paragraph and reference pages above say it differs.
...
- **Idle state** — every byte from the client is forwarded to the focused pane's PTY, except the configured prefix when `JACKIN_PREFIX` is set. Prefix mode is disabled by default; `JACKIN_PREFIX=C-b` opts into the tmux-style `Ctrl+B` prefix.
...
Prefix bindings when prefix mode is enabled (subject to refinement during implementation):
...
Prefix `u` is reserved for the focused-agent usage overlay. The canonical TUI contract lives in [Capsule usage overlay](/roadmap/capsule-usage-overlay-responsiveness-and-tabs/): status-chrome signal, CodexBar-inspired quota bucket modal, focused-provider rules, daemon-disconnected fallback, and read-only host-side-effects boundary.
...
- **Phase 3b — Prefix-key input.** Replace the `0x0A` palette intercept and the standalone Alt+arrow shortcut with the prefix-key state machine described above. Prefix mode disabled unless `JACKIN_PREFIX` is set; `JACKIN_PREFIX=C-b` enables `Ctrl+B`. The command palette still exists, opened by `prefix + Space`. Update <RepoFile path="crates/jackin-capsule/src/tui/input.rs">crates/jackin-capsule/src/tui/input.rs</RepoFile> and the dialog dispatch in <RepoFile path="crates/jackin-capsule/src/daemon.rs">crates/jackin-capsule/src/daemon.rs</RepoFile>.
...
- **Herdr's SSH remote tunneling.** Herdr's transparent PTY forwarding over SSH has a different threat model than the explicit operator-controlled tunneling planned for [jackin-remote](/roadmap/jackin-remote/).
...
### Phase 3 — In-container multiplexer *(replace tmux — partially implemented)*

The shipped Phase 3 work landed PID 1 ownership, the PTY layer, a binary pane tree, a status bar, a Unix socket attach channel, jackin-term `DamageGrid`, prefix-key support, and the dockerfile changes that drop tmux from the derived image. Remaining work is not another tmux replacement; it is the still-open control/event surface, host aggregation, and integrations listed below.
...
- **Phase 3b — Prefix-key input.** Replace the `0x0A` palette intercept with the prefix-key state machine. Prefix mode disabled unless `JACKIN_PREFIX` is set; `JACKIN_PREFIX=C-b` enables `Ctrl+B`. Bindings as listed in "Input model: prefix key" above when prefix mode is enabled. Add the "Escape forwards immediately" rule (zero-delay disambiguation).
- **Phase 3c — Persistent server + binary attach channel.** The tag-plus-length binary attach channel is shipped. Persistent exited-tab retention is not: the current daemon exits cleanly when no live sessions remain, so exited tabs are not retained for replay after the last session ends. Keep any future exited-tab retention as explicit remaining work.
...
- Shipped the [agent runtime status authority](/roadmap/agent-runtime-status/): raw `working` / `blocked` / `idle` / `unknown`, derived `done`, foreground-process detection, semantic runtime reports, visible-screen signals, stale-report arbitration, and stuck diagnostics. Silence alone is not a blocker signal.
- Implement the two-stage `done` / `idle` split and the operator-acknowledgement mechanism.
- Expand control-channel API: `session.create`, `session.kill`, `session.title`, and event streaming remain future work; the shipped control channel is request/reply today.
...
- See [jackin❯ daemon](/roadmap/jackin-daemon/) and [jackin❯ Desktop Agent Hub](/reference/research/desktop/agent-hub/jackin-desktop-agent-hub/).

## Relationship to other roadmap items

- **[Agent Orchestration Program](/reference/research/agent-orchestration/program-research/)** — Herdr is evaluated there as the strongest prior art for the multiplexer vision. The full comparative table (Herdr vs. jackin❯ values) lives in the research overview.
- **[Console agent session control](/roadmap/console-agent-session-control/)** — Phase 4 of that item unblocks once Phase 2 of this item ships: the binary exposes live session state, eliminating manifest-snapshot reconciliation.
- **[Agent runtime status authority](/roadmap/agent-runtime-status/)** — the Phase 3 event stream is the delivery mechanism for blocked/working/done/idle/stuck indicators in the console and hardline.
- **[Agent attention prompts](/roadmap/agent-attention-prompts/)** — `blocked` events replace PTY polling from the host; `done` events trigger the "ready for review" notification path.
- **[jackin❯ daemon](/roadmap/jackin-daemon/)** — `jackin-capsule` is the per-container endpoint the daemon subscribes to. Phase 4 of this item and the daemon's container-watch phase are designed together.
- **[jackin❯ Desktop Agent Hub](/reference/research/desktop/agent-hub/jackin-desktop-agent-hub/)** — Phase 3 of this item is the prerequisite: the desktop app's live session view is driven by `jackin-capsule` socket events aggregated by the daemon.
- **[jackin-remote](/roadmap/jackin-remote/)** — remote containers will need a `jackin-capsule` socket accessible over the tunnel; the socket mount and auth model need to account for the remote path.
