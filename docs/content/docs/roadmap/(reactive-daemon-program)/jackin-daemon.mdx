...
jackin❯ now has a per-user host daemon command group documented at [`jackin daemon`](/commands/daemon/). The daemon owns lifecycle, a same-user Unix socket at `~/.jackin/run/jackin-daemon.sock`, JSONL request/response framing, `hello`/`status`/`shutdown`, launchd/systemd user-unit file writers, an `attention_snapshot` consumer, and immutable process-owned OTLP providers. Startup failures remain on the invoking product error surface; the control protocol reports a credential-free configuration fingerprint and typed current health. The in-container `jackin-capsule` PID 1 process remains a separate per-container control plane; Capsule Phase 4 still depends on future host-daemon event subscriptions.
