...
**Status**: Lifecycle foundation plus first attention adapter shipped — `jackin daemon serve/start/stop/restart/status/install/uninstall` exists with a private Unix control socket, protocol/build skew checks, direct OTLP export, typed live telemetry health, per-user launchd/systemd unit writers, and a muted-by-default one-way attention snapshot adapter. Reactive features that still depend on the daemon ([live auth sync](/roadmap/live-auth-sync/), [host bridge](/roadmap/host-bridge/), broader Desktop Agent Hub APIs) remain deferred until their own adapter plans land.
