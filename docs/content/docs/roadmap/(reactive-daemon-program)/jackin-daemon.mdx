...
The daemon draft PR remains the right place to adjust the base contract while Desktop is being designed. The [jackin❯ Desktop Agent Hub](/reference/research/desktop/agent-hub/jackin-desktop-agent-hub/) roadmap depends on this daemon, so daemon API gaps discovered there should be fixed here instead of papered over in the macOS app.
...
- <RepoFile path="crates/jackin-runtime/src/runtime/caffeinate.rs">crates/jackin-runtime/src/runtime/caffeinate.rs</RepoFile> — today's per-command keep-awake reconciler; first migration target in Phase 3.
