...
- **No durable raw log.** Raw bytes are dropped after parsing, so there is no lossless replay source unless [terminal observation and automation](/roadmap/terminal-observation-automation/) adds opt-in recording first.
- **~10k-line horizon.** Anything older than the retained scrollback is gone.
- **Secrets.** Rendered terminal output can contain tokens, paths, and file contents; screen-scrape capture must be opt-in and redaction-aware, consistent with the recording stance in [terminal observation and automation](/roadmap/terminal-observation-automation/).
...
- **Semantic / vector search.** V1 is index + keyword. Semantic retrieval waits until there are enough captured conversations to evaluate quality, aligned with the [persistent storage layer](/roadmap/persistent-storage-layer/) memory direction.
...
- **Orchestration.** Déjà Vu provides the read surface a future orchestrator consumes (see [Agent workflow orchestration](/roadmap/agent-workflow-orchestration/)); it does not schedule or drive agents.
...
| Déjà Vu-private index vs a table in the persistent storage layer? | Prefer reusing the [persistent storage layer](/roadmap/persistent-storage-layer/) once it lands; ship a small private SQLite index first if Déjà Vu moves earlier. |
...
- [Persistent storage and workspace memory layer](/roadmap/persistent-storage-layer/) — the index/DB home Déjà Vu should reuse, and the cross-agent memory direction it complements.
- [Terminal observation and automation](/roadmap/terminal-observation-automation/) — owns the PTY/`DamageGrid` read APIs and opt-in recording the screen-scrape fallback would build on.
...
- [Agent workflow orchestration](/roadmap/agent-workflow-orchestration/) — future consumer of the captured-conversation read surface.
