...
2. **Author a new `docs/.rumdl.toml`.** Start from the default rule set, then disable rules that conflict with Fumadocs conventions:
   - `MD057` (relative-link existence) — fires on Fumadocs' site-absolute routes (`/guides/mounts/`) which are rendered URLs, not filesystem paths. lychee already covers this.
   - `MD041` (first-line H1) — Fumadocs renders titles from frontmatter; the first heading in MDX is `##`.
