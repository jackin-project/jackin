...
- jackin❯ positioning is "one tool for everyone" — a queue that only ingests
...
- *No abstraction in multicode* — this leaf proposes a `TaskSource` trait jackin❯-side; multicode hardcodes GitHub. Source code: [`lib/src/manager.rs`](https://github.com/graemerocher/multicode/blob/main/lib/src/manager.rs) is where their queue logic lives.
...
Each source also returns or references a `TaskPolicy` so the queue does not
hardcode GitHub issue assumptions:
...
The policy is user-facing because it controls what a queued agent may publish,
where it may publish, and which manual gates are required. It belongs in the
resolved session contract for every autonomous dispatch.
...
[GitHub link tracking](/roadmap/github-link-tracking/). Tasks
include the issue URL in `links` so the dispatched agent automatically
sees `<jackin:issue>...</jackin:issue>` (the [tag
protocol](/roadmap/agent-tag-protocol/) seam emits one for the
...
- **Source authentication.** GitHub PAT comes from the existing
  ...
  via the [credential source pattern](/roadmap/credential-source-pattern/)*.
...
- <RepoFile path="crates/jackin-config/src/app_config.rs">crates/jackin-config/src/app_config.rs</RepoFile> — `[task_sources]` parsing
- <RepoFile path="crates/jackin/src/workspace.rs">crates/jackin/src/workspace.rs</RepoFile> — workspace's task_sources
  field
...
- [Agent tag protocol](/roadmap/agent-tag-protocol/) — tasks
