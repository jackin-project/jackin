...
**Status**: Open — design pattern (Phase 5, [Agent Orchestrator Research Program](/roadmap/agent-orchestrator-research/))

## Problem

jackin❯ already has several credential-resolution paths, documented in
[Environment Variables](/guides/environment-variables/) and
[Authentication](/guides/authentication/). They do not yet share one
typed credential-source abstraction.

Several upcoming items need *more* sources:

- [GitHub link tracking](/roadmap/github-link-tracking/) needs
  a GitHub PAT.
- [Task source abstraction](/roadmap/task-source-abstraction/)
  may need per-source tokens.
- [Claude auth strategy](/roadmap/claude-auth-strategy/)
...
- multicode has already tried three sources (`env`, `command`, keychain)
  ...
  one from scratch.
- It pairs with [jackin-remote](/roadmap/jackin-remote/) and
...
2. **`env`** — read host-operator env var; error if unset. This is never
   broad environment inheritance into the agent container.
3. **`command`** — run a command without a shell by default, with a minimal
   environment; the command must exit 0 and may only return the secret on
   stdout.
4. **`op`** — route to 1Password CLI using the current operator-env behavior.
5. **OS secret stores** — Apple Keychain via `security` on macOS,
   libsecret/`secret-tool` on Linux, and Windows Credential Manager when
   supported. New backend.
...
- Replace existing ad-hoc credential lookups with the unified type while
  preserving the behavior documented under Environment Variables and
  Authentication.
...
- **OS secret-store implementation.** macOS Keychain and Linux
  `secret-tool` are the first practical targets; Windows Credential Manager
  should share the same resolver shape once a Windows host story matters.
- **Forwarding interaction with [Claude auth
  strategy](/roadmap/claude-auth-strategy/).** That item is
...
- <RepoFile path="crates/jackin-config/src/app_config.rs">crates/jackin-config/src/app_config.rs</RepoFile> — TOML deserialization
...
- [Task source abstraction](/roadmap/task-source-abstraction/) —
  new consumer
- [jackin-remote](/roadmap/jackin-remote/) — credential
  forwarding model
