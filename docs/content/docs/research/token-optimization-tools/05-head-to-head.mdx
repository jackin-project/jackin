...
| Language-aware code outlining (per-read) | ✗ (passes code verbatim) | ✓ | ✓ (8 languages) | ✓ (tree-sitter, 18 languages) |
...
| Parser / structural | none | per-type (AST outline, JSON, log) | per-command + a `filter.rs` code filter (8 langs) | tree-sitter (18 langs) + persistent property graph + call graph |
