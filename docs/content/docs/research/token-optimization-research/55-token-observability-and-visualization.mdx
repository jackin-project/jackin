---
title: "55 — Token observability and session visualization (token-optimizer and peers)"
---
...
- **The one token class it still cannot show is thinking — and neither can any JSONL tool.** Claude Code redacts thinking from the transcript; output_tokens is thinking + visible fused. token-optimizer visualizes the input/output/cache split (the actionable ~80% of the bill) but cannot directly graph the thinking-vs-visible slice the dossier had to *infer* via `count_tokens(visible)` (file 02). "Full visibility for every token" has this one structural blind spot on hosted Claude.
...
- **Per-turn breakdown:** input / output / cache-read / cache-write, with **spike detection** highlighting context jumps. This is the dossier's headline token-class split (32% cache-read / 29% cache-write / 20% thinking / 17% visible output / 2% uncached, file 00/02) rendered per turn — except thinking stays inside the output bar (see the blind spot below).
...
The observability niche splits into four classes by data source, which determines cache-safety and what is visible. (This section is seeded from the dossier's record 18 plus the direct sweep; the parallel market re-sweep enriches the table with the newer 2026 session-viewers and any per-token visualizers it surfaces — load-bearing additions will be merged with sources before this file's ranking is treated as final.)

| Tool | Class | What it makes visible | Surface | Cache-safe? | Notes |
|---|---|---|---|---|---|
| **token-optimizer** (alexgreensh) | JSONL reader | Per-turn input/output/cache split, spike detection, quality grades, 30-day trends | Web dashboard + status line + CLI | **Yes** (no proxy) | The anchor; richest local session viz found; PolyForm-NC license |
| **ccusage** | JSONL reader | Daily / session / model cost + token totals | CLI (tables) | **Yes** | Record 18; 16,080★; totals, not per-turn cache split or viz |
| **Claude Code `/usage`** | Built-in | Usage by skill / subagent / plugin / MCP server; cache-miss line (2.1.174) | In-CLI | **Yes** | Record 18 / file 46; first-party, no external tool |
| **`tools/session_cost.py`** (this dossier) | JSONL reader | $/token split deduplicated by `message.id`, the 32/29/20/17/2 decomposition | Script | **Yes** | The dossier's own instrument; reproducible, no UI |
| **OpenTelemetry (Claude Code OTel export) → Grafana/Prometheus** | OTel metrics | Token counts/cost as time-series; fleet-level | Dashboards | **Yes** (read-only export) | Reach across many sessions; needs a metrics stack |
| **Langfuse / Helicone / Phoenix(Arize) / OpenLLMetry(Traceloop) / Lunary** | Proxy or SDK tracing | Per-call/per-trace token + cost, latency, often per-message | Web platform | **Proxy variants: audit** | General LLM observability; self-hostable (most); a proxy in the path can affect caching — prefer SDK/callback or read-only modes |

The pattern mirrors file 54's compression sweep: **the JSONL-reading, no-proxy class is the safe default for a coding agent** (token-optimizer, ccusage, `/usage`, `tools/`), because it cannot perturb the request and adds zero prefix rent. Proxy/OTel observability buys cross-session and cross-tool reach at the cost of a moving part in (or beside) the request path — worth it for a fleet, overkill for "visualize my latest session."

## jackin' fit

- **Adopt the local-transcript observability class as the measurement front-end of the validation harness** (31, and the per-tool harnesses in 51/53). token-optimizer's per-turn cache-split dashboard and `tools/session_cost.py` answer the same question the harness needs ("where did the tokens go this session?") with no proxy and no prefix cost — pure negative-cost, the file-47 "measure first" rule with a UI.
...
- token-optimizer stats (1,341★ / 8 watchers / 110 forks / created 2026-02-26): `gh api repos/alexgreensh/token-optimizer`; source tree (`session-parser.js`, `dashboard.js`, `jl-sketcher.js`, `pricing.js`, `quality.js`, `read-cache.js`, `smart-compact.js`, `drift.js`) confirms the JSONL-parse → dashboard architecture
- dossier cross-references: measurement method and ccusage/`/usage` — [`03-prior-art-and-market-scan.md`](/research/token-optimization-research/03-prior-art-and-market-scan/) (record 18); the token-class decomposition and thinking-invisibility — [`02-baseline-audit.md`](/research/token-optimization-research/02-baseline-audit/) and [`00-executive-summary.md`](/research/token-optimization-research/00-executive-summary/); keepalive graveyard #3 — `00`/`20`; outline lever — [`51-code-intelligence-tools.md`](/research/token-optimization-research/51-code-intelligence-tools/); subscription/quota metric — [`41-subscription-and-quota-economics.md`](/research/token-optimization-research/41-subscription-and-quota-economics/); online quality + context rot — [`47-meta-cost-governance-and-online-quality.md`](/research/token-optimization-research/47-meta-cost-governance-and-online-quality/) and [`46-fresh-literature-and-market-delta.md`](/research/token-optimization-research/46-fresh-literature-and-market-delta/); runnable instruments — [`tools/`](/research/token-optimization-research/tools/)
- compression companions: [`53-headroom-and-context-compression.md`](/research/token-optimization-research/53-headroom-and-context-compression/), [`54-context-compression-literature-and-market.md`](/research/token-optimization-research/54-context-compression-literature-and-market/)
