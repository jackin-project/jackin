---
title: "53 — Headroom and the context-compression layer (vs the caveman ecosystem)"
---
...
- **Evidence is vendor self-report (T3-weak / T4), and the two independent hands-on measurements that exist both land at ~47–50% whole-session — below the 60–95% headline.** A third-party deploy (Miya-Gadget, 2026-06-03) measured 59,742→31,358 tokens (**47.5%**) with RAG prose compressed **0%** and logs only 31%, calling the "95%" claim "oversold"; an HN user reported "~50%." The headline survives only as a per-payload ratio on redundant JSON/logs (see the benchmarks section). The *mechanisms*, meanwhile, inherit **T1** from the dossier's own local reproductions (log-filter −94.2%, outline −91%, symbol-search −98%, JSON-minify −34.3%/−41.2%). Headroom productizes proven levers; it does not invent a new physics.
...
**Independent measurement exists now, and it lands below the headline.** A third party (Miya-Gadget, 2026-06-03) deployed headroom on a real coding session and measured **59,742 → 31,358 tokens = 47.5% overall**, broken down as code 79.8%, JSON 59.2%, logs 31.0%, and **RAG/prose 0.0% (untouched by default)** — concluding the "95% token reduction" marketing "feels oversold," with realistic sessions at ~20–30% and 80%+ only on high-redundancy JSON/logs. An HN user independently reported "~50%." Both corroborate the per-payload-vs-whole-bill gap exactly: 60–95% is real on redundant content and evaporates on prose/code-heavy mixes. The press that drove headroom's visibility — a 2026-05-31 Register piece repeating a vendor "$700K saved / 90% redundant" figure, echoed by ~20 downstream outlets — ran no independent test (file 54 §A).
...
## Deep comparison: headroom vs the caveman ecosystem

The operator runs the caveman family (caveman + cavemem + cavekit + cavecrew + caveman-compress/commit/review). The honest comparison is that **the two ecosystems barely overlap** — they attack opposite ends of the token pipeline from opposite engineering philosophies — and the one place they do overlap (memory) is the place to choose between them.

| Axis | Caveman ecosystem | Headroom |
|---|---|---|
| Direction | **Output** — what the model writes | **Input/context** — what the model reads |
| Primary bucket | Visible prose (~17% of dollars) | Tool outputs/logs/RAG/files/history (the 61% cache lines) |
| Mechanism | Skill-prompt register change (terse English / Classical Chinese) | Typed compressors (AST/JSON/log/search) + a trained text model + drop-low-value |
| Lossiness | **Lossy, no recovery** — dropped caveats are gone | **Reversible** — originals in CCR, fetched via `headroom_retrieve` |
| Touches code? | Passes code/diffs verbatim | Collapses function bodies, keeps signatures (outline); passes raw code ~0% |
| Touches thinking? | **No** (README concedes it) | **No** |
| Cache interaction | Neutral (output side) | Safe in MCP/library, risk in proxy |
| Form factor | Claude Code plugin / skills (minutes to adopt) | pip/npm/docker + Rust core + local ML runtime (minutes–hours) |
| Self-cost | ~940 tok/session skill-listing prefix (file 03) | Per-request ML model + proxy latency + MCP schema rent |
| Evidence | Output ultra **−58.5% local** (token) vs the 75% claim | **−66.1% self-report**, 0% on code, no independent replication |
| Adoption | caveman 72,759★ (created 2026-04-04) | 28,199★ (created 2026-01-07) — both gh api 2026-06-15, **both star counts PR-inflated in this niche; rank by evidence, not stars (file 54)** |
| Engineering depth | Prompt/skill design | 190 releases, Rust core, provider-specific proxy |

Mapped component by component, the two ecosystems are near-mirror images rather than rivals:

- **caveman (output register) ↔ headroom compression (input).** Different token classes; they **stack**. The combined ceiling is still bounded by the dossier's structural walls: caveman cuts a slice of the 17% you write, headroom cuts a slice of the 61% you read, and the 20% thinking bucket is untouched by both. There is no double-counting because they never touch the same tokens.
- **cavemem (compressed memory MCP) ↔ headroom cross-agent memory + CCR.** This is the real overlap. cavemem stores caveman-compressed (lossy prose) memory in SQLite+FTS5 with BM25/vector retrieval, single-agent. Headroom's memory is **reversible** (the lossy form is a view; the original is retrievable) and **cross-agent** (one store shared across Claude, Codex, Gemini, with auto-dedup). On architecture, headroom's memory is the stronger design — reversibility removes cavemem's "confidently-wrong recalled fact" failure mode, and cross-agent dedup matters for a multi-tool operator. **Both still lack the net-accounting the dossier demanded** (white-space #5): neither publishes injection-cost-vs-re-exploration-saved.
- **cavekit (spec-driven build loop) ↔ `headroom learn`.** Different mechanisms, same meta-layer ambition: persist hard-won context so it is not re-derived. cavekit writes a durable SPEC.md up front; `headroom learn` mines *failed* sessions after the fact and writes corrections into CLAUDE.md/AGENTS.md. `headroom learn` is the more novel idea (closed-loop self-correction) and is new to the dossier (H3).
- **cavecrew (compressed subagents) ↔ headroom `SharedContext`.** cavecrew compresses subagent reports back into the main thread (dossier measured −43.9%, not the claimed 60%); `SharedContext().put/.get` passes compressed context across a multi-agent workflow. Same goal (cheaper agent-to-agent transfer); cavecrew is Claude-Code-native, SharedContext is framework-level.

**The decisive asymmetry (2024–2026 literature, file 54): output brevity is cache-neutral; input compression is cache-breaking unless done at write-time on new content.** This is the structural reason caveman's lever is *safer* than headroom's, not merely smaller. Caveman shrinks generated output — it never touches the cached prefix, so it composes with caching for free and hits the 5×-priced output class. Headroom shrinks input, which on an already-caching Claude either (a) recompresses the cached prefix and busts it (the record-19 trap), or (b) is scoped to *new* observations at write-time (the live-zone / MCP path, H1) — the only cache-safe way to compress input. A pre-registered RCT on 358 Claude Sonnet 4.5 runs (arXiv 2603.23525) shows the failure mode: moderate input compression cut cost 27.9%, but aggressive compression *raised* cost 1.8% as output expanded — and it did not even price the cache it also breaks. Practical ordering: adopt output brevity (caveman) first because it is unconditionally cache-safe; adopt input compression (headroom) only at write-time on observations.

The verdict the operator actually needs: **keep caveman for output, evaluate headroom (MCP mode) for input, and treat the memory layer as an either/or** — run cavemem *or* headroom memory, not both, and pick headroom's if the operator works across Claude + Codex + Gemini and values reversibility, cavemem's if the operator is Claude-only and wants the lighter, plugin-native option. Running both memory layers is pure overhead.
...
|---|---|---|
| H-K1 | "headroom cuts 60–95% of your tokens" | Per-compressible-payload ratio, not whole-bill. Repetitive logs/JSON hit 87–94%; code and grep compressed **0%** in v0.5.18 ("passes through to preserve correctness"); the representative mix is 66.1%. **Independently measured at 47.5% whole-session** (Miya-Gadget, 2026-06-03; RAG prose 0%, logs 31%) and "~50%" (HN) — both below the headline. Whole-bill effect = compressible-observation share × compression × (write + 0.1×read) — low-double-digit % of dollars, same category as the caveman K1 correction. |
...
- cross-references (caveman/cavemem/cavekit/cavecrew records, K1/K4, white-space map): [`03-prior-art-and-market-scan.md`](/research/token-optimization-research/03-prior-art-and-market-scan/)
- cache-conflict + CompressionAttack + CAG/FL1/FL3: [`46-fresh-literature-and-market-delta.md`](/research/token-optimization-research/46-fresh-literature-and-market-delta/)
- log-filter −94.2% / TOON −41.2% / minify −34.3% local reproductions: [`03-prior-art-and-market-scan.md`](/research/token-optimization-research/03-prior-art-and-market-scan/)
- outline −91% / symbol-search −98% local reproductions: [`51-code-intelligence-tools.md`](/research/token-optimization-research/51-code-intelligence-tools/)
- fresh-literature sources (write-time compression, code-domain SWE-bench gains, cache break-even, output-brevity dominance, context rot) and the compression-tool market sweep: [`54-context-compression-literature-and-market.md`](/research/token-optimization-research/54-context-compression-literature-and-market/). Key inline IDs: Squeez arXiv 2604.04979; AgentDiet arXiv 2509.23586; "Don't Break the Cache" arXiv 2601.06007; Claude 4.5 compression RCT arXiv 2603.23525; SWEzze arXiv 2603.28119; SWE-Pruner arXiv 2601.16746; Perplexity Paradox arXiv 2602.15843; Chain-of-Draft arXiv 2502.18600; brevity-hierarchy arXiv 2604.00025.
