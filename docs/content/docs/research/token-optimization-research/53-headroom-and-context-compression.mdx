...
- **Evidence is vendor self-report (T3-weak / T4): no independent replication exists** (confirmed across the README, the docs site, and every secondary write-up found — all repeat the maintainer's numbers and most explicitly say "measure on your own workloads"). The *mechanisms*, however, inherit **T1** from the dossier's own local reproductions (log-filter −94.2%, outline −91%, symbol-search −98%, JSON-minify −34.3%/−41.2%). Headroom productizes proven levers; it does not invent a new physics.
...
| Adoption | 28,185 stars, 1,908 forks, 30 contributors, 268 open issues (gh api, 2026-06-15) |
...
| Adoption | caveman 71,891★ | 28,185★ |
...
|---|---|---|
| H-K1 | "headroom cuts 60–95% of your tokens" | Per-compressible-payload ratio, not whole-bill. Repetitive logs/JSON hit 87–94%; code and grep compressed **0%** in v0.5.18 ("passes through to preserve correctness"); the representative mix is 66.1%. Whole-bill effect = compressible-observation share × compression × (write + 0.1×read) — low-double-digit % of dollars, same category as the caveman K1 correction. |
