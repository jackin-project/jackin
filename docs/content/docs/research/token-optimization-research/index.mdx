...
> **The Caveman vs Headroom vs RTK three-way comparison now has its own dedicated, diagram-driven folder:** [Caveman vs Headroom vs RTK](/research/caveman-headroom-rtk/) consolidates and deepens the material in files 53/54/56 — equal-depth design teardowns of all three, a feature has/lacks matrix, best-case-of-each, and a straight answer to whether one product can combine all three. Files 53/54/56 below remain for their broader scope and full source ledgers.
...
- **[`53-headroom-and-context-compression.md`](53-headroom-and-context-compression/)** — deep dive on
  ...
  caveman ecosystem and RTK is consolidated in the dedicated [Caveman vs Headroom vs RTK](/research/caveman-headroom-rtk/) folder. Headroom compresses what the model *reads* (tool outputs/logs/RAG/files, the 61%
...
- **[`56-rtk-and-write-time-observation-compression.md`](56-rtk-and-write-time-observation-compression/)** —
  deep dive on `rtk-ai/rtk` ("Rust Token Killer") — the dossier's RTK record. The **three-way** Caveman vs Headroom vs RTK
  comparison it originally carried now lives, expanded, in the dedicated [Caveman vs Headroom vs RTK](/research/caveman-headroom-rtk/) folder (single source of truth). RTK is the deterministic, Claude-Code-native productization of the
