...
Method: deterministic samples piped through the free count_tokens harness (`python3 /tmp/ct.py &lt;model&gt; < sample`), one user message per call. Samples: first 2,000 B of `crates/jackin-core/src/agent.rs`; first 4,096 B and full 6,366 B of this repo's <RepoFile path="AGENTS.md">AGENTS.md</RepoFile>; a fixed 528 B English paragraph; a 558 B JSON tool schema. Counts include a few tokens of message-wrapper overhead (ratios are marginally understated).
