...
Nothing. When jackin' downloads `jackin-capsule` at launch it automatically verifies a signed manifest before accepting the binary. The verification is silent and transparent — you only notice it if something is wrong (in which case the launch aborts with an error message).
