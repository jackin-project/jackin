...
| `JACKIN_TELEMETRY_LEVEL` | Overrides `[telemetry].level` from `config.toml`: `info` (default), `debug`, or `trace`. `debug`/`trace` enable debug-category diagnostics even when `--debug` is not set. |
