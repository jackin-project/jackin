...
| `JACKIN_DEBUG` | Backs `--debug` for verbose local diagnostics and capsule debug logging. |
| `JACKIN_TELEMETRY_LEVEL` | Overrides telemetry verbosity: `info` (default), `debug`, or `trace`. `debug`/`trace` enable debug-category diagnostics even when `--debug` is not set. |
| `JACKIN_TELEMETRY_CATEGORIES` | Optional comma-separated debug-category allowlist such as `docker,launch`; `*` allows every debug category. Applies before debug-category events enter JSONL or OTLP. |
