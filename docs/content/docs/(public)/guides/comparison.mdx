...
| **Environment model** | Versioned role repo with Dockerfile + `jackin.role.toml` | Official agent templates, custom templates, snapshots |
...
| **Persistence** | Mounted workspace files plus per-agent state (Claude Code, Codex, Amp, Kimi, OpenCode) and `gh` state | Sandbox-local packages, private Docker state, and sandbox filesystem state persist until removal |
...
- centered on reusable roles as code
...
### 2. Roles are first-class source artifacts

An role is a normal git repo with a Dockerfile and manifest. That gives you:
...
This is one of the biggest jackin❯ advantages.
...
- which role should be the default
...
Docker Sandboxes can run multiple sandboxes, but the jackin❯ role and workspace model makes this style of work more explicit.
...
So the baseline cost per sandbox will usually be materially higher than the jackin❯ model of:
...
It is a strength because it pushes you toward role repos as the real source of truth.

It is a weakness because ad-hoc runtime setup work disappears when the container is removed.

### 5. Runtime binaries are cached by jackin❯

jackin❯ resolves agent runtime metadata on the host, uses that metadata to find the latest Linux binary for the host's container architecture, downloads that binary once per version, and stores it under jackin❯ local cache. During image preparation, all supported agent binaries are resolved and prepared together; cache misses use jackin❯ `fast-down` parallel downloader instead of a sequential file fetch. Derived image builds then copy those cached binaries into the image instead of running each agent's upstream installer. This keeps repeated rebuilds fast while still checking for newer Claude Code, Codex, Amp, Kimi, OpenCode, and Grok versions.

If jackin❯ cannot resolve or download the current hard-coded binary URL because an upstream release layout changed or a server is unavailable, it first tries the newest executable already present in the local cache. If no usable cached executable exists, the derived Dockerfile switches that agent to the official upstream installer script instead of copying a missing binary. That fallback is slower, but it follows the agent vendor's own install path, and jackin❯ emits a warning so the operator can see that binary pre-download failed and an installer fallback was used.
...
That last point matters for jackin❯ roadmap. Starting with local Docker is reasonable. The same operator and workspace model can later grow into Kubernetes-backed debug workflows.
...
- you care about reusable roles as code
