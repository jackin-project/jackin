...
- **All logs** — the same event stream as the diagnostics file: lifecycle
  ...
  external `docker`/`git` command firehose after diagnostics redaction. Raw
  PTY bytes, rendered frame bytes, and decoded keystroke/input payloads stay
  local to the capsule `multiplexer.log` and are not exported. The OTLP
