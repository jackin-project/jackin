...
Every `jackin` command can record its full story — logs, lifecycle events, and per-stage timings — keyed by a **run id** (printed again at run exit). It goes to one of two sinks: a live external view streamed to any OpenTelemetry (OTLP) backend, or a local diagnostics file. Set one environment variable to stream to a backend; with none configured, jackin❯ falls back to the file. See [Where the run is recorded](#where-the-run-is-recorded) for how the two relate.
...
`--debug` controls how much detail is recorded (see [Severity and category controls](#severity-and-category-controls)) — not whether the file is written. For telemetry-only overrides, set `JACKIN_TELEMETRY_LEVEL=info|debug|trace` and optionally `JACKIN_TELEMETRY_CATEGORIES=docker,launch`. If an export is configured but can't be delivered, jackin❯ surfaces a notice and falls back to writing the file so the run is never lost silently.

At exit, jackin❯ prints the run id and the active sink. File-mode runs include the JSONL path. Backend-mode runs tell you to query the backend for `parallax.run.id=<run-id>`; debug dialogs and failure popups only show a `Diagnostics log` / reveal row when that local file actually exists.
...
Diagnostics log records also carry `event.name`, `event.outcome`,
`jackin.component`, `jackin.operation`, and `jackin.category`, so backend
queries do not need to parse console prefixes or freeform bodies.
...
- **All logs** — the same event stream as the diagnostics file: lifecycle
  ...
  external `docker`/`git` command firehose after diagnostics redaction. With
  `JACKIN_TELEMETRY_LEVEL=trace`, raw PTY bytes, rendered frame bytes, and
  decoded keystroke/input payloads are captured at TRACE. When OTLP is active
  they go to the backend and are not mirrored to `multiplexer.log`; without
  OTLP they fall back to the local capsule log. The OTLP transport's own logs
  are filtered out, so the export is jackin❯ story, not its plumbing.
...
- **Process metrics** — while the process runs, metrics export every 5
  ...
  one timeline.
- **Run-domain metrics** — the active run exports low-cardinality counters such
  as `jackin.diagnostics.events` (tagged by `kind`), `jackin.cache.hits`, and
  `jackin.cache.misses`. Dependency-internal storage spans such as Turso VM
  internals stay out of the default export; use workflow/domain counters and
  the allowlisted jackin❯ targets for normal analysis.
...
`JACKIN_TELEMETRY_LEVEL` can override the telemetry side without changing the
CLI's existing debug UI behavior:

- `info` exports compact lifecycle telemetry.
- `debug` exports debug-category diagnostics.
- `trace` raises the OTLP allowlist to TRACE for jackin❯ targets and enables
  debug-category diagnostics plus raw capsule PTY/frame/input payload records.

Set `JACKIN_TELEMETRY_CATEGORIES` to a comma-separated allowlist such as
`docker,launch` to keep unrelated debug categories out of JSONL and OTLP. Use
`*` to allow every category.
