...
`jackin diagnostics validate` emits one governed marker for each signal, flushes the exporters, and exits successfully only when traces, logs, and metrics reach the configured endpoint. It reports a non-zero exit when delivery is disabled, unconfigured, inactive, or unsuccessful.
...
For the complete schema, identity, propagation, and health contract, see [Application observability](/reference/runtime/diagnostics/).
