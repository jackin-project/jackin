...
```bash
brew install --cask jackin-project/tap/jackin-usage-menu-bar
```

That installs `JackinUsageMenuBar.app` from the immutable GitHub Release ZIP. Floor: **macOS 14 (Sonoma)+**. Uninstall: `brew uninstall --cask jackin-usage-menu-bar` (removes the app; no root daemon or LaunchDaemon).

### Direct download

Download `jackin-usage-menu-bar-<VERSION>-universal-apple-darwin.zip` plus `.sha256` / `.bundle` / `.sbom.json` from the [GitHub Releases](https://github.com/jackin-project/jackin/releases) page, verify with `cargo xtask release-verify <zip>`, then open the app. See [Verifying releases](/getting-started/verifying-releases/).
