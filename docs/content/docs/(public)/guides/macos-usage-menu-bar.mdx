...
Grok's plan/tier is read from xAI's billing service (the true subscription tier), not guessed from how you logged in, so a Free-tier browser login is labeled correctly; its prepaid credits and on-demand cap are shown as spending **limits**, never token prices or usage trends.
...
Footer: a single **Refresh** (⌘R) row plus the next-refresh caption. When no surfaces are enabled, the first-run note points you at Settings — there is no credential-harvesting UI.
