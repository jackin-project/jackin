...
CodexBar-style **per-provider** menu-bar strip (clean-room): one chip per enabled agent with data — provider glyph + stacked short percents (session/weekly remainings from Rust). Empty providers stay hidden. Settings **Display** mode:
