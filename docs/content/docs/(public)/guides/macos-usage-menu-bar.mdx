...
- **Detail** — Overview list or a full provider card (account, plan, auth origin, quota gauges with OpenUsage-style **`{remaining}% left`** primary lines, pace/reset/money strings, estimate caption when Rust marks local estimates). Settings **Percent style** also applies here.
