...
OpenUsage-inspired status strip (clean-room look-and-feel): template jackin❯ logomark, then compact **mini capacity bars** tinted by severity next to monospaced Rust labels. Empty metrics stay hidden. Settings **Display** mode:

  - **Focus percent** (default) — worst enabled account as one chip (`Cl 63%` + mini bar)
  - **Icon only** — logomark alone
  - **Pinned surface** — one surface’s chip only
  - **Multi-surface strip** — worst-first chips, capped (default 3), e.g. three mini bars + labels
