...
- [OrbStack isolated machine backend](/roadmap/orbstack-isolated-machine-backend/) — research a macOS backend that preserves Docker workflows inside an OrbStack isolated machine with explicit shares.
- [smolvm backend research](/roadmap/smolvm-backend-research/) — research an open Rust/libkrun VM backend for true VM-per-workload isolation, OCI-image execution, and network-off-by-default operation.
