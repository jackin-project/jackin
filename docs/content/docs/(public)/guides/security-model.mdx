...
- [OrbStack isolated machine backend](/reference/research/isolation/orbstack-isolated-machines/) — research a macOS backend that preserves Docker workflows inside an OrbStack isolated machine with explicit shares.
- [smolvm backend research](/reference/research/isolation/smolvm-backend/) — research an open Rust/libkrun VM backend for true VM-per-workload isolation, OCI-image execution, and network-off-by-default operation.
- [Selectable sandbox backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) — keep Docker, OrbStack, Kubernetes, and later true microVM providers under one backend-selection umbrella without forcing one boundary on every workload.
...
For the latest plan and current research items, the [Roadmap](/roadmap/) is the source of truth.
