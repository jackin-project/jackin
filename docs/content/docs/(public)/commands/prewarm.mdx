...
From the jackin❯ workspace console, select a saved workspace and press `W` to run the same path as `jackin prewarm --image --workspace <NAME>`. The console exits before the prewarm command owns the terminal, so the operation has the same host-side effects and progress behavior as the CLI command.
