...
`jackin daemon` manages the per-user host daemon foundation. This production surface ships lifecycle, install files, a private Unix control socket, protocol/build skew checks, status reporting with typed telemetry health, and the first one-way attention adapter. It does not enable live auth sync, host bridge, keep-awake migration, or Desktop Agent Hub adapters.
...
| `jackin daemon status` | Print daemon PID, protocol version, build id, socket path, enabled adapter list, coredump policy, and per-signal telemetry export health. |
