...
The gate checks the archive against `<file>.tar.gz.sha256`, verifies `<file>.tar.gz.bundle` with the same cosign identity and issuer policy shown below, verifies the GitHub artifact attestation for `jackin-project/jackin`, and parses `<file>.tar.gz.sbom.json`.
