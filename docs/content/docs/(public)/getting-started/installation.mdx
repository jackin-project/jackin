...
On macOS 26 (Tahoe) with Apple Silicon, a future jackin❯ release may use Apple
Container as an alternative runtime backend (per-session Linux kernels via
Virtualization.framework). That backend is **not shipped yet** — there is no
`jackin load … --backend apple-container` flag today. Install Docker (or
OrbStack) and use the default Docker backend on all supported platforms.
