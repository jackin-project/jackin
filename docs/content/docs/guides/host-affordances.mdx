...
| Open visible URL | In a mouse-disabled pane such as a shell or plain transcript, `Alt`-click or `Ctrl`-click an `http`, `https`, or `mailto` URL. | Capsule resolves the URL using the same token rules as double-click selection, then asks the host attach client to open it. Non-URL clicks keep the normal mouse fallback. |
...
9. Print or display an `http`, `https`, or `mailto` URL in a shell pane, then test `Alt`-click and `Ctrl`-click. Confirm a non-URL modified click still behaves like normal mouse input.
10. Move the pane cursor onto an `http`, `https`, or `mailto` URL token and run **Open link under cursor**. Then move the cursor onto non-URL text and confirm the command shows a no-link notice.
