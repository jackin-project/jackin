...
| Reveal diagnostics log | Launch with `--debug`, open Debug info, then press `R`. | Capsule asks the host attach client to reveal the diagnostics JSONL file. The host client only accepts canonical paths under jackin's `data/diagnostics/runs/` directory and rejects all other host paths. |
...
6. Copy an absolute image path as text and run **Stage image from clipboard path**; confirm a staged path appears.
...
8. Launch with `--debug`, open Debug info, click the diagnostics row to copy it, then press `R` and confirm the host file manager reveals the same JSONL file.
