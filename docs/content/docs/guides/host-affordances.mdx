...
Ordinary text paste remains ordinary terminal input. A text clipboard containing `/tmp/example.png` is not auto-imported. Use **Stage image from clipboard path** when you intentionally want jackin' to treat host clipboard text as an image path.
...
4. Copy an absolute image path as text and run **Stage image from clipboard path**; confirm a staged path appears.
5. Run **Export file** for a small workspace file and confirm the host copy appears under `~/Downloads/jackin/<instance>/`.
6. Open the GitHub context dialog on a PR branch and test **Open PR** and **Open CI**.
7. Print or display an HTTP(S) URL in a shell pane, then test `Alt`-click and `Ctrl`-click. Confirm a non-URL modified click still behaves like normal mouse input.
