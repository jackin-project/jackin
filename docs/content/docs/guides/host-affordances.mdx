...
7. Run **Export file** for a small workspace file and confirm the host copy appears under `~/Downloads/jackin/<instance>/`.
8. Open the GitHub context dialog on a PR branch and test **Open PR** and **Open CI**.
9. Print or display an HTTP(S) URL in a shell pane, then test `Alt`-click and `Ctrl`-click. Confirm a non-URL modified click still behaves like normal mouse input.
10. Move the pane cursor onto an HTTP(S) URL token and run **Open link under cursor**. Then move the cursor onto non-URL text and confirm the command shows a no-link notice.
