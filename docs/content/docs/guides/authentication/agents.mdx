...
The row is visible only while the agent is in `sync` mode. Press `Enter` on **Source folder** to open the directory picker; hidden folders are visible there so default credential locations like `~/.claude` and `~/.codex` are reachable. Press `s` to select the current directory. Press `d` on the Source folder row to clear an explicit value and return to the inherited or default folder.

The row label tells you whether the folder is `default`, `explicit`, or `inherited`, and includes the environment-variable hint that the agent itself honors (`CLAUDE_CONFIG_DIR`, `CODEX_HOME`, `AMP_DATA_HOME`, or `XDG_DATA_HOME` where applicable).
