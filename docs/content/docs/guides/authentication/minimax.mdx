...
description: Use MiniMax's Anthropic-compatible endpoint as an alternative provider for Claude Code sessions
---

[MiniMax's Token Plan](https://platform.minimax.io/subscribe/token-plan) provides an Anthropic-compatible API endpoint (`https://api.minimax.io/anthropic`) backed by the MiniMax-M3 model. Because the endpoint speaks the same protocol as Anthropic's API, Claude Code works against it without modification — jackin' injects the redirect at session-spawn time.

When `MINIMAX_API_KEY` is configured, opening a new Claude tab presents a provider picker with **MiniMax** as one of the options. Operators without a MiniMax key see no change: MiniMax does not appear in the picker.
...
When you select MiniMax in the provider picker, jackin' injects environment variables into the spawned Claude Code process at launch time — not into any shared config file:
