...
<Aside type="tip">
The Kimi provider is configured in the **Auth** tab of `jackin console` under the **Kimi Code** row (named to distinguish it from the Kimi runtime agent row). Configure it the same way as Z.AI or MiniMax — it is not by hand-editing any config file.
</Aside>
...
2. Scroll to the **Kimi Code** row and press `Enter` or `Space` to open the auth form. (This row is for the Kimi provider credential. The **Kimi** row above it controls the Kimi Code CLI agent runtime.)
