...
This is one of jackin' biggest advantages.
...
Docker Sandboxes can run multiple sandboxes, but jackin' role and workspace model makes this style of work more explicit.
...
So the baseline cost per sandbox will usually be materially higher than jackin' model of:
...
jackin' resolves agent runtime metadata on the host, uses that metadata to find the latest Linux binary for the host's container architecture, downloads that binary once per version, and stores it under jackin's local cache. During image preparation, all supported agent binaries are resolved and prepared together; cache misses use jackin's `fast-down` parallel downloader instead of a sequential file fetch. Derived image builds then copy those cached binaries into the image instead of running each agent's upstream installer. This keeps repeated rebuilds fast while still checking for newer Claude Code, Codex, Amp, Kimi, OpenCode, and Grok versions.
