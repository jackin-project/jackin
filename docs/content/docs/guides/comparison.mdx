...
jackin' resolves agent runtime metadata on the host, downloads each required Linux binary once per version and architecture, and copies the cached binary into derived image builds. This keeps repeated rebuilds fast while still checking for newer Claude Code, Codex, Amp, Kimi, and OpenCode versions.
