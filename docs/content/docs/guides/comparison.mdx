...
jackin' resolves agent runtime metadata on the host, uses that metadata to find the latest Linux binary for the host's container architecture, downloads that binary once per version, and stores it under jackin's local cache. Derived image builds then copy those cached binaries into the image instead of running each agent's upstream installer. This keeps repeated rebuilds fast while still checking for newer Claude Code, Codex, Amp, Kimi, OpenCode, and Grok versions.
