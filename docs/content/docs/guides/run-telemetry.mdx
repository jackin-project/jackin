...
attribute `jackin.run.id`, plus `parallax.run_id` for backends that group on
it). One run id is the thread that ties the whole invocation together — even
though, as below, it is split across several traces on purpose.
...
Because all of these share `jackin.run.id`, a backend that understands run ids
— [Parallax](https://github.com/tailrocks/parallax) is the reference target —
still answers "show me everything for run `<id>`" while letting you walk the
links from screen to screen.
...
- **All logs** — the same event stream as the diagnostics file: lifecycle
  ...
  full firehose including every external `docker`/`git` command. The OTLP/HTTP
...
- **Process metrics** — while the process runs, metrics export every 5
  ...
  runtime counters `tokio.runtime.workers`, `tokio.runtime.alive_tasks`, and
  `tokio.runtime.global_queue_depth`. The same run-id resource attribute rides
...
into one timeline) and the same `jackin.run.id`, and the session links back to
...
The container reaches a backend running on the host loopback
(`127.0.0.1`/`localhost`) automatically: jackin' rewrites the endpoint to
`host.docker.internal` and wires it to the host gateway. Point
`JACKIN_OTLP_ENDPOINT` at an already-routable address and it is passed through
unchanged.
