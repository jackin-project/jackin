...
- `allowlist` network tier (iptables/ipset egress filtering, `JACKIN_ALLOWED_HOSTS` assembled at launch)
...
2. **Default flip** (<RepoFile path="crates/jackin-runtime/src/runtime/docker_profile.rs" />): change `impl Default for DockerSecurityProfile` from `Compat` to `Standard`. One line.
