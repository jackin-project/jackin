...
Everything described on this page is fully implemented and available now:
- All four profiles with enforced capability sets
- `[docker.grants]` in config and workspace for per-dimension overrides
- `[docker]` in `jackin.role.toml` for role declarations (`min_profile`, `requires_inner_engine`, `dind`, `network_allow`, `capabilities_add`)
- `allowlist` network tier via `init-firewall.sh` (iptables/ipset, `JACKIN_ALLOWED_HOSTS` assembled at launch)
- All validation errors at launch time (bad cap names, user+sudo conflict, memory sizing, profile conflicts)
- Full telemetry under `--debug` (profile source, cap decisions, cgroup version, AppArmor, session contract)
...
To use a stricter profile today: pass `--docker-profile <profile>` or set `[docker] default_profile = "…"` in config.

### What will change in a future PR

**`standard` will become the default.** This requires one code change and one base-image change:

1. **Sudo audit** (<RepoFile path="docker/construct/Dockerfile" /> line 116): replace `NOPASSWD:ALL` with per-binary file capabilities. This is the blocker — adding `--security-opt no-new-privileges` to `standard` containers would silently break any agent script that calls `sudo`. Until the audit confirms every privileged operation is handled without blanket sudo, `standard` stays `sudo = true` with `no-new-privileges` off.

2. **Default flip** (<RepoFile path="src/runtime/docker_profile.rs" />): change `impl Default for DockerSecurityProfile` from `Compat` to `Standard`. One line.

Both items are tracked in `TODO.md` with full context. Until they land, the default stays `compat` and **no existing user is affected**.
...
### Remaining research items (explicitly deferred)

These are **not** blocked by the sudo audit — they require separate research before implementation:

| Item | What's needed | Where tracked |
|---|---|---|
| Rootless DinD | Compatibility matrix with Testcontainers, Compose, BuildKit on cgroup v2 | [Docker Runtime Hardening Contract](/reference/roadmap/docker-runtime-hardening-contract/) Phase B |
| Sysbox backend | Host-install policy decision (must not mutate host silently) | Phase C |
| Network proxy sidecar | V2 allowlist implementation covering non-HTTP protocols | [Network egress policy](/reference/roadmap/network-egress-policy/) |
