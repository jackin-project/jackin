...
- All four profiles with enforced capability sets (`locked`, `hardened`, `standard`, `compat`)
...
- `allowlist` network tier: iptables/ipset OUTPUT allowlist installed in the container via `docker exec --user root` before the agent session starts; `JACKIN_ALLOWED_HOSTS` and `JACKIN_NETWORK_ENFORCEMENT` injected; fail-closed (launch aborts if firewall install fails)
- `locked` profile uses a Docker-internal network (`--internal=true`) — traffic cannot leave the Docker network even without iptables rules
- `standard`/`hardened`/`locked` sudo off by default — enabled only via an explicit `sudo = true` grant; `compat` keeps sudo on unchanged
- DinD off by default in `standard`/`hardened`/`locked` — enabled via grant (`"none"` / `"rootless"` / `"privileged"`); rootless DinD wired and selectable (requires cgroup v2); `compat` keeps privileged DinD unchanged
- Full telemetry under `--debug`: per-decision lines for profile, capabilities, `no_new_privileges`, read-only root, resources, cgroup version, AppArmor, network mode, and session contract summary
