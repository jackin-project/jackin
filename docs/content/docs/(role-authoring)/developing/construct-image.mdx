...
| Starship prompt | Shared zsh/fish prompt with the `j❯` sigil and canonical green chevron |
...
- Starship prompt configured for both zsh and fish so the prompt surface stays the same when switching shells: `j❯` by default, with the active `JACKIN_AGENT` prefix when an agent session sets it
...
<RepoFile path="docker/construct/versions.env" /> remains the source of truth for the pinned `tirith`, `shellfirm`, and `MISE_VERSION` build args used by <RepoFile path="docker/construct/Dockerfile" />. `mise` is installed from the official rolling apt repository during the Docker build; pinning the apt package version gives Buildx a stable cache key, and Renovate bumps invalidate the mise install layer when upstream publishes a new package.
...
2. **Agent installation** — copies jackin❯ cached agent binaries for every agent declared in the role manifest
