...
The construct source starts at <RepoFile path="docker/construct/Dockerfile" /> in the jackin' repository. The declarative build definition lives in <RepoFile path="docker-bake.hcl" /> at the repo root, and the supported command surface is the `construct-*` mise tasks defined in <RepoFile path="mise.toml" />, backed by the <RepoFile path="crates/jackin-xtask/src/construct.rs" /> Rust task runner.
