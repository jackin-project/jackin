...
The pinned `shellfirm` binary is prepared before Docker Buildx runs and staged at <RepoFile path="docker/construct/prebuilt/shellfirm">docker/construct/prebuilt/shellfirm</RepoFile>. CI builds or restores that native binary per architecture from the GitHub Actions cache, then the Dockerfile copies it into the image. This keeps the heavy Rust compile out of the Docker build while upstream still lacks a Linux arm64 release artifact.
