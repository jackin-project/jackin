...
jackin' enforces **strict parsing** — unknown fields are rejected with an error. This catches typos and prevents silent misconfiguration. Top-level `version` records the minimum manifest schema the file requires; older stamps remain valid until the manifest uses a newer field or enum value. On a desktop, use `jackin role migrate <role-repo-path>` to update a local manifest before adopting newer schema features. In CI and automated migration workflows, use the standalone `jackin-role migrate <role-repo-path>` binary instead of the full `jackin` operator CLI. See [Schema Versions](/reference/schema-versions/) for the full version history and migration matrix.
...
requires_inner_engine = true      # needs DinD (default; omit for same result)
network_allow = ["crates.io"]     # extra hosts for allowlist network mode
...
| `requires_inner_engine` | bool | `false` = role declares it does not need Docker-in-Docker. jackin' skips the DinD sidecar even under profiles that would normally enable it (e.g. `standard`). Omit or set `true` to leave DinD controlled by the operator profile. |
| `dind` | string | Minimum DinD tier the role requires when it needs inner Docker: `none`, `rootless`, or `privileged`. Only meaningful when `requires_inner_engine` is omitted or `true`. |
| `network_allow` | array of strings | Extra hosts/IPs merged into `JACKIN_ALLOWED_HOSTS` when the operator's network grant is `allowlist`. Each entry is a domain, IPv4/IPv6 CIDR, wildcard subdomain (`*.example.com`), or `domain:port`. |
...
network_allow = ["registry.npmjs.org", "crates.io", "static.crates.io"]
```
