...
<Aside type="note">
Keys use dotted namespacing (`jackin.construct.version`, `jackin.role.git.sha`). A `jackin.manifest.version` label (the `jackin.role.toml` schema version) is being added to this contract — `jackin-role` is gaining a `manifest-version` subcommand to extract it for the workflow. Until your role re-publishes with the updated `jackin-role-action`, jackin falls back to a local workspace rebuild for that role (correct, just not the fast published path). See [Image Labels & Recipe Hash](/reference/runtime/image-labels/) for the full label schema jackin verifies.
</Aside>
...
1. **Determine the construct version**
   ...
   If the `jackin-role` binary is available in your CI environment (download it from the [jackin releases](https://github.com/jackin-project/jackin/releases)), you can extract it automatically:
