...
Adds the `on_demand` flag to `OpRef` and the new `Extended { value, on_demand }` variant in `EnvValue` for on-demand credential injection via `jackin-exec`, plus the optional `[workspaces.X.runtime]` section with a `backend` field for per-workspace Apple Container backend override. Both changes are additive; the noop migration leaves existing workspace files parsing unchanged (the new fields are omitted when unset).
