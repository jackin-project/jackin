...
**Shell into the checkout.** A shell action drops the operator into an interactive shell at the root of the isolated checkout — the same throwaway worktree the recipe commands run in, with the same redirected configuration and state. So anything run by hand (`git log`, `cargo build`, a one-off script) sees exactly what a recipe command sees, behaves the same way, and still cannot touch the operator's real setup. On exit the terminal returns to review mode.
...
- **Shell / interactive child** — the checkout shell hands the terminal over while it runs and must restore the review surface cleanly on exit; like the launched binary, it inherits the redirected configuration and state so it defaults to the isolated bundle, not the operator's real setup.
