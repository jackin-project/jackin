...
Packaging is part of this work. The release and preview artifacts must include `jackin-dev`, and the generated Homebrew formulae in the `jackin-project/homebrew-tap` repository must install it beside `jackin` and `jackin-role`. Any future PR that changes `jackin-dev` as an installed binary must keep that packaging path current: build artifacts, archive contents, release workflow, preview workflow, and Homebrew tap formula output move together. Operators reviewing jackin' PRs should be able to install or update from the tap and immediately run `jackin-dev pr sync` without a prior source checkout.
