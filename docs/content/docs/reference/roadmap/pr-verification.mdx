...
**First focus: verifying the pull request a running instance is working on, host-side. Nothing more.** Browsing a repo's pull requests with no instance, branches as a standalone target, role-repo PRs as a standalone target, merge-preview, coordinated multi-repo verification, and any scripted/non-interactive entry are explicitly deferred (see "Deferred"). The one branch-shaped capability kept in the first slices is **pinning a companion role to a branch while verifying a PR** — because that is a real need for PRs that pair a CLI change with a role change.
...
- **Browsing a repo's PRs with no instance.** The first slices reach review only from an instance that is on a PR branch. Picking any open PR on a workspace's repo (creating a checkout on demand) is the natural next slice.
...
- [Console agent session control](/reference/roadmap/console-agent-session-control/) — review is anchored to an instance and shares the console's instance-selection layout; it reads the instance's pull-request context but runs its commands host-side, not inside the instance.
...
The novel combination: host-side PR verification reached from the instance the operator is working in, with companion-role-branch pinning and an isolated jackin❯ configuration and state home so jackin❯ can verify itself safely.
