...
- <RepoFile path="crates/jackin/src/cli/role.rs" /> — adds `jackin open` / `jackin diff`
  / `jackin web` subcommands
- <RepoFile path="crates/jackin/src/console/tui/state.rs" /> — keybindings
- <RepoFile path="crates/jackin-config/src/resolve.rs" /> — primary-mount resolution
