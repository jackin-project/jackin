...
- <RepoFile path="src/cli/role/mod.rs" /> — adds `jackin open` / `jackin diff`
  / `jackin web` subcommands
- <RepoFile path="src/console/tui/state/mod.rs" /> — keybindings
- <RepoFile path="src/workspace/resolve/mod.rs" /> — primary-mount resolution
