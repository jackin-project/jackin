...
`JACKIN_AGENT_CODENAME` is added to the reserved runtime env list in <RepoFile path="crates/jackin-core/src/env_model.rs" /> alongside the existing `JACKIN_*` vars so role authors cannot override it via `[env.JACKIN_AGENT_CODENAME]` declarations in their manifest.
...
- `JACKIN_AGENT_CODENAME` added to the reserved runtime env list in <RepoFile path="crates/jackin-core/src/env_model.rs" />.
