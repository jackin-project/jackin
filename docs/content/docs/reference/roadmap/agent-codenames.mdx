...
- Tab hover tooltip in the Capsule tab strip: shown on mouse hover only; displays codename, agent, provider, start time, exit time, and status. The tab label itself is unchanged.
