...
**Tab hover tooltip.** When the operator moves their mouse cursor over a tab cell in the strip, the multiplexer renders a small box overlay positioned directly below the hovered tab cell. The box uses a single-line Unicode border, is approximately 30 characters wide, and shows the codename on the first line followed by agent type, provider, start time, and exit time or `active`. This is the only place the codename appears in the TUI. The tooltip is painted inside `compose_chrome_hover_frame` using the existing ANSI save/restore cursor sequence, left-aligned to the hovered tab cell's column from `StatusBar::tab_regions`. The mouse pointer changes to a hand cursor (`PointerShape::Pointer`) when over any tab cell — this must be added to the `pointer_shape_at` match alongside the existing `HoverTarget::DialogCopyTarget` arm.
...
- Tab hover tooltip: small box (~30 chars wide, single-line Unicode border) rendered below the hovered tab cell; shows codename, agent, provider, start time, exit time/`active`; `PointerShape::Pointer` hand cursor added to `pointer_shape_at` for `HoverTarget::Tab`. Tab label itself unchanged.
...
- Host-side `jackin status` codename integration is tracked separately in [Operator CLI Hygiene](/reference/roadmap/operator-cli-hygiene/) Phase 2.
