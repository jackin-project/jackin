...
**Tab hover tooltip.** When the operator moves their mouse cursor over a tab cell in the strip, the multiplexer renders a small overlay showing the full identity record for that tab: the codename, agent type, provider, start time, and exit time (or `active`). The tooltip design follows the overlay component conventions documented in [`tui-design-decisions.mdx`](/reference/tui-design-decisions/).

When a custom tab label is set, the tab strip shows `my-label [badger]` — the custom label as the primary display, the codename in brackets — so the operator can rename tabs for their own orientation without losing the machine-readable codename.
...
```json
[
  {"codename": "badger", "agent": "claude", "provider": "anthropic", "started_at": "2026-06-04T10:15:02Z", "exited_at": null,                    "status": "active", "self": true},
  {"codename": "falcon", "agent": "claude", "provider": "anthropic", "started_at": "2026-06-04T10:16:30Z", "exited_at": null,                    "status": "active", "self": false},
  {"codename": "reef",   "agent": "codex",  "provider": "openai",    "started_at": "2026-06-04T10:17:45Z", "exited_at": null,                    "status": "active", "self": false},
  {"codename": "crater", "agent": "claude", "provider": "anthropic", "started_at": "2026-06-04T10:12:00Z", "exited_at": "2026-06-04T10:40:11Z", "status": "exited", "self": false}
]
```
...
**Should `jackin status` on the host report codenames?** Exposing per-tab codenames through the host-side `jackin status` command alongside the existing instance overview would let host-side scripts and the future workflow runner address tabs by human-readable name without entering the container. Deferred to Phase 3.
...
- Tab hover tooltip in the Capsule tab strip showing codename, agent, provider, start time, exit time, and status.
...
- [Persistent storage and workspace memory layer](/reference/roadmap/persistent-storage-layer/) — a future durable storage surface relevant to Phase 3 registry persistence.
- [Terminal observation and automation](/reference/roadmap/terminal-observation-automation/) — the typed read/wait/send Capsule API that a coordination-aware workflow runner would use alongside codename routing.
