...
session/workspace attribution; runtime samples still use a zero-cost placeholder
until provider-specific pricing adapters attach exact cost. These APIs are not
yet the host-global daemon API or final `account/list` / `account/refresh` /
`usage/*` CLI surface.
