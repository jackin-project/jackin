...
`UsageFocused` / `UsageRefreshFocused`, `UsageAccountList`, `UsageWorkspace`,
and `UsageSession` message variants. These read the Capsule daemon cache and
`/jackin/state/usage/telemetry.db`. The Capsule daemon persists account quota
buckets, local-log samples, and token-bearing runtime-stream JSON samples with
session/workspace attribution; samples keep exact USD cost only when the source
record exposes an explicit USD cost field, and otherwise report token totals
without invented pricing. These APIs are not yet the host-global daemon API or
final `account/list` / `account/refresh` / `usage/*` CLI surface.
...
7. **Capsule focused signal.** The first Capsule/control/TUI bridge has landed:
   Capsule can request a normalized focused usage view, render a compact
   focused-agent status label, and open the read-only prefix `u` overlay from a
   daemon-owned cache. The remaining work is to replace the conservative local
   cache with the canonical account snapshot service, provider-authoritative
   quota adapters, and scheduled refresh/backoff described above. Rendering
   contract lives in [Capsule usage and quota overlay](/reference/roadmap/capsule-usage-quota-overlay/).
...
- **Capsule multiplexer.** Focused-tab provider only — see [Capsule usage and quota overlay](/reference/roadmap/capsule-usage-quota-overlay/) for the rendering contract (status-line layout, prefix `u`, modal contents, daemon-disconnected and needs-login fallbacks).
...
- Capsule multiplexer quota signal — Capsule's rendering contract lives in [Capsule usage and quota overlay](/reference/roadmap/capsule-usage-quota-overlay/).
...
- <RepoFile path="crates/jackin/src/cli/role.rs" /> — `jackin usage` subcommand
- <RepoFile path="crates/jackin/src/console/tui/state.rs" /> — column rendering
...
- [Capsule usage and quota overlay](/reference/roadmap/capsule-usage-quota-overlay/) —
  Capsule TUI rendering contract
