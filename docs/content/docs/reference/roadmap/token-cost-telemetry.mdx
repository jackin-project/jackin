...
**Implementation note**: The Capsule usage overlay work has landed the first
Capsule-local Turso cache for this design: `usage_samples` rows now carry
`source_hash` for idempotent log/runtime ingestion plus `cost_source` so
summaries can distinguish provider-reported exact USD costs from price-table
estimates and unpriced samples. This remains a local bridge, not the final
host-global telemetry daemon.
