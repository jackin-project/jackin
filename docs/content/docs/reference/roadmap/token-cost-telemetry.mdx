...
**Implementation note**: The Capsule usage overlay work has landed the
account-quota side of this design: a Capsule-local Turso
`account_usage_snapshots` cache feeds the focused status signal and provider
overlay. It deliberately removed the earlier local `usage_samples` bridge,
workspace/session summaries, price-table estimates, and cost grids. The
per-message token/cost plane described below remains open and still belongs to
the final host-global telemetry daemon.
