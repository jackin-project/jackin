...
- **TUI render layer** — `console/tui/render/` and all widget files: 296 `Style::default().fg()` calls, 357 `Span::styled()` calls, 27 `Block::default()` chains, inline color constants duplicated across 10+ files instead of using the centralized palette in `console/tui/widgets/mod.rs`
...
| Centralized color palette import | Inline `Color::Rgb(...)` values duplicated in `input/save.rs`, `scope_picker.rs`, `save_discard.rs`, `mount_dst_choice.rs`, `source_picker.rs`, `file_browser/render.rs` instead of importing from `console/tui/widgets/mod.rs` | 6+ files | Removes ~20 inline magic color values |
...
| <RepoFile path="src/console/tui/widgets/op_picker/mod.rs" /> | **2331** | 4-level state machine + loader + key handler. | — |
