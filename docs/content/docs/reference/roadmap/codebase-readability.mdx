...
- **TUI render layer** — `console/tui/render/` and all widget files: 296 `Style::default().fg()` calls, 357 `Span::styled()` calls, 27 `Block::default()` chains, inline color constants duplicated across 10+ files instead of using the centralized palette in `console/tui/components/mod.rs`
...
The unified TUI scrollbar extraction has shipped: shared metrics, clamping, drag/click mapping, tail-relative launch-progress scrolling, and capsule scrollback now live in `jackin_tui::scroll`; the durable contract is documented in [TUI Design Decisions](/reference/tui-design-decisions/#scrollable-blocks--shared-component).
...
| `fn app_layout(area) -> Rc<[Rect]>` for header/body/footer | 4 identical `Layout::default()` splits with `[Length(3), Min(..), Length(2)]` | `render/mod.rs`, `render/settings.rs` | Removes 4 duplicate layout constructions |
...
| <RepoFile path="src/console/tui/render/editor.rs" /> | **3331** | 25-arm match on tab. | `contextual_row_items` (205L), `render_secrets_tab` (104L) |
...
| <RepoFile path="src/console/tui/render/list.rs" /> | **2920** | 41 `TestBackend::new()` calls. | `render_details_pane` (116L) |
...
| <RepoFile path="src/console/tui/render/mod.rs" /> | **1025** | `fn render` is 201L. Central render orchestrator; crossed the 1000L line since the previous re-bake. |
...
| <RepoFile path="src/console/tui/render/settings.rs" /> | **893** | Settings rendering |
