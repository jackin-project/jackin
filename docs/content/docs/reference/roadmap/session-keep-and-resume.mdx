...
| F13 | Medium | No `docker start` tier: `start_container` exists in <RepoFile path="src/docker_client.rs" /> but is never called, so a stopped-but-present container forces a full pipeline re-run instead of a cheap restart (the highest-fidelity restore is left on the table). |
...
- [Session snapshot and rollback](/reference/roadmap/session-snapshot-rollback/) — the orthogonal direction: *undo* a session back to its pre-launch host state, where this item *resumes* a session forward to continue it. The deferred immutable-snapshot recovery model lives there.
- [Déjà Vu](/reference/roadmap/deja-vu/) — the backstop for "never lose valuable context": even when a session is unrecoverable, the cross-agent conversation archive survives instance teardown, so the operator can still re-read what was discussed and decided.
