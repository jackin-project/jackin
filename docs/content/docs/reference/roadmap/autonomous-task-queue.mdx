...
suffix already produced by <RepoFile path="src/instance/naming/mod.rs" />.
...
- `src/queue/task_source.rs` (from previous leaf) —
  source polling
- <RepoFile path="src/runtime/launch/mod.rs" /> — invoked by the
  dispatcher per task; small refactor to accept programmatic
  invocation instead of CLI args only
- <RepoFile path="src/instance/naming/mod.rs" /> — per-task instance name
  generator extension
- <RepoFile path="src/cli/role/mod.rs" /> — `jackin queue` subcommands
