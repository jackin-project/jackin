...
suffix already produced by <RepoFile path="crates/jackin-runtime/src/instance/naming.rs" />.
...
- `src/queue/task_source.rs` (from previous leaf) —
  source polling
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — invoked by the
  dispatcher per task; small refactor to accept programmatic
  invocation instead of CLI args only
- <RepoFile path="crates/jackin-runtime/src/instance/naming.rs" /> — per-task instance name
  generator extension
- <RepoFile path="crates/jackin/src/cli/role.rs" /> — `jackin queue` subcommands
