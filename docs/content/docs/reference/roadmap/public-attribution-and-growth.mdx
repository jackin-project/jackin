...
- The current jackin❯ repository policy requires one agent `Co-authored-by` trailer for each AI agent involved in a commit; see <RepoFile path="AGENTS.md" />. Any jackin❯ attribution mechanism must respect local project rules like that instead of assuming the product can append unrelated attribution lines.
...
| `Orchestrated-by: jackin❯ <url>` trailer | Medium; visible in raw commit message | Medium | Best semantic fit for the container/session orchestrator role. Needs ecosystem research because custom trailers vary by project. |
