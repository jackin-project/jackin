...
Backend operations should emit `debug_log!("orbstack", …)` (the existing macro in <RepoFile path="crates/jackin/src/tui.rs" />) so the firehose under `JACKIN_DEBUG=1` reconstructs the full machine lifecycle. Minimum required lines:
...
All three are additive. The instance manifest at <RepoFile path="crates/jackin-runtime/src/instance/manifest.rs" /> also needs a `backend_kind`/`provider_kind` axis split, which is per-instance metadata (not a versioned schema in the AGENTS.md sense) but still needs a forward-compatible default for existing Docker-only instances.
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> - launch orchestration that needs a backend split.
- <RepoFile path="crates/jackin-runtime/src/runtime/image.rs" /> - role image build path.
- <RepoFile path="crates/jackin-docker/src/docker_client.rs" /> - current Docker lifecycle boundary.
- <RepoFile path="crates/jackin-runtime/src/instance/manifest.rs" /> - instance metadata that needs backend/provider fields.
