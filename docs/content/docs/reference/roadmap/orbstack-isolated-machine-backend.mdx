...
All three are additive. The instance manifest at <RepoFile path="src/instance/manifest/mod.rs" /> also needs a `backend_kind`/`provider_kind` axis split, which is per-instance metadata (not a versioned schema in the AGENTS.md sense) but still needs a forward-compatible default for existing Docker-only instances.
...
- <RepoFile path="src/runtime/launch/mod.rs" /> - launch orchestration that needs a backend split.
- <RepoFile path="src/runtime/image/mod.rs" /> - role image build path.
- <RepoFile path="src/docker_client/mod.rs" /> - current Docker lifecycle boundary.
- <RepoFile path="src/instance/manifest/mod.rs" /> - instance metadata that needs backend/provider fields.
