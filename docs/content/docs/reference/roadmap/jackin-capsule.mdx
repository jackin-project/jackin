...
**Event stream:** Phase 3 now exposes `EventsSubscribe` with `AgentStateChanged`, `SessionSpawned`, `SessionExited`, token usage, and workspace roll-up events. Phase 4 daemon integration consumes that stream across containers instead of inventing a second status channel.
...
- Shipped event stream: `AgentStateChanged`, `SessionSpawned`, `SessionExited`, `TokenUsageChanged`, and `WorkspaceStatusChanged`.
