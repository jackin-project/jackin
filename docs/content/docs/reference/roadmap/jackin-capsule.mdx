...
The shipped renderer is a Zellij-style dirty-output layer on top of `DamageGrid`, not a raw PTY-byte passthrough. Directly forwarding active-pane bytes was rejected because pane content must be offset into an inner rectangle, clipped against borders, dimmed when inactive, hidden behind opaque modal backdrops, and kept consistent with scrollback and selection state.

**Hot path — dirty pane body**. When any pane PTY emits bytes, the server:

1. Feeds the bytes into the pane's `DamageGrid` so the screen stays current. This is cheap and unavoidable; it is what makes tab switches work later.
2. Marks that pane's body dirty and lets the render ticker coalesce bursts at roughly 30 fps.
3. For the focused live pane-only case, drains `dirty_spans()`, borrows a `GridPatch`, and emits changed rows directly through `SocketBackend::draw_grid_patch()`, each with explicit cursor moves and style state.
4. Falls back to the Ratatui frame path when chrome, selection, dialogs, scrollback, or multi-pane complexity must be synchronized with the pane body.
...
2. For each visible pane in the active tab, render `DamageGrid` rows through `PaneBodyWidget`.
3. Render pane borders and scrollbars.
4. Render dialog overlay if one is open.
5. Position the host terminal cursor at the focused pane's `DamageGrid::cursor_position()` and honour `DamageGrid::hide_cursor()`.
