...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — current runtime launch path
- <RepoFile path="crates/jackin-config/src/resolve.rs" /> — workspace resolution
- <RepoFile path="crates/jackin-config/src/planner.rs" /> — resolved mount planning
- <RepoFile path="crates/jackin-runtime/src/instance/auth.rs" /> — per-instance auth behavior
- <RepoFile path="crates/jackin-docker/src/shell_runner.rs" /> — Docker command boundary
