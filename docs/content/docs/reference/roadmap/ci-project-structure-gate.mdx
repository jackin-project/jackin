...
<RepoFile path="PROJECT_STRUCTURE.md" /> has drifted from the actual module layout because there is no automated check. PRs that add new `.rs` module files can merge without mentioning them in the navigation map.
...
- **Greps for basename** (e.g., `op_picker` for <RepoFile path="src/console/tui/components/op_picker/mod.rs" />) — PROJECT_STRUCTURE.md uses prose descriptions containing the module name.
