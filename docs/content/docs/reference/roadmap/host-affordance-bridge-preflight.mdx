...
- The opt-in host path now treats a lone `Ctrl+V` as a macOS image-paste probe: it uses `osascript` to coerce the host clipboard to PNG, sends a typed image frame when that succeeds, and forwards the original `Ctrl+V` byte when no valid image is available. Browser TIFF coercion, Finder file-url resolution, Linux backends, visible rejection feedback, and chunked oversize image transfer remain follow-up work.
