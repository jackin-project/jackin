...
**Status**: Partially implemented — Phase 0 evidence captured; opt-in Phase 1A host attach path added
...
- The opt-in host path currently owns raw mode, stdin/stdout relay, SIGWINCH resize, focus/mouse bytes, cleanup, direct-socket transport, and `attach-proxy` transport. It still needs shared OSC 10/11 terminal-color probing before it can become the default path.
