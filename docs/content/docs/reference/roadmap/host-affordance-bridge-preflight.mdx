...
**Status**: Partially implemented — Phase 0 evidence captured; opt-in Phase 1A host attach path added with terminal-color parity, bounded single-frame image paste path, first GitHub host-open actions, and minimal chunked file export
...
Current result: passed for the first implementation slice. Image paste remains a bounded single-frame `ClientFrame::ClipboardImage` path with a 16 MiB frame cap and needs chunking before large-image support. File export uses chunked server frames with transfer id, offsets, size, 1 MiB chunks, and SHA-256 verification, plus a 64 MiB first-pass cap. Remaining probe work is live cancellation behavior, live export latency, and whether very large exports should move from attach chunks to a negotiated side stream.
