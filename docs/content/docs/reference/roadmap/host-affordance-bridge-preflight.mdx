...
- Debug launches now inject `JACKIN_RUN_DIAGNOSTICS_PATH` alongside `JACKIN_RUN_ID`. Capsule's Debug info dialog copies that exact host JSONL path for the diagnostics row, with the previous in-container path derivation retained only as a fallback for older launches. The dialog also exposes an explicit `R` / `O` reveal shortcut and **Reveal diagnostics** row so Debug info owns a distinct copy target and reveal target.
- The launch failure popup now has a host-side `R` reveal action for the visible **run diagnostics** and **docker output** artifact rows. It uses the same host desktop reveal helper as file export, keeps the failure popup open, and shows visible `Revealed!` feedback.
- The command palette now has `Export file` and `Export file and reveal` actions. Capsule canonicalizes the requested container path, allows only workspace paths and `/jackin/run/`, rejects non-regular files, caps the first implementation at 64 MiB, and streams `FileExportStart` / `FileExportChunk` / `FileExportEnd` frames with 1 MiB chunks, SHA-256, and an explicit reveal flag. The opt-in host attach client writes to `~/Downloads/jackin/<instance>/` through a temporary file, verifies size and digest, then renames to a collision-resistant final path. It asks the host OS file manager to reveal the copy only for the explicit reveal action. Live validation through a throwaway jackin' instance remains pending.
