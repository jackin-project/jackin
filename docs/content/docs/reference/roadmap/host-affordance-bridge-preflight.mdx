...
**Status**: Partially implemented — Phase 0 evidence captured; opt-in Phase 1A host attach path added with terminal-color parity, image paste with chunked large-image transfer, explicit image-path staging, first GitHub host-open actions, and minimal chunked file export
...
| Ghostty mouse modifiers | Plain motion/clicks emitted SGR mouse frames. Option motion used modifier-coded frames such as `cb=43`; Ctrl motion used frames such as `cb=51`. Cmd was not distinguishable in the process-visible stream in the manual probe. | V1 should expose command-palette actions for paste/open-link and may bind only modifiers proven to reach the host attach process. Cmd-click must not be assumed. |
