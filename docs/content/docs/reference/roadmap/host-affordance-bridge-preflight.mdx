...
**Status**: Partially implemented — Phase 0 evidence captured; opt-in Phase 1A host attach path added with terminal-color parity, bounded single-frame image paste path, and first GitHub host-open actions
...
| GitHub CI URL queries | `gh pr view <url> --json statusCheckRollup` exposes check objects with detail URLs. `gh pr checks <url> --json bucket,completedAt,description,link,name,startedAt,state,workflow` uses `link`; `detailsUrl` is not a valid field for `gh pr checks`. | Implemented lookup asks for `bucket,link,name,workflow`, stores a validated HTTP(S) CI link, and prefers failed links before cancelled, pending, passing, and skipped links. `statusCheckRollup.detailsUrl` remains a fallback source for later refinement. |
...
- GitHub context now records a best-effort CI URL from `gh pr checks` `link` fields, renders separate copy/open PR/open CI actions, and emits a scheme-limited host-open attach frame handled by the opt-in host attach client. Live browser-open validation remains pending.
...
Current result: passed for the first implementation slice. Capsule stores one validated HTTP(S) CI target from `gh pr checks` `link`, and the dialog exposes an explicit Open CI action. Remaining work is fallback depth when `gh pr checks` lacks a useful link and live host-browser validation.
