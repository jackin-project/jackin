...
- Host-open and file-export outcomes now return bounded host-notice frames to Capsule so success and rejection are visible in the TUI instead of only in host logs. Host desktop open/reveal helpers now wait for the host command exit status and report nonzero exits without logging stdout/stderr or private payloads. File-export compact logs record source category, basename, byte count, destination category, and coarse rejection reason instead of full container or host paths; operator-visible notices still show the exported destination.
