...
**Status**: Partially implemented — Phase 0 evidence captured; opt-in Phase 1A host attach path added with terminal-color parity, image paste with chunked large-image transfer, explicit host image paste, explicit image-path staging, first GitHub host-open actions, and minimal chunked file export
...
- `Alt`-click and `Ctrl`-click on visible HTTP(S) URLs in mouse-disabled panes now resolve through the same URL-token rules as double-click selection and emit the existing scheme-limited host-open attach frame. Non-URL modified clicks keep the raw mouse fallback. `Cmd`-click and OSC 8 hyperlink-region lookup remain unimplemented until live terminal evidence proves the correct signal shape.
