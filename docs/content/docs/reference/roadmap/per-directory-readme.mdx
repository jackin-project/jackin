...
| `src/console/` | Operator console TUI. Entry: `run_console()`. Two subsystems: `manager/` and `tui/components/`. **Note: has 2 production dependencies on `runtime/`** (`register_agent_repo`, `RepoError`). |
