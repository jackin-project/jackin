...
Use the Docker stats API (`docker stats --no-stream` or, via the `DockerApi` trait in <RepoFile path="src/docker_client/mod.rs" />, the typed `ContainerStats` endpoint). Sample every 2 seconds while the console is open; pause sampling when the panel is collapsed.
...
- <RepoFile path="src/console/tui/state/mod.rs" /> — keybinding wiring
- <RepoFile path="src/runtime/launch/mod.rs" /> — sampling adapter trait
