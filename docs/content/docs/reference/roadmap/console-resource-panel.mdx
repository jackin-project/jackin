...
Use the Docker stats API (`docker stats --no-stream` or, via the `DockerApi` trait in <RepoFile path="crates/jackin-docker/src/docker_client.rs" />, the typed `ContainerStats` endpoint). Sample every 2 seconds while the console is open; pause sampling when the panel is collapsed.
...
- <RepoFile path="crates/jackin/src/console/tui/state.rs" /> — keybinding wiring
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — sampling adapter trait
