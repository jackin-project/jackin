...
| **Kimi Code** | [kimi.com/membership/pricing](https://www.kimi.com/membership/pricing) (shares Kimi membership quota; 7-day refresh, no carryover) | [Kimi Code Console](https://www.kimi.com/code/console) → API Keys | Kimi Code API key (shown once) | `kimi-for-coding` |
...
Codex routes to a custom backend through a `[model_providers.<id>]` table plus a `[profiles.<id>]` table in `~/.codex/config.toml`, for example:
...
### Phase 2 — Codex backend routing

Add the `[model_providers]` + `[profiles]` renderer and wire it into `jackin-capsule runtime-setup`. Resolve the wire-protocol question (see Open Questions) before claiming Codex support for `chat`-only providers.
...
1. **Codex wire protocol.** Codex moved to the Responses API and is reported to have removed Chat Completions support in early 2026 ([openai/codex#9612](https://github.com/openai/codex/issues/9612)). MiniMax documents `wire_api = "responses"` and is safe; GLM and Kimi are documented with `wire_api = "chat"`. Phase 2 must verify, against the pinned Codex version in the construct image, whether `chat`-wire providers still route at all — and if not, whether GLM/Kimi expose a Responses-compatible endpoint. This gates which `(provider, codex)` cells are actually deliverable.
