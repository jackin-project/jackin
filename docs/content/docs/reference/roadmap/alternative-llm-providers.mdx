...
> **PR #495 port note.** This feature shipped on `main` as PR `#523` against the pre-migration monolith and was ported onto the workspace crate structure in PR #495. The design record for the runtime/provider registry work lives in [agent-runtime-trait](/reference/roadmap/agent-runtime-trait/).
