...
On every startup, `AppConfig::load_or_init` (in <RepoFile path="crates/jackin-config/src/app_config_persist.rs" />) runs the migration step before deserializing into typed structs:
...
1. Add `version` extraction to the raw TOML read layer in <RepoFile path="crates/jackin-config/src/app_config_persist.rs" /> (before deserialization into `AppConfig`).
2. Implement the migration registry and the fold-and-rewrite logic using `toml_edit` (<RepoFile path="crates/jackin-config/src/editor.rs" />).
...
1. Add `version: String` (`version` in TOML) to `RoleManifest` in <RepoFile path="crates/jackin-manifest/src/manifest.rs" />.
...
- <RepoFile path="crates/jackin-config/src/app_config_persist.rs" /> — `AppConfig::load_or_init`, the migration entry point.
- <RepoFile path="crates/jackin-config/src/editor.rs" /> — `ConfigEditor`, used for comment-preserving file rewrites.
- <RepoFile path="crates/jackin-manifest/src/manifest.rs" /> — `RoleManifest`, the role manifest struct.
- <RepoFile path="crates/jackin/src/config.rs" /> — `AppConfig` and workspace config structs.
