...
On every startup, `AppConfig::load_or_init` (in <RepoFile path="src/config/persist/mod.rs" />) runs the migration step before deserializing into typed structs:
...
1. Add `version` extraction to the raw TOML read layer in <RepoFile path="src/config/persist/mod.rs" /> (before deserialization into `AppConfig`).
2. Implement the migration registry and the fold-and-rewrite logic using `toml_edit` (<RepoFile path="src/config/editor/mod.rs" />).
...
- <RepoFile path="src/config/persist/mod.rs" /> — `AppConfig::load_or_init`, the migration entry point.
- <RepoFile path="src/config/editor/mod.rs" /> — `ConfigEditor`, used for comment-preserving file rewrites.
