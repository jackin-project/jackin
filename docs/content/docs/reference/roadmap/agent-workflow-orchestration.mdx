...
| Context and memory | Resolve a workflow-scoped memory brief from the [persistent storage and workspace memory layer](/reference/roadmap/persistent-storage-layer/) before each agent session. Do not depend on one runtime's private memory as the shared source of truth. |
...
| Container-only isolation may be weaker than VM/forkable sandbox competitors. | Keep [selectable sandbox backends](/reference/roadmap/selectable-sandbox-backends/) compatible with the run model. Do not bake "Docker container" into workflow state. |
...
`MemoryBrief` is the compact context package resolved for one phase/session. It should include only the memories relevant to the current run, workspace, role, workflow, and agent: operator preferences, workspace facts, workflow rules, recent run summaries, and explicit operator notes. It is built from the [persistent storage and workspace memory layer](/reference/roadmap/persistent-storage-layer/) and delivered through runtime adapters; it is not copied from Claude Code's or Codex's private memory store.
...
The current `status` and `snapshot` control calls are enough for display, but not enough for reliable workflow orchestration. The runner needs a typed control surface that can manipulate sessions without pretending terminal text is an API. [Terminal observation and automation](/reference/roadmap/terminal-observation-automation/) owns the detailed read/wait/send/capture/tab/session API shape; this section names the subset the workflow runner depends on.
...
- Now that [Agent runtime status authority](/reference/roadmap/agent-runtime-status/) has shipped, what additional verification or review signal should a workflow require before it advances from "agent is done" to "task is ready"?
...
- [Native APM support](/reference/roadmap/native-apm-role-support/) — package/install layer for skills, prompts, and MCP clients used by workflow profiles.
