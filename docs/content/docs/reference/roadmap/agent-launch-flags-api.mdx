...
<RepoFile path="docker/runtime/entrypoint.sh" /> hardcodes launch commands per agent:
...
Move the launch dispatch out of bash into `jackin-capsule runtime-setup` (which already runs). Bash becomes a thin wrapper, not the policy layer. Each `AgentRuntime` impl (after the agent runtime trait extraction) returns its launch argv from a Rust function the operator can shadow via manifest.
...
- Do not implement this before the agent runtime trait extraction lands.

## Implementation Phases

### Phase 1 — Depends on agent runtime trait

This item is blocked on the agent runtime trait roadmap item. After it lands, each `AgentRuntime` impl has a `launch_argv()` method.
...
- <RepoFile path="src/agent/mod.rs" />
- <RepoFile path="crates/jackin-capsule/src/runtime_setup/mod.rs" />
