...
- **Secret-redacted logs.** Daemon log output must redact tokens the same way `tracing::debug!` already does for `GithubAuthContext` and `GithubProvisionOutcome` (manual `Debug` impls; see <RepoFile path="crates/jackin-runtime/src/instance.rs" />).
...
- **Keep-awake `caffeinate`.** Today's per-command reconciler (see <RepoFile path="crates/jackin-runtime/src/runtime/caffeinate.rs" />) becomes a daemon-owned watcher: the daemon subscribes to "any role from a keep-awake workspace running" via Docker events and holds the caffeinate lock. CLI commands no longer reconcile.
...
- <RepoFile path="crates/jackin-runtime/src/runtime/caffeinate.rs" /> — today's per-command keep-awake reconciler; first migration target in Phase 2.
