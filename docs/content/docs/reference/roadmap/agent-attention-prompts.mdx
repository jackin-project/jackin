...
- <RepoFile path="crates/jackin-capsule/src/runtime_setup/mod.rs" /> — current MCP-registration pattern for `tirith` / `shellfirm`. The `jackin-attention` MCP server registers through the same code path.
