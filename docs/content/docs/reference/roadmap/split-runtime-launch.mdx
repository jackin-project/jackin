...
- `build_mode_resolution()` at ~line 2977 duplicates the 3-layer × 4-agent match (12 arms) from <RepoFile path="crates/jackin-config/src/app_config_roles.rs" /> `resolve_mode()` solely to capture the resolution trace for error rendering. Extract `resolve_mode_with_trace()` in `config/roles.rs` instead.
- DinD sidecar launch args (~line 777) are duplicated identically in <RepoFile path="crates/jackin-runtime/src/runtime/attach.rs" /> at ~line 472. Extract shared `fn run_dind_sidecar()`.
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — target file (7575L)
- <RepoFile path="crates/jackin-runtime/src/runtime/attach.rs" /> — shares DinD sidecar and network logic (1130L)
- <RepoFile path="crates/jackin-runtime/src/runtime/test_support.rs" /> — `FakeRunner` used by the test suite
- <RepoFile path="crates/jackin-runtime/src/runtime/cleanup.rs" /> — shares container teardown pattern
- <RepoFile path="crates/jackin-config/src/app_config_roles.rs" /> — `resolve_mode()` duplicated for trace in launch.rs
