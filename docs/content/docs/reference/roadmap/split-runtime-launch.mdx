...
- `build_mode_resolution()` at ~line 2977 duplicates the 3-layer × 4-agent match (12 arms) from <RepoFile path="src/config/roles/mod.rs" /> `resolve_mode()` solely to capture the resolution trace for error rendering. Extract `resolve_mode_with_trace()` in `config/roles.rs` instead.
- DinD sidecar launch args (~line 777) are duplicated identically in <RepoFile path="src/runtime/attach/mod.rs" /> at ~line 472. Extract shared `fn run_dind_sidecar()`.
...
- <RepoFile path="src/runtime/launch/mod.rs" /> — target file (7575L)
- <RepoFile path="src/runtime/attach/mod.rs" /> — shares DinD sidecar and network logic (1130L)
- <RepoFile path="src/runtime/test_support.rs" /> — `FakeRunner` used by the test suite
- <RepoFile path="src/runtime/cleanup/mod.rs" /> — shares container teardown pattern
- <RepoFile path="src/config/roles/mod.rs" /> — `resolve_mode()` duplicated for trace in launch.rs
