...
1. Bump `CURRENT_WORKSPACE_VERSION` in <RepoFile path="crates/jackin-config/src/migrations.rs" />
...
5. A new entry in <RepoFile path="docs/content/docs/reference/runtime/schema-versions.mdx" />
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — DinD launch, workspace registry ensure call, shared-net attachment
- <RepoFile path="crates/jackin-runtime/src/runtime/attach.rs" /> — DinD recovery path, same DinD additions
- <RepoFile path="crates/jackin-runtime/src/runtime/naming.rs" /> — naming functions for registry container, volume, shared network
- <RepoFile path="crates/jackin-runtime/src/runtime/cleanup.rs" /> — teardown hook, stop registry when last workspace instance exits
- <RepoFile path="crates/jackin/src/workspace.rs" /> — `ContainerRegistryConfig` struct, `WorkspaceConfig` extension, new `WorkspaceResources` struct for workspace-level resource tracking
- <RepoFile path="crates/jackin-config/src/migrations.rs" /> — `CURRENT_WORKSPACE_VERSION` bump, migration step
- <RepoFile path="docs/content/docs/reference/runtime/schema-versions.mdx" /> — new timeline entry
