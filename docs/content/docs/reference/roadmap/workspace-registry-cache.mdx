...
1. Bump `CURRENT_WORKSPACE_VERSION` in <RepoFile path="src/config/migrations/mod.rs" />
...
- <RepoFile path="src/runtime/launch/mod.rs" /> — DinD launch, workspace registry ensure call, shared-net attachment
- <RepoFile path="src/runtime/attach/mod.rs" /> — DinD recovery path, same DinD additions
- <RepoFile path="src/runtime/naming/mod.rs" /> — naming functions for registry container, volume, shared network
- <RepoFile path="src/runtime/cleanup/mod.rs" /> — teardown hook, stop registry when last workspace instance exits
- <RepoFile path="src/workspace/mod.rs" /> — `ContainerRegistryConfig` struct, `WorkspaceConfig` extension, new `WorkspaceResources` struct for workspace-level resource tracking
- <RepoFile path="src/config/migrations/mod.rs" /> — `CURRENT_WORKSPACE_VERSION` bump, migration step
