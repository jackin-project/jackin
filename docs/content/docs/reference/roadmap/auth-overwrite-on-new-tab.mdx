...
`run_agent_setup()` dispatches to a per-agent `setup_*()` function — `setup_claude()`, `setup_codex()`, `setup_amp()`, `setup_kimi()`, `setup_opencode()`. Each of these unconditionally copies the provisioned credential snapshot from the `/jackin/<agent>/` bind-mount into the agent's live config directory (`~/.claude/.credentials.json`, `~/.codex/auth.json`, etc.).

The `/jackin/<agent>/` mount is a bind-mount of a file that was written once, at `RoleState::prepare()` time in <RepoFile path="crates/jackin-runtime/src/instance/auth.rs" />, during the initial container launch. That snapshot is frozen at the moment the container was first created. It does not update when the host's credentials rotate, and it does not reflect token refreshes the agent performs inside the container.
...
The fix gates every credential copy on whether the current `jackin-capsule new` invocation is a first-boot or a subsequent session. The container-init marker at `/jackin/state/container-init.done` is the discriminant: it is absent on first boot (before `run_container_init_once()` writes it) and present on every subsequent exec.

`runtime_setup::run()` captures `is_first_init = !Path::new(CONTAINER_INIT_MARKER).exists()` before calling `run_container_init_once()`, then passes the flag through `run_agent_setup(copy_auth)` to each `setup_*()` function. When `copy_auth` is `false`, the credential copy blocks are skipped entirely. The non-credential setup — `seed_home_dir()` (uses `CopyMode::SkipExisting`, already idempotent), MCP tool registration (`claude mcp add tirith`, `claude mcp add shellfirm`), and provider config writes (Codex MiniMax block, OpenCode JSON) — continues to run on every new-tab invocation because those are either idempotent or depend on stable env vars that do not change between tabs.

On first boot, `is_first_init` is `true`, `copy_auth` is `true`, and the container gets the same initial auth bootstrap it always received. On every subsequent `jackin-capsule new`, `copy_auth` is `false` and whatever credentials the agent has on disk are left untouched.

The relevant files changed:

- <RepoFile path="crates/jackin-capsule/src/runtime_setup.rs" /> — `run()`, `run_agent_setup()`, `setup_claude()`, `setup_codex()`, `setup_amp()`, `setup_kimi()`, `setup_opencode()`.
...
The correct long-term answer is the [Live bidirectional auth sync](/reference/roadmap/live-auth-sync/) feature, which depends on the [jackin' daemon](/reference/roadmap/jackin-daemon/). The architecture is already designed in those two roadmap items; this section analyses the specific properties that make it the right answer to the auth fragility problem, along with the challenges it introduces.
...
1. **This PR** — new-tab overwrite closed. Auth on disk inside a container is now stable for the container's lifetime unless the operator explicitly re-launches.
...
- <RepoFile path="crates/jackin-capsule/src/runtime_setup.rs" /> — the fixed file; `run()`, `run_agent_setup()`, and `setup_*()` are the changed functions.
- <RepoFile path="crates/jackin-runtime/src/instance/auth.rs" /> — `provision_*_auth()` functions that write the launch-time credential snapshot. The snapshot model is unchanged by this PR; it remains the input to first-boot auth bootstrap.
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — `agent_mounts()` that bind-mounts the provisioned snapshot into the container.
