...
3. Add `cargo:cargo-audit`, `cargo:cargo-deny`, and `cargo:cargo-shear` to <RepoFile path="mise.toml" /> with pinned versions; add a documented `cargo-shear` ignore only for a confirmed false positive.
