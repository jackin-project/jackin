...
For jackin' specifically, coverage is the instrument that surfaces the untested branches in exactly the auth / Docker / TUI code where <RepoFile path="AGENTS.md" /> already flags pervasive duplication — the places a missing call site in one of two near-identical paths hides.
