...
3. Add `cargo:cargo-audit`, `cargo:cargo-deny`, and `cargo:cargo-machete` to <RepoFile path="mise.toml" /> with pinned versions; add `[package.metadata.cargo-machete] ignored = [...]` for any macro-only false positives.
...
**Not independently verified** (reasoned synthesis — re-confirm at implementation): RustSec's official CI recommendation; `cargo-shear`-vs-`cargo-machete` displacement; `cargo-mutants` runtime cost and adoption; and `taplo` / `cocogitto` / `committed` / `cargo-hakari` / `cargo-chef` / `cargo-about` / `miri` adoption. Tool versions are time-sensitive — pin to a current release at implementation time.
