...
- <RepoFile path="src/runtime/launch/mod.rs" /> — Docker reachability is implicit in the first `bollard` call, not a separate up-front check. The launch pipeline does resolution + execution in one pass (~750 LOC starting at line 1479). `LoadOptions` has `debug`, `rebuild`, `force` — no `dry_run`.
- No `Cli::Doctor`, `Cli::Status`, `Cli::Init`, or `Cli::Ps` variant in <RepoFile path="src/cli/mod.rs" />.
- Searched `src/cli/` and `src/app/`; no `status`, `ps`, or top-level `list` subcommand. Instance discovery code exists in <RepoFile path="src/runtime/discovery/mod.rs" /> and <RepoFile path="src/instance/mod.rs" />, fully consumable.
...
Reuses the instance discovery logic the console already uses (<RepoFile path="src/runtime/discovery/mod.rs" />). CPU/MEM via `docker stats --no-stream --format json`.
...
- Reuse instance discovery from <RepoFile path="src/runtime/discovery/mod.rs" />. CPU/MEM via Docker stats.
...
- <RepoFile path="src/runtime/launch/mod.rs" /> — launch pipeline, dry-run insertion point
- <RepoFile path="src/runtime/discovery/mod.rs" /> — instance discovery for `status`
- <RepoFile path="src/instance/auth/mod.rs" /> — auth checks that doctor should surface
- <RepoFile path="src/capsule_binary/mod.rs" /> — capsule download verification
