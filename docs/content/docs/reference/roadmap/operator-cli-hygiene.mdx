...
Add `jackin status` (and `jackin ps` as an alias) that prints a structured overview of all running jackin' instances from the host. Each instance block shows the instance identity, resource usage, git branch, pull request details with CI status, and the full per-agent identity table sourced from the in-container registry.

```
jackin' fleet status

2 instances running · 3 agents active · 1 agent exited
...
jk-x3n1q8vz   myproj / agent-smith   running   47m   3% cpu   180M

  branch   fix/token-expiry
  pr       #138  Fix JWT token expiry edge case
  url      https://github.com/acme/myproj/pull/138
  ci       ✗ failing — check_lint

  codename   agent     provider      started    exited     status
  ───────────────────────────────────────────────────────────────
  reef       codex     openai        13:20:44   —          active
  crater     claude    anthropic     10:12:00   10:40:11   exited

────────────────────────────────────────────────────────────────────────────────
```

**Instance block fields:**

- Header line: instance ID, workspace, role, state, uptime, CPU%, memory.
- `branch` — the git branch currently checked out in the container's workdir.
- `pr` — PR number and title, resolved via `gh pr view` inside the container or via the GitHub API using the branch name. Shows `—` when no open PR exists for the branch.
- `url` — direct GitHub link to the pull request. The operator can open it immediately without searching.
- `ci` — aggregated check-run status for the PR's head commit: `✓ passing`, `✗ failing — <check-name>`, or `⏳ pending`. Sourced from `gh pr view --json statusCheckRollup`. Shows `—` when there is no PR or no check runs.

**Agent table fields:** same as `jackin-capsule agents` — codename, agent type, provider, started timestamp, exited timestamp (`—` when still active), and status. Active agents sort first; exited agents follow.

**When agent data is unavailable:** instances whose control socket is not yet reachable (still starting up) show `agents  pending`. Stopped instances show `agents  —` since the registry is in-memory and not persisted across container restarts (see [agent codenames](/reference/roadmap/agent-codenames/) Phase 3 for deferred registry persistence).
...
Supports `--filter workspace=myproj`, `--filter agent=claude`, `--state running`. Reuses the instance discovery logic the console already uses (<RepoFile path="src/runtime/discovery.rs" />). CPU/MEM via `docker stats --no-stream --format json`. Branch and PR data via `gh` CLI or the GitHub REST API.
