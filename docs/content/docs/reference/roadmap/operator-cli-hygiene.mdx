...
- Create <RepoFile path="src/cli/doctor.rs" /> with `pub async fn run(args: DoctorArgs) -> Result<()>`.
...
- Create <RepoFile path="src/cli/format.rs" /> with `enum OutputFormat { Human, Json }` and `OutputEnvelope<T>`.
...
- Create <RepoFile path="src/error.rs" /> with `enum JackinError` and `impl JackinError::user_message(&self) -> UserMessage`.
