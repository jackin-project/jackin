...
- <RepoFile path="crates/jackin/src/main.rs" /> shows three sites where anyhow chains are rendered via `tui::fatal(&format!("{error:#}"))`. No structured failure-to-friendly-message mapping exists.
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — Docker reachability is implicit in the first `bollard` call, not a separate up-front check. The launch pipeline does resolution + execution in one pass (~750 LOC starting at line 1479). `LoadOptions` has `debug`, `rebuild`, `force` — no `dry_run`.
- No `Cli::Doctor`, `Cli::Status`, `Cli::Init`, or `Cli::Ps` variant in <RepoFile path="crates/jackin/src/cli.rs" />.
- Searched `src/cli/` and `src/app/`; no `status`, `ps`, or top-level `list` subcommand. Instance discovery code exists in <RepoFile path="crates/jackin-runtime/src/runtime/discovery.rs" /> and <RepoFile path="crates/jackin-runtime/src/instance.rs" />, fully consumable.
...
Add `jackin status` (and `jackin ps` as an alias) that prints a table of all running jackin' instances outside the TUI.

```
INSTANCE                 WORKSPACE  ROLE               AGENT(S)          STATE    UPTIME   CPU%  MEM
proj-the-architect-...   myproj     the-architect      claude            running  2h 14m   12%   420M
proj-agent-smith-...     myproj     agent-smith        claude, codex     running    47m    3%   180M
```

One workspace can run multiple agents, so the agent column may list several. When available, show PR info per instance (the branch/PR the agent is working on).

Supports `--format json` (see item 3 below), `--filter workspace=myproj`, `--filter agent=claude`, `--state running`.

Reuses the instance discovery logic the console already uses (<RepoFile path="crates/jackin-runtime/src/runtime/discovery.rs" />). CPU/MEM via `docker stats --no-stream --format json`.
...
- Add `Command::Doctor(DoctorArgs)` to <RepoFile path="crates/jackin/src/cli.rs" />.
...
- Add `Command::Status(StatusArgs)` and `Command::Ps` alias to <RepoFile path="crates/jackin/src/cli.rs" />.
- Create `src/cli/format.rs` with `enum OutputFormat { Human, Json }` and `OutputEnvelope<T>`.
- Reuse instance discovery from <RepoFile path="crates/jackin-runtime/src/runtime/discovery.rs" />. CPU/MEM via Docker stats.
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — launch pipeline, dry-run insertion point
- <RepoFile path="crates/jackin-runtime/src/runtime/discovery.rs" /> — instance discovery for `status`
- <RepoFile path="crates/jackin-runtime/src/instance/auth.rs" /> — auth checks that doctor should surface
- <RepoFile path="crates/jackin-image/src/capsule_binary.rs" /> — capsule download verification
