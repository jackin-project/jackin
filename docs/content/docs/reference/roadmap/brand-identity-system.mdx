...
**Status**: Partially implemented — web/docs/README, CLI/Capsule brand text, the phosphor green-block mark, and the frozen-rain `--help` banner shipped; a full multi-line `--version` splash redesign still deferred
...
- A full multi-line `--version` splash / ASCII-wordmark redesign. The frozen-rain `--help` banner already shipped — the binary prints it above clap's pill on a wide interactive terminal (clap reflows multi-line ANSI art through `before_help`, so a dedicated printer with its own TTY and width detection renders it), and `--version` shows the one-line pill splash on a TTY. Only the larger multi-line splash art remains deferred.
