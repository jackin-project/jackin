...
|---|---|---|
| Prompt sigil | `j❯` stayed legible at prompt size and favicon size on the `#5CF07A` block, with a black `j` and a white chevron, square corners. | Chosen for prompt, favicon, app/PWA icons, and monogram surfaces. |
...
- Wordmark/logotype surfaces use the full form (`jackin❯` or `jackin>` fallback) on the green block. There is one colorway, not separate light/dark variants: black letters and a white chevron on `#5CF07A` in every context.
...
- **Color recognition:** every rendered logo/documentation treatment that supports styling must show the mark on the `#5CF07A` block with black letters and a white chevron, not merely spell the mark as text.
...
| Token | Hex | Use |
|---|---:|---|
| Logo block green | `#5CF07A` | The solid block every mark sits on; also the canonical phosphor green |
...
The mark has one colorway, not several: a `#5CF07A` block, `#0A0A0A` letters, a `#FFFFFF` chevron, square corners. It renders identically on dark canvases, light docs, cream stickers, and inside terminals — the block carries the brand color so the surrounding surface never has to.
...
- Every logo surface renders the mark on the `#5CF07A` block with a black word and a white chevron; only surrounding UI accents may use muted green variants.
