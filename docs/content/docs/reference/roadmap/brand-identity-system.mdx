...
The root `jackin --help` banner uses frozen brand rain, not animation. Because clap reflows multi-line ANSI art passed through `before_help`, the binary prints the rain field itself — drawn from the shared phosphor rain pool — above clap's one-line `jackin❯` pill, and only on a wide interactive terminal; piped or narrow output falls back to the pill alone. The field is deterministic (a fixed per-cell seed) and bounded to stay under the terminal width. It is static, not the live launch progress rain, which the Launch Progress TUI lifecycle owns.
