...
|---|---|---|
| `claude` | yes (built-in `LSP` tool, v2.0.74+) | **full navigation** — go-to-def / refs / hover / impls / call-hierarchy, via the official `rust-analyzer-lsp@claude-plugins-official` plugin (Anthropic-authored → passes the marketplace allow-list) + the binary on `PATH` |
...
# rust-analyzer: official LSP as a rustup component (no compile). Agent-facing
# only where the runtime is a native LSP client: `claude` drives it for full
# navigation via the official `rust-analyzer-lsp@claude-plugins-official` plugin
# (added in jackin.role.toml [claude].plugins; Anthropic-authored, allow-list
# clean), `opencode` auto-detects it on PATH for diagnostics. Every runtime can
# shell `rust-analyzer diagnostics <path>` as a verification gate. No LSP->MCP
# bridge — codex/amp/kimi/grok have no native LSP and fall back to codedb +
# ast-grep for navigation.
