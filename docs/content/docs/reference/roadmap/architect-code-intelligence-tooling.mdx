...
**Status**: Open — pilot ready to implement (the-architect role only, opt-in, no jackin-core code change). Pilot stack decided: `ast-grep` + `rust-analyzer` + cargo verifiers + `fff`. `codedb` rides along as an opt-in A/B evaluation arm; `ast-index` was evaluated and deferred. See [Tool evaluation and comparison](#tool-evaluation-and-comparison) for the reasoning and the verified findings behind these choices.
...
This item also records the **evaluation** of the candidate tools (`ast-grep`, `codedb`, `fff`, `ast-index`) that produced this stack, so the choice is auditable and the deferred candidates have a documented home. The broad market sweep and the token-economics A/B protocol live in the research dossier [Code-intelligence tools: codedb, fff, CodeGraff, and alternatives](/research/token-optimization-research/51-code-intelligence-tools/); this roadmap item is the **decision record** and the implementation plan, not a duplicate of that evidence.
...
- **No jackin-core change.** The pilot rides existing extension points: the role `Dockerfile`, a role `setup_once` hook, and default-home seeding of the agent home (<RepoFile path="crates/jackin-image/src/derived_image.rs">crates/jackin-image/src/derived_image.rs</RepoFile>).
...
The full market scan — `codedb`/`fff` benchmarked against `Serena`, `Code Context Engine`, `Augment`, `Sourcegraph`, `Qodo`, `Claude Context`, `CodeGraff`, and others, plus the token-economics equation, local measurements, the validation harness, and the acceptance rule — lives in the research dossier [Code-intelligence tools: codedb, fff, CodeGraff, and alternatives](/research/token-optimization-research/51-code-intelligence-tools/). That dossier is the evidence and the A/B protocol; this item is the decision and the implementation. The dossier's conclusion (keep the fff pilot; add a codedb A/B arm with a strict bounded-output policy; do not adopt CodeGraff Pro by default) is the basis for the stack above.
...
| Symbols / callers / deps / task-context | `codedb` | MCP, registered at user scope — **A/B evaluation arm**, telemetry off, installer hooks skipped |
...
### 2. Register the MCP servers (role hook)

Register `fff` (and, for the evaluation, `codedb`) at **user scope** so they write to the container's `~/.claude.json`, never to a project `.mcp.json` inside the operator's mounted repo. Use a `setup_once` hook so it runs once per instance, after auth handoff:
...
# Register codedb (A/B evaluation arm) at user scope. Telemetry is disabled via
# CODEDB_NO_TELEMETRY in the image. Pass the mounted workspace as the project
# root so codedb never indexes the agent home. Do NOT install codedb's block-legacy
# hook — the router keeps fff/rg as the search tools, codedb for relationships.
if command -v codedb >/dev/null 2>&1; then
  claude mcp list 2>/dev/null | grep -q '^codedb' || claude mcp add -s user codedb -- codedb mcp "${JACKIN_WORKSPACE:-$PWD}"
fi
```
...
The equivalent `codex mcp add` / OpenCode MCP config registers the same servers for those runtimes when the evaluation extends beyond Claude; keep them behind the same `command -v` guards.
...
2. The copies under `~/.claude/` and `~/.codex/` are captured by the derived layer's default-home step (see <RepoFile path="crates/jackin-image/src/derived_image.rs">crates/jackin-image/src/derived_image.rs</RepoFile>) and reseeded on first boot; `~/.config/AGENTS.md` and `~/.config/opencode/AGENTS.md` are baked into the image and present at runtime.
...
- the agent invokes `ast-grep` for structural queries, `rust-analyzer` for symbol resolution, `fff` for file/content lookup, and (in the codedb arm) `codedb` for caller/dependency/context questions — instead of ad-hoc text search;
...
- the `codedb` arm meets the dossier's acceptance rule (target-file hit rate ≥ native, edit/test success ≥ native, total tokens per solved task lower by a meaningful margin, no unbounded-output path) — or codedb is dropped while the rest of the stack stays. Run the arms with the fixed task suite and metrics from the [validation harness](/research/token-optimization-research/51-code-intelligence-tools/).
...
- **codedb output guardrail.** If codedb stays past the A/B, add a hook (or instruction) requiring `limit` / `prefix` / compact-read on `codedb_tree`, `codedb_snapshot`, and `codedb_remote action=tree` so a code-intelligence call never dumps more than native tools would. This is the dossier's bounded-output policy made concrete.
...
- [Code-intelligence tools: codedb, fff, CodeGraff, and alternatives](/research/token-optimization-research/51-code-intelligence-tools/) — the research dossier behind this decision: market sweep, token economics, local measurements, validation harness, and acceptance rule.
