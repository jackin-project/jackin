...
embedded migrations. "SQLite-first" names the on-disk **format**, not the
client: all SQLite access uses `turso` (the workspace's single, async DB stack,
already used by the Capsule-local cache) — never `rusqlite`, Diesel-on-SQLite,
or any other binding. A sync caller makes its path async rather than reaching
for a sync client. Document the production rationale in an ADR before promoting
the current in-container cache pattern.
