...
Partial implementation note: the [Capsule usage and quota overlay](/reference/roadmap/capsule-usage-quota-overlay/) now creates a Capsule-local rusqlite store at `/jackin/state/usage/telemetry.db` with the `account_usage_snapshots` and `usage_samples` schema subset. That store persists normalized account quota buckets for the in-container Capsule daemon only. The full per-instance `~/.jackin/data/<container>/jackin.db`, host-global `~/.jackin/data/daemon/accounts.db`, migrations, memory tables, and per-message `usage_samples` ingestion described below remain open.
