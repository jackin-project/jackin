...
on individual instances. The daemon creates and migrates this file on first
start; CLI / `jackin console` / Capsule never write to it. The daemon
process is the single writer; CLI tools and the console open the file
read-only. WAL mode handles the daemon-write / multi-reader-process pattern
