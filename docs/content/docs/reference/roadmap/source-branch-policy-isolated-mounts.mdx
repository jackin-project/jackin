...
- <RepoFile path="crates/jackin/src/console/tui/state.rs" /> and <RepoFile path="crates/jackin/src/console/tui/input/editor.rs" /> — replace isolation cycling with the mount settings dialog, staged mount form, source-folder browser handoff, destination text input, and mode-gated source branch rows.
...
- Console editor mount settings dialog for source folder, container destination, read-only state, isolation type, and mode-gated source branch policy.
