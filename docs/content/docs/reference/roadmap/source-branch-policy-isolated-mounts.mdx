...
The workspace editor should replace the current one-key isolation cycling model with a focused **Mount isolation settings** dialog. The mount row can still show a compact badge, but editing the badge should open a modal where the operator chooses the isolation type and the settings that belong to that type.

### Mounts tab row

The mounts table should keep one row per mount and make the source policy visible only when it matters:
...
```text
Enter edit mount · I isolation settings · A add mount · D remove · Esc back
```

`I` can remain the mnemonic, but it should open the settings dialog instead of cycling through every mode. Fast cycling made sense when the only decision was `shared | worktree | clone`; once each mode has settings, cycling hides important state and makes accidental changes easy.

### Isolation settings dialog

The dialog should use the existing console modal vocabulary: centered panel, one focused control at a time, footer-only hints, and a visible summary of what will happen. The first screen chooses the isolation type; the right side previews the consequence.

```text
╭─ jackin' · mount isolation settings ─────────────────────────────╮
│ Mount                                                             │
│   host      ~/Projects/api                                        │
...
│                                                                   │
│ Preview                                                           │
│   New agents get a private worktree from the repo default branch.  │
╰───────────────────────────────────────────────────────────────────╯
  ↑/↓ choose · Tab next group · Enter apply · Esc cancel
```

When `shared` is selected, the `Source branch` group should disappear or render disabled with `not used for shared mounts`. When `clone` is selected, the same policy labels apply, but the preview should say `private clone` rather than `private worktree`.
...
- <RepoFile path="crates/jackin-config/src/schema.rs" /> — extend `MountConfig` with the policy fields and serde defaults.
- <RepoFile path="crates/jackin-config/src/migrations.rs" /> — workspace schema bump and migration chain.
- <RepoFile path="crates/jackin-runtime/src/isolation/materialize.rs" /> — resolve the policy before `worktree add` / `clone` materialization and use the resolved commit/ref without mutating the host checkout.
- <RepoFile path="crates/jackin-runtime/src/isolation/state.rs" /> — confirm the existing `base_commit` remains sufficient; add the resolved policy/ref only if cleanup or diagnostics needs it.
- <RepoFile path="crates/jackin/src/cli/workspace.rs" /> — parse `--mount-source-branch` for create/edit.
- <RepoFile path="crates/jackin/src/console/tui/state.rs" /> and <RepoFile path="crates/jackin/src/console/tui/input/editor.rs" /> — replace isolation cycling with the mount-isolation settings dialog.
- <RepoFile path="crates/jackin-runtime/src/runtime/progress.rs" /> — reuse the existing launch dialog surface for `ask` policies instead of adding a separate prompt renderer.

## Scope (V1)

- Workspace schema for per-mount source branch policy.
- Runtime resolution for `worktree` and `clone` mounts.
- CLI create/edit support.
- Console editor settings dialog for isolation type plus source branch policy.
- Launch cockpit prompt for `Ask on launch`.
- Docs updates for the Workspaces guide, Mounts guide, workspace command reference, configuration reference, and schema versions.
- Tests covering parsing, migration, policy resolution, `worktree` materialization, `clone` materialization, invalid combinations, and TUI state transitions.
...
- [Ephemeral mount modes](/reference/roadmap/ephemeral-mount-modes/) — another sign that isolation is becoming a configurable mount strategy, not a single enum.
- [Session keep and resume](/reference/roadmap/session-keep-and-resume/) — resume becomes more predictable when every isolated checkout records exactly which branch/commit it started from.
- [Launch Progress TUI](/reference/roadmap/launch-progress-tui/) — provides the rich launch dialog surface for `Ask on launch`.
- [Task source abstraction](/reference/roadmap/task-source-abstraction/) — future PR/task flows can reuse this policy once a task carries its own branch source.
