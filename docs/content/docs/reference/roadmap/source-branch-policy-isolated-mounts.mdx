...
- <RepoFile path="crates/jackin/src/console/tui.rs" /> — replace isolation cycling with the mount settings dialog, staged mount form, source-folder browser handoff, destination text input, and mode-gated source branch rows (inline `state` and `input::editor` modules).
