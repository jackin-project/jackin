...
<RepoFile path="src/operator_env/mod.rs" /> is **3573L** total (was 2975L at original analysis — +20% growth). It has grown into a shared cross-cutting module that serves runtime launch, config validation/persistence, console widgets, and the console op-cache layer. The file mixes public traits/parsers, subprocess client logic, metadata parsing for the picker, env-layer composition, launch diagnostics, and tests.
...
- <RepoFile path="src/operator_env/mod.rs" /> — target file (3573L)
- <RepoFile path="src/runtime/launch/mod.rs" /> — resolves operator env and emits launch diagnostics
- <RepoFile path="src/console/tui/op_picker/mod.rs" /> — uses `OpStructRunner` / picker metadata
