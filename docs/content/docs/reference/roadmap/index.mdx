...
- **[Brand identity system](/reference/roadmap/brand-identity-system/)** — non-Rust surfaces now use the terminal-native `jackin❯` / `j❯` system, canonical color tokens, docs chrome, landing hero, README hero, favicon/app icons, Open Graph artwork, prose sweep, and role-container Starship prompt. Rust CLI/Capsule output, help banners, and Rust snapshot coverage remain deferred while refactoring is in flight (status: partially implemented)
