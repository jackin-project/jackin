...
- **[Workspace-native PR verification](/reference/roadmap/pr-verification/)** — `jackin dev pr verify <n>` builds the per-PR Verify-locally loop into the binary: a diff-driven rich surface that surfaces and runs the build/test/serve commands a PR's changes call for, host-side, against an isolated config/state home so jackin' can verify its own PRs without touching the operator's real setup (status: open — design proposal)
