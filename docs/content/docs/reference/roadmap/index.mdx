...
- Shared TUI scroll model — scrollbar geometry, clamping, cursor-follow offsets, drag/click mapping, launch-progress tail scrolling, and capsule scrollback all route through `jackin_tui::scroll` while keeping console and capsule visuals distinct; see [TUI Design Decisions](/reference/tui-design-decisions/#scrollable-blocks--shared-component)
- Docs site migration to Fumadocs on TanStack Start — the docs build now runs through Vite/Bun, statically prerenders GitHub Pages output under `.output/public`, ships static Orama search, AI-readable `llms.txt` and per-page Markdown, Takumi-generated OG/social/icon images, custom `/sitemap.xml`, and the React landing/docs chrome. Contributor details live in [Codebase Map](/reference/codebase-map/).
...
- **[Agent runtime trait](/reference/roadmap/agent-runtime-trait/)** — replace the closed 5-variant `Agent` enum and its 414 dispatch references across 31 files with a per-runtime `AgentRuntime` trait and `src/runtime/adapters/<runtime>.rs` modules, so adding a sixth runtime is one new file plus one registration line rather than a 31-file edit. Phase 1.5 follow-up under the readability program — commits the trait shape that the program names but does not specify (status: open — design proposal)
- **[Structured tracing and metrics](/reference/roadmap/structured-tracing-metrics/)** — adopt the `tracing` ecosystem (`tracing` + `tracing-subscriber` + `tracing-appender`, optional feature-gated `opentelemetry`) behind the existing observability surface, collapsing the three hand-rolled stacks (88 `eprintln!`, the capsule `clog!` / `cdebug!` macros, and the `RunDiagnostics` JSONL whose `span_id` is defined-but-never-set) into one instrumentation API with composable output layers. Real span correlation across the concurrent launch pipeline, the two-tier compact/verbose contract reduced to a level filter, byte-stable JSONL, and metrics for free — while keeping local-first JSONL the default and OTLP opt-in (status: open — design proposal)
...
- **[TUI architecture — Ratatui application patterns](/reference/roadmap/tui-architecture/)** — north-star design for restructuring both TUI surfaces (the host operator console and the `jackin-capsule` multiplexer) around Ratatui's Model/Update/View and component patterns: an explicit `Action`/`Message` boundary, a pure view layer, decomposed state, background I/O off the draw thread, and elevating the existing `jackin-tui` crate into a shared design-component library that ends the per-surface copy-paste. Coordinates with the readability program's snapshot-test net and file splits (status: open — design proposal)
