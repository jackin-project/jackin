...
- **[Workspace-native PR verification](/reference/roadmap/pr-verification/)** — a console flow that builds the per-PR Verify-locally loop into `jackin console`: select a workspace, pick one of its primary repo's pull requests, and run a diff-driven rich surface that surfaces and runs the build/test/serve commands the PR's changes call for, host-side, against an isolated config/state home so jackin' can verify its own PRs without touching the operator's real setup (status: open — design proposal)
