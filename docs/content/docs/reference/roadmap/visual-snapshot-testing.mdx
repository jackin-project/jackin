...
- **The console snapshot helper captures glyphs only.** It dumps each cell's `.symbol()` and discards colour and every text attribute (see <RepoFile path="crates/jackin/src/console/tui/view/snapshot_tests.rs">crates/jackin/src/console/tui/view/snapshot_tests.rs</RepoFile>). A regression that turns a `Blocked` badge from red to grey, drops the underline on a link, or removes `REVERSED` from a selected row passes every existing snapshot.
...
- **Text-only console snapshots** — four `insta` snapshots (list, settings, editor-general, editor-mounts) in <RepoFile path="crates/jackin/src/console/tui/view/snapshot_tests.rs">crates/jackin/src/console/tui/view/snapshot_tests.rs</RepoFile> with committed `.snap` files, generated with `INSTA_UPDATE=new`. These capture glyphs only.
