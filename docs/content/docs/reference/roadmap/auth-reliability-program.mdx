...
**Status**: Partially implemented — Phase 0 shipped; Phases 1–7 open
...
*Fixed in Phase 0 — this PR. See [Auth overwrite on new tab](/reference/roadmap/auth-overwrite-on-new-tab/).*
...
*Addressed in Phase 2. See [Auth health and operator visibility](/reference/roadmap/auth-health-and-visibility/).*

**3. Multi-company auth requires manual account switching** (frequent for multi-project operators). An operator with a personal Claude account and a company Claude account cannot tell different workspaces to read from different credential directories. Every workspace reads from the same hardcoded `~/.claude` path. The operator must manually switch accounts on the host before launching each workspace, or all workspaces share one account.

*Addressed in Phase 1. See [Per-scope auth-sync source folder](/reference/roadmap/auth-sync-source-folder/).*
...
*Addressed in Phase 4. Requires [jackin' daemon](/reference/roadmap/jackin-daemon/) and [live bidirectional auth sync](/reference/roadmap/live-auth-sync/).*
...
*Partially addressed. See [Workspace Claude token setup](/reference/roadmap/workspace-claude-token-setup/).*
...
*Addressed in Phase 6 (parallel track). See [Container credential exposure](/reference/roadmap/container-credential-exposure/).*
...
(shipped)   (next PR)   (after P1)   (daemon)   (live sync)   (abstraction)
                                          │
...
**Detail:** [Auth overwrite on new tab](/reference/roadmap/auth-overwrite-on-new-tab/).
...
**Exact PR scope:** `AgentAuthConfig` schema change (one new optional field), version bumps for both `CURRENT_CONFIG_VERSION` and `CURRENT_WORKSPACE_VERSION`, migration steps, fixtures for both config and workspace file kinds, Timeline entry in schema-versions doc, `resolve_sync_source_dir` resolver in <RepoFile path="src/config/roles.rs" />, provisioning updates in <RepoFile path="src/instance/auth.rs" />, `show_hidden` on `FileBrowserState`, Source folder row and shared picker helper in the auth modal for both console surfaces.

**Prerequisite for:** Phase 4 (live sync should consume the resolved source rather than the hardcoded path).

**Detail:** [Per-scope auth-sync source folder](/reference/roadmap/auth-sync-source-folder/).
...
First, a `probe_auth_health()` function is added to <RepoFile path="src/instance/auth.rs" />. It runs as part of `RoleState::prepare()` before `docker run`, takes the same resolved inputs `provision_*_auth` takes, and checks: credential file exists and is non-empty (sync mode), file parses as valid JSON (Claude, Codex, Amp, OpenCode), JWT `exp` claim decoded from the payload (no signature check — just read the expiry field) with a 7-day warning window, env var set and non-empty (api_key and oauth_token modes). Never makes a network call. Result icons (`✓` / `⚠` / `✗`) appear in the per-agent auth rows that jackin' already prints in the launch summary.
...
**Exact PR scope:** `probe_auth_health()` + `AuthHealthResult` struct in <RepoFile path="src/instance/auth.rs" />; integration into `RoleState::prepare()` launch summary; `jackin auth status` subcommand in `src/cli/auth.rs` + dispatch in <RepoFile path="src/app/mod.rs" />; health column in console Auth tab (background probe, cached per focus); health glyph in instance rows. No schema change. No daemon dependency.

**Detail:** [Auth health and operator visibility](/reference/roadmap/auth-health-and-visibility/).
...
**Detail:** [jackin' daemon](/reference/roadmap/jackin-daemon/).
...
**Exact PR scope:** Per-axis daemon adapters; `jackin-auth-watcher` in-container binary; shared-store mount strategy in <RepoFile path="src/runtime/launch.rs" />; `sync` → `forward` rename with full schema migration artifacts for all three versioned file kinds; construct image update to include `jackin-auth-watcher`.

**Prerequisite for:** Phase 7 benefits from Phase 4 but does not strictly require it.

**Detail:** [Live bidirectional auth sync](/reference/roadmap/live-auth-sync/).
...
**Detail:** [Credential source pattern](/reference/roadmap/credential-source-pattern/).
...
**Detail:** [Container credential exposure](/reference/roadmap/container-credential-exposure/).
...
**Detail:** [Host bridge — secrets and approved host actions](/reference/roadmap/host-bridge/).
...
| 1 — multi-company isolation | Per-workspace credential source path | No | Complete | Next auth PR |
| 2 — health and visibility | Pre-launch probe, `jackin auth status`, console indicators | No | Complete | After Phase 1 |
...
| Item | Phase | Status |
|---|---|---|
| [Auth overwrite on new tab](/reference/roadmap/auth-overwrite-on-new-tab/) | 0 | Shipped |
| [Per-scope auth-sync source folder](/reference/roadmap/auth-sync-source-folder/) | 1 | Open — ready to implement |
| [Auth health and operator visibility](/reference/roadmap/auth-health-and-visibility/) | 2 | Open — design complete |
| [Workspace Claude token setup](/reference/roadmap/workspace-claude-token-setup/) | 2 (validity probe integration point) | Partially implemented |
| [GitHub CLI authentication strategy](/reference/roadmap/github-cli-auth-strategy/) | 2–4 (scope pre-flight; bidirectional sync) | Partially implemented |
| [jackin' daemon](/reference/roadmap/jackin-daemon/) | 3 | Open — design questions remain |
| [Live bidirectional auth sync](/reference/roadmap/live-auth-sync/) | 4 | Open — design complete, awaits daemon |
| [Credential source pattern](/reference/roadmap/credential-source-pattern/) | 5 | Open — design complete |
| [Container credential exposure](/reference/roadmap/container-credential-exposure/) | 6 | Open — design complete |
| [Host bridge — secrets and approved host actions](/reference/roadmap/host-bridge/) | 7 | Open — design complete, awaits daemon |
| [Reliable Claude authentication strategy](/reference/roadmap/claude-auth-strategy/) | Historical context for current mode design | Deferred |
