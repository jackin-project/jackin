...
| **F** | **Multiplexer screen scrape** | jackin's own `vt100` screen + scrollback for the pane | Live / post-hoc | Yes — jackin owns the PTY | n/a (jackin-internal) |
...
What the multiplexer can actually see (validated in <RepoFile path="crates/jackin-capsule/src/session.rs" />):

- Capsule keeps one `vt100::Parser` per session; its `Screen` is the source of truth, exposed via `screen()`. Raw PTY bytes are read in a blocking reader loop and fed to the parser byte-by-byte, then **dropped** — there is no raw-byte or asciicast log retained by default.
- It retains **`SCROLLBACK_LEN = 10_000`** lines of native `vt100` scrollback per session (the comment cites exactly this use case: "Operators need scrollback to read Codex / Claude responses that exceed one viewport"), plus a separate `inline_scrollback` buffer for Codex-style top-anchored scroll regions that `vt100` does not record as scrollback. The dirty-row renderer in <RepoFile path="crates/jackin-capsule/src/render.rs" /> already reads cells from the `Screen`.
...
The default capture path is host-side and read-only against agent state: it reads the per-instance homes jackin' already restores and runs read-only vendor exports/streams (`amp threads`/`--stream-json`). The screen-scrape fallback consumes Capsule's existing `vt100` screen layer. The only writes are into Déjà Vu's own `~/.jackin/data/deja-vu/` tree.
...
Wire interception (D) for the exact system prompt / thinking where an operator wants it, and the multiplexer screen-scrape (F) last-resort tier sharing Capsule's `vt100` layer with the [agent runtime status authority](/reference/roadmap/agent-runtime-status/). Both opt-in and redaction-aware.
...
| Should the screen-scrape fallback and the status authority share one screen-reading layer? | Yes — both read Capsule's `vt100::Screen`; build one reusable screen-extraction layer rather than two. |
...
- [Terminal observation and automation](/reference/roadmap/terminal-observation-automation/) — owns the PTY/`vt100` read APIs and opt-in recording the screen-scrape fallback would build on.
- [Agent runtime status authority](/reference/roadmap/agent-runtime-status/) — the herdr-inspired status work; shares Capsule's `vt100` screen-reading substrate and the "structured signals first, screen last, never trust scrollback as authority" discipline.
