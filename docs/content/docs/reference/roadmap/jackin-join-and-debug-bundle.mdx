...
- <RepoFile path="crates/jackin-diagnostics/src/run.rs" /> — existing diagnostics framework
- <RepoFile path="crates/jackin-env/src/resolve.rs" /> — env resolution (redaction source)
- <RepoFile path="crates/jackin-runtime/src/instance/auth.rs" /> — credential locations (redaction targets)
