...
**Status**: Partially implemented — image recipe labels, warm local-image reuse, live current-instance attach-first, and nested launch timing diagnostics shipped; stopped/startable restore, credential laziness, selected-runtime foreground prep, build-path surgery, and prewarm remain open
...
5. Selects the agent and resolves restore candidates. A live current-role candidate now attaches immediately through hardline before credentials, image decisions, binary prep, or Docker build run.
