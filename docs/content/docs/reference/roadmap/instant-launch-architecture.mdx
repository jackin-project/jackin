...
Remaining Phase 3 work is to move sibling-runtime image refresh into background/prewarm flows and to move background refresh decisions behind a stronger launch-plan model. The broader item still needs plan-gated credential laziness for non-empty non-required credential graphs, recipe-keyed plugin bundle artifact caching, deeper build-path surgery, role/image prewarm, and warmed runtime resources.
