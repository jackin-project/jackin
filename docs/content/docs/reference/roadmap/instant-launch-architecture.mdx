...
Remaining Phase 3 work is to move background refresh decisions behind a stronger launch-plan model and add daemon prewarm surfaces beyond the CLI, explicit image prewarm, and saved-workspace console action. The broader item still needs plan-gated credential laziness for non-empty non-required credential graphs, recipe-keyed plugin bundle artifact caching, deeper build-path surgery, and warmed runtime resources.
