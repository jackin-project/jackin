...
- `jackin prewarm --sidecar` now reuses the launch path's `docker:dind` image constant and pulls that sidecar image only when it is missing locally. It does not create sidecar containers, networks, cert volumes, workspaces, or host config, so it warms one runtime resource without changing restart semantics.
