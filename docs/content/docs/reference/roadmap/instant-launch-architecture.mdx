...
**Status**: Partially implemented — image recipe labels, warm local-image reuse, live current-instance attach-first, stopped/startable current-instance restore, missing-container recreation from valid images, nested launch timing diagnostics including sidecar/Capsule readiness steps, parsed Docker build-step diagnostics, `jackin diagnostics summary`, concurrent operator-env reads, concurrent GitHub env reads, and conditional `/home/agent` ownership repair shipped; broader credential laziness, selected-runtime foreground prep, deeper build-path surgery, and prewarm remain open
