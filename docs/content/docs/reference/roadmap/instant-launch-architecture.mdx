...
**Status**: Partially implemented — image recipe `v2` labels with component-level invalidation reasons, warm local-image reuse, agent-scoped derived image tags, selected image decisions before credential resolution, source-specific `ImageDecision::BuildFromPublished` / `BuildFromWorkspace` rebuild plans, live/stopped current-instance attach-first before role repo refresh when the agent is already known, stopped/startable current-instance restore that also bypasses workspace `git_pull_on_entry`, missing-container recreation from valid images, selected-runtime foreground binary/auth prep and builds without a separate latest-release update probe, `jackin prewarm` for runtime binary/`jackin-capsule` caches, DinD sidecar image prewarm, configured, targeted, workspace, and all-workspace default role-repo caches, explicit role images, workspace images, and all-workspace image prewarm, saved-workspace console image prewarm, background sibling auth prewarm, warm-hit-only sibling runtime binary/image prewarm, prefetched selected-agent version recording without a foreground Docker probe, launch-plan selection/rejection diagnostics, nested launch timing diagnostics including role repo refresh and restore candidate scans, build-source selection diagnostics, build-context size diagnostics, explicit workspace `git_pull_on_entry` timing, per-auth-slot role-state preparation, sidecar/Capsule readiness, hardline exec, and post-attach finalization steps, parsed Docker build-step diagnostics, `jackin diagnostics summary` with skipped-timing sections, `jackin diagnostics compare` with launch-plan/cache/skipped-timing/build-context/build-step/startup-delta comparisons and fastest/first-run baselines, startup-vs-full-session timing summaries, concurrent operator-env reads, empty operator-env and manifest-env resolution skips, concurrent GitHub env reads, ignored GitHub env resolution skip, filtered GitHub env resolution for only runtime-consumed keys, configured-token GitHub sync skip, GitHub ignore-mode absent-state role prep skip, Dockerfile-demand GitHub build-token lookup, concurrent GitHub/agent role-state auth preparation, fresh-launch DinD/auth/workspace overlap, `.git`/`.jackin-runtime`-free derived build contexts, single-layer Claude plugin installation, BuildKit-cached fallback installer layers, and host UID/GID remap removal shipped; broader plan-gated credential laziness, deeper build-path surgery, artifact-cached plugin prep, daemon prewarm surfaces, and warmed sidecar containers remain open
...
3. If the launch request or workspace already names the selected agent and no branch override is being loaded, checks current-role restore candidates before role source resolution. A live current-role candidate attaches immediately through hardline before role repo fetch/update, workspace `git_pull_on_entry`, credentials, image decisions, binary prep, or Docker build run. A stopped or created current-role candidate starts and reconnects through Capsule before the same expensive foreground work. A missing current-role container records the existing container name and continues to role/image validation for the smallest recreate repair.
...
12. Creates/updates the instance manifest and prepares selected-agent auth state.
...
- Derived builds stamp `jackin.image_recipe_version=v2`, `jackin.image_recipe_hash`, `jackin.selected_agent`, `jackin.selected_agent_version` when known, `jackin.role_git_sha`, `jackin.construct_image`, and `jackin.recipe.*` diagnostic labels onto the local image. The diagnostic labels cover role source ref, base image, generated runtime hash, supported-agent set, selected-agent install recipe, cache-bust value, `jackin-capsule` version, hook content hash, Claude plugin recipe hash, and host identity strategy.
- Warm launches inspect the local image tag/labels first. When the recipe matches, launch returns `ImageDecision::Reuse` and skips `prepare_runtime_binaries`, `create_derived_build_context`, `resolve_github_token`, `docker build`, and the foreground `docker run --rm --entrypoint <agent> --version` probe. When the recipe misses, launch returns `ImageDecision::BuildFromPublished` or `BuildFromWorkspace` before binary prep so the rebuild path already knows whether it should layer on top of a declared `published_image` or rebuild from the workspace Dockerfile. The `image_cache_hit` diagnostic includes the skipped work list and the selected-agent version when that image label is present.
...
- The recipe currently covers role SHA/ref, construct and base image identity, generated runtime Dockerfile shape, supported agents, selected agent install recipe, `jackin-capsule` package version, hook file hashes, Claude plugin config hash, cache-bust value, and host identity strategy.
...
- Rebuild paths now prepare and install only the selected foreground runtime plus `jackin-capsule`; `create_derived_build_context_for_agents` renders the derived Dockerfile for that selected agent instead of forcing a Claude launch to wait on Kimi, OpenCode, or other sibling runtime binary checks. The selected agent remains part of the image recipe, so switching runtimes invalidates the local image explicitly rather than silently launching a missing CLI.
- After a selected foreground image is reused, <RepoFile path="crates/jackin-runtime/src/runtime/image.rs" /> starts a non-blocking sibling-runtime binary prewarm task. It writes only jackin-owned binary cache state, emits `runtime_prewarm_*` diagnostics, and does not block hardline or move agent setup into the entrypoint. When the selected image had to rebuild, sibling binary prewarm is skipped with a diagnostic instead of competing with the cold foreground launch.
- <RepoFile path="crates/jackin/src/cli/prewarm.rs" /> adds the first explicit `jackin prewarm` command. The command fills jackin-owned agent-binary and `jackin-capsule` caches for all agents or repeated `--agent` filters. With `--roles`, it clones or updates configured role repos under `~/.jackin/roles/` without touching host repos or host git config; `--roles --role <selector>` narrows that repo prewarm to one configured or `--role-git`-provided source. With `--image --role <selector>`, it resolves the configured or overridden role source, runs the same image recipe decision used by launch, reuses valid local labels, and builds missing/stale selected role image tags. With `--image --workspace <name>`, it prewarms the saved workspace's default role and default agent when one is configured, otherwise every supported image for that role. Both image paths avoid creating containers or touching host repos, host git config, shell config, `gh` config, or agent configs outside jackin-owned state.
- `jackin prewarm --sidecar` and every `jackin prewarm --image ...` path now reuse the launch path's `docker:dind` image constant and pull that sidecar image only when it is missing locally. Sidecar image lookup/pull starts alongside `jackin-capsule` and agent binary cache prewarm, then prints in a deterministic section before image prewarm output. These paths do not create sidecar containers, networks, cert volumes, workspaces, or host config, so explicit image prewarm prepares the fresh-start sidecar image without changing restart semantics.
...
- <RepoFile path="crates/jackin/src/cli/prewarm.rs" /> also supports `jackin prewarm --image --all-workspaces`, expanding explicit image prewarm across every saved workspace that declares a default role while preserving selected-default-agent narrowing for binary prewarm.
- <RepoFile path="crates/jackin/src/console/tui/input/list.rs" /> and <RepoFile path="crates/jackin/src/app/load_cmd.rs" /> now expose saved-workspace image prewarm from the workspace console with `W`. The action exits the TUI and dispatches through the existing `jackin prewarm --image --workspace <name>` implementation instead of adding a second prewarm path.
...
- Rebuild paths now emit a `build_context_snapshot` diagnostics event after creating the immutable Docker context. The event records file count and byte count, and `jackin diagnostics summary` / `compare` surface those values plus per-run cache decisions so real-world timing comparisons can separate context-copy cost from Docker build execution and warm-image reuse.
...
- Live and stopped current-role restore candidates now also bypass role repo refresh when the agent is already known, plus `git_pull_on_entry`; explicit blocking pulls still run for fresh creates and missing-container recreate repairs, but they no longer delay an already-valid hardline attach.
...
- <RepoFile path="crates/jackin-runtime/src/instance.rs" /> now exposes `RoleState::prepare_for_agents`, and the foreground launch path provisions auth state only for the selected runtime. A Claude launch no longer resolves or prepares sibling Codex, Amp, Kimi, OpenCode, or Grok auth slots before hardline; the existing full `RoleState::prepare` path remains available for callers that intentionally need every manifest-supported slot.
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch/launch_pipeline.rs" /> now starts non-blocking sibling-auth prewarm after selected-runtime auth preparation succeeds. The background task calls <RepoFile path="crates/jackin-runtime/src/instance.rs" /> `RoleState::prewarm_auth_for_agents`, skips the GitHub auth axis, writes only jackin-owned per-instance agent state, and emits `sibling_auth_prewarm_*` diagnostics without delaying hardline.
- <RepoFile path="crates/jackin-runtime/src/runtime/image.rs" /> now starts non-blocking sibling-image prewarm only after the selected image was reused. The background task revalidates the jackin-owned cached role repo under the normal role lock, reuses valid sibling image recipes, and builds missing or invalid sibling agent-scoped tags with fresh `ShellRunner`/`BollardDockerClient` handles so it never borrows the foreground mutable runner. When the selected image had to rebuild, sibling image work is skipped with a diagnostic instead of competing with the cold foreground launch.
- <RepoFile path="crates/jackin-runtime/src/runtime/image.rs" /> records nested timings for local image tag lookup, role SHA lookup, image recipe hashing, image label inspection, selected-agent binary checks, `jackin-capsule` lookup, build-context creation, GitHub token lookup, Docker build, and selected-agent version probing. It also emits `image_cache_hit`, `image_cache_miss`, and `image_build_source` diagnostics with the precise reuse, invalidation, published-base, or workspace-Dockerfile reason, so warm and cold runs explain why Docker build was skipped or which build source was selected.
...
- <RepoFile path="crates/jackin-diagnostics/src/summary.rs" /> and <RepoFile path="crates/jackin/src/cli/diagnostics.rs" /> add `jackin diagnostics summary <run-id|path>`, which prints stage durations, nested timings, Docker build steps, cache decisions, and startup duration through the first `hardline` stage event from one run artifact so cold/warm/restart launches can be inspected without hand-parsing JSONL or confusing attached-session time with startup time.
- <RepoFile path="crates/jackin/src/cli/diagnostics.rs" /> also adds `jackin diagnostics compare <run-id|path> <run-id|path>...`, which ranks broad stages and nested timings across multiple run artifacts by the slowest observed duration and compares startup duration, selected launch plans, build-context sizes, parsed Docker build steps, and cache decisions per run. This gives cold/warm/attach/restart comparisons a first-class command instead of making agents and operators build ad hoc spreadsheets from JSONL.
...
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> now copies all declared runtime hooks first and sets their executable bits in one `RUN chmod +x ...` instruction. Hook behavior and restart semantics stay the same, but rebuilds no longer add one extra Docker layer per hook.
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> also sets the runtime entrypoint and `jackin-capsule` executable bits in one Docker layer after copying both files, preserving the same `/jackin/runtime/` baked image contract while removing another cold-build layer.
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> now combines the shell-title shim append with `/jackin/run` and `/jackin/state` directory setup in one Docker layer. The shim remains image-baked, idempotent, and outside <RepoFile path="docker/runtime/entrypoint.sh" />; the build path just avoids an extra `RUN`.
- Claude marketplace/plugin installation now uses a BuildKit cache mount for `/home/agent/.cache` in the same image-build layer. Plugin state still bakes into the derived image and default-home snapshot; the cache only preserves transient downloader/tool caches across rebuilds.
...
Remaining Phase 3 work is to move background refresh decisions behind a stronger launch-plan model and add daemon prewarm surfaces beyond the CLI and saved-workspace console action. The broader item still needs plan-gated credential laziness for non-empty non-required credential graphs, recipe-keyed plugin bundle artifact caching, deeper build-path surgery, and warmed runtime resources.
...
The current implementation still has one structural blocker before this graph can be expressed directly: <RepoFile path="crates/jackin-core/src/runner.rs" /> defines `CommandRunner` with mutable `run` / `capture` / `capture_secret` methods, and <RepoFile path="crates/jackin-runtime/src/runtime/launch/launch_pipeline.rs" /> passes one mutable runner through image build, workspace materialization, role launch, and attach. DinD sidecar startup has moved to `DockerApi`, but starting the remaining runner-bound work in parallel still requires a prior runner-boundary change: either split the command executor into independent handles or move command recording behind a synchronized shared runner implementation. Without that boundary change, the apparent `tokio::join!` refactor would only fight Rust's borrow rules or serialize through the same mutable seam.
...
The current `build_agent_image()` still owns the late published-image staleness check and only short-circuits a stale published image to a fresh local workspace image in one path. It no longer prepares runtime binaries before the local-image reuse decision, and the foreground decision is now source-specific for `Reuse`, `BuildFromPublished`, and `BuildFromWorkspace`; the remaining deeper surgery is to move the published-image freshness decision fully into the pre-binary-prep `ImageDecision` step and add the non-blocking `RefreshInBackground` arm.
