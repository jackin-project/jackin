...
- <RepoFile path="crates/jackin-runtime/src/instance.rs" /> records nested timings for GitHub auth provisioning and each supported agent's role-state auth provisioning under `role_state_prepare:*_auth`, so the credential stage can identify whether GitHub, Claude, Codex, Amp, Kimi, OpenCode, or Grok state preparation is blocking a warm launch without logging credential values. GitHub auth and independent supported-agent auth slots now prepare concurrently and are merged back into the typed `ProvisionedAuth` structure deterministically, so a slow sibling auth file or `gh` lookup no longer serializes every other slot.
...
Remaining Phase 3 work is to move sibling-runtime preparation into background/prewarm flows and to move background refresh decisions behind a stronger launch-plan model. The broader item still needs plan-gated credential laziness for non-empty credential graphs, UID/GID remap removal, plugin bundle preparation, deeper build-path surgery, and prewarm.
