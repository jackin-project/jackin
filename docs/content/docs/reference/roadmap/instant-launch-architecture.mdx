...
- Fresh create-container launches now start the per-instance Docker network and `docker:dind` sidecar concurrently with workspace materialization. The role container still waits for both the ready sidecar and materialized mounts, and sidecar/materialization failures mark the instance `FailedSetup` and run the normal cleanup path so the overlap does not leave orphaned Docker resources.
