...
- <RepoFile path="crates/jackin-runtime/src/instance.rs" /> now defers creating the jackin-owned `.config/gh` role-state directory until GitHub provisioning actually has to run. `auth_forward = ignore` with no prior `hosts.yml` records `skipped_no_state` without creating empty GitHub config state, while stale `hosts.yml` still enters the wipe path.
