...
3. Resolves the role source, fetches/updates the cached role repository, validates the role manifest, and passes role/branch trust gates.
4. Selects the agent and resolves restore candidates. A live current-role candidate now attaches immediately through hardline before workspace `git_pull_on_entry`, credentials, image decisions, binary prep, or Docker build run. A stopped or created current-role candidate starts and reconnects through Capsule before the same expensive foreground work. A missing current-role container recreates in place with the existing name and uses the normal image decision, so a valid local image avoids rebuild work.
...
- Live and stopped current-role restore candidates now also bypass `git_pull_on_entry`; explicit blocking pulls still run for fresh creates and missing-container recreate repairs, but they no longer delay an already-valid hardline attach.
