...
- <RepoFile path="crates/jackin-runtime/src/runtime/image.rs" /> now starts non-blocking sibling-image prewarm after the selected image decision completes. The background task revalidates the jackin-owned cached role repo under the normal role lock, reuses valid sibling image recipes, and builds missing or invalid sibling agent-scoped tags with fresh `ShellRunner`/`BollardDockerClient` handles so it never borrows the foreground mutable runner or delays hardline.
