...
**Status**: Partially implemented — image recipe labels, warm local-image reuse, live current-instance attach-first, stopped/startable current-instance restore, missing-container recreation from valid images, and nested launch timing diagnostics shipped; credential laziness, selected-runtime foreground prep, build-path surgery, and prewarm remain open
