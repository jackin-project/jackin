...
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> no longer runs recursive `chown -R agent:agent /home/agent` unconditionally. The generated Dockerfile only repairs `/home/agent` ownership after a UID/GID remap or when `/home/agent` itself is owned by the wrong numeric user/group, removing a measured no-op layer from warm derived builds.

Remaining Phase 3 work is to move sibling-runtime binary and auth preparation into background/prewarm flows and to move background refresh decisions behind a stronger launch-plan model. The broader item still needs plan-gated credential laziness for non-empty non-required credential graphs, full UID/GID remap removal, recipe-keyed plugin bundle artifact caching, deeper build-path surgery, and prewarm.
