...
| Honor `--rebuild` (D1) | To do | In the `launch_pipeline.rs` early-restore gate, exclude `--rebuild` from the attach/start short-circuit: running → attach now plus a background image rebuild; stopped / created / missing → foreground rebuild and recreate through the normal `decide_agent_image` path (`ExplicitRebuild`) | New `launch_pipeline` tests covering `--rebuild` against running, stopped, and missing containers reaching the expected plan; manual `jackin load <role> --rebuild` against a warm running container rebuilds the image (diagnostics show `explicit_rebuild`) without dropping the session |
| Wire console `W` prewarm footer hint | To do | Set `show_prewarm` from the production workspace-list footer builder, not only in tests | `cargo nextest run -p jackin-console`; in `jackin console` the workspace list footer shows the `W` prewarm hint |
