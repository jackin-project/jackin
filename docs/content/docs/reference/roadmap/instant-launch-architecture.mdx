...
- <RepoFile path="crates/jackin-diagnostics/src/summary.rs" /> and <RepoFile path="crates/jackin/src/cli/diagnostics.rs" /> add `jackin diagnostics summary <run-id|path>`, which prints stage durations, nested timings, Docker build steps, and cache decisions from one run artifact so cold/warm/restart launches can be compared without hand-parsing JSONL.
