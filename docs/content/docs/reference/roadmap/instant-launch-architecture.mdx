...
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> now also folds the selected-runtime default-home snapshot into that same runtime finalization layer after the `COPY` instructions. Restart behavior stays image-baked under `/jackin/runtime/` and `/jackin/default-home`; cold builds avoid separate trailing Docker layers for runtime title setup and default-home capture.
