...
- After the selected foreground image path is known, <RepoFile path="crates/jackin-runtime/src/runtime/image.rs" /> starts a non-blocking sibling-runtime binary prewarm task. It writes only jackin-owned binary cache state, emits `runtime_prewarm_*` diagnostics, and does not block hardline or move agent setup into the entrypoint.
