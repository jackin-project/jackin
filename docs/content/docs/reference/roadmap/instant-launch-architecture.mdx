...
- Rebuild paths now emit a `build_context_snapshot` diagnostics event after creating the immutable Docker context. The event records file count and byte count so real-world timing comparisons can separate context-copy cost from Docker build execution.
