...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch/launch_pipeline.rs" /> now filters operator-env credential refs through the selected launch plan before resolving them: manifest-declared vars and the selected agent's required credential key still resolve, while non-required agent credential refs remain lazy and no longer probe `op` or host env during unrelated warm launches.
...
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> now copies all declared runtime hooks first, then folds hook directory setup, state ownership, hook executable bits, and the source-hook `.zshenv` shim into the shared runtime finalization `RUN`. Hook behavior and restart semantics stay the same, but rebuilds no longer add separate hook setup, chmod, or source-shim Docker layers.
...
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> now also folds entrypoint/Capsule executable-bit setup and the selected-runtime default-home snapshot into that same runtime finalization layer after the `COPY` instructions. Restart behavior stays image-baked under `/jackin/runtime/` and `/jackin/default-home`; cold builds avoid separate trailing Docker layers for runtime chmod/title setup and default-home capture.
...
Remaining Phase 3 work is to move background refresh decisions behind a stronger launch-plan model and add daemon prewarm surfaces beyond the CLI, explicit image prewarm, and saved-workspace console action. The broader item still needs deeper build-path surgery, daemon-maintained prewarm surfaces, and persistent warmed runtime resources.
