...
11. Runs `docker build`, then stores the selected agent version from prefetched release metadata. If the selected install used a fallback script or metadata is missing, it runs `docker run --rm --entrypoint <agent> <image> --version` and stores the parsed version.
...
- Derived builds stamp `jackin.image_recipe_version=v2`, `jackin.image_recipe_hash`, `jackin.selected_agent`, `jackin.role_git_sha`, `jackin.construct_image`, and `jackin.recipe.*` diagnostic labels onto the local image. The diagnostic labels cover role source ref, base image, generated runtime hash, supported-agent set, selected-agent install recipe, cache-bust value, `jackin-capsule` version, hook content hash, Claude plugin recipe hash, and host identity strategy.
...
Remaining Phase 3 work is to move sibling-runtime binary and auth preparation into background/prewarm flows and to move background refresh decisions behind a stronger launch-plan model. The broader item still needs plan-gated credential laziness for non-empty non-required credential graphs, recipe-keyed plugin bundle artifact caching, deeper build-path surgery, and prewarm.
