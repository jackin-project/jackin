...
The foreground decision now covers `Reuse`, `BuildFromPublished`, and `BuildFromWorkspace`, and stale published bases are rejected before runtime binary prep. Remaining deeper surgery is to add the non-blocking `RefreshInBackground` arm and make background-refresh inputs explicitly non-foreground.
