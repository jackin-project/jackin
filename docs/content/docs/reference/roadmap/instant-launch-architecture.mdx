...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> now records nested restore timings for current-role candidate lookup, each current container inspect, and related-candidate lookup. Attach/start/recreate decisions can therefore explain time spent before credentials, image prep, or Docker build.
