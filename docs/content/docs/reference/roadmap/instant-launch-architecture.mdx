...
5. Selects the agent and resolves restore candidates. A live current-role candidate now attaches immediately through hardline before credentials, image decisions, binary prep, or Docker build run. A stopped or created current-role candidate starts and reconnects through Capsule before the same expensive foreground work. A missing current-role container recreates in place with the existing name and uses the normal image decision, so a valid local image avoids rebuild work.
