...
**Status**: Partially implemented — image recipe `v2` labels with component-level invalidation reasons, warm local-image reuse, live current-instance attach-first, stopped/startable current-instance restore, missing-container recreation from valid images, launch-plan selection/rejection diagnostics, nested launch timing diagnostics including per-auth-slot role-state preparation and sidecar/Capsule readiness steps, parsed Docker build-step diagnostics, `jackin diagnostics summary`, `jackin diagnostics compare`, concurrent operator-env reads, concurrent GitHub env reads, concurrent GitHub/agent role-state auth preparation, and conditional `/home/agent` ownership repair shipped; broader credential laziness, selected-runtime foreground prep, deeper build-path surgery, and prewarm remain open
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch/launch_dind.rs" />, <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" />, and <RepoFile path="crates/jackin-runtime/src/runtime/attach.rs" /> now time Docker network creation, DinD `docker run`, DinD readiness polling, socket/config bind preparation, role `docker run`, pre-attach exit inspection, stopped-container restore inspection/start, and Capsule socket readiness. The broad `sidecar` and `capsule` stages can now explain which runtime-start boundary consumed a warm launch.
...
The current implementation has one structural blocker before this graph can be expressed directly: <RepoFile path="crates/jackin-core/src/runner.rs" /> defines `CommandRunner` with mutable `run` / `capture` / `capture_secret` methods, and <RepoFile path="crates/jackin-runtime/src/runtime/launch/launch_pipeline.rs" /> passes one mutable runner through image build, workspace materialization, DinD startup, and role launch. Starting DinD in parallel with workspace materialization or Docker build therefore requires a prior runner-boundary change: either split the command executor into independent handles or move command recording behind a synchronized shared runner implementation. Without that boundary change, the apparent `tokio::join!` refactor would only fight Rust's borrow rules or serialize through the same mutable seam.
