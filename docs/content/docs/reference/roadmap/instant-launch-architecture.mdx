...
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> now also folds entrypoint/Capsule executable-bit setup into that same runtime finalization layer after the `COPY` instructions. Restart behavior stays image-baked under `/jackin/runtime/` and `/jackin/default-home`; cold builds avoid another trailing Docker layer.
