...
10. Creates a temporary derived build context in <RepoFile path="crates/jackin-image/src/derived_image.rs" /> by copying the role repo without `.git` internals and writing `.jackin-runtime/DerivedDockerfile` for rebuild paths.
...
- <RepoFile path="crates/jackin-image/src/derived_image.rs" /> now excludes `.git` directories from the temporary derived Docker context copy. Rebuild paths still use the validated role files and generated `.jackin-runtime` assets, but avoid copying repository object databases into jackin-owned build contexts.
