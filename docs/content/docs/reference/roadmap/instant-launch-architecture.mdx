...
**Status**: Partially implemented — image recipe labels, warm local-image reuse, live current-instance attach-first, stopped/startable current-instance restore, and nested launch timing diagnostics shipped; credential laziness, selected-runtime foreground prep, build-path surgery, missing-container recreation, and prewarm remain open
...
5. Selects the agent and resolves restore candidates. A live current-role candidate now attaches immediately through hardline before credentials, image decisions, binary prep, or Docker build run. A stopped or created current-role candidate starts and reconnects through Capsule before the same expensive foreground work.
...
Remaining Phase 3 work is to split selected-agent-only prep from sibling-runtime prep and to move background refresh decisions behind a stronger launch-plan model. The broader item still needs missing-container recreation from a valid image, lazy/parallel credential resolution, UID/GID remap removal, plugin bundle preparation, and prewarm.
