...
**Status**: Partially implemented — image recipe labels, warm local-image reuse, live current-instance attach-first, stopped/startable current-instance restore, missing-container recreation from valid images, nested launch timing diagnostics, parsed Docker build-step diagnostics, concurrent operator-env reads, and concurrent GitHub env reads shipped; broader credential laziness, selected-runtime foreground prep, build-path surgery, and prewarm remain open
