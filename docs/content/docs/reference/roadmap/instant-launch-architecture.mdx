...
**Status**: Partially implemented — image recipe labels, warm local-image reuse, live current-instance attach-first, stopped/startable current-instance restore, missing-container recreation from valid images, nested launch timing diagnostics, and concurrent operator-env reads shipped; broader credential laziness, selected-runtime foreground prep, build-path surgery, and prewarm remain open
...
|---|---:|---|
| Credentials black box | 55.5s | The baseline run predated nested credential timings. Code inspection showed <RepoFile path="crates/jackin-env/src/resolve.rs" /> probing `op` once, then resolving attributed values sequentially; each `op://` read went through <RepoFile path="crates/jackin-env/src/op_runner.rs" /> / <RepoFile path="crates/jackin-env/src/op_cli.rs" /> with a 30s default timeout. The current implementation times each operator-env key and resolves independent operator-env entries concurrently, but the broader credential stage still needs laziness and plan-gating so attach plans do not resolve fresh secrets at all. |
