...
**Not yet implemented — the interactive operator credential picker.** The capsule daemon's `ExecCommand` handler currently **fails closed**: every `jackin-exec` call returns `ExecDenied` ("credential picker is not yet available") because the `Dialog::ExecPicker` dialog and its daemon event-loop integration are not built. No credential is ever resolved or injected without operator approval. Remaining work: the picker dialog + event-loop wiring, temp-file materialization for key-type credentials, and end-to-end behavioral coverage.
