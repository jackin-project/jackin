...
- <RepoFile path="src/workspace/mounts/mod.rs" /> — parser for the new
  modes; rejects `src` for `tmpfs`/`ephemeral`
- <RepoFile path="src/runtime/launch/mod.rs" /> — Docker arg construction
- <RepoFile path="src/runtime/cleanup/mod.rs" /> — volume cleanup on
  `purge`
- <RepoFile path="src/manifest/mod.rs" /> — `[[runtime.mounts]]`
  role-level defaults
- <RepoFile path="src/cli/workspace/mod.rs" /> — `--mount-isolation tmpfs`
