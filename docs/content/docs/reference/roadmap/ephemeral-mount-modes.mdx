...
- <RepoFile path="crates/jackin/src/workspace.rs" /> — `MountConfig`,
  `MountIsolation` enum extension
- <RepoFile path="crates/jackin-config/src/mounts.rs" /> — parser for the new
  modes; rejects `src` for `tmpfs`/`ephemeral`
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> — Docker arg construction
- <RepoFile path="crates/jackin-runtime/src/runtime/cleanup.rs" /> — volume cleanup on
  `purge`
- <RepoFile path="crates/jackin-manifest/src/manifest.rs" /> — `[[runtime.mounts]]`
  role-level defaults
- <RepoFile path="crates/jackin/src/cli/workspace.rs" /> — `--mount-isolation tmpfs`
