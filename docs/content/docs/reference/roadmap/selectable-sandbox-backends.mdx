...
2. [smolvm backend research](/reference/roadmap/smolvm-backend-research/) — evaluate an open Rust/libkrun VM backend for true VM-per-workload isolation, OCI-image execution, and potential SDK integration; compare it directly with Docker Sandboxes. This is now the primary VM isolation track.
3. **Apple Container framework evaluation** (macOS 26 Tahoe) — each OCI container runs in its own lightweight VM via Virtualization.framework on Apple Silicon, giving a true per-container kernel boundary. Evaluate once operator macOS 26 adoption reaches a usable threshold; no dedicated roadmap item yet.
4. Kubernetes platform support — future work, not part of the current Docker hardening or smolvm design pass.

**[OrbStack isolated machine backend](/reference/roadmap/orbstack-isolated-machine-backend/) is deferred.** Research in June 2026 established that OrbStack isolated machines share OrbStack's Linux kernel — they add namespace isolation, not a hypervisor boundary — and that for the current macOS ARM operator audience (all of whom already use OrbStack as their Docker engine), the role container is already inside OrbStack's Linux VM. An isolated machine adds a namespace layer within the same kernel, which does not address the kernel escape risk that motivates microVM adoption. The OrbStack item retains its historical research as reference material.
...
| Suspicious repository | Let an agent inspect/build unknown code without giving it broad host access. | Docker `hardened` first; smolvm if Docker controls are insufficient and smolvm compatibility is proven. | The first wins are network deny/allowlist, resource caps, read-only root, reduced credentials, and no unnecessary DinD. |
...
| Shared workstation or team laptop | Keep one operator's agent from reaching local services, broad files, or shared Docker state. | smolvm (own kernel per workload, network off by default) or Docker Sandboxes-style controls as benchmark. OrbStack isolated machines are deferred — they share OrbStack's Linux kernel and do not provide a new kernel boundary. | The key need is a true VM boundary and private Docker daemon, not only stricter namespace flags. |
...
Docker container names cannot remain the source of truth once multiple backend families exist. Today (May 2026) per-instance metadata lives in <RepoFile path="src/instance/manifest.rs" /> as `InstanceManifest` (schema v1), with a Docker-specific `DockerResources { role_container, dind_container, network, certs_volume }` field. The registry shape needs to evolve to:
...
Multi-backend changes what `jackin console` shows per instance. Today the per-instance details pane at <RepoFile path="src/console/manager/render/list.rs" /> renders Docker-specific state (container name, network, DinD). The pane needs a backend-aware variant:
