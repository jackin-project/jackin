...
Multi-backend changes what `jackin console` shows per instance. Today the per-instance details pane at <RepoFile path="src/console/tui/render/list.rs" /> renders Docker-specific state (container name, network, DinD). The pane needs a backend-aware variant:
