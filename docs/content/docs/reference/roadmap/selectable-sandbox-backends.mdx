...
Docker container names cannot remain the source of truth once multiple backend families exist. Today (May 2026) per-instance metadata lives in <RepoFile path="crates/jackin-runtime/src/instance/manifest.rs" /> as `InstanceManifest` (schema v1), with a Docker-specific `DockerResources { role_container, dind_container, network, certs_volume }` field. The registry shape needs to evolve to:
...
Multi-backend changes what `jackin console` shows per instance. Today the per-instance details pane at <RepoFile path="crates/jackin/src/console/tui/view/list.rs" /> renders Docker-specific state (container name, network, DinD). The pane needs a backend-aware variant:
...
- <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> - current backend-specific launch orchestration.
- <RepoFile path="crates/jackin-runtime/src/runtime/image.rs" /> - role image build path that should stay reusable across backends.
- <RepoFile path="crates/jackin-docker/src/docker_client.rs" /> - Docker backend API boundary.
- <RepoFile path="crates/jackin-runtime/src/instance/manifest.rs" /> - instance metadata that needs to become backend-neutral.
