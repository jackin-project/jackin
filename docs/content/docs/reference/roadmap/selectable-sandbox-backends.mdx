...
Multi-backend changes what `jackin console` shows per instance. Today the per-instance details pane at <RepoFile path="crates/jackin/src/console/tui/view/list.rs">crates/jackin/src/console/tui/view/list.rs</RepoFile> renders Docker-specific state (container name, network, DinD). The pane needs a backend-aware variant:
