...
- **Image (build once, UID-agnostic):**
  - `/home/agent` is normalized to **group 0** with **group == owner** permissions (`chgrp -R 0 /home/agent && chmod -R g=u /home/agent`) as the final layer touching the home, so any UID running in group 0 can use the baked home. See <RepoFile path="crates/jackin-image/src/derived_image.rs">derived_image.rs</RepoFile>.
