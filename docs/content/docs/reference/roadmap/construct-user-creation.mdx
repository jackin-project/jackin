...
- **Image (build once, UID/GID-agnostic):**
  - `/home/agent` and baked home/config paths are born **group 0** with group-readable/writable permissions: the construct image creates `agent` with primary group 0, makes `/home/agent` group-writable, applies `umask 0002` to build `RUN` steps, and generated role-image `COPY` instructions use `--chown=agent:0`. The derived image no longer runs a recursive `chgrp/chmod` finalization layer; instead it explicitly creates the mutable runtime home roots (`.cache`, `.config`, `.local/bin`, `.local/share`, `.local/state`) as `agent:0` `0775` and fails the build if any expected mutable root is not group-writable. See <RepoFile path="docker/construct/Dockerfile">construct/Dockerfile</RepoFile> and <RepoFile path="crates/jackin-image/src/derived_image.rs">derived_image.rs</RepoFile>.
