...
repo under `docs/` (entry point: <RepoFile path="docs/source.config.ts" />, with sidebar metadata in `docs/content/docs/**/meta.json`).
...
- A behaviour change in `src/cli/**` ships with the matching update to
  `docs/content/docs/commands/<cmd>.mdx` in the same commit.
...
- [split workspace-file layout](/reference/configuration/#one-file-per-workspace-by-design) — orthogonal, but in the same "configuration / workflow ergonomics" cluster.
- [Developer Reference Fumadocs section](/reference/roadmap/developer-reference-setup/) — also a docs-discoverability item, since both depend on docs being easy to find and edit.
