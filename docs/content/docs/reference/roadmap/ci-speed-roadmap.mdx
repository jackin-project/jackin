...
5. Done in PR #632 for release builds: extend the same `sccache` wrapper, GitHub cache runtime export, target-specific release namespaces, step-summary stats, and short-retention stats artifacts to release archive builds. Keep `clippy`/`check-default`/nextest prepare as the next measured expansion point, and do not remove `Swatinem/rust-cache` until measured data shows `sccache` plus cargo registry/tool caches is faster and less cache-heavy. `rust-cache` still handles dependency downloads, target reuse, and same-run package fan-out well.
...
3. Partly done upstream in Velnor: opt-in persistent `target/` stores are now scoped under `_velnor_targets/<repo>/<workflow>/<job-bucket>`, so warm build state cannot cross repository, workflow, or job-class boundaries. Finish the security model before trusting the optional Velnor lane for parity evidence: fork PRs must not reach secrets or a shared Docker daemon, runner pools still need per-trust-level isolation, and resource caps must keep one run from starving others. These are separate security decisions, not speed work.
...
8. When a job needs something velnor does not yet provide, improve `velnor` itself rather than forking the pipeline: give it a Docker daemon for `docker-e2e` and construct builds, an arm path (or keep the native-arm construct leg hosted-only until it has one), and lane-awareness for `build-validator`/`construct-e2e-image`/preview/release through the same `matrix-setup` mechanism. The warm-state hygiene model for persistent `target/` buckets has started upstream with repository/workflow/job-class scoping; keep extending those runner capabilities, then re-verify on both lanes.
