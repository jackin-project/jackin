...
Mozilla `sccache` caches compiler outputs through `RUSTC_WRAPPER` and supports local plus remote/GHA-backed storage. Its Rust guidance requires incremental compilation to be disabled for cacheability, which already matches the project's compile-heavy jobs. PR #632 pilots host `sccache` on preview and release archive builds where release-profile rebuild time dominates, emits `sccache --show-stats`, and keeps `Swatinem/rust-cache` for registry/index/target reuse until data says otherwise.
