...
3. Partly done upstream in Velnor: opt-in persistent `target/` stores are now scoped under `_velnor_targets/<repo>/<workflow>/<job-bucket>`, so warm build state cannot cross repository, workflow, or job-class boundaries, and Velnor job containers now receive daemon-level CPU and memory caps. Finish the security model before trusting the optional Velnor lane for parity evidence: fork PRs must not reach secrets or a shared Docker daemon, and runner pools still need per-trust-level isolation. These are separate security decisions, not speed work.
