...
|---|---|---|
| Role container `docker run` flag assembly | <RepoFile path="src/runtime/launch.rs" /> | No `--cap-add` / `--cap-drop`, no `--security-opt seccomp=`, no `--security-opt apparmor=`, no `--security-opt no-new-privileges`, no `--read-only`, no `--tmpfs`, no `--pids-limit`, no `--memory*`. Docker defaults apply implicitly. |
| DinD sidecar start | <RepoFile path="src/runtime/launch.rs" /> | `--privileged`. TLS cert dir at `DOCKER_TLS_CERTDIR=/certs` with `DOCKER_TLS_SAN=DNS:<dind-name>`. Cert volume bound per launch. No rotation. |
| Workspace mount string builder | <RepoFile path="src/runtime/launch.rs" /> | Translates `MaterializedWorkspace` to `-v src:dst[:ro]`. `:ro` already enforced at the kernel layer. |
