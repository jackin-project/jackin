...
| Should `asciinema/avt` replace `vt100`? | Not now. `avt` is attractive for offline replay and dump/diff APIs, but the jackin' live path depends on OSC/unhandled-CSI callback behavior. Revisit as part of the VT parser dependency strategy. |
...
- [TUI Design Decisions](/reference/tui//) and [snapshot tests for TUI](/reference/roadmap/snapshot-tests-tui/) — renderer/golden-test work that should converge with the terminal-frame artifact model.
