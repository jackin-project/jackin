...
**Status**: Partially implemented — four `insta` snapshot tests (list, settings, editor-general, editor-mounts) land in <RepoFile path="src/console/tui/render/snapshot_tests.rs" /> with committed `.snap` files; additional coverage (global mounts, auth tab, PR context) follows as render work proceeds.
...
- <RepoFile path="src/console/tui/render/editor.rs" /> is now **3231L** (was ~1682L — nearly doubled)
- <RepoFile path="src/console/tui/render/list.rs" /> is now **2816L** (was ~2069L)
- <RepoFile path="src/console/tui/render/mod.rs" /> is now **1165L** (was ~500L)
- <RepoFile path="src/console/tui/render/global_mounts.rs" /> is now **839L** (new sub-module)
...
|---|---|---|
| `render_sentinel_description_pane` | <RepoFile path="src/console/tui/render/list.rs" /> | Static "+ New workspace" panel. Zero state. 80×10 terminal. |
| `render_tab_strip` | <RepoFile path="src/console/tui/render/editor.rs" /> | Tab bar with `EditorTab` active. 4 variants × 1 snapshot each. 80×3 terminal. |
| `render_mounts_subpanel` | <RepoFile path="src/console/tui/render/list.rs" /> | Mount list subpanel. 3 cases: empty, 1 mount, 3 mounts. 60×20 terminal. |

**Additional high-value targets identified in re-analysis:**

| Function | File | What to snapshot |
|---|---|---|
| Global mounts editor rendering | <RepoFile path="src/console/tui/render/global_mounts.rs" /> | Per-subtab renders. New 839L file with no snapshot coverage. |
| Per-tab editor body renders | <RepoFile path="src/console/tui/render/editor.rs" /> | At minimum: General, Mounts, Agents, Secrets tab body renders. |
| Workspace list renders | <RepoFile path="src/console/tui/render/list.rs" /> | Active workspace detail panel with various mount configurations. |
