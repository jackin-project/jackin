...
**Status**: Partially implemented — four `insta` snapshot tests (list, settings, editor-general, editor-mounts) land in <RepoFile path="src/console/manager/render/snapshot_tests.rs" /> with committed `.snap` files; additional coverage (global mounts, auth tab, PR context) follows as render work proceeds.
