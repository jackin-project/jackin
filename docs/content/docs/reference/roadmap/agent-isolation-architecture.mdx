...
**smolvm**: uses Hypervisor.framework on macOS and libkrun on Linux. Pre-1.0. Has a documented Docker-in-VM path (v0.7.0, requires Alpine base + ext4 bind mount + virtio-net). See [smolvm backend research](/reference/roadmap/smolvm-backend-research/).
...
|---|---|
| [Docker runtime hardening contract](/reference/roadmap/docker-runtime-hardening-contract/) | Layer 1 — prerequisite for Apple Container (rootless DinD). Also independently valuable for current Docker backend operators. |
| [smolvm backend research](/reference/roadmap/smolvm-backend-research/) | Contains the Apple Container detailed comparison + Phase 0 plan. Also the fallback if Apple Container fails Docker-inside validation. |
| [Process-level sandboxing](/reference/roadmap/process-level-sandboxing/) | Cross-layer — covers both zerobox per-operation sandboxing and the jackin-exec secure execution wrapper design. |
...
| [Container credential exposure](/reference/roadmap/container-credential-exposure/) | The problem that jackin-exec (Layer 3) solves. Documents the trajectory from current env injection to jackin-exec credential isolation. |
| [Host bridge — secrets and approved host actions](/reference/roadmap/host-bridge/) | The host-side resolver that jackin-exec calls to look up credentials from the operator's vault or keychain. |
| [Session contract and explain mode](/reference/roadmap/session-contract-explain-mode/) | How the operator sees which layers are active, what each boundary enforces, and what residual risks remain. |
| [Selectable sandbox backends](/reference/roadmap/selectable-sandbox-backends/) | Umbrella for the backend selection model. Apple Container and smolvm are the active candidates. |
| [OrbStack isolated machine backend](/reference/roadmap/orbstack-isolated-machine-backend/) | Deferred — shared-kernel architecture does not address the kernel escape risk. Historical research preserved. |
