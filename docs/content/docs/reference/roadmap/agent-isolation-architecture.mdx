...
The answer: a wrapper binary (`jackin-exec`) installed in the construct image. The agent calls `jackin-exec ssh sentry` instead of `ssh sentry` directly. jackin-capsule intercepts, resolves the SSH key from the operator's host vault at execution time, executes the command with the key injected as a temporary file under `/jackin/run/`, returns sanitized output, and deletes the key. The raw credential value never enters the agent's context.
