...
**Conclusion**: [OrbStack isolated machine backend](/reference/roadmap/orbstack-isolated-machine-backend/) deferred. Namespace isolation within a shared kernel does not address the kernel escape risk that motivates VM-level separation.
...
**Two env tiers:**
- `env.*` (always-available): injected into container at launch, all processes see them — used for non-sensitive config
- `secure_exec.*` (protected): NOT in container env, NOT visible via `printenv`, resolved by the host at launch and stored capsule-private, materialized only during `jackin-capsule exec` invocations

**`jackin-exec` = `jackin-capsule exec`**: a symlink to the capsule binary, not a separate artifact. The agent calls `jackin-exec ssh sentry`; the capsule subcommand injects the SSH key as a temp file under `/jackin/run/secrets/`, runs the command, scans output for secret patterns, deletes the key, and returns sanitized output. The raw credential value never appears in the agent's process environment.

This is the same architecture Docker Sandboxes uses for its host-side credential proxy — except jackin' builds it explicitly, with a TUI-native operator approval dialog, instead of relying on Docker's opaque implementation.
...
See: [Docker runtime hardening contract](/reference/roadmap/docker-runtime-hardening-contract/)
...
See: [smolvm backend research](/reference/roadmap/smolvm-backend-research/) (contains the Apple Container comparison and Phase 0 plan)
...
See: [Process-level sandboxing](/reference/roadmap/process-level-sandboxing/) (jackin-exec design), [Container credential exposure](/reference/roadmap/container-credential-exposure/), [Host bridge](/reference/roadmap/host-bridge/)
...
See: [Process-level sandboxing](/reference/roadmap/process-level-sandboxing/), [Network egress policy](/reference/roadmap/network-egress-policy/)
...
If rootless DinD fails inside apple/container: smolvm is the fallback (documented Docker-in-VM path exists, though constrained). See [smolvm backend research](/reference/roadmap/smolvm-backend-research/) for the full fallback plan.
...
3. **Approval UX for high-frequency sessions**: "Approve All Similar" and "Always for Session" modes need TUI design — see [TUI design decisions](/reference/tui-design-decisions/)
...
4. Network egress policy (after Apple Container or smolvm backend lands)
   ...
   └─ See: [Network egress policy](/reference/roadmap/network-egress-policy/)
...
5. zerobox per-operation sandboxing (after jackin-exec, parallel with egress)
   ...
   └─ See: [Process-level sandboxing](/reference/roadmap/process-level-sandboxing/)
```
...
- <RepoFile path="crates/jackin-capsule/src/main.rs" /> — PID-1 detection (`process::id() == 1`); must add `JACKIN_CAPSULE_FORCE_DAEMON` env var check for Apple Container
- <RepoFile path="crates/jackin-capsule/src/daemon.rs" /> — daemon control plane; add jackin-exec message type here
- <RepoFile path="src/instance/manifest.rs" /> — `InstanceManifest` + `DockerResources`; add `BackendResources` tagged union with `AppleContainerResources` variant
- <RepoFile path="src/runtime/launch.rs" /> — launch orchestration; backend split point
- <RepoFile path="src/docker_client.rs" /> — Docker lifecycle boundary; Apple Container gets a peer module
...
- <RepoFile path="src/config/mod.rs" /> — `AppConfig` + `WorkspaceConfig`; `WorkspaceConfig` gains `secure_exec` field (jackin-exec)

## Related Roadmap Items

| Item | Role in this architecture |
|---|---|
| [Docker runtime hardening contract](/reference/roadmap/docker-runtime-hardening-contract/) | Layer 1 — prerequisite for Apple Container (rootless DinD must validate). Independently valuable for current Docker backend operators. |
| [Apple Container backend](/reference/roadmap/apple-container-backend/) | Layer 2 — dedicated item with Phase 0-4, `AppleContainerResources` struct, schema migration, `JACKIN_CAPSULE_FORCE_DAEMON` code task. |
| [jackin-exec](/reference/roadmap/jackin-exec/) | Layer 3 — dedicated item with workspace secret bindings schema migration, implementation phases, MCP tool spec, operator approval UI. |
| [Process-level sandboxing](/reference/roadmap/process-level-sandboxing/) | Cross-layer — zerobox per-operation filesystem/network sandboxing only (jackin-exec is separate). |
| [Network egress policy](/reference/roadmap/network-egress-policy/) | Layer 4 — session-level outbound policy, enforced at VM egress when Apple Container backend lands. |
| [Container credential exposure](/reference/roadmap/container-credential-exposure/) | The problem that jackin-exec (Layer 3) solves. |
| [Host bridge — secrets and approved host actions](/reference/roadmap/host-bridge/) | Host-side resolver that jackin-exec calls for `op://` credentials. |
| [Session contract and explain mode](/reference/roadmap/session-contract-explain-mode/) | How the operator sees which layers are active and what residual risks remain. |
| [Selectable sandbox backends](/reference/roadmap/selectable-sandbox-backends/) | Umbrella for Docker and Apple Container as the two runtime families; backend selection model, instance registry design. |
| [smolvm backend research](/reference/roadmap/smolvm-backend-research/) | Deferred — fallback if Apple Container Phase 0 fails Docker-inside test. |
| [OrbStack isolated machine backend](/reference/roadmap/orbstack-isolated-machine-backend/) | Deferred — shared-kernel namespace, not hypervisor boundary. Historical research preserved. |
