...
Env-pilot guard (`cargo xtask lint arch` → `check_curated_pub_mods`) allowlists root `pub mod`s for curated crates (`jackin-env` → `test_support` only). `jackin-core::tui_theme` is an intentional namespace because its Ratatui `Color` adapters share names with the product-owned RGB tokens re-exported at the crate root; keeping the adapters namespaced makes the representation explicit at call sites. Full `jackin-config` / `jackin-core` root narrowing is STOP-deferred (import blast radius); the ratchet bounds the surface until module-cluster slices land.
