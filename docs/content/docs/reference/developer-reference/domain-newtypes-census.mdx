...
| `SessionId` | `jackin_core::SessionId` | core | **Adopted** — non-zero `u64`; designated daemon/session-supervisor and attach/control boundaries use the newtype, with raw `u64` retained only at the schema-preserving protocol edge. |
| `ContainerId` | `jackin_core::ContainerId` | core | **Adopted** — non-empty name without whitespace/path separators; designated runtime/docker instance boundaries use the newtype, converting to `&str` only at Docker CLI/wire edges. |
