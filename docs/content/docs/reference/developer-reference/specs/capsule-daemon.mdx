...
| INV | Description | Verify by | Tests |
|---|---|---|---|
| INV-D19 | Last-session exit handler classifies whether the daemon should exit | `handle_last_session_exit` | MISSING (behavior covered indirectly by integration paths) |
| INV-D20 | File export paths are categorized as jackin-run / jackin-owned / container-absolute / relative via `container_paths` | `requested_export_path_category` in `daemon/file_export.rs` | MISSING |
...
5. **INV-D19 / INV-D20** — cleanup classification and file-export category matrix need direct tests.
