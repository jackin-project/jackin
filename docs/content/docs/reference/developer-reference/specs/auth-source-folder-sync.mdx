...
|---|---|---|---|
| INV-1 | `sync_source_dir` is a direct agent credential/config directory, not a home directory that gets agent defaults appended under it | Unit tests for Codex, Amp, Kimi, OpenCode, and Grok copy credentials directly from `source_dir/<credential-file>` or the selected Kimi directory | `jackin_instance::auth::tests::sync_source_dir_copies_direct_auth_json` `jackin_instance::auth::tests::sync_source_dir_copies_direct_secrets_json` `jackin_instance::auth::tests::sync_source_dir_copies_direct_kimi_dir` |
| INV-2 | Claude Code source-folder sync copies `.claude.json` from the selected `CLAUDE_CONFIG_DIR` | A regression test prepares Claude state from a source folder containing `.claude.json` and confirms the role-state account file contains that metadata | `jackin_instance::auth::tests::sync_source_dir_copies_claude_config_dir_without_nested_home_layout` |
| INV-3 | Claude Code credential extraction for an explicit source folder reads only that folder (its `.credentials.json`, else its per-config-dir macOS Keychain entry) and never falls back to the default host credentials | A regression test prepares Claude state from a source folder with no credentials while default host credentials exist, and confirms the role-state `credentials.json` is not the default account (`HostMissing`, not `Synced`) | `jackin_instance::auth::tests::sync_source_dir_does_not_fall_back_to_default_host_credentials` `jackin_instance::auth::tests::sync_source_dir_uses_source_folder_own_credentials` |
| INV-4 | Amp source-folder sync reads `secrets.json` from the selected Amp data directory | A regression test calls Amp source-folder provisioning with `source_dir/secrets.json` and confirms the role-state secret is mounted | `jackin_instance::auth::tests::sync_source_dir_copies_direct_secrets_json` |
| INV-5 | Runtime launch mounts prepared auth state only when preparation produced or preserved a credential handoff file | Launch mount tests assert Claude, Codex, and Amp include auth mounts under synced states and omit them under ignored/missing states | `jackin_runtime::runtime::launch::tests::agent_mounts_for_claude_sync_mode_forwards_auth_files` `jackin_runtime::runtime::launch::tests::agent_mounts_for_claude_ignore_mode_mounts_state_but_no_auth_handoff` `jackin_runtime::runtime::launch::tests::agent_mounts_for_codex_host_missing_omits_auth_json` |
| INV-6 | Source-folder resolution preserves the three-layer account-selection scope: workspace-role override, then workspace, then global | Resolver tests prove role overrides win over workspace/global, and launch-boundary tests prove the same resolver shape is used per agent and per role before `RoleState::prepare` | `jackin_runtime::runtime::launch::tests::sync_source_resolution_uses_workspace_role_scope_per_agent` |
| INV-7 | The Source Folder picker validates a candidate folder against the selected agent's credential structure and rejects a wrong folder inline instead of saving it | `validate_sync_source_dir` unit tests cover every sync agent (valid, empty, and wrong-folder cases); the picker commit handler surfaces the rejection reason and keeps the picker open | `jackin_instance::auth::tests::validate_claude_accepts_file_credentials_rejects_bare_folder` `jackin_instance::auth::tests::validate_single_file_agents` `jackin_instance::auth::tests::validate_kimi_requires_config_and_credentials_tree` |
| INV-8 | When a role sets an agent's standard config-dir env var (`CLAUDE_CONFIG_DIR`, `CODEX_HOME`, `XDG_DATA_HOME`), `runtime-setup` writes the forwarded credential and config files to the directory that env var selects inside the container, not the fixed default — otherwise the CLI reads a path jackin never wrote to and falls back to interactive login | `runtime_setup` resolver tests assert the `*_from` cores compose the env-selected path (e.g. `CLAUDE_CONFIG_DIR=/home/agent/.claude-work` ⇒ both `.claude.json` and `.credentials.json` land inside that dir) and the default path when unset | `jackin_capsule::runtime_setup::tests::claude_paths_follow_config_dir_when_set` `jackin_capsule::runtime_setup::tests::claude_paths_default_when_config_dir_unset` `jackin_capsule::runtime_setup::tests::codex_home_honors_env_else_defaults` |
...
## Container-side config-dir resolution

The host-side rules above decide *which folder jackin reads credentials from*. A symmetric rule governs *where jackin writes those credentials inside the container*. The agent CLIs honor the same standard config-dir env vars at runtime, so when a role exports one, jackin must write to the directory it selects rather than the fixed default — the CLI reads the env-selected path, and a credential written anywhere else is invisible to it.

`runtime-setup` resolves each in-container path through the env var the agent itself respects, falling back to the default when unset:

| Agent | Env var honored | Credential / config target |
|---|---|---|
| Claude Code | `CLAUDE_CONFIG_DIR` | `.credentials.json` inside the dir; `.claude.json` inside the dir when the var is set, else `$HOME/.claude.json` (the CLI's own asymmetry) |
| Codex | `CODEX_HOME` | `auth.json` and `config.toml` inside the dir |
| Amp | `XDG_DATA_HOME` | `$XDG_DATA_HOME/amp/secrets.json` |
| OpenCode | `XDG_DATA_HOME` | `$XDG_DATA_HOME/opencode/auth.json` |
| Kimi / Grok | none | fixed `~/.kimi-code` / `~/.grok/auth.json` |

Roles should set these env vars only for genuinely custom multi-profile needs; the defaults already match each CLI's standard layout, so an unnecessary override only adds a path to keep aligned (and previously broke Claude auth by relocating `.claude.json` lookup away from where jackin wrote it).
