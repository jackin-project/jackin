...
**Summary**: adds optional global and per-workspace Docker security profile/grants config. This is additive with serde defaults, so existing files round-trip cleanly and the migration is a no-op restamp.

**Example with explicit policy**:

```toml
version = "v1alpha7"
dirty_exit_policy = "keep"
```
...
| File kind | Predecessor | Migration mode | Fixture |
|---|---|---|---|
| Config (`config.toml`) | `v1alpha6` | Automatic on startup | <RepoFile path="crates/jackin/tests/fixtures/migrations/config/from-v1alpha6/meta.toml" /> |
| Workspace (`workspaces/<name>.toml`) | `v1alpha6` | Automatic on startup | <RepoFile path="crates/jackin/tests/fixtures/migrations/workspace/from-v1alpha6/meta.toml" /> |

**Summary**: introduces `[docker].profile` (optional string: `locked`, `hardened`, `standard`, `compat`) and `[docker].grants` (optional table with per-dimension capability overrides) on both `config.toml` and workspace files. Both fields are additive with serde defaults — absent means no profile override / no grants. The migration is a no-op restamp for both file kinds.
