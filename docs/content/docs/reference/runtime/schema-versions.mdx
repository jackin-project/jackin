...
|---|---|---|---|
| `~/.config/jackin/config.toml` | `v1alpha9` | jackin | Automatic on startup |
| `~/.config/jackin/workspaces/<name>.toml` | `v1alpha8` | jackin | Automatic on startup |
| `jackin.role.toml` | `v1alpha6` | Role repository | Desktop: `jackin role migrate <role-repo-path>`; CI/automation: `jackin-role migrate <role-repo-path>` |
...
| `[<agent>.providers.<id>].model` | string | No | `v1alpha5` | Per-provider model override for `[claude]`, `[codex]`, and `[opencode]`; map defaults to empty |
