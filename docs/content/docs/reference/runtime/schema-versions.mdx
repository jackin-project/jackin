...
**After** (v1alpha7 — no change needed; dirty_exit_policy defaults to None):
...
| `dirty_exit_policy` | string | No | `v1alpha7` | `ask`, `keep`, or `discard`; global dirty/unpushed exit policy; omitted means built-in `ask` |
...
| `dirty_exit_policy` | string | No | `v1alpha7` | `ask`, `keep`, or `discard`; workspace override for dirty/unpushed exit policy; omitted means inherit from global config |
