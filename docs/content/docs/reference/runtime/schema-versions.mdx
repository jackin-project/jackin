...
**Summary**: adds the optional `[runtime] default_backend = "docker"` field to `config.toml`, the optional `[runtime] backend = "apple-container"` field to per-workspace files, and on-demand env metadata for `jackin-exec`. `OpRef` values now accept `on_demand = true`, and env values may use `{ value = "...", on_demand = true }` for literal or host-env sources that should be resolved only through `jackin-exec`. All fields are additive and serde-defaulted, so the migration is a no-op restamp.
...
**After** (v1alpha7 config — no change needed; runtime defaults to Docker):
...
[env]
API_KEY = { op = "op://vault/item/field", path = "Vault/Item/Field", on_demand = true }
...
| `[env].<name>.on_demand` | bool | No | `v1alpha7` | For op refs only; when true, resolve through `jackin-exec` instead of launch-time injection |
| `[env].<name>.value` | string | No | `v1alpha7` | Extended literal or `$HOST_ENV` value form used with `on_demand` metadata |
...
| `[runtime].default_backend` | string | No | `v1alpha7` | Runtime backend default: `docker` or `apple-container`; omitted means `docker` |
...
| `[runtime].backend` | string | No | `v1alpha7` | Per-workspace runtime override: `docker` or `apple-container`; omitted inherits the global default |
