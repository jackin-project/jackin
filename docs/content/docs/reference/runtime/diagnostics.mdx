...
| `container_started` | After `docker run -d` succeeds | `{"container_name": "...", "capsule_log": "/path/to/multiplexer.log"}` fallback path |
...
A single run may start multiple containers — the role container plus a Docker-in-Docker sidecar. The `container_started` event records the container name and the **host fallback path of the in-container capsule diagnostics log** (`~/.jackin/data/<container-name>/state/multiplexer.log`) so an agent reading the run JSONL can locate the per-container crash log without knowing the on-disk layout when OTLP is unavailable or disabled.
...
diagnostics even when the operator did not pass `--debug`; the trace level also
enables raw capsule PTY/frame/input payload records.
