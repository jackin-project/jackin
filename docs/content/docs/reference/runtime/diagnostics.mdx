...
Every `RunDiagnostics` write also emits a `tracing::info!` or `tracing::debug!` event (Defect 47). These events feed **only** the optional OTLP exporter, installed when jackin' is compiled with `--features otlp` and `JACKIN_OTLP_ENDPOINT` is set. The default build installs no terminal subscriber.
