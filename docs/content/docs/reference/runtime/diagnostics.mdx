...
A run ID is a bare unique value with no prefix: a jackin-minted id is six hex characters (for example, `8b4766`); when a wrapper such as Parallax propagates its run id, jackin adopts that value instead. The host CLI prints the run ID at startup in `--debug` mode. Debug launches also inject `JACKIN_RUN_ID` and `JACKIN_RUN_DIAGNOSTICS_PATH` into the role container so Capsule's Debug info dialog can copy the exact host JSONL path instead of guessing from the container's home directory.
