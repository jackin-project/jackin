...
`JACKIN_TELEMETRY_LEVEL=debug|trace` enables debug-category diagnostics even
when the operator did not pass `--debug`; `JACKIN_TELEMETRY_CATEGORIES` narrows
those debug-category records by category. These controls affect telemetry
