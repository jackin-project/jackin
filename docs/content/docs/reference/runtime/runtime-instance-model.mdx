...
Implementation lives in <RepoFile path="src/instance/naming/mod.rs" /> and <RepoFile path="src/runtime/naming/mod.rs" />. The full naming reference — including image names, derived resource names, lock files, roles cache layout, CLI selector formats, and worked examples — is in [Instance and Resource Naming](/reference/runtime/instance-resource-naming/).
...
Implementation lives in <RepoFile path="src/instance/manifest/mod.rs" />, <RepoFile path="src/isolation/state/mod.rs" />, and <RepoFile path="src/instance/mod.rs" />.
...
Implementation lives in <RepoFile path="src/runtime/launch/mod.rs" /> (`resolve_restore_candidate`, related restore helpers, name claiming, and launch orchestration).
...
Implementation lives in <RepoFile path="src/runtime/attach/mod.rs" /> and the hardline dispatch paths in <RepoFile path="src/app/mod.rs" />.
...
Implementation lives in <RepoFile path="src/console/tui/state/mod.rs" />, <RepoFile path="src/console/tui/view/list.rs" />, and <RepoFile path="src/console/tui/input/list/mod.rs" />.
...
Implementation lives in <RepoFile path="src/runtime/cleanup/mod.rs" />.

## Source References

| Area | Source |
|---|---|
| DNS-safe names and instance IDs | <RepoFile path="src/instance/naming/mod.rs" /> |
| Docker labels and derived resource names | <RepoFile path="src/runtime/naming/mod.rs" /> |
| Instance manifest and rebuildable index | <RepoFile path="src/instance/manifest/mod.rs" /> |
| Launch, restore discovery, name claiming | <RepoFile path="src/runtime/launch/mod.rs" /> |
| Attach, inspect, session inventory | <RepoFile path="src/runtime/attach/mod.rs" /> |
| Eject, purge, orphan cleanup | <RepoFile path="src/runtime/cleanup/mod.rs" /> |
| Workspace/materialized mount state | <RepoFile path="src/isolation/materialize/mod.rs" /> and <RepoFile path="src/isolation/state/mod.rs" /> |
| Foreground finalizer | <RepoFile path="src/isolation/finalize/mod.rs" /> |
| Console instance rendering/actions | <RepoFile path="src/console/tui/view/list.rs" /> and <RepoFile path="src/console/tui/input/list/mod.rs" /> |
