...
All naming logic lives in <RepoFile path="crates/jackin-runtime/src/instance/naming.rs" /> (container names, instance IDs, role-family matching) and <RepoFile path="crates/jackin-runtime/src/runtime/naming.rs" /> (image tags, derived resource names, display formatting).
...
Set once at launch time, never renamed. The Docker container name and the state directory name are always identical — they are derived from the same string in <RepoFile path="crates/jackin-runtime/src/instance.rs" />.
