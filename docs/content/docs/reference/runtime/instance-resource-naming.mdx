...
All naming logic lives in <RepoFile path="src/instance/naming/mod.rs" /> (container names, instance IDs, role-family matching) and <RepoFile path="src/runtime/naming/mod.rs" /> (image tags, derived resource names, display formatting).
