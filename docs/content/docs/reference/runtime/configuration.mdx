...
| `sync_source_dir` | Optional host-side source folder for file-backed `sync` auth. The value is the agent credential/config directory itself, such as `CLAUDE_CONFIG_DIR`, `CODEX_HOME`, or `AMP_DATA_HOME`, and is valid on global, workspace, and workspace-role agent blocks for agents that support file-backed sync. | agent runtime default |
