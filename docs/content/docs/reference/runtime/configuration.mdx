...
Each file carries its own `version = "v1alpha7"` stamp, so a dotfile-managed setup that is partially old (some workspaces stamped, some not) migrates per-file on first load. See [Schema Versions](/reference/runtime/schema-versions/) for the full version history and migration matrix.
...
Top-level `version` is the config schema version. jackin' writes `version = "v1alpha7"` today. Older config files with no `version` are treated as legacy and migrated automatically on startup before parsing. See [Schema Versions](/reference/runtime/schema-versions/) for the full version history and migration matrix.
...
| `sync_source_dir` | Optional host-side source folder for file-backed `sync` auth. It is valid on global, workspace, and workspace-role agent blocks for agents that support file-backed sync. | agent runtime default |
...
The workspace editor stages Auth-tab edits in memory until the operator confirms the workspace save. The save path in <RepoFile path="crates/jackin/src/console/services/config.rs">crates/jackin/src/console/services/config.rs</RepoFile> applies separate diff passes for general workspace fields, agent auth modes, agent sync source folders, and credential env values; the sync-source pass writes through `ConfigEditor` setters in <RepoFile path="crates/jackin-config/src/editor.rs">crates/jackin-config/src/editor.rs</RepoFile> for both workspace and role layers. This keeps the dialog staging model consistent: the Auth dialog changes pending state only, and disk writes happen only after the workspace save confirmation.
...
API_TOKEN = { op = "op://abc123.../def456.../fld789...", path = "Personal/api/token" }
...
[roles.agent-smith.env]
API_TOKEN = { op = "op://abc123.../def456.../fld789...", path = "Work/shared-smith/token" }
```
...
- `{ op = "op://...", path = "Vault/Item/Field" }` — 1Password reference, resolved via `op read` at launch. `op` is the UUID-based URI (stable if the item is renamed); `path` is a human-readable display snapshot. Use `jackin config env set` to create these — it resolves the UUID and writes the correct form automatically.
- `$NAME` or `${NAME}` — read from the host env at launch
- Anything else — literal string passed through as-is

<Aside type="caution" title="Plain op:// strings are not resolved">
Do not write `KEY = "op://Vault/Item/Field"` as a plain string in config files — it will be passed to the container literally, not resolved. Only the inline-table form `{ op = "...", path = "..." }` is resolved via 1Password. Use `jackin config env set` to create 1Password references correctly.
</Aside>
...
The default runtime backend is Docker. On macOS 26 ARM, operators can opt into the experimental Apple Container backend globally or per workspace.
...
# ~/.config/jackin/workspaces/my-project.toml
[runtime]
backend = "apple-container"
```

The launch-time `--backend` flag overrides both settings for one `jackin load` invocation.
...
The `[git]` table is omitted from `config.toml` when all fields are at their defaults (the table is not written unless the operator sets it). See [Schema Versions](/reference/runtime/schema-versions/#v1alpha7) for the field-level reference.
