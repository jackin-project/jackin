...
- **Fresh published image** → pull `docker.io/.../jackin-the-architect:latest` and **restamp it** as the local `jk_<role>__base:<sha>` (a `FROM <published>` build — no role rebuild). The overlay then derives `FROM` that local tag.
...
Either way the local base carries `jackin.role.git.sha` + `jackin.construct.image` and is **reused** when the tag already exists locally and its construct matches — so warm launches skip the published pull entirely, and the heavy role layers are materialized once per (role commit, construct) (see [Local role base](#local-role-base)).
...
jackin pulls the published base and verifies `jackin.role.git.sha` against the role checkout's HEAD (plus the construct version) to decide whether to restamp the published image into the local base, or build the base from the workspace Dockerfile. `jackin.manifest.version` will join this contract once `jackin-role-action` is updated to stamp it.

## Local role base

The role base is always materialized locally as `jk_<role>__base:<sha>` — the role content with **no** jackin overlay — so the derived overlay builds `FROM` a stable local tag rather than the published `:latest`. The base is either a restamp of the pulled published image (`FROM <published>`) or a local build of the role Dockerfile (construct `FROM` overridden when `JACKIN_CONSTRUCT_IMAGE` is set). Either way it carries two labels:
...
On the next launch the local base is **reused** when `jk_<role>__base:<sha>` already exists and its `jackin.construct.image` matches the current construct — the role commit is the tag, the construct label distinguishes a custom-construct base from an official one. So warm launches skip the published pull, and the heavy role layers are materialized once per (role commit, construct); the overlay derives `FROM` the base.
