...
| Supply chain (capsule binary) | SHA256 + signed manifest | <RepoFile path="src/capsule_binary.rs" /> | See [signed releases](/reference/roadmap/security-threat-model-and-signed-releases/) |

## Security functions

All six functions live in <RepoFile path="src/instance/auth.rs" /> and are called during credential provisioning at every `jackin load` / `jackin console` launch.
...
Security scanner that validates container runtime state against a policy. Guards against configuration drift and unexpected privilege escalation during a session. Downloaded in <RepoFile path="docker/construct/Dockerfile" /> at line 51 via a SHA256-verified curl + `tar` extraction. Shell hook wired in the same shell config files as shellfirm.

Disable per-session by setting `JACKIN_DISABLE_TIRITH=1` in the container environment.

## High-autonomy launch flags

jackin' launches each agent with flags that remove tool-approval prompts. These are defined in <RepoFile path="docker/runtime/entrypoint.sh" /> and applied at container start before the agent binary runs.
...
| Kimi | `--yolo --auto` CLI flags (`--yolo` auto-approves actions; `--auto` starts in auto permission mode) | line 58 |
| OpenCode | `OPENCODE_CONFIG_CONTENT='{"permission":"allow"}'` env var + `~/.config/opencode/opencode.json` written with `"permission": "allow"` by capsule runtime setup | line 64 + <RepoFile path="crates/jackin-capsule/src/runtime_setup.rs" /> |
