...
| Supply chain (capsule binary) | SHA256 + signed manifest | <RepoFile path="crates/jackin-image/src/capsule_binary.rs" /> | See [signed releases](/reference/roadmap/security-threat-model-and-signed-releases/) |

## Security functions

All six functions live in <RepoFile path="crates/jackin-runtime/src/instance/auth.rs" /> and are called during credential provisioning at every `jackin load` / `jackin console` launch.
