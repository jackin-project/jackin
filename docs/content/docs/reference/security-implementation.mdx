...
| Supply chain (capsule binary) | SHA256 + signed manifest | <RepoFile path="crates/jackin-image/src/capsule_binary.rs">crates/jackin-image/src/capsule_binary.rs</RepoFile> | See [signed releases](/reference/roadmap/security-threat-model-and-signed-releases/) |
...
Security scanner that validates container runtime state against a policy. Guards against configuration drift and unexpected privilege escalation during a session. Downloaded in <RepoFile path="docker/construct/Dockerfile">docker/construct/Dockerfile</RepoFile> at line 51 via a SHA256-verified curl + `tar` extraction. Shell hook wired in the same shell config files as shellfirm.
