...
Area H. Siblings: cost model and heavy-day profile in [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/) (§5); register mechanics in [04 — Style and Language Compression — Beyond Caveman](/reference/research/token-optimization-techniques/04-style-and-language-compression/); tokenizer effects in [05 — Tokenizer arbitrage](/reference/research/token-optimization-techniques/05-tokenizer-arbitrage/); cache mechanics in [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/); worker down-routing in [10 — Model routing and tiered delegation](/reference/research/token-optimization-techniques/10-model-routing-and-delegation/); self-host-only inference plays in [13 — Infrastructure-level (self-hosted / gateway tier)](/reference/research/token-optimization-techniques/13-infrastructure-level/); eval design in [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/reference/research/token-optimization-techniques/16-validation-harness/).

Arithmetic in this file uses the **$17/day floor variant** of the modeled profile from [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/) unless
explicitly tied to the local 25-agent fleet case study. Multiply floor-profile dollar figures by
~1.28 to compare against the $22/day working variant used in [15 — Composed Stacks: Conservative / Aggressive / Unbelievable](/reference/research/token-optimization-techniques/15-composed-stacks/).
...
**Expected savings:** Keeping a 10k-tok log/test payload out of a parent with 30 turns left avoids ~10k×(1.25+2.9)×$10/M ≈ $0.42 input spend per occurrence, against ~$0.11 spawn overhead (local) plus report cost → net ~$0.25-0.30/occurrence. On the [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/) §5 profile ($17.00/day), five such delegations/day ≈ $1.3-1.5 ≈ 8-9% (ESTIMATE), plus deferred compaction. Counterweight: agents ≈4x chat, parallel research multi-agent ≈15x — delegation saves only on verbose side-tasks.
...
**Composability:** Substrate for H2-H5; pairs with worker down-routing ([10 — Model routing and tiered delegation](/reference/research/token-optimization-techniques/10-model-routing-and-delegation/)); conflicts with nothing.
...
**Composability:** Multiplies with H1 (applies exactly at the re-read interface); stacks with Haiku down-routing (same payload 135 vs 170 tok under the Sonnet/Haiku tokenizer — [05 — Tokenizer arbitrage](/reference/research/token-optimization-techniques/05-tokenizer-arbitrage/)); choose one register per interface vs H3.
**Validation protocol:** count_tokens A/B on same-information report pairs (harness per [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/reference/research/token-optimization-techniques/16-validation-harness/)); always count with the Anthropic tokenizer, never tiktoken.
...
**Composability:** Interacts with everything that fattens the shared prefix — CLAUDE.md (2,744 tok), MCP schemas (1,420 tok loaded vs ~60 deferred, local) — the fatter the prefix, the more staggering pays; see [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/).
...
**Availability:** NOT-USER-ACCESSIBLE (GATEWAY-OR-SELF-HOST at absolute best, same-architecture pairs, custom serving — see [13 — Infrastructure-level (self-hosted / gateway tier)](/reference/research/token-optimization-techniques/13-infrastructure-level/)).
...
**Validation protocol:** Track rework rate on compressed vs full task specs; run the 1-2-hop decode eval per [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/reference/research/token-optimization-techniques/16-validation-harness/).
...
| 28 | Modeled day profile used for % arithmetic | $17.00/day; $1.70/task | [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/) §5 (local model) |
| 29 | Tokenizer divergence (Fable vs Sonnet/Haiku) on identical text | 15-38% sample-dependent; register payload 170 vs 135 tok | local measurement; [05 — Tokenizer arbitrage](/reference/research/token-optimization-techniques/05-tokenizer-arbitrage/) |
