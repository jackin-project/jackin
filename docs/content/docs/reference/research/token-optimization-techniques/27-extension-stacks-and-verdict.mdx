...
- **But for this operator the metric is wrong.** The local credential is Max (file 19); a subscriber's
...
- **42 genuinely-new techniques** (files 19–25), each with a coverage-delta note proving absence from
...
- Volume I's binding constraints are structural: (1) frontier-model thinking bills as output and only
 ...
 the cache floor is self-host-only on hosted Claude (file 24) — same wall as Volume I's K1/K2.
- The new dollar levers are real but small: vision-tier routing saves 67% of *image* tokens (file 20),
...
Max), "$ per task" is the wrong denominator. The quota model (file 19):
...
- The cap weighting itself is **officially unpublished**; community triangulation puts cache-read at
 ~0.1× (T3, file 19), and the **token denominator is opaque** (bounded INCOMPLETE). So "tasks per cap"
 ...
 **Recorded as version/path-dependent; operator should audit their own subagent JSONL** (file 22 F4)
