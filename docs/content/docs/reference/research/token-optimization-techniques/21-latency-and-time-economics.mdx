...
credits (real dollars) but **do not count against the cap** ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)) — so on a quota-bound Max seat,
...
- **Mechanism:** `speed: "fast"` runs the same Opus 4.8 at up to 2.5× throughput for 2× the per-token
 ...
 [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)). Separate rate-limit pool from standard Opus; falls back silently to standard on
...
- **Mechanism:** Anthropic's multi-agent research system "cut research time by up to 90%" by spawning
 ...
 wave forfeits cache, [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 8).
...
- **Composability:** batch × caching × cheaper model is the deepest dollar stack ([07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 9);
...
At the subscription cap, the only remaining lever *is* a time-vs-dollar trade (bridges [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/) Q6).
...
- **Quality risk:** **NEUTRAL.**
- **Availability:** CLAUDE-CODE-TODAY.
- **Effort to adopt:** minutes (set the policy + a monthly credit cap, [25 — Meta layer: the cost of optimizing, budget governance, and online quality guarding](/reference/research/token-optimization-techniques/25-meta-cost-governance-and-online-quality/)).
- **Composability:** the join point of [19 — Subscription & quota economics](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/) (quota), [21 — Latency, wall-clock, and human-time](/reference/research/token-optimization-techniques/21-latency-and-time-economics/) (time), and [25 — Meta layer](/reference/research/token-optimization-techniques/25-meta-cost-governance-and-online-quality/) (governance).
...
- Caching is the only lever that is **negative-cost on dollars, quota, *and* time** simultaneously —
 which is why prefix hygiene ([07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/), [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/) Q2) is the highest-leverage habit on every axis.
