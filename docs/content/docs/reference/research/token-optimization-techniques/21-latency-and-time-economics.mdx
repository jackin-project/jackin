...
credits (real dollars) but **do not count against the cap** (file 19) — so on a quota-bound Max seat,
...
- **Mechanism:** `speed: "fast"` runs the same Opus 4.8 at up to 2.5× throughput for 2× the per-token
 ...
 Volume I 07). Separate rate-limit pool from standard Opus; falls back silently to standard on
...
- **Mechanism:** Anthropic's multi-agent research system "cut research time by up to 90%" by spawning
 ...
 wave forfeits cache, Volume I 07 tech 8).
...
- **Composability:** batch × caching × cheaper model is the deepest dollar stack (Volume I 07 tech 9);
...
At the subscription cap, the only remaining lever *is* a time-vs-dollar trade (bridges file 19 Q6).
...
- **Quality risk:** **NEUTRAL.**
- **Availability:** CLAUDE-CODE-TODAY.
- **Effort to adopt:** minutes (set the policy + a monthly credit cap, file 25).
- **Composability:** the join point of files 19 (quota), 21 (time), and 25 (governance).
...
- Caching is the only lever that is **negative-cost on dollars, quota, *and* time** simultaneously —
 which is why prefix hygiene (Volume I 07, file 19 Q2) is the highest-leverage habit on every axis.
