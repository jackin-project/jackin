...
paired-task harness (file 16) and kills `max_tokens` as an optimizer keeping it as a safety rail
...
- **The optimizer has a cost, and for some levers it exceeds the saving.** A `count_tokens`
 ...
 (file 21); the offline harness (file 16) costs n≥10 paired runs per technique; an online
 ...
 **async so it adds no tail latency**, **validate the judge before trusting it**, and **alarm rather
 than block**. The pattern: **offline gate (file 16) + online canary (this file)**. Note: OSS Arize
 Phoenix does *not* do production monitoring (paid AX only); Helicone is a score *sink*, not a judge.

Dollar/quota context from Volume I and files 19/21; tiers per finding.
...
- **Levers that need a hot-path proxy or a custom judge carry a high guard tax** and must clear a high
 bar — which is why a `count_tokens`-per-step compression proxy (RPM-bound, file 21) or an
 LLMLingua-class compressor (needs red-teaming, file 24 FL3) rarely pay off for code.
- **The harness itself should be sampled/batched, not inline.** Run validation offline (batch, file
 ...
 step (file 21 L2). The meta-rule: *don't let the meter cost more than the thing it measures.*
...
## C. Online quality guarding — the live complement to file 16
...
- **Validate the judge first** — the judge itself needs a harness (echoes Volume I 16 §5).
...
- **Expected savings:** loss-avoidance — bounds the worst case (a runaway loop, a 5-deep subagent
 recursion, file 24) instead of discovering it on the invoice. No token saving on the happy path.
...
- **Quality risk:** **NEUTRAL** (governors don't change outputs) — except a too-tight rate limit
 throttles legitimate work (429 backoff, file 21); size it from the per-user TPM table.
...
- **Composability:** the fleet governor for file 22; pairs with degrade-don't-die routing (G5).
...
- **Composability:** the governance partner of G1; reconfirms Volume I 09's kill.
...
The live complement to file 16: catch the regressions an offline suite never sees.
...
- **Mechanism:** a reference-free LLM-as-judge scores a sampled fraction of production traces async
 ...
 threshold breaches (drift, caveat-drop, format failures). It watches the deployed stack; file 16
...
- **Composability:** governs G3; same discipline as file 21 L2 (the optimizer's latency tax).
...
- **Coverage-delta:** New connection. Volume I 10 covers routing; triggering it from a *budget* event
...
- **Mechanism:** Cloudflare Spend Limits can route to a fallback model after a cap is hit; the same
 ...
 wait-vs-pay-vs-downgrade choice at the cap edge (files 19 Q6, 21 L6).
