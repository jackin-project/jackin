...
- **Token counts** via the free OAuth `count_tokens` endpoint (`/tmp/ctimg.py`, `/tmp/ctpdf.py`)
 ...
 I, file 07). Raw `input_tokens` includes a constant ~6–8-token user-message wrapper (Volume I
...
- **Expected savings:** per full-frame screenshot, roughly 4,760 → 1,520–1,570 tokens = **~−67%** on the image
 ...
 large in **quota** (file 19) and in window pressure. A 25-page PDF: 78,806 → 68,804 tokens
...
- **Composability:** pairs with context editing/observation masking (Volume I 06/12) applied to image
 blocks specifically.
