...
estimate in [03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/):267` and [12 — Provider-level features, current and beta](/reference/research/token-optimization-techniques/12-provider-features/):165`. Every number
...
Pricing and the modeled session profile are inherited from [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/). The
...
- **Token counts** via the free OAuth `count_tokens` endpoint (`/tmp/ctimg.py`, `/tmp/ctpdf.py`)
 ...
 I, [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)). Raw `input_tokens` includes a constant ~6–8-token user-message wrapper (Volume I
...
- **Expected savings:** per full-frame screenshot, roughly 4,760 → 1,520–1,570 tokens = **~−67%** on the image
 ...
 large in **quota** ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)) and in window pressure. A 25-page PDF: 78,806 → 68,804 tokens
...
- **Composability:** pairs with context editing/observation masking ([06 — Context architecture](/reference/research/token-optimization-techniques/06-context-architecture/)/12) applied to image
 blocks specifically.
