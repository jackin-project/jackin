...
- **Net for the operator:** the Volume I stack is ~80% portable as discipline and ~60% as feature.
 ...
 (file 19), and fast mode (file 21). The biggest *gains* from looking outward are reverse-ported:
...
- **Quota/subscription specifics — DO NOT PORT.** Each provider's cap structure, cache-vs-storage
 billing, and auth-gating differ (below); file 19's Anthropic quota model is Anthropic-specific.
...
- **Expected savings:** avoids a full prefix re-ingestion across idle gaps on 5-min-TTL paths — the
 same per-bust economics as Volume I 07, realized as a shipped flag.
...
- **Availability:** Aider today; the *pattern* is portable to any SDK harness (Volume I 07 tech 3).
...
- **Availability:** Aider / OpenCode today; portable to Claude Code as subagent-model pinning +
 Edit-tool discipline (Volume I 09/10).
...
- **Expected savings:** bounds tool-output bloat (a top hidden cost in long sessions) and avoids
 always-loading rules that only matter on some paths — the conditional-context idea Volume I 08
...
- **Mechanism:** (a) `cache_control` breakpoint placement is Anthropic-only — elsewhere you protect
 ...
 19) and **fast mode** (file 21) are Claude-Code-specific; (c) caveman register compression is a
...
|---|---|---|---|
| Claude Code | sub cap (file 19) / API $ | 0.1× (cache_read) | `cache_control`, 1h-TTL on sub, fast mode |
