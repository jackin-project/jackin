...
`count_tokens` now accepts `claude-fable-5` directly (live probe HTTP 200, count identical to `claude-opus-4-8`); it rejected the id at Volume II research time (see 40), which is why the measurements below were counted on `claude-opus-4-8` — same tokenizer either way, so they stand.
