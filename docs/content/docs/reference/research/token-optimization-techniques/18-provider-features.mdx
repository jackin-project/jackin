...
- Two of Anthropic's own live pages still say prior-turn thinking is "automatically stripped"; the adaptive-thinking and context-editing pages say Opus 4.5+/Sonnet 4.6+/Fable 5 keep ALL prior thinking turns in context and bill them as input. The contradiction is live and worth real money on long sessions. [Re-verified 2026-07-05: the context-windows page is fixed — it now states per-model behavior (Opus 4.5+/Sonnet 4.6+/Fable 5/Mythos 5/Mythos Preview keep previous thinking blocks by default; earlier Opus/Sonnet and all Haiku models strip them). Only the token-counting page still carries the blanket "Thinking blocks from previous assistant turns are ignored and do not count toward your input tokens" note.]
...
- **Effort to adopt:** hours (one edits entry + append full `response.content` so compaction blocks persist; track cost via `usage.iterations[]` — re-verified 2026-07-05: with the compaction beta enabled every response includes `usage.iterations`; only the compaction ENTRY appears solely on turns that trigger a new compaction, and re-applying an existing compaction block incurs no additional compaction cost).
...
- **Mechanism:** Requires `code_execution_20260120` or later (re-verified 2026-07-05: `code_execution_20260521` — same runtime, discloses a 90s per-cell limit — is accepted and interchangeable in `allowed_callers`); tools opt in via `allowed_callers: ["code_execution_20260120"]` (guidance, "not a security boundary"). Claude writes a script in the sandbox; each in-script tool call pauses execution, client supplies tool_result, script resumes. Verbatim: "Tool results from programmatic invocations do not count toward your input/output token usage. Only the final code execution result and Claude's response count." Platforms: Claude API, Claude Platform on AWS, MS Foundry — NOT Bedrock/Vertex. Exclusions: MCP-connector tools, `strict: true` tools.
...
- KILL (nuanced) "prior-turn thinking is always stripped, so reasoning never costs input" — false on Opus 4.5+/Sonnet 4.6+/Fable 5 (kept, billed as input). Two Anthropic pages (context-windows, token-counting) still propagate the dead claim (re-verified 2026-07-05: context-windows now corrected to per-model behavior; only token-counting still carries it).
...
7. **Fast mode** as a spend-more lever (Opus 4.8 at $10/$50 — Fable 5 sticker for an Opus model; Opus 4.6/4.7 at $30/$150 = 6x; research preview, no batch, stacks with caching) deserves a quality-per-dollar comparison vs Fable 5 at high effort. [Re-verified 2026-07-05: Opus 4.6 fast mode ended 2026-06-29 (runs/bills standard); Opus 4.7 fast mode is deprecated, removal 2026-07-24 — only Opus 4.8 remains long-term.]
...
| Fast mode: Opus 4.8 $10/$50; Opus 4.6/4.7 $30/$150; research preview; stacks with caching/data-residency; no batch (re-verified 2026-07-05: only Opus 4.8 $10/$50 and Opus 4.7 $30/$150 — deprecated, removed 2026-07-24 — remain; Opus 4.6 fast mode ended 2026-06-29 and now runs/bills standard) | same pricing page |
| Tool-use system tax table (290/410, 675/804, 497/589, 496/588; Fable 5 absent); bash +245; text editor +700; computer use 735 & 466-499; web search $10/1k; web fetch $0, ~2,500/~25k/~125k tok examples (re-verified 2026-07-05: table gained Sonnet 5 = 354/474) | same pricing page |
...
| Tool search: ~55k example, ">over 85%" reduction, 30-50 tool degradation, "prefix is untouched, so prompt caching is preserved", 10,000 max, 3-5 refs, &lt;10-tools guidance, no beta header (re-verified 2026-07-05: tool-use examples now compatible — input_examples expand on discovery; "up to 5 matching tools by default"; defer_loading tools reject cache_control with 400) | platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool |
