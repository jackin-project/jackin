...
| **SDK/`claude -p`/GitHub-Actions usage off the subscription cap** → separate API-rate credit | | T1 | Bifurcates the cost model by surface (file 19 Q7) |
...
hit the *subscriber* directly and are folded into files 19 and the verdict (27). The verdict carries a Sonnet 5 exception — see the Sonnet 5 row above.
...
- **OrcaRouter** (2605.30736, May 2026): production LinUCB contextual-bandit router, RouterArena #2 at
 submission. Resolves part of Volume I 10's open "no current router eval" question — still a
...
- **Coverage-delta:** New. CAG is 0-hit in Volume I (confirmed 18-overview); Volume I 08 debates
...
- **Expected savings:** converts per-turn RAG retrieval+injection into a one-time cached-prefix write
 ...
 subscription the cached reads are also ~0.1× against the cap (file 19). Bounded by the 200K
 subscription context (file 19) — large corpora still need retrieval.
...
- **Composability:** stacks with prompt caching (07) and the codebook/pointer ideas (Volume I 08/14);
...
- **Coverage-delta:** New (Claude Code 2.1.169). Volume I 07 enumerates cache-safe vs
...
- **Expected savings:** one avoided full-history rewrite per directory change — the per-bust economics
 of Volume I 07 tech 1 / file 19 Q2 (≈150K × (1.25–2.0 − 0.1) tokens).
...
- **Composability:** extends Volume I 07's cache-safe operation list; pairs with prefix-stability (19 Q2).
...
- **Coverage-delta:** New risk axis. Volume I 13 covers the *cost* trap of compression proxies;
...
- **Expected savings:** none — loss-avoidance. Reinforces routing compression to ingestion-only seams
 (Volume I 14 K15) or avoiding it entirely for code.
...
- **Effort to adopt:** minutes (policy: no compressor in the hot path; see file 25).
...
- **Coverage-delta:** New (0-hit in Volume I). Extends Volume I 13 (self-host) and 14 K2; the
...
- **Composability:** self-host stack only (with Volume I 13/K2); orthogonal to everything hosted.
