...
| **SDK/`claude -p`/GitHub-Actions usage off the subscription cap** → separate API-rate credit | | T1 | Bifurcates the cost model by surface ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/) Q7) |
...
hit the *subscriber* directly and are folded into [19 — Subscription & quota economics](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/) and the [27 — Volume II capstone](/reference/research/token-optimization-techniques/27-extension-stacks-and-verdict/). The verdict carries a Sonnet 5 exception — see the Sonnet 5 row above.
...
- **OrcaRouter** (2605.30736, May 2026): production LinUCB contextual-bandit router, RouterArena #2 at
 submission. Resolves part of [10 — Model routing and tiered delegation](/reference/research/token-optimization-techniques/10-model-routing-and-delegation/)'s open "no current router eval" question — still a
...
- **Coverage-delta:** New. CAG is 0-hit in Volume I (confirmed 18-overview); [08 — Retrieval, memory, and state offloading](/reference/research/token-optimization-techniques/08-retrieval-memory-and-state/) debates
...
- **Expected savings:** converts per-turn RAG retrieval+injection into a one-time cached-prefix write
 ...
 subscription the cached reads are also ~0.1× against the cap ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)). Bounded by the 200K
 subscription context ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)) — large corpora still need retrieval.
...
- **Composability:** stacks with prompt caching (07) and the codebook/pointer ideas ([08 — Retrieval, memory, and state offloading](/reference/research/token-optimization-techniques/08-retrieval-memory-and-state/)/14);
...
- **Coverage-delta:** New (Claude Code 2.1.169). [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) enumerates cache-safe vs
...
- **Expected savings:** one avoided full-history rewrite per directory change — the per-bust economics
 of [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 1 / [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/) Q2 (≈150K × (1.25–2.0 − 0.1) tokens).
...
- **Composability:** extends [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)'s cache-safe operation list; pairs with prefix-stability (19 Q2).
...
- **Coverage-delta:** New risk axis. [13 — Infrastructure-level](/reference/research/token-optimization-techniques/13-infrastructure-level/) covers the *cost* trap of compression proxies;
...
- **Expected savings:** none — loss-avoidance. Reinforces routing compression to ingestion-only seams
 ([14 — Frontier](/reference/research/token-optimization-techniques/14-frontier-ideas/) K15) or avoiding it entirely for code.
...
- **Effort to adopt:** minutes (policy: no compressor in the hot path; see [25 — Meta layer: the cost of optimizing, budget governance, and online quality guarding](/reference/research/token-optimization-techniques/25-meta-cost-governance-and-online-quality/)).
...
- **Coverage-delta:** New (0-hit in Volume I). Extends [13 — Infrastructure-level](/reference/research/token-optimization-techniques/13-infrastructure-level/) (self-host) and 14 K2; the
...
- **Composability:** self-host stack only (with [13 — Infrastructure-level](/reference/research/token-optimization-techniques/13-infrastructure-level/)/K2); orthogonal to everything hosted.
