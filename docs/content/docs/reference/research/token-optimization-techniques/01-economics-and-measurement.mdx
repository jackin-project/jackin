...
- **Modeled heavy-day profile (basis for stack math):** the $17 floor variant is ~25k uncached
 ...
 variant in [15 — Composed Stacks](/reference/research/token-optimization-techniques/15-composed-stacks/) scales this to six sessions. The measured split is profile-specific; the stable
...
- **Long context:** Fable 5, Opus 4.8/4.7/4.6, Sonnet 4.6 include the full 1M-token window **at
 ...
 not (see [06 — Context architecture — what enters the window at all](/reference/research/token-optimization-techniques/06-context-architecture/)).
...
2. **Thinking is output.** Locally measured at 54.8% of output tokens in a max-effort session
 ([02 — Baseline Audit of This Environment (Phase 0)](/reference/research/token-optimization-techniques/02-baseline-audit/)). Any output-side technique that doesn't touch thinking (all style
...
4. **Break-even arithmetic for cache writes:** 5-min write (1.25×) pays for itself after **one**
 ...
 done in [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/).
...
Local confirmation ([02 — Baseline Audit of This Environment (Phase 0)](/reference/research/token-optimization-techniques/02-baseline-audit/)): identical text counts +15% (Python code) to +38%
...
- **Cross-tier routing saves more than list prices imply.** Moving prose-heavy work Fable 5 →
 ...
 input classes. Quantified per task class in [10 — Model routing and tiered delegation](/reference/research/token-optimization-techniques/10-model-routing-and-delegation/).
...
- Tokens/char measured locally on Fable 5: ~2.3–3.4 for English/markdown, ~1.4–1.6 for CJK, more
 granular tables in [05 — Tokenizer arbitrage](/reference/research/token-optimization-techniques/05-tokenizer-arbitrage/).
...
automatic saving documented in [12 — Provider-level features, current and beta](/reference/research/token-optimization-techniques/12-provider-features/)).
...
- **OpenTelemetry** (`CLAUDE_CODE_ENABLE_TELEMETRY=1`, OTLP exporters): metrics
 ...
 exactly what's needed to measure subagent economics ([11 — Multi-agent protocols and subagent compression](/reference/research/token-optimization-techniques/11-multi-agent-protocols/)).
 (code.claude.com/docs/en/monitoring-usage.)
- **Session JSONL transcripts** (`~/.claude/projects/&lt;project&gt;/&lt;session&gt;.jsonl`) — per-call
 usage including cache fields. Two traps found locally ([02 — Baseline Audit of This Environment (Phase 0)](/reference/research/token-optimization-techniques/02-baseline-audit/)): the same
...
- **Console usage pages / ccusage-style analyzers** — covered with the market scan in
 [03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/); JSONL-based analyzers inherit both transcript traps above.

## 5. The modeled session profile (basis for all stack arithmetic)

Measured base (this environment, [02 — Baseline Audit of This Environment (Phase 0)](/reference/research/token-optimization-techniques/02-baseline-audit/)): one heavy orchestration session = 19 API
...
is modeled separately in [11 — Multi-agent protocols and subagent compression](/reference/research/token-optimization-techniques/11-multi-agent-protocols/); (b) cache behavior healthy (92.8% read
...
[09 — Output discipline and structured generation](/reference/research/token-optimization-techniques/09-output-discipline/)). Sensitivity: at 55% thinking the output rows become $3.44/$2.81
reversed; total unchanged. At a 70% cache-read share (sloppier sessions), day cost rises ~$3–4
from extra writes.

**The $/task metric (brief §4).** All stack claims in [15 — Composed Stacks: Conservative / Aggressive / Unbelievable](/reference/research/token-optimization-techniques/15-composed-stacks/) use:
`Nx = (baseline $/completed task) ÷ (optimized $/completed task)` at statistically
indistinguishable success on the validation suite ([16 — Validation Harness: the No-Quality-Loss Proof Protocol](/reference/research/token-optimization-techniques/16-validation-harness/)). With ~10 completed
...
| Session decomposition, thinking 54.8%, prompt mix 0.44/6.73/92.83, tokenizer +15–38% local (prose-specific; code/CJK near-neutral); session split is profile-specific | Local measurements, methods in [02 — Baseline Audit of This Environment (Phase 0)](/reference/research/token-optimization-techniques/02-baseline-audit/) |
