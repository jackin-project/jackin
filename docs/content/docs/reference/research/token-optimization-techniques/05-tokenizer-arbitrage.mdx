...
Dollar arithmetic uses the modeled heavy-day profile from [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/): ~$22/day at Fable 5 list prices, split 32% cache reads / 29% cache writes / 20% thinking / 17% visible output / 2% uncached input (local measurement).
...
- **Composability:** multiplies through caching (smaller static content = cheaper cache write AND 0.1x reads thereafter; see [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)); stacks with numeric/ID normalization below (those dominate per-row cost); orthogonal to model routing.
...
- **Composability:** rescales every other technique's denominator — minification saves 41% on Sonnet but 29% on Fable; never port format-ROI numbers across the 4.7 boundary. Pairs with subagent delegation patterns in [06 — Context architecture — what enters the window at all](/reference/research/token-optimization-techniques/06-context-architecture/).
...
- **Effort to adopt:** minutes.
- **Composability:** stacks inside tabular re-serialization; pairs with output-side rules in [09 — Output discipline and structured generation](/reference/research/token-optimization-techniques/09-output-discipline/) (told-to-emit numbers ride the ~5x output price).
...
- **Composability:** output-side wins ride the ~5x output price multiplier (see [09 — Output discipline and structured generation](/reference/research/token-optimization-techniques/09-output-discipline/)); input-side stacks with everything.
...
- **Availability:** CLAUDE-CODE-TODAY (trivially — by *not* doing it; use English-register compression instead, covered in [03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/) and [09 — Output discipline and structured generation](/reference/research/token-optimization-techniques/09-output-discipline/)).
- **Effort to adopt:** n/a.
...
- **Composability:** applies to MCP schema design (see [02 — Baseline Audit of This Environment (Phase 0)](/reference/research/token-optimization-techniques/02-baseline-audit/) tool-schema overhead) and any codegen target.
...
- **Effort to adopt:** minutes.
- **Composability:** prerequisite for all of the above; feeds the regression harness in [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/reference/research/token-optimization-techniques/16-validation-harness/).
...
5. Cache interplay shown only as arithmetic (smaller content → cheaper write and 0.1x reads immediately); not yet demonstrated end-to-end in a live session — see [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/).
...
| Phase-0 ground truth: thinking 54.8% of output; wenyan-full 80.9% chars→56.6% tokens; wenyan-ultra 74.5%; caveman-ultra 58.5%; Fable premium ~+15% code / ~+38% prose corpus; day split 32/29/20/17/2 | Local Phase-0 measurement (see [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/)) |
