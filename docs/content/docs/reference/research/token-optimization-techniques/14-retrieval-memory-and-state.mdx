...
### 6. Anthropic API memory tool (memory_20250818; beta at time of writing, GA per re-verification 2026-07-05) — cross-session file memory
...
- **Availability:** SDK (Developer Platform, Bedrock, Vertex; public beta — re-verified 2026-07-05: now generally available on the Messages API, no beta header required; tool type still `memory_20250818`, all Claude 4+ models; SDK helpers remain in beta namespaces despite GA). NOT-USER-ACCESSIBLE inside Claude Code — auto memory (technique 3) is the product analogue.
...
| memory_20250818 commands, injected protocol prompt text, multi-session pattern, "persists... across compaction boundaries" quote, ZDR, no perf numbers on page (re-verified 2026-07-05: tool now GA, no beta header; compaction-section wording changed to "memory preserves the information that must survive summarization") | https://platform.claude.com/docs/en/agents-and-tools/tool-use/memory-tool — live fetch |
