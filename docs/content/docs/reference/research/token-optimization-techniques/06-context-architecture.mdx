...
- Prefix economics compound through caching: every 10K of always-on prefix ≈ $0.01/turn in cache reads ≈ **$1.89/day** on the modeled profile (114 reads + 6 writes/day). Cache reads are 32% of heavy-session dollars (local Phase-0; see [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/)).
...
- **Composability:** the meta-technique that prioritizes all others; prefix cuts compound via the 0.1x cache-read multiplier on every turn (cache reads = 32% of heavy-session dollars; see [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)).
...
- **Composability:** complements deferral (06.1: prefix vs history); interacts with caching — naive client-side deletion invalidates the cached prefix; Claude Code's cache-edit approach exists precisely for this (see [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)).
...
| Fable 5 $10/$50/MTok; cache read 0.1x, write 1.25x (5min); all $/day arithmetic | dossier reference pricing (see [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/)) + arithmetic shown inline, labeled ESTIMATE |
