...
- **Quality risk:** **NEUTRAL.** A transparent proxy that mis-handles `cache_control` could *harm*
 caching — verify it passes breakpoints through ([03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/) record 17).
...
- **Composability:** precondition for Q3/Q5; same discipline as [13 — Infrastructure-level](/reference/research/token-optimization-techniques/13-infrastructure-level/) tech 1 with a bigger payoff.
...
- **Coverage-delta:** **Partial inversion** of [13 — Infrastructure-level](/reference/research/token-optimization-techniques/13-infrastructure-level/) tech 4 / 16. Vol I's break-even is in
...
- **Coverage-delta:** [13 — Infrastructure-level](/reference/research/token-optimization-techniques/13-infrastructure-level/) tech 2 covers the 1h/5m split as a *dollar* lever; the
...
- **Composability:** the jackin❯-fleet angle (a launcher placing fleet work on the SDK credit vs the
 interactive seat) is developed in 44; stacks with batch ([18 — Volume II extension](/reference/research/token-optimization-techniques/18-extension-overview/)).
