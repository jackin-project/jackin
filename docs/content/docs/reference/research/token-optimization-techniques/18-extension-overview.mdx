...
Volume I dossier (files 00–17), to find the cells Volume I left blank or drew too thin. It is
deliberately pushed before the deep dives so the gap map can be reviewed before research spends on
it. Volume I is treated as finished and correct; nothing here edits files 00–17.
 ...
 **Max subscription** (`~/.claude/.credentials.json` → `subscriptionType: max`).
 ...
 quota question; "quota" appears **≥5 times in file 07 and is modeled zero times**. For a flat-rate
...
- Volume II will ship area files 19–25 (one per pursued gap), a frontier file 26 (≥6 new ideas not
 duplicating Volume I's sixteen K-ideas), and a stacks-and-verdict file 27 (coverage-delta ledger,
...
2. **Overlay by coverage sweep.** A 14-agent read-only workflow (`vol1-coverage-map`, one agent per
 Volume I file 01–17, model Explore, structured output) rated each file's depth on each of the
 eight seeded blind spots — `absent` / `mention` / `partial` / `full` — with a `file:line` citation
 and an evidence quote. Files 00, 03, 07, and 14 were read in full by the main process instead.
