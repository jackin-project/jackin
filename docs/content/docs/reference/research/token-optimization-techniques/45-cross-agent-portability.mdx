...
- **Five of Volume I's lever classes port to essentially every coding agent** as first-class
 ...
 `model_reasoning_effort` none→xhigh, Gemini `thinkingLevel`/`thinkingBudget` (re-verified 2026-07-05: the Gemini thinking docs now expose only `thinking_level`; `thinkingBudget` no longer appears on the thinking page), Cursor named variants,
...
| **Effort / thinking tier** | `/effort`, MAX_THINKING | named variants (`-high`/`-max`) | configurable levels | `model_reasoning_effort` none→xhigh | `thinkingLevel` / `thinkingBudget` (re-verified 2026-07-05: only `thinking_level` documented) | `--reasoning-effort` / `--thinking-tokens` | `reasoningEffort` none→xhigh |
...
- **Effort tiering — PORTS as a first-class feature everywhere**, with finer granularity than Volume
 ...
 minimal/low/medium/high — re-verified 2026-07-05: `thinking_level` values are per-model: gemini-3-pro-preview low/high; gemini-3-flash-preview low/medium/high; gemini-3.5-flash minimal/low/medium/high; gemini-3.1-pro-preview low/medium/high; `thinkingBudget` was dropped from the thinking page). Thinking-bills-as-output is universal, so "lower effort on routine work"
...
| 3 | Gemini implicit cache on 2.5+; `thinkingLevel` (G3) / `thinkingBudget` (re-verified 2026-07-05: thinking page now documents only per-model `thinking_level`; `thinkingBudget` removed); cache 10% + storage $4.50/1M-hr; CLI per-tool budgets, `compressionThreshold` 0.5 | ai.google.dev/gemini-api/docs/{caching,thinking,gemini-3,pricing}; github.com/google-gemini/gemini-cli configuration.md |
| 4 | Copilot token billing (1 credit=$0.01); Auto −10%; configurable reasoning levels; copilot-instructions.md + path-specific; 1M context toggle | docs.github.com/copilot/{billing,models-and-pricing,auto-model-selection,supported-models,custom-instructions,optimize-ai-usage} (re-verified 2026-07-05: models-and-pricing now lives at docs.github.com/en/copilot/reference/copilot-billing/models-and-pricing; the old concepts/billing/models-and-pricing path 404s) |
