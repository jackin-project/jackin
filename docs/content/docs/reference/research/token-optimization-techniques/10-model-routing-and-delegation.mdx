...
- **Expected savings:** ESTIMATE on a 5-worker exploration fan-out, each 40K in / 3K out in Fable tokens: all-Fable = 5×(40K×$10 + 3K×$50)/1M = **$2.75**. Haiku with no tokenizer premium = **$0.275** (−90.0%); Haiku at the official/prose 1.3x divergence = $0.21 (−92.3%); repo-markdown-like 1.43x = **$0.19 (−93.0%)**. On the modeled heavy day ($22, Fable): two such fan-outs previously done inline at parent rates ≈ $5.50 → $0.38–0.55, ~22–23% off the day IF that work was being done at all (the context-isolation win compounds it; see [06 — Context architecture — what enters the window at all](/reference/research/token-optimization-techniques/06-context-architecture/)).
...
- **Composability:** stacks with `effort: low` frontmatter, parent prompt caching (untouched), and caveman-style output compression ([09 — Output discipline and structured generation](/reference/research/token-optimization-techniques/09-output-discipline/); the cavecrew agents in this repo compose both). Anti-synergy: fork mode (inherits parent model); `CLAUDE_CODE_SUBAGENT_MODEL` is global, so it also downtiers subagents you wanted strong.
...
- **Composability:** explicitly cache-safe, so composes with all caching techniques ([07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)). Complements technique 1: advisor = escalate UP at decision points; subagents = delegate DOWN for volume. Anti-synergy: Fable-main sessions; very long transcripts (consult tax grows linearly, uncached).
...
- **Validation protocol:** 15 fixed tasks × {high, medium, low} on Sonnet 4.6; record output tokens, tool-call count, tests-pass rate. Publish the per-level % — it would be the first public number (see [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/reference/research/token-optimization-techniques/16-validation-harness/)).
...
- **Composability:** combines techniques 1 + 5 (effort: low workers) + compressed worker output ([09 — Output discipline and structured generation](/reference/research/token-optimization-techniques/09-output-discipline/)). Anti-synergy: parallel identical-prefix workers all pay full cache-write price unless the first request is staggered (API parallel-cache trap; see [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)).
...
- **Composability:** precondition for every other technique's math; see [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/).
...
| Phase-0: thinking 54.8% of output; prompt mix 0.44/6.73/92.83%; dollar split 32/29/20/17/2; ~$22/heavy day | local phase-0 measurement (see [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/)) |
