...
- **Composability:** multiplies with prompt caching (smaller stable prefix, see [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)); survives /compact (root index re-injected from disk); topic files compress further with caveman-compress (local phase-0: 58.5% token cut on prose). Anti-synergy: none known.
...
- **Expected savings:** local measurement : restatement 233 tok vs pointer 53 tok = **77.3% per recalled item** (sweep's independent sample: 79.3%). Kills the "a pointer is ~10 tokens" folklore — useful pointers carry path + gist. Scales with restatement frequency (unmeasured; needs transcript mining). Cache side-benefit: pointers are often byte-identical across turns, restatements never are — preserves prefix stability ([07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)). Manus frames the payoff via cache economics: cached input 10x cheaper (their $0.30 vs $3.00/MTok matches Anthropic's 0.1x cache-read ratio, cross-checked against reference pricing); "KV-cache hit rate is the single most important metric for a production-stage AI agent"; Manus input:output ≈ 100:1 (vendor figure, no methodology).
