...
- Synthesis: [`15`](15-composed-stacks/) composed stacks with dollar math,
 [`16`](16-validation-harness/) runnable no-quality-loss protocol,
 [`17`](17-adoption-roadmap/) day-1/week-1/month-1 with automatic-vs-disciplined split.
...
The former Volume III chapters 51–57 left this dossier entirely and now live in the [token-optimization tools hub](/reference/research/token-optimization-tools/) as chapters 19–25.
...
7. Mid-run, five workflow draft agents died on a session rate limit (reset 19:20 UTC); the run
 continued on usage credits per the operator's local action. Files 11 and 14 were re-drafted
...
- [x] **≥40 techniques** across files 04–13: **110 cataloged**, every one carrying the full
...
layer on top of the frozen Volume I (files 00–17 unedited); it fills the gaps Volume I left blank or
...
- [`18`](18-extension-overview/) — gap audit: independent six-axis taxonomy overlaid on Volume I,
 the blind-spot map with `file:line` evidence, and the Volume II index.
- [`19`](19-subscription-and-quota-economics/) — the quota-weighted cost model for a capped
 subscriber (blind spot 1).
- [`20`](20-multimodal-token-economics/) — image/screenshot/PDF token costs, measured locally
 (blind spot 2).
- [`21`](21-latency-and-time-economics/) — wall-clock/human-time as a second cost axis (blind spot 3).
- [`22`](22-fleet-and-multitenant-cache/) — hosted cross-container/fleet cache economics (blind spot 4).
- [`23`](23-cross-agent-portability/) — portability matrix across coding agents (blind spot 5).
- [`24`](24-fresh-literature-and-market-delta/) — clean-room re-sweep; KV-eviction family, CAG,
 changelog drift (blind spot 6).
- [`25`](25-meta-cost-governance-and-online-quality/) — cost of optimizing, budget governance,
 online quality guards (blind spot 8).
- [`26`](26-extension-frontier/) — 8 new frontier ideas (not duplicating K1–K16).
- [`27`](27-extension-stacks-and-verdict/) — coverage-delta ledger, verdict delta, Corrections to
...
- **50 genuinely-new techniques** (42 in files 19–25 with the full §10 record + 8 frontier), each with
...
distributed, collected in 27). Full map with `file:line` evidence and per-cell stake: `18`.
...
7. **Seven area files (19–25)** were written, exceeding the ≥5 floor; fleet (22) was kept distinct
 (not merged) because the hosted-fleet material proved genuinely separate from Volume I 13's
 self-host tier.
...
- [x] **Blind-spot map** built by overlaying an independent taxonomy on Volume I with `file:line`
 evidence of thin/absent coverage (`18`).
- [x] **≥5 new area files** (19–25 = seven), writing rules followed; **≥25 new techniques** (50, each
 with a coverage-delta note); **≥10 with the full record** (all 42 in 19–25 carry it). (`27` ledger)
- [x] **≥6 new frontier ideas** with feasibility verdicts + math (8 in `26`).
- [x] **Subscription/quota cost model delivered** with an explicit bounded **INCOMPLETE** naming the
 unpublished denominator and what was measured instead (`19`).
- [x] **Multimodal/vision/PDF token costs measured locally via `count_tokens`** with the method shown
 (zlib-generated assets, validated against real PNGs + the published table) (`20`).
- [x] **Every Volume II headline number survived the adversarial pass**; the two most novel were
 ...
 included (`27`).
- [x] **Verdict delta with arithmetic** — dollars unchanged, metric reframed for a subscriber (`27`).
- [x] **Cross-layer caveats captured** in `27` (cache scope, subagent caching).
...
- **[`22-context-compression-literature-and-market.md`](/reference/research/token-optimization-tools/22-context-compression-literature-and-market/)** —
  ...
  accuracy (Squeez, AgentDiet, SWEzze, SWE-Pruner, LongCodeZip) — refuting file 24's "no compressor safe
...
- **[`23-token-observability-and-visualization.md`](/reference/research/token-optimization-tools/23-token-observability-and-visualization/)** — the
  *observability* layer (distinct from compression): a deep dive on `alexgreensh/token-optimizer` and a
  ...
  not a Max subscription (file 19). The JSONL-reading, no-proxy class is the safe measurement front-end of
...
- **[`24-rtk-and-write-time-observation-compression.md`](/reference/research/token-optimization-tools/24-rtk-and-write-time-observation-compression/)** —
  ...
  cache-safe **write-time observation-compression** design point files 21 (H1) and 22 named: it compresses
  ...
  per file 22 §A), corrected to low-double-digit whole-bill, same as the caveman K1 / headroom H-K1 moves.
  ...
  three, pilot it role-scoped with the host-write/hook-conflict guardrails. File 19's ast-grep coverage was
...
- [x] **Adversarial validation applied:** the independent `28` pass found arithmetic/tokenizer/profile/cap issues; load-bearing corrections are now applied in the live summaries and affected reports.
- [x] **Composed stacks and 10x verdict are current:** `15` carries corrected ≈2.4–2.5x aggressive math, ≈5–6.2x validated-routing ceiling, and no defensible 10x at zero quality loss.
- [x] **Negative-cost set, graveyards, harness, and roadmap are present:** negative-cost set in `15`, claim graveyards in `00`/area files/`27`, runnable validation protocol in `16`, adoption sequence in `17`.
