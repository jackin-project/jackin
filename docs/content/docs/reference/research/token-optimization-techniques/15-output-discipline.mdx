...
Method: free `count_tokens` API, model `claude-fable-5`, each payload sent as a single user message; 1-char baseline message = 7 tok envelope, subtracted in % columns. Measurement script reproduced in 31-validation-harness.md style: simulate the exact JSON a tool call would carry. File under test: <RepoFile path="crates/jackin-config/src/app_config/workspaces.rs">crates/jackin-config/src/app_config/workspaces.rs</RepoFile> (200 lines); change under test: a realistic 5-line method addition with a 5-line unique anchor.
...
| Claude Code effort defaults (high on Fable 5/Opus 4.8/4.6/Sonnet 4.6; xhigh on Opus 4.7); max session-only (re-verified 2026-07-05: persists when set via `CLAUDE_CODE_EFFORT_LEVEL`) + "prone to overthinking"; ultracode/ultrathink semantics; per-model calibration; frontmatter `effort` | quotes | code.claude.com/docs/en/model-config |
