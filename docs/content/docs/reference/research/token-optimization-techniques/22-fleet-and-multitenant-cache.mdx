...
**self-hosted** fleet tier fully in [13 — Infrastructure-level](/reference/research/token-optimization-techniques/13-infrastructure-level/) (HiCache/LMCache across replicas) and the
excludeDynamicSections *mechanism* in [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/), but left the **hosted** (Claude
API / subscription) cross-container story thin and explicitly flagged the dynamic-section size as
unmeasured in [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/). This file builds the hosted-fleet model: when N containers share one
...
- **The server prompt cache is workspace-scoped , not machine-scoped.** N
 ...
 rules Volume I cited ([07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 7) describe Claude Code's **local file cache** (GitHub #17531), a
 different layer.
...
- **Multi-tenant boundary: org isolation is absolute; workspace is the sharing unit.** Tenant-per-org
 ...
 pooled cap** ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)); a headless/SDK fleet can draw the separate API-rate credit, off
...
The original Volume I text ([07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 7 + surprising-findings: "your git state is in the cache key… worktrees never
...
Claude Code's *observed* reuse blends both layers. This correction has now been applied to [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/);
...
5. **Prefix ≥ the model minimum** (Opus 4.8 = 1,024; Fable 5 = 512; Haiku 4.5 = 4,096 — live values
 match [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 11; sub-minimum prefixes silently cache nothing).
...
- **Coverage-delta:** New. [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 7 covers the SDK flag; the **workspace-scope rule** (the
...
- **Coverage-delta:** [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 7 names the flag; **the enumerated six fields, the measured
...
- **Coverage-delta:** [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 8 covers the concurrency rule for subagent waves; applying it
...
- **Quality risk:** **NEUTRAL** (adds one TTFT of latency to the wave start — [21 — Latency, wall-clock, and human-time as a second cost axis](/reference/research/token-optimization-techniques/21-latency-and-time-economics/)).
...
- **Composability:** interacts with [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/) tech 4 (subagent fan-out economics assume caching);
...
- **Coverage-delta:** Volume I K16 in [14 — Frontier](/reference/research/token-optimization-techniques/14-frontier-ideas/) proposes baking the optimization pack into jackin❯; the
...
- **Availability:** BUILDABLE (jackin❯ roadmap; insertion points mapped in [14 — Frontier](/reference/research/token-optimization-techniques/14-frontier-ideas/) K16 / 17).
...
| 7 | Min cacheable prefix Opus 4.8 = 1,024; Fable 5 = 512; Haiku 4.5 = 4,096 (matches [07 — Caching exploitation](/reference/research/token-optimization-techniques/07-caching-exploitation/)) | platform.claude.com prompt-caching |
...
| 10 | Subscription fleet shares one pooled cap; headless/SDK draws separate API-rate credit | support.claude.com ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/) ledger) |
