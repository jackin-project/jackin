...
**self-hosted** fleet tier fully (`13:116-187`, HiCache/LMCache across replicas) and the
excludeDynamicSections *mechanism* as one technique (`07` tech 7), but left the **hosted** (Claude
API / subscription) cross-container story thin and explicitly flagged the dynamic-section size as
unmeasured (`07` Gaps #6). This file builds the hosted-fleet model: when N containers share one
...
- **The server prompt cache is workspace-scoped , not machine-scoped.** N
 ...
 rules Volume I cited (`07` tech 7) describe Claude Code's **local file cache** (GitHub #17531), a
 different layer.
...
- **Multi-tenant boundary: org isolation is absolute; workspace is the sharing unit.** Tenant-per-org
 ...
 pooled cap** (file 19); a headless/SDK fleet can draw the separate API-rate credit, off
...
The original Volume I text (`07` tech 7 + surprising-findings: "your git state is in the cache key… worktrees never
...
Claude Code's *observed* reuse blends both layers. This correction has now been applied to `07`;
...
5. **Prefix ≥ the model minimum** (Opus 4.8 = 1,024; Fable 5 = 512; Haiku 4.5 = 4,096 — live values
 match Volume I 07 tech 11; sub-minimum prefixes silently cache nothing).
...
- **Coverage-delta:** New. Volume I 07 tech 7 covers the SDK flag; the **workspace-scope rule** (the
...
- **Coverage-delta:** Volume I 07 tech 7 names the flag; **the enumerated six fields, the measured
...
- **Coverage-delta:** Volume I 07 tech 8 covers the concurrency rule for subagent waves; applying it
...
- **Quality risk:** **NEUTRAL** (adds one TTFT of latency to the wave start — file 21).
...
- **Composability:** interacts with Volume I 07 tech 4 (subagent fan-out economics assume caching);
...
- **Coverage-delta:** Volume I K16 (`14`) proposes baking the optimization pack into jackin❯; the
...
- **Availability:** BUILDABLE (jackin❯ roadmap; insertion points mapped in Volume I 14 K16 / 17).
...
| 7 | Min cacheable prefix Opus 4.8 = 1,024; Fable 5 = 512; Haiku 4.5 = 4,096 (matches Volume I 07) | platform.claude.com prompt-caching |
...
| 10 | Subscription fleet shares one pooled cap; headless/SDK draws separate API-rate credit | support.claude.com (file 19 ledger) |
