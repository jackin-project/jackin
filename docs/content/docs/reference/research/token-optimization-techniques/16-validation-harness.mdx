...
- **Tokens by class** come from `usage` in `claude -p --output-format json` (plus OTel or session
 ...
 [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/).
...
negations, and qualifiers carry disproportionate meaning per token ([04 — Style and Language Compression — Beyond Caveman](/reference/research/token-optimization-techniques/04-style-and-language-compression/)).
...
([01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/) §1) as a cross-check. Per-call class decomposition when needed:
enable OTel (`claude_code.token.usage` by `type`) or parse the session JSONL **deduplicating by
`message.id`** (trap documented in [02 — Baseline Audit of This Environment (Phase 0)](/reference/research/token-optimization-techniques/02-baseline-audit/) §1).
...
Every technique record in files [04 — Style and Language Compression — Beyond Caveman](/reference/research/token-optimization-techniques/04-style-and-language-compression/) through [14 — Frontier — unrealistic but maybe real](/reference/research/token-optimization-techniques/14-frontier-ideas/) names its falsification experiment; they all reduce to an
...
| Price table used for $ math | [01 — Token Economics and Measurement](/reference/research/token-optimization-techniques/01-economics-and-measurement/) (live-) |
| JSONL dedup + thinking-redaction traps | Local measurement, [02 — Baseline Audit of This Environment (Phase 0)](/reference/research/token-optimization-techniques/02-baseline-audit/) |
