...
do **not** duplicate Volume I's sixteen ([20 — Multimodal token economics](/reference/research/token-optimization-techniques/20-multimodal-token-economics/) K1–K16). Each is worked mechanism → savings math →
...
profile (note: Fable-priced; ~half for an Opus-4.8 subscriber once Fable 5 is removed, [24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/)); quota figures
use [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)'s model.
...
**Mechanism:** the subscription cap is a rolling 5-hour window plus a fixed weekly anchor ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)).
...
work. It is the quota-axis analogue of batch scheduling ([21 — Latency, wall-clock, and human-time as a second cost axis](/reference/research/token-optimization-techniques/21-latency-and-time-economics/) L5).
...
**Coverage-delta:** New. Directly attacks [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)'s bounded INCOMPLETE (the unpublished cap
...
0.1× ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)).

**Savings math:** no direct saving; it converts [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)'s "tasks-per-cap is unquantifiable" into a
...
*if* the proxy preserves `cache_control` (a careless proxy busts the cache — [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/) Q1). **Effort:**
medium.
...
the 3.05× per-image cap divergence ([20 — Multimodal token economics: images, screenshots, PDFs](/reference/research/token-optimization-techniques/20-multimodal-token-economics/)), which Volume I never measured.
...
**Savings math:** per full-frame screenshot, 4,784 → 1,568 image tokens = **−67%** ([20 — Multimodal token economics: images, screenshots, PDFs](/reference/research/token-optimization-techniques/20-multimodal-token-economics/) measured).
A 20-frame debugging session: 20 × (4,784 − 1,568) = 64,320 tokens shifted off the expensive family —
modest in dollars (image tokens at input price) but real in quota ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)) and window pressure, and
...
**Tier:** T1 (measured caps, [20 — Multimodal token economics: images, screenshots, PDFs](/reference/research/token-optimization-techniques/20-multimodal-token-economics/)). **Quality risk:** QUALITY-TRADE if the summary drops a visual
...
**Coverage-delta:** New. Volume I has zero multimodal; this operationalizes [20 — Multimodal token economics: images, screenshots, PDFs](/reference/research/token-optimization-techniques/20-multimodal-token-economics/)'s "text beats
...
tokens (exact, scrollable) instead of the 1,568–4,784 image cap or the 1.98–2.30× PDF tax ([20 — Multimodal token economics: images, screenshots, PDFs](/reference/research/token-optimization-techniques/20-multimodal-token-economics/)).
...
**~−50%** ([20 — Multimodal token economics: images, screenshots, PDFs](/reference/research/token-optimization-techniques/20-multimodal-token-economics/) measured). Plus exact characters and downstream grep-ability.

**Feasibility verdict:** BUILDABLE — needs a local OCR/extraction tool in the container (jackin❯ can
bake it in, [22 — Fleet, team & multi-tenant cache economics (hosted)](/reference/research/token-optimization-techniques/22-fleet-and-multitenant-cache/) F6). For born-digital PDFs `pdftotext` is trivial; OCR for screenshots is heavier.
...
**Coverage-delta:** New. Volume I never models latency; this automates [21 — Latency, wall-clock, and human-time as a second cost axis](/reference/research/token-optimization-techniques/21-latency-and-time-economics/)'s v·t·s > Δ$
inequality.
...
for 2× price ([21 — Latency, wall-clock, and human-time as a second cost axis](/reference/research/token-optimization-techniques/21-latency-and-time-economics/)), worth it when a developer-minute (~$0.83–1.25) times the minutes saved exceeds
...
returns ~3 minutes ≈ $3.75 of developer time (≈7:1, [21 — Latency, wall-clock, and human-time as a second cost axis](/reference/research/token-optimization-techniques/21-latency-and-time-economics/) ESTIMATE); on autonomous work it saves the
premium entirely (t≈0 → never buy speed). Net: the same total-cost optimum [21 — Latency, wall-clock, and human-time as a second cost axis](/reference/research/token-optimization-techniques/21-latency-and-time-economics/) derives, applied
automatically.
...
it re-bills the prefix, [21 — Latency, wall-clock, and human-time as a second cost axis](/reference/research/token-optimization-techniques/21-latency-and-time-economics/)).
...
**Coverage-delta:** New synthesis of [24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/) FL1 (CAG-via-caching) + [22 — Fleet, team & multi-tenant cache economics (hosted)](/reference/research/token-optimization-techniques/22-fleet-and-multitenant-cache/) (fleet workspace cache) +
...
`cache_control` prefix; pin the fleet to one workspace ([22 — Fleet, team & multi-tenant cache economics (hosted)](/reference/research/token-optimization-techniques/22-fleet-and-multitenant-cache/) F1) with `excludeDynamicSections` (F2)
so every container shares one cached copy; keep it warm with 1h TTL + a pre-warm/keepalive ping (Vol I
K13 / Aider's pattern, [23 — Cross-agent / cross-provider portability matrix](/reference/research/token-optimization-techniques/23-cross-agent-portability/) P2). Every container then reads the repo at 0.1× instead of
...
**Savings math:** the shared-prefix fleet math ([22 — Fleet, team & multi-tenant cache economics (hosted)](/reference/research/token-optimization-techniques/22-fleet-and-multitenant-cache/) F1/F3): N containers → 1 write + (N−1) 0.1×
reads of the repo core; cold-start ~10× cut (F3). Per turn, the repo core costs 0.1× instead of fresh
exploration tokens. Bounded by the 200K subscription context ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/reference/research/token-optimization-techniques/19-subscription-and-quota-economics/)) — the *core*, not the whole
repo, fits.

**Feasibility verdict:** BUILDABLE — jackin❯'s launcher is the natural home (it already owns the
insertion points, Vol I K16 / [22 — Fleet, team & multi-tenant cache economics (hosted)](/reference/research/token-optimization-techniques/22-fleet-and-multitenant-cache/) F6). The hard part is curating "the stable core" and keeping it
...
**Coverage-delta:** New. Connects [25 — Meta layer: the cost of optimizing, budget governance, and online quality guarding](/reference/research/token-optimization-techniques/25-meta-cost-governance-and-online-quality/)'s online judge (blind spot 8) to compression; Volume I's
...
default, with a sampled async LLM-as-judge ([25 — Meta layer: the cost of optimizing, budget governance, and online quality guarding](/reference/research/token-optimization-techniques/25-meta-cost-governance-and-online-quality/) G3) watching production traces for caveat-drop /
...
saving minus the guard tax ([25 — Meta layer: the cost of optimizing, budget governance, and online quality guarding](/reference/research/token-optimization-techniques/25-meta-cost-governance-and-online-quality/) G4: sampling 1–10%); positive when the compressed lane is large
and the judge is cheap.

**Feasibility verdict:** BUILDABLE — wire a validated reference-free judge (LangSmith/Braintrust/Arize
AX) over the compressed lane's traces with a revert webhook. The blocker is judge calibration ([25 — Meta layer: the cost of optimizing, budget governance, and online quality guarding](/reference/research/token-optimization-techniques/25-meta-cost-governance-and-online-quality/):
...
**Coverage-delta:** New. Operationalizes [23 — Cross-agent / cross-provider portability matrix](/reference/research/token-optimization-techniques/23-cross-agent-portability/)'s portability matrix; Volume I is single-agent.
...
prevents the silent loss when a team moves tools ([23 — Cross-agent / cross-provider portability matrix](/reference/research/token-optimization-techniques/23-cross-agent-portability/): ~80% of the stack ports as discipline, ~60%
...
**Tier:** T1 (each target's config surface, [23 — Cross-agent / cross-provider portability matrix](/reference/research/token-optimization-techniques/23-cross-agent-portability/)). **Quality risk:** NEUTRAL (config translation).
...
Volume I left them — blocked behind the hosted API (soft-prompts, KV export: K1/K2/[24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/)) — and the
