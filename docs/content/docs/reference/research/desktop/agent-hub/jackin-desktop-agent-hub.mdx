...
Source priorities, confidence labels, and accuracy reasoning for these apps live in [Token & Cost Telemetry → Better accuracy references](/reference/research/agent-telemetry/token-cost-telemetry/#better-accuracy-references). Desktop borrows their **macOS UI patterns**:
...
The data model and the `UsageConfidence` + `UsageSnapshotStatus` labels are the [telemetry roadmap's contract](/reference/research/agent-telemetry/token-cost-telemetry/#account-quota-snapshots); Desktop renders them.
...
- [Token & Cost Telemetry](/reference/research/agent-telemetry/token-cost-telemetry/) — broader
