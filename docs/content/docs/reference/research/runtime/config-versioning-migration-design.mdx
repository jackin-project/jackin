...
- **Config migration failure recovery.** Writes use `atomic_write` (temp file + rename) in <RepoFile path="crates/jackin-config/src/app_config_persist.rs">crates/jackin-config/src/app_config_persist.rs</RepoFile>, so a failed write aborts startup with a clear error instead of leaving a half-written file.
