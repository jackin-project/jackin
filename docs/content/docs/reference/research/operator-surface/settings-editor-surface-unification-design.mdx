...
**Status**: Design rationale and target architecture for the settings/editor convergence now tracked under [Codebase health: Rust strictness, structure, and AI reviewability](/roadmap/codebase-health-enforcement/). The auth source-folder validation and shared error dialog described here have shipped; the modal-enum collapse, host-trait, and per-domain convergence below are proposed target shape, not yet implemented. This page keeps the duplication inventory and architecture reasoning out of the roadmap item's day-to-day tracking.
...
Per the [TUI architecture](/reference/tui/): the shared config-surface model + a `ConfigModal` enum would live in `jackin-console` beside the existing shared components; pure decision/flatten logic lives in the screens' update modules (already partly there); dispatch/apply helpers would move into a shared `jackin-console` update module over time. The current handlers live in the app crate's input layer (<RepoFile path="crates/jackin-console/src/tui/input/editor.rs" />, <RepoFile path="crates/jackin-console/src/tui/input/global_mounts.rs" />); any move toward a `jackin-console` update layer should stage so each slice stays small and reviewable. This now relates to the consolidated [codebase-health roadmap](/roadmap/codebase-health-enforcement/), which owns both the remaining editor input split and settings/editor convergence.
...
- [Codebase health: Rust strictness, structure, and AI reviewability](/roadmap/codebase-health-enforcement/) — the consolidated roadmap item that now owns this convergence work.
