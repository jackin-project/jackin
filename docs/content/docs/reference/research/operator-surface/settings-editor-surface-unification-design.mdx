...
Generic state structs live once in `jackin-console`; concrete instantiations live in the `jackin` app crate (<RepoFile path="crates/jackin/src/console/tui.rs" />). The structural asymmetry at the root: the editor uses **one** unified modal enum `ConsoleModal` (<RepoFile path="crates/jackin-console/src/tui/app.rs" />); settings splits the same modal taxonomy across **three** enums (`GlobalMountModal`, `SettingsEnvModal`, `SettingsAuthModal`) plus a screen-level error-popup field (<RepoFile path="crates/jackin-console/src/tui/screens/settings/model.rs" />).
...
`decide_auth_source_folder` would wrap the already-shared `validate_auth_source_folder` (<RepoFile path="crates/jackin/src/console.rs" />) → runtime gate `validate_sync_source_dir` (<RepoFile path="crates/jackin-runtime/src/instance/auth.rs" />). The plan: one `handle_auth_modal<H>` and one of each `apply_*`/`open_*`/`restore_*` would replace the auth twins, with the same shape generalizing to the other domains.
...
- [Unify the settings and workspace-editor surfaces](/roadmap/unify-settings-editor-surfaces/) — the roadmap item this design rationale supports.
