...
| `terminal` | optional Crossterm event adapter and scoped session guard | Tokio, signal policy, panic-hook installation, logging initialization |
...
The optional terminal adapter should provide a scoped session with these rules:
...
| `terminal_modes.rs` | refactor into partial-init-safe optional Crossterm session | `terminal` |
