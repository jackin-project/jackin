...
- never use product-global atomics such as the current rich-surface/host-screen flags in `crates/jackin-tui/src/ownership.rs`.
...
The existing `SelectListState` in `crates/jackin-tui/src/components/select_list.rs` demonstrates why refactoring precedes publication: it owns `Vec<String>`, returns indices, performs fixed ASCII substring filtering, measures labels with `chars().count()`, and embeds action labels. The existing `TextField` in `crates/jackin-tui/src/components/text_input.rs` combines editing with product validation and action wording. Those behaviors are valuable donor implementations, but their current signatures are not the reusable API.
