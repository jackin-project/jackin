...
| `runtime` | executor-neutral `Dirty`/`UpdateResult`/`Subscription`/`Component`/`View` contracts and the `drive_frame` frame driver; base module optional by use | Tokio, spawning, effect execution, panic/signal policy, any requirement that widgets use it |
...
The executor-neutral runtime contracts — `Dirty`, `UpdateResult<E>`, `SubscriptionPoll`, the `Subscription` trait with std-channel and closure adapters, `Component<Ev, Msg>`, `View<Model>`, and the `drive_frame` frame driver — extract into the ordinary base `runtime` module. It is optional by use, not a Cargo feature: the initial manifest still has only the `crossterm` feature, no widget references `runtime`, and no architecture example requires it. The contracts are pure Ratatui, `jackin❯`'s canonical TUI rules already mandate the one shared frame driver across its console, Capsule, and launch surfaces, and deleting the donor would otherwise strand that driver in a new product-local crate. The module stays convenience, not framework, and future consumers are not required to adopt it. The Tokio receiver implementations, spawn helpers, and fallback runtime in <RepoFile path="crates/jackin-tui/src/runtime.rs">crates/jackin-tui/src/runtime.rs</RepoFile> stay in `jackin❯` with `jackin-console` as the target home; Rust coherence prevents a consumer crate from implementing the extracted `Subscription` trait directly for Tokio receiver types, so the shared module ships a closure-based adapter and consumers may wrap receivers in local newtypes. No fallback Tokio runtime is created inside the component library.
...
| `runtime.rs` | extract executor-neutral contracts and `drive_frame` into the base module, optional by use; Tokio adapters stay in `jackin❯` | `runtime`; Tokio helpers stay local |
