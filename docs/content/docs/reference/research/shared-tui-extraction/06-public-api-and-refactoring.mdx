...
| `runtime` | optional executor-neutral `Dirty`/`UpdateResult`/`Subscription`/`Component`/`View` contracts and the `drive_frame` frame driver | Tokio, spawning, effect execution, panic/signal policy, any requirement that widgets use it |
...
The executor-neutral runtime contracts — `Dirty`, `UpdateResult<E>`, `SubscriptionPoll`, the `Subscription` trait with std-channel and closure adapters, `Component<Ev, Msg>`, `View<Model>`, and the `drive_frame` frame driver — extract as one optional `runtime` module. They are pure Ratatui, `jackin❯`'s canonical TUI rules already mandate the one shared frame driver across its console, Capsule, and launch surfaces, and deleting the donor would otherwise strand that driver in a new product-local crate. The module stays convenience, not framework: no widget or example requires it, and Holla is not expected to adopt it. The Tokio receiver implementations, spawn helpers, and fallback runtime in <RepoFile path="crates/jackin-tui/src/runtime.rs">crates/jackin-tui/src/runtime.rs</RepoFile> stay in `jackin❯` with `jackin-console` as the target home; Rust coherence prevents a consumer crate from implementing the extracted `Subscription` trait directly for Tokio receiver types, so the shared module ships a closure-based adapter and consumers may wrap receivers in local newtypes. No fallback Tokio runtime is created inside the component library.
...
| `runtime.rs` | extract executor-neutral contracts and `drive_frame` as one optional module; Tokio adapters stay in `jackin❯` | `runtime` (optional); Tokio helpers stay local |
