...
- Conventional Commits and DCO sign-off from the first commit;
- during the roadmap bootstrap only, signed buildable checkpoints push directly to `main` with no TermRock PR, and source/story/SVG/docs changes stay in the same commit;
- after the roadmap completes, protect `main`, require one PR per branch, and require docs in the same PR as API changes;
...
and Fumadocs page: one signed commit during direct-`main` bootstrap and one PR
after bootstrap. Required docs jobs:
