...
|- crates/
|  |- termrock/
|  |  `- src/{input,interaction,layout,scroll,style,terminal,text,widgets}/
|  `- termrock-lookbook/
|     |- src/lib.rs
...
Start with the same current compatibility floor as the first consumer: Rust edition 2024, `rust-version = "1.95"`, and the tested toolchain pinned to `1.97.0`, matching <RepoFile path="Cargo.toml">Cargo.toml</RepoFile> and <RepoFile path="rust-toolchain.toml">rust-toolchain.toml</RepoFile> at research time. Before repository creation, check Holla's MSRV. The shared crate may lower the floor to the lowest intended consumer only if all dependency versions and all feature combinations pass there. Cargo expects all targets and features promised by a package to work at its declared `rust-version`; see the [Cargo rust-version reference](https://doc.rust-lang.org/cargo/reference/rust-version.html).
...
- Conventional Commits, DCO sign-off, protected `main`, one PR per branch, and docs in the same PR as API changes;
...
Use path classification and two required aggregators, `rust-required` and `docs-required`, so branch protection does not change when an expensive job is skipped. Every workflow has least-privilege permissions, a timeout, and PR concurrency cancellation.
...
| `examples` | check direct, TEA, component, Flux, and buffer-only examples through public APIs | application-pattern neutrality |

The base crate must also pass `cargo check -p termrock --no-default-features` and a dependency assertion that finds no Tokio/Crossterm package in that graph. `buffer_only` is checked with no default features; terminal examples are checked with the declared Crossterm feature. The Crossterm feature receives a separate all-target check on Linux, macOS, and Windows if the project claims those platforms.
...
- verify package archives include license, readme, source, and required generated metadata but exclude target output and docs build caches;
...
| `dco` | every commit carries a valid `Signed-off-by` trailer |
...
REUSE requires comprehensive, machine-readable license and copyright information per file. Preserve existing SPDX headers during history extraction, put the Apache-2.0 text under `LICENSES/`, and annotate generated SVG/JSON assets through headers, sidecar files, or `REUSE.toml`. See the [REUSE 3.3 specification](https://reuse.software/spec/) and [REUSE FAQ](https://reuse.software/faq/).
...
Only one platform owns canonical render files, initially Linux with a fixed locale and Unicode-width dependency lock. Other platforms run buffer/behavior tests but do not rewrite fixtures. This prevents line-ending or font-tool differences from creating multiple authorities.
...
The same pull request that changes a public component changes its story, SVG, and Fumadocs page. Required docs jobs:
...
direct, TEA, component-oriented, Flux, and buffer-only integrations. These are
different consumer compositions over the same public components, not separate
TermRock runtime APIs.
...
Keep required PR latency bounded by moving broad but valuable checks to a scheduled workflow:
...
- run semver, package, license, docs, and consumer-smoke gates;
