...
|- fixtures/renders/
|- examples/{direct,tea,component,flux,buffer_only}.rs
...
The shared project may expose small executor-neutral contracts such as modal outcomes, focus ownership, and dirty/redraw state. It does not expose effect envelopes that can execute themselves, and it must not prescribe one product's root `Model`, effect executor, persistence, daemon, or Tokio task tree. Standard Ratatui `Widget`/`StatefulWidget` rendering remains usable without the optional component/update layer.

The repository includes compile-checked direct, TEA, component-oriented, Flux,
and buffer-only examples. Every example uses the same public widgets and
interaction types while defining its own root state and routing, proving that
TermRock supports application patterns instead of prescribing one.

The base `termrock` crate depends on `ratatui-core`, following Ratatui's guidance for widget-library authors. Crossterm raw mode, alternate screen, mouse capture, event conversion, and restoration live behind an optional `crossterm` feature. The lookbook enables that feature; a buffer-only consumer does not compile a terminal backend. The initial library has no Tokio feature.
