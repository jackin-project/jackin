...
The Fumadocs application lives in the same repository. Component source, stories, generated SVGs, docs pages, and their verification gate therefore advance in one change unit and cannot drift across repositories: one signed direct-`main` commit during bootstrap, then one normal TermRock PR after the roadmap completes.
...
Repository governance has one explicit bootstrap exception: while this roadmap's single `jackin❯` PR is open, DCO-signed, buildable TermRock checkpoints push directly to `main` and do not open TermRock PRs. After the final roadmap checkpoint, TermRock enables its normal protected-`main`, branch, and pull-request workflow for all later changes.
