...
7. **Executor restraint.** Widgets and state do not require Tokio. The initial base crate has no async-runtime dependency; add an adapter only after two consumers prove the same executor-neutral need.
