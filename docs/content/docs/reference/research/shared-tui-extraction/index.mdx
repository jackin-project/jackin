...
- **Implementation delivery:** one future `jackin❯` branch/PR covers the complete roadmap; TermRock bootstrap checkpoints are signed commits pushed directly to its `main`, and `jackin❯` pins each exact revision.
...
- **Runtime:** executor-neutral update/view/subscription contracts and the `drive_frame` frame driver live in the base `runtime` module, optional by use and required by no widget; Tokio integration stays consumer-owned.
