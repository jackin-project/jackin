...
`Subscription`, or `EffectExecutor` trait. An optional executor-neutral
`runtime` module (dirty/update contracts, subscriptions with std-channel and
closure adapters, component/view traits, and the `drive_frame` frame driver)
exists for consumers that want the donor's TEA conventions; no widget or other
module references it, and nothing requires adopting it.
...
|  |- request.rs
|  `- encode.rs
|- runtime/                  # optional executor-neutral contracts
|  |- contract.rs
|  |- subscription.rs
|  `- frame.rs
|- widgets/
...
| `scroll.rs` plus scroll helpers in `jackin-core` | consolidate | Create one canonical offset/axis/thumb/viewport implementation. Retain the maintained `tui-scrollbar` dependency as the single proportional thumb-metrics source. Choose one integer policy, document saturation, and remove parallel `usize`/`u16` APIs unless the conversion is unavoidable at the Ratatui edge. |
...
| `runtime.rs` | split | Extract the executor-neutral contracts and `drive_frame` as the optional `runtime` module: pure Ratatui, and `jackin❯` mandates one shared frame driver across console, Capsule, and launch, so donor deletion would otherwise strand it. Tokio receiver implementations, spawn helpers, and the fallback runtime stay in `jackin❯` (`jackin-console`), connecting through the module's closure adapter because coherence forbids direct foreign impls. |
