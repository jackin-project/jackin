...
- Subagent effort pins: exploration/read-only agents get `effort: low` + cheap model frontmatter — jackin generates agent definitions, so the dossier's "subagents are NOT automatically cheap (`model: inherit`)" trap is structurally closed.
...
- **Cheap-fleet pinning**: read-only fan-out to Haiku-tier (5-worker fan-out $2.75 → ~$0.20–0.28 class, −90–93% ESTIMATE); code-writing teammates stay Sonnet-tier (Haiku SWE-bench gap ~6pp — the quality-delta map).
- **Tokenizer-aware routing** (dossier 11/16): the Fable/Opus-4.8 tokenizer family bills ASCII prose up to +59% vs the Sonnet/Haiku family — effective downtier ratios reach ~13–14.5× on prose/markdown work, exactly 3× Sonnet→Haiku. The engine counts routed corpora with `count_tokens` against the *target* model before committing a route; never ports counts across families.
