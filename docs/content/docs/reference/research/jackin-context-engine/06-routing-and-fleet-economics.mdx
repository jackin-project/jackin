...
- **Tokenizer-aware routing** (dossier 11/16): the Fable/Opus-4.8 tokenizer family bills ASCII prose up to +59% vs the legacy Sonnet-4.6/Haiku-4.5 family — effective downtier ratios reach ~13–14.5× on prose/markdown work, exactly 3× Sonnet→Haiku. Scope matters (updated 2026-07-05): Sonnet 5 (`claude-sonnet-5`, shipped 2026-06-30, $2/$10 per MTok intro through 2026-08-31 then $3/$15, 1M context) uses the **new** Opus-4.7+ tokenizer family, so the extra ~1.2–1.45× tokenizer discount on downtiering applies only to legacy Sonnet 4.6 / Haiku 4.5 targets; routing to Sonnet 5 is a pure price-ratio play — an unusually cheap one during the intro window — with zero tokenizer bonus ([ch. 50](/reference/research/token-optimization-techniques/50-live-reverification/)). The engine counts routed corpora with `count_tokens` against the *target* model before committing a route; never ports counts across families.
...
- **`excludeDynamicSections` enforced fleet-wide** with an SDK version floor: six per-container fields (~111 tokens: cwd, git flag, platform, …) placed ahead of the prefix otherwise un-share the entire ~28–34k system prefix across every container in the fleet — the single biggest hosted-fleet lever (Vol II ch. 44).
