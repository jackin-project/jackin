...
5. **Subscriber (tasks-per-cap) economics per tool.** The dossier has the metric (chapter 19); the hub never asks *which of the three most extends a Max-plan 5-hour window* — a different ranking from $-per-task, and the metric most operators actually feel.
