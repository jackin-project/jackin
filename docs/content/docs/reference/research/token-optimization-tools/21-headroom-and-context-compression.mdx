...
Volume III deep-dive requested after the code-intelligence sweep ([19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/)) and the vector-database follow-up ([20 — Qdrant and vector databases](/reference/research/token-optimization-tools/20-qdrant-and-vector-databases/)): analyze `chopratejas/headroom`, compare it in depth to the caveman ecosystem the operator already runs, re-sweep the internet for context-compression projects the dossier missed, and fold the result back into the dossier without editing the frozen Volume I/II files. Research conducted 2026-06-15; every external claim carries a source + access date in the ledger; headroom's product numbers are vendor self-reported and are tiered accordingly.
...
- **jackin❯ verdict:** pilot headroom's **MCP mode** as an A/B arm against the levers the dossier already recommends (hook filtering — record 20; code-intelligence outlines — [19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/); serialization — record 14), measured on incremental tokens-per-solved-task. **Do not default to proxy mode** in a jackin❯ container: it adds a cache-bust risk, a per-request ML model in the hot path, a CompressionAttack surface ([techniques 24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/) FL3), and a double-compaction conflict with Claude Code's own context management.
...
|---|---|---|---|
| **LogCompressor** | Keep errors/stack traces/levels, drop passing noise | Record 20 (preprocessing/hooks filtering) | **T1, local −94.2%** on a synthetic cargo log, all failures preserved ([03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/)) |
| **CodeAwareCompressor** | Keep imports/signatures/types, collapse bodies | Record 16 (aider repo map) + [techniques 06 — Context architecture — what enters the window at all](/reference/research/token-optimization-techniques/06-context-architecture/) + [19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/) | **T1, local −91%** outline vs whole-file read ([19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/)) |
| **SearchCompressor** | `file:line:content`, drop verbose detail | [19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/) (codedb/fff symbol search) | **T1, local −98%** symbol-search vs file read ([19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/)) |
| **SmartCrusher** | JSON arrays → sampled/typed, keep anomalies | Record 14 (TOON + JSON minification) | **T1, local −34.3% minify / −41.2% TOON** vs minified ([03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/)) |
...
| **CacheAligner** | Extract volatile content, stabilize prefix, insert `cache_control` | Record 05/13 (cache hygiene) + [techniques 24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/) FL1/FL2 | T1 (Anthropic caching mechanics) |
...
| **Cross-agent memory** | Shared, auto-dedup store across Claude/Codex/Gemini | Record 02 (cavemem) + 15 (claude-mem) + [techniques 23 — Cross-agent / cross-provider portability matrix](/reference/research/token-optimization-techniques/23-cross-agent-portability/) | New cross-agent angle (see H4) |
...
- **Coverage-delta:** Refines record 19 + [techniques 24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/) FL3. Volume I/II treated input compression as monolithically cache-hostile; this records the cache-compatible sub-design.
- **Layer:** input + cache.
...
- **Evidence tier:** T1 for the mechanism (the underlying log/outline/minify levers are locally reproduced in [03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/) and [19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/)); **T3-weak for headroom's specific product numbers** (vendor self-report, no independent replication); **T2 academic backing for the write-time pattern itself** — Squeez (arXiv 2604.04979 — 92% tool-output token removal at 0.86 recall, run as a write-time Unix pipe, cache-safe, code-domain) and AgentDiet (arXiv 2509.23586 — Claude 4 Sonnet 64.5%→66.5% with input −40–60%, the only paper in the class that nets out the compressor's own +5–15% cost, the net-accounting white-space #5 demanded).
...
- **Composability:** feeds record 07 (CLAUDE.md) and the jackin❯ `[token_policy]` idea ([techniques 17 — Adoption Roadmap: Day 1 / Week 1 / Month 1](/reference/research/token-optimization-techniques/17-adoption-roadmap/)); anti-synergy with prefix slimming if left unbounded.
...
- **Coverage-delta:** Extends record 02 (cavemem) / record 15 (claude-mem) / [techniques 23 — Cross-agent / cross-provider portability matrix](/reference/research/token-optimization-techniques/23-cross-agent-portability/) (cross-agent portability) with a cross-tool, auto-dedup angle none of them cover.
- **Layer:** input (memory).
...
A clean-room sweep for compression-layer projects the dossier's [03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/) and [24 — Fresh literature & market delta](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/), plus hub [19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/) and [20 — Qdrant and vector databases](/reference/research/token-optimization-tools/20-qdrant-and-vector-databases/) do not already cover. Code-intelligence retrievers (codedb, fff, Serena, Code Context Engine, Claude Context, Sourcegraph, Augment, Qodo) are in [19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/); vector backends are in [20 — Qdrant and vector databases](/reference/research/token-optimization-tools/20-qdrant-and-vector-databases/); this section is the *compression/proxy/memory* layer specifically. Numbers are vendor self-report unless marked; none is locally reproduced here.
...
| **CompactPrompt** | Prompt compression guide/lib | Prune + abbreviate + quantize data | "up to 60%" | Self-host | T4 ([techniques 24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/)) |
...
Headroom is a direct test of the literature trends [techniques 24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/) already tracked, and it sharpens two of them:
...
Headroom fits the same role-scoped, opt-in, measured-locally pattern [19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/) set for code-intelligence tools, with one extra guardrail because it sits closer to the model.
...
3. **A/B against the levers the dossier already banks**, not against a naive baseline: hook filtering (record 20), code-intelligence outlines ([19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/)), and serialization (record 14) already capture most of headroom's compressible wins, cache-safely and with no extra dependency. Headroom earns its place only if it beats that stack net of MCP schema rent and retrieve round-trips.
...
Run the same shape as [19 — Code-intelligence tools](/reference/research/token-optimization-tools/19-code-intelligence-tools/), with cache continuity added as a first-class metric:
...
- cross-references (caveman/cavemem/cavekit/cavecrew records, K1/K4, white-space map): [03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/)
- cache-conflict + CompressionAttack + CAG/FL1/FL3: [24 — Fresh literature & market delta (clean-room re-sweep)](/reference/research/token-optimization-techniques/24-fresh-literature-and-market-delta/)
- log-filter −94.2% / TOON −41.2% / minify −34.3% local reproductions: [03 — Prior art and market scan](/reference/research/token-optimization-techniques/03-prior-art-and-market-scan/)
- outline −91% / symbol-search −98% local reproductions: [19 — Code-intelligence tools: codedb, fff, CodeGraff, and alternatives](/reference/research/token-optimization-tools/19-code-intelligence-tools/)
- fresh-literature sources (write-time compression, code-domain SWE-bench gains, cache break-even, output-brevity dominance, context rot) and the compression-tool market sweep: [22 — Context-compression literature and market delta (2024–2026)](/reference/research/token-optimization-tools/22-context-compression-literature-and-market/). Key inline IDs: Squeez arXiv 2604.04979; AgentDiet arXiv 2509.23586; "Don't Break the Cache" arXiv 2601.06007; Claude 4.5 compression RCT arXiv 2603.23525; SWEzze arXiv 2603.28119; SWE-Pruner arXiv 2601.16746; Perplexity Paradox arXiv 2602.15843; Chain-of-Draft arXiv 2502.18600; brevity-hierarchy arXiv 2604.00025.
