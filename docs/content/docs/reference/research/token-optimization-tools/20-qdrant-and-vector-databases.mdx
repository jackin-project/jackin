...
- **The dossier already covered the category, but not Qdrant specifically.**
  File `08` called semantic/RAG repo indexing the largest evidence gap; file `13`
  rejected semantic response caching for coding agents; chapter `19` compared
...
- whole-response semantic caching, which file `13` already killed for coding
  agents.
