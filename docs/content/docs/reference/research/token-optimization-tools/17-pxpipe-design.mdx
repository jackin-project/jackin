...
| https://registry.npmjs.org/pxpipe | Empty — bare name unclaimed; squat risk |
...
| https://raw.githubusercontent.com/teamchong/pxpipe/HEAD/src/core/ — `proxy.ts`, `transform.ts`, `render.ts`, `history.ts`, `applicability.ts`, `atlas.ts` | Mechanism verification at source (T1 basis) |
