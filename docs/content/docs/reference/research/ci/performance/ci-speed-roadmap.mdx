...
After every successful crate job, the configured canonical lane publishes a seven-day result marker
...
parallel matrix even when each cache-hit job did no work. The writer also
publishes an exact-SHA alias for release workflows. GitHub is the canonical
writer when present, while a Velnor-only dispatch makes Velnor the writer; both
lanes read the same marker names and execute the same lookup and test contract.
