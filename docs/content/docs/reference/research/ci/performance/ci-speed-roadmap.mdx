...
entries; the prepared tools and xtask own about `86MB` per active ref, use no
prefix fallback, and remain subject to the existing per-run cache-budget audit.
