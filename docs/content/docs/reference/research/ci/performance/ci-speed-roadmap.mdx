...
each selected crate's single job owns all applicable test, check, lint, MSRV,
...
closure. Default/all-feature checks, clippy, nextest, doctests, MSRV, applicable
...
run also exposed 1,191 unchanged third-party builds in Velnor's MSRV check:
the runner-local current target was warm, but `target/msrv` was not. A local
target is now accepted only when both current and MSRV outputs exist; otherwise
the job restores the portable target. Finally, registry verification triggered
