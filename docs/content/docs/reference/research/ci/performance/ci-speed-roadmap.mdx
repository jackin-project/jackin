...
For an explicit `lanes=both` producer run, Velnor is listed first under that
same lock so its purpose-built persistent runner performs a cold build before
the hosted parity consumer; GitHub-only automatic and manual runs are
unchanged. This prevents optional release work from occupying the hosted slots
needed by required pull-request admission.
