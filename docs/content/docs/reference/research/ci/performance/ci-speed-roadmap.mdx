...
latest pointer whose run did not own the referenced target parts. Cache and
...
Latest pointers independently resolve the run owning the matching ready marker,
and are republished only alongside newly uploaded target parts. The same
