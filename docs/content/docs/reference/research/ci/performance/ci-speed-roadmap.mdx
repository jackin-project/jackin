...
added `quick-xml`: `Cargo.lock` and the root workspace manifest were treated as
unconditional full-workspace changes, and every crate target key hashed both
files wholesale. The run therefore launched all 28 crate jobs even though the
dependency was consumed only by `jackin-xtask`; completed cold jobs included
