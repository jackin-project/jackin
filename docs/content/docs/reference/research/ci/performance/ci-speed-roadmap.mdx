...
The shared setup job now treats prepared xtask/tool hits as a complete warm
state: it skips the Rust toolchain, Cargo registry, mise, and clippy setup that
only cold artifact population requires. On a miss, registry preparation runs
