...
toolchain directories. Warmup is now the only installation owner, the invalid
rustup cache namespace advances to `v2`, and downstream jobs use the
download-free `ci-toolchain activate` command against that prepared cache.
