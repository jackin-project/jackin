...
| `cargo-hakari` | **No-adopt pending first measured delta** | Hygiene job `hakari-timing` now records real tool exit status and before/after workspace build wall-clock. Decision flips to **Adopt** only if a scheduled run shows a clear wall-time win worth the workspace-hack maintenance cost; otherwise stays no-adopt. See plan 012. |
