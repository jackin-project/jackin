...
- **Availability:** CLAUDE-CODE-TODAY (`NO_COLOR=1`, `--no-unicode` flags on CLIs the agent invokes; a style line in <RepoFile path="AGENTS.md" /> / output style).
...
- **Expected savings:** up to 60% of the identifier component in pathological constants-heavy text (355→143); realistically 1–5% of a code-heavy session — ESTIMATE; compounds in always-loaded surfaces (tool schemas, <RepoFile path="AGENTS.md" />) via cache reads.
- **Evidence tier:** T1 (local).
