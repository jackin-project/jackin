...
- **Expected savings:** the economics died before the block matters: this repo's root agent-rules file (<RepoFile path="AGENTS.md" />) = 2,744 fable-5 tokens , riding at cache-read ≈ **$0.0027/request**; absolute daily ceiling if deleted entirely: 2,744 × 150 req × $1e-6 ≈ $0.41 reads + ~$0.07 expiry writes ≈ **$0.48/day (2.8%)** (ESTIMATE). Not worth a training pipeline even where one existed.
...
- **Feasibility verdict:** BUILDABLE today — in this repo it belongs beside the existing pre-merge gates (roadmap freshness, docs) described in <RepoFile path="PULL_REQUESTS.md" />.
