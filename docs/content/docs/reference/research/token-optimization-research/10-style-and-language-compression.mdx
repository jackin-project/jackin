...
- Because one visible-output token costs as much as **50 cache-read tokens** ($50 vs $1/MTok on Fable 5), compressing the always-cached instruction side is ~50x less valuable per token: a 60% cut of this repo's 2,738-token <RepoFile path="AGENTS.md" /> saves **≈$0.05/session (~1.4%)** — and carries rule-misread risk disproportionate to that.
...
Compressing always-on instructions saves every turn, but at cache-read rates: a 60% cut of this repo's 2,738-token <RepoFile path="AGENTS.md" /> is worth about a nickel per session.
...
| Caveman-ultra 58.5%; wenyan-full 56.6% (80.9% char); wenyan-ultra 74.5%; thinking 54.8% of output; prompt mix 0.44/6.73/92.83%; dollar split 32/29/20/17/2; <RepoFile path="AGENTS.md" /> 2,738 tok; MCP 1,420 vs ~60 tok | Local phase-0 measurements (see 01-economics-and-measurement.md, 02-baseline-audit.md) |
| ≈$0.05/session for 60% <RepoFile path="AGENTS.md" /> cut; 50:1 output:cache-read value; 10.2% / 12.7% session-dollar arithmetic | ESTIMATE, arithmetic shown inline, on modeled session profile + reference pricing ($10/$50, cache read 0.1x, write 1.25x) |
