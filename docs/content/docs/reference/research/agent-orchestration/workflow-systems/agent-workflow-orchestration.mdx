...
| Context and memory | Resolve a workflow-scoped memory brief from the [persistent storage and workspace memory layer](/reference/research/agent-orchestration/memory/persistent-storage-layer/) before each agent session. Do not depend on one runtime's private memory as the shared source of truth. |
...
| Container-only isolation may be weaker than VM/forkable sandbox competitors. | Keep [selectable sandbox backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) compatible with the run model. Do not bake "Docker container" into workflow state. |
...
`MemoryBrief` is the compact context package resolved for one phase/session. It should include only the memories relevant to the current run, workspace, role, workflow, and agent: operator preferences, workspace facts, workflow rules, recent run summaries, and explicit operator notes. It is built from the [persistent storage and workspace memory layer](/reference/research/agent-orchestration/memory/persistent-storage-layer/) and delivered through runtime adapters; it is not copied from Claude Code's or Codex's private memory store.
...
| [Persistent storage layer](/reference/research/agent-orchestration/memory/persistent-storage-layer/) | Durable run records, event logs, GitHub links, review results, and retry/cancel state. |
...
The current `status` and `snapshot` control calls are enough for display, but not enough for reliable workflow orchestration. The runner needs a typed control surface that can manipulate sessions without pretending terminal text is an API. [Terminal observation and automation](/reference/research/agent-orchestration/terminal-observation/terminal-observation-automation/) owns the detailed read/wait/send/capture/tab/session API shape; this section names the subset the workflow runner depends on.
...
- [Agent Orchestration Program](/reference/research/agent-orchestration/program-research/) — parent research program.
...
- [Persistent storage layer](/reference/research/agent-orchestration/memory/persistent-storage-layer/) — durable run records and event logs.
...
- [Native APM support](/reference/research/agent-runtimes/package-manager-support/native-apm-role-support/) — package/install layer for skills, prompts, and MCP clients used by workflow profiles.
