...
**Status**: Open — phased implementation program (fleet-operations track plus containment/recovery track)
...
This page is the task plan for the orchestration program. It keeps only the decision inputs needed to explain the roadmap shape; full external-product dossiers live under [/reference/research/](/reference/research/).
...
- [cellshot](https://github.com/kitlangton/cellshot) and adjacent terminal recorder/test tools are the strongest reference for **agentic terminal observation and automation**: structured terminal frames, visible-text waits, input injection, artifact bundles, and replayable traces. They directly inform [Terminal observation and automation](/reference/research/agent-orchestration/terminal-observation/terminal-observation-automation/), the Phase 2 work that makes Capsule sessions script-addressable — Playwright-like control for jackin❯ instances, tabs, prompts, waits, and captures — without treating Docker logs or screenshots as the API.
- [Orca](https://github.com/stablyai/orca) is the strongest reference for **desktop-native UX for multi-agent workflows**: worktree-per-task visual orchestration, WebGL multi-pane terminals, diff-annotation-to-agent feedback loops, embedded Chromium per worktree for live UI inspection, and 30+ pre-configured agent runtimes at zero config. Its isolation model (host-native worktrees, agents run as host user) is the opposite of the jackin❯ model, making it a useful UX benchmark without being a viable architecture borrow. See [Orca ADE research](/reference/research/agent-orchestration/orca-ade/) for the full evaluation.
...
| Run Docker/Compose without trusting the host daemon | Docker Sandboxes private Docker Engine; Hazmat Tier 3 | Keep host socket mounting out of scope; make private-daemon backends the Docker-capable path under [selectable sandbox backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) |
...
| Script, test, and replay a visible agent terminal | cellshot `wait`/`send`/`snapshot`; VHS `.tape`; asciinema recordings; tui-test `getByText` and traces | [Terminal observation and automation](/reference/research/agent-orchestration/terminal-observation/terminal-observation-automation/) should turn Capsule sessions into typed read/wait/send/snapshot/record targets for CLIs, tests, future workflow runners, and external adapters |
...
| Docker tier decision | Treat host Docker daemon access as a policy boundary, not a convenience default | [Selectable sandbox backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/), [network egress policy](/roadmap/network-egress-policy/) |
...
|---|---|---|
| Agent isolation | `bwrap` + `systemd-run`; Apple `container` | Have — Docker + DinD; planned — [selectable backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) |
...
| Per-agent token / cost / OOM tracking | Yes — usage aggregation service | *Gap — see [Token & Cost Telemetry](/reference/research/agent-telemetry/token-cost-telemetry/)* |
...
| Persistent storage backend | Per-workspace SQLite + per-workspace JSON | *Gap — see [Persistent storage layer](/reference/research/agent-orchestration/memory/persistent-storage-layer/)* |
...
| Strong local isolation | macOS user + seatbelt + `pf`; VM tier for hardest cases | Per-sandbox microVM | [Selectable sandbox backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) |
...
| Credential delivery | Host-owned secret store, materialized/brokered per harness | Host proxy injects credentials; values stay outside VM | [Container credential exposure](/reference/research/security/credential-exposure/container-credential-exposure/) and [host bridge](/roadmap/host-bridge/) |
...
| [Terminal observation and automation](/reference/research/agent-orchestration/terminal-observation/terminal-observation-automation/) | cellshot-style visible waits, terminal frames, input injection, artifact bundles, and replay traces | jackin❯ Capsule control plane; agent runtime status for semantic waits |
| [Déjà Vu — cross-agent conversation capture](/reference/research/agent-orchestration/conversation-capture/deja-vu/) | SpecStory-style capture of every operator prompt and agent answer, normalized and archived for browse/replay | Multi-runtime adapter seam; persistent storage; jackin❯ Capsule control plane |
...
| [Public attribution and project growth](/reference/research/product-growth/attribution/public-attribution-and-growth/) | Orca-style Git commit trailers and PR metadata as discoverability surfaces | Operator handler system; local repo policy |
...
| Item | Inspiration in multicode | Depends on |
|---|---|---|
| [Persistent storage layer](/reference/research/agent-orchestration/memory/persistent-storage-layer/) | Per-workspace SQLite | — |
| [Token & Cost Telemetry](/reference/research/agent-telemetry/token-cost-telemetry/) | Usage aggregation | Persistent storage; agent runtime status |
...
| [Agent workflow orchestration](/reference/research/agent-orchestration/workflow-systems/agent-workflow-orchestration/) | Multi-agent task pipeline with parallel review fan-out and deterministic phase routing | Task source; persistent storage; agent runtime status; Capsule session control |
...
| [Container credential exposure](/reference/research/security/credential-exposure/container-credential-exposure/) | Docker Sandboxes credential proxy; Hazmat secret store | Host bridge; credential source pattern |
...
| Item | Primary inspiration | Depends on |
|---|---|---|
| [Selectable sandbox backends](/reference/research/security/sandbox-backends/selectable-sandbox-backends/) | Docker Sandboxes microVM + private daemon; Hazmat Tier 3 decision | Runtime backend refactor |
...
| [Devcontainer parity](/reference/research/runtime/devcontainer-parity/) | Claude/devcontainer firewall and reproducibility pattern | Network egress policy |
...
- [Container credential exposure](/reference/research/security/credential-exposure/container-credential-exposure/) —
...
- [Terminal observation and automation](/reference/research/agent-orchestration/terminal-observation/terminal-observation-automation/) — jackin❯ roadmap item for `cellshot`-style terminal frames, visible waits, input injection, artifact bundles, recording/replay, and automation scripts on top of Capsule sessions.
