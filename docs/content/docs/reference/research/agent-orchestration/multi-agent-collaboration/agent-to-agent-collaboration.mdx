...
- **Codenames** come from [agent codenames](/roadmap/agent-codenames/): every worker already gets a stable, never-reused `JACKIN_AGENT_CODENAME`, so the orchestrator can address "falcon, take task 1" and the operator can read the same handle on screen.
