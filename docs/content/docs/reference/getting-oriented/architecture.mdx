...
Implementation: <RepoFile path="crates/jackin-capsule/src/daemon.rs" />, <RepoFile path="src/runtime/launch/mod.rs" /> (`launch_role_runtime`), <RepoFile path="src/runtime/attach/mod.rs" /> (`reconnect_or_create_session`, `spawn_agent_session`, `spawn_shell_session`).
