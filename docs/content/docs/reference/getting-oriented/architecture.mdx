...
| `derived image` | Decide reuse from local image tags and recipe labels; on a miss, generate the role-specific `DerivedDockerfile`, assemble the temporary Docker build context, copy cached runtime assets into it, run `docker build`, and capture Docker stdout/stderr to a plain run sidecar log on failure. Reuse launches do not block on published-image pulls or agent release lookups; those staleness checks run after attach in a background sentinel, and a stale result rebuilds the image for the next launch. |
