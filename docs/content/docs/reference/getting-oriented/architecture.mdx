...
`JACKIN_AGENT` is set per agent PTY by <RepoFile path="crates/jackin-capsule/src/session/mod.rs" />, immediately before `/jackin/runtime/entrypoint.sh` runs. The daemon's own process does not carry `JACKIN_AGENT`, so shell panes and future mixed-agent sessions do not inherit a misleading container-wide agent value.
...
Implementation: <RepoFile path="crates/jackin-capsule/src/daemon/mod.rs" />, <RepoFile path="src/runtime/launch/mod.rs" /> (`launch_role_runtime`), <RepoFile path="src/runtime/attach/mod.rs" /> (`reconnect_or_create_session`, `spawn_agent_session`, `spawn_shell_session`).
...
Implementation: <RepoFile path="crates/jackin-capsule/src/client.rs" />, <RepoFile path="crates/jackin-capsule/src/tui/input/mod.rs" />, <RepoFile path="crates/jackin-capsule/src/session/mod.rs" />, and <RepoFile path="crates/jackin-capsule/src/daemon/mod.rs" />.
