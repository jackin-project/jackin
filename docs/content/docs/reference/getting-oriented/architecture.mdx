...
The built-in agent set still has a typed identity enum, but runtime behavior now rides through a sealed adapter registry. `AgentRuntime` owns the questions that vary by agent: slug and label, install block, version parsing, auth capabilities, state-path layout, source-folder hints, manifest model access, and related runtime metadata. Adding a built-in runtime should mean adding one adapter plus the small number of centralized accessor rows that remain because the on-disk schemas still use named fields.

Providers follow the same direction. The provider registry owns provider labels, availability, token selection, environment overrides, and generated provider-config fragments for Codex/OpenCode-style runtimes. A provider feature must extend the registry rather than adding closed matches at launch, capsule spawn, and console UI call sites.

Some named-field accessors deliberately remain. `AppConfig`, `WorkspaceConfig`, `RoleConfig`, and `RoleManifest` keep `claude` / `codex` / `amp` / `kimi` / `opencode` fields because that is the versioned TOML surface operators and role authors already read. Collapsing those fields to a map would require a schema bump and migrations, and the current centralized accessors keep the dispatch local enough that a sixth runtime is still one adapter plus one row in each named-field accessor. The exception is acceptable only while the matches stay centralized and documented; new scattered `Agent::...` or `Provider::...` matches are regressions.
...
Role repositories are normal source repositories, so jackin' keeps role maintenance split between two surfaces. The main operator CLI owns desktop ergonomics: its role-authoring dispatch validates checked-out role repositories, applies manifest migrations to local copies, and writes small starter repositories for authors. That path lives in <RepoFile path="crates/jackin/src/role_authoring.rs" /> and reuses the same manifest migration registry and repository contract checks used by runtime launch.

Automation stays on the small role-focused binary in <RepoFile path="crates/jackin/src/bin/role.rs" />. CI workflows and future Renovate-style migration jobs should not depend on the full operator CLI just to validate a role repo or restamp a manifest; they call the standalone validator, which shares <RepoFile path="crates/jackin-manifest/src/repo.rs" /> and <RepoFile path="crates/jackin-manifest/src/migrations.rs" /> with the desktop path.
...
`JACKIN_AGENT` is set per agent PTY by <RepoFile path="crates/jackin-capsule/src/session.rs" />, immediately before `/jackin/runtime/entrypoint.sh` runs. The daemon's own process does not carry `JACKIN_AGENT`, so shell panes and future mixed-agent sessions do not inherit a misleading container-wide agent value.
...
Implementation: <RepoFile path="crates/jackin-capsule/src/daemon.rs" />, <RepoFile path="crates/jackin-runtime/src/runtime/launch.rs" /> (`launch_role_runtime`), <RepoFile path="crates/jackin-runtime/src/runtime/attach.rs" /> (`reconnect_or_create_session`, `spawn_agent_session`, `spawn_shell_session`).
...
Implementation: <RepoFile path="crates/jackin-capsule/src/client.rs" />, <RepoFile path="crates/jackin-capsule/src/tui/input.rs" />, <RepoFile path="crates/jackin-capsule/src/session.rs" />, and <RepoFile path="crates/jackin-capsule/src/daemon.rs" />.
...
- `state_paths()` — `AgentStatePaths { credential_dir, credential_file, folder_env_var }` — the per-agent credential location on the host and the environment hint for the operator. Used by provisioning (Defect 46 Phase B's `sync_source_dir` reads from these).
