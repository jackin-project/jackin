...
| --- | --- |
| `jackin-dev pr sync <PR_NUMBER>` | Clone or refresh the PR checkout, prepare isolated config/state, build local `jackin`, auto-build capsule/construct inputs from the diff, and write `env.sh`. |
