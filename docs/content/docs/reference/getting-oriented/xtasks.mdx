...
`research check` and `roadmap audit` are read-only and deterministic — good candidates for a CI sidebar gate so a PR that adds a page without wiring its `meta.json` (or vice versa) fails before merge.
