...
`research check` is read-only and deterministic. Roadmap sidebar parity is already gated in CI by `cargo xtask roadmap audit` (in the docs workflow), so `roadmap audit` is the local equivalent rather than a second CI gate.
