...
The authoritative control-channel wire types live in <RepoFile path="crates/jackin-protocol/src/control.rs" /> so the host CLI and the in-container binary share the same serde schema without making `jackin` depend on `jackin-capsule`'s PTY and terminal stack. The attach-channel frame codec lives in <RepoFile path="crates/jackin-protocol/src/attach.rs" /> because both the host attach client and the in-container Capsule daemon need the same binary framing.
