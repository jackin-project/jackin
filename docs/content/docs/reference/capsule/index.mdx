...
The terminal engine itself was originally a forked `vt100` parser; it has since been replaced by jackin-term's owned `DamageGrid`, which records dirty rows at mutation time (`DirtyTracker`) instead of diffing whole screens, and emits typed OSC/CSI passthrough events directly. `vt100` is no longer a `jackin-capsule` dependency. The crate options evaluated during the original parser selection were:

- `vt100` was the original parser and grid source. `Screen::contents_diff` is useful for whole-screen origins, but it does not know jackin' pane offsets, borders, status rows, or dialog overlays, so jackin' snapshots visible rows itself. Inactive split panes use a light ANSI-dim cue; modal dialogs hide everything behind an opaque solid-black backdrop instead. The fork it pinned (for `Screen::clear_scrollback()` and `CSI 3J`) is now retired from the capsule; `DamageGrid` owns scrollback natively.
- `vt100-ctt` is a newer fork of `vt100`, but it has far lower ecosystem adoption and its default feature set pulls in `tui-term`, `ratatui`, and `ratatui-core`.
- `termwiz::Surface` has a mature change-log and diff API, but adopting it would mean translating every cell into a second surface or replacing the parser. That is more architecture churn than the problem needed.
...
|---|---|---|---|---|
| Internal Zellij-style dirty patch over the owned grid (shipped) | Record damage in `DamageGrid`, encode focused live patch rows directly, and route complex frames through Ratatui | Low: the compositor coordinate rules are unchanged across the `vt100` → `DamageGrid` engine swap | Small local API; coupled to jackin' pane geometry by design | Apache-compatible dependencies; reusable inside `jackin-capsule`, not a separate crate boundary |
...
- `pointer` over clickable chrome owned by jackin', such as tabs, the top menu button, the bottom-row GitHub and container-info entry points, and clickable dialog values such as the Container ID and GitHub URL copy targets.
