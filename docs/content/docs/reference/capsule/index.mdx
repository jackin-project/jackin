...
**Debug builds for crash triage.** When a container crashes with an exit-101 panic and the backtrace in `~/.jackin/data/<container>/state/multiplexer.log` shows `<unknown>` frames, rebuild the capsule with symbols:
...
The debug binary is ~10× larger than release but resolves every frame to function + file:line. The `RUST_BACKTRACE=full` env var causes `Backtrace::force_capture()` in the panic hook to capture the full stack trace with inlined frames. After reproducing the crash, share the run id with `jackin --debug`; the run log (from Defect 41's `container_started` event) points to the container's `multiplexer.log` where the resolved backtrace is written.
