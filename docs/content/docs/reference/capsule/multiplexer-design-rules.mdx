...
- **Every scrollback-offset change repaints the pane body and the footer hint together, through a full-body frame.** A partial/dirty-span repaint is never sufficient after the offset moved (including the wheel return to offset 0): the visible window changed wholesale even though no grid cell did. `Session::scroll_by`, `set_scrollback_offset`, and the snap-to-live reset all mark the pane body repaint-pending so the dirty-span fast path auto-rejects.
- **While the view is scrolled back, the operator cursor is hidden.** The live VT cursor position is meaningless against history rows; `?25h` may only be re-asserted once the view is back at the live tail. This holds across focus swaps and attach: every surface that re-asserts mode state (`current_mode_state`) applies the same rule.
