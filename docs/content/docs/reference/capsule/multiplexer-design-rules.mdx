...
Unknown CSI from an agent never reaches the outer terminal. The grid handles what it emulates, answers device/mode queries itself (DA, DSR, DECRQM, kitty query, OSC 10/11 default-color queries — to the agent, never the host), and **default-denies everything else**. Raw terminal bytes are not telemetry; allowlist changes use an explicit reviewed PTY fixture or synthetic reproduction. The forward allowlist, with reasons:
