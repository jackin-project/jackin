...
The evaluated alternatives each helped clarify the design, but none replaced the need for `jackin-term`. The full survey from the pre-build audit:
...
The result is intentionally hybrid: depend on `vte` for the parser, borrow proven grid/damage ideas from mature terminals and multiplexers, and keep jackin❯ capsule-specific model in a small Rust crate that the workspace controls.

## Borrow and re-implement ledger

`jackin-term` depends on one parser (`vte`) and re-implements grid/damage ideas from mature terminals with attribution. Every borrow site carries an inline attribution comment in source naming the project and license.

| Source | What jackin-term takes | How | License | Attribution |
|---|---|---|---|---|
...
Non-Rust emulators (libvterm, Ghostty) inform the design only; their code is re-implemented in Rust with attribution, never linked. This keeps `jackin-term` pure-Rust with no `unsafe`, FFI, or non-Rust build dependency.
