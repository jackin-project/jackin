...
The debug capsule uses `[profile.capsule-debug]` in <RepoFile path="Cargo.toml" /> (inherits release optimisation, retains symbols + DWARF line tables, does **not** strip). The resulting binary is ~10× larger but is otherwise byte-compatible with the release capsule.
