...
Implementation: <RepoFile path="src/runtime/launch/mod.rs" /> (`launch_role_runtime`, `diagnose_premature_exit`) and <RepoFile path="crates/jackin-capsule/src/main.rs" /> (`resolve_initial_agent`).
...
Implementation: <RepoFile path="src/runtime/attach/mod.rs" /> (`reconnect_or_create_session`, `spawn_agent_session`, `spawn_shell_session`).
...
Implementation: <RepoFile path="src/runtime/attach/mod.rs" /> (`inspect_agent_sessions`) and <RepoFile path="crates/jackin-capsule/src/client.rs" /> (`run_status`).
