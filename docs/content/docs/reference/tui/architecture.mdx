...
**Component library.** Repeated product-neutral visual patterns live in [TermRock](https://github.com/tailrocks/termrock). Surface-specific components live under that surface's `src/tui/components/` and graduate upstream only when multiple consumers need the same neutral implementation. TermRock owns its terminal lookbook, generated SVG catalog, and drift checks; jackin❯ owns product compositions and their render tests.
