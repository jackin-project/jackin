...
| View | `tui/view.rs`, screen `view.rs` | Pure Ratatui frame composition from the current model; the only residual raw-ANSI is a small tail appended after the frame (cursor positioning/visibility, the OSC outer-terminal title, and the bottom chrome) |
...
- Ratatui widget implementations; the only residual raw-ANSI is the small post-frame tail (cursor, OSC title, bottom chrome)
...
The only raw-ANSI is a small tail appended after the Ratatui frame (`take_output()`): cursor positioning/visibility, the OSC outer-terminal title, and the bottom chrome (branch/PR bar, hint row, debug run-id chip). These ride outside the cell buffer by design.
