...
2. **`crates/jackin-tui/`** — Product-specific compositions and Ratatui adapters shared by at least two jackin❯ surfaces. It may own cross-surface product update/effect, external-subscription, and one-draw adapter contracts, but cannot own a generic theme facade, neutral widgets, terminal lifecycle, surface event loops, schedulers, or external effects.
