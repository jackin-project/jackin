...
3. `PanelEmphasis` is border styling only. It is computed from screen-level or dialog-level focus state when rendering; do not store it as independent state.
...
TermRock `Viewport` (console `scroll_block` adapter) paints `PanelEmphasis` from the caller's `focused` flag. Passive `focused` means **interaction ownership** — forms, tab content, and other containers that accept keys stay green even when content fits. The passive-scroll focusability rule is enforced in state/input: list-detail surfaces clear `list_scroll_focus` when content no longer overflows (`focused_block_still_scrollable`), so a passive detail block does not remain the focus owner without overflow.
