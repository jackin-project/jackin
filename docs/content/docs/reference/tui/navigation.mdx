...
1. `jackin_tui::runtime::SurfaceFocus<Target>` is the product projection over TermRock's `FocusRing` for tabbed screens. It answers which region owns input: the tab bar, or one named content block. Borders, tab underlines, cursors, and footer scope derive from that one owner value.
