...
The mechanism is the `rich_surface_active` flag plus the active diagnostics run: `emit_debug_line` / `active_debug` (<RepoFile path="src/tui/mod.rs" />, <RepoFile path="src/diagnostics/mod.rs" />) route to the run file, and the command runner (<RepoFile path="src/docker/mod.rs" />) suppresses live streaming whenever `--debug` is on or a rich surface is active. Streaming child output straight to `stdout`/`stderr` while a rich surface is up is a bug; route it through the diagnostics run instead.
