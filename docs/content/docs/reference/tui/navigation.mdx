...
**All bordered scrollable content blocks must use `render_scrollable_block`** (defined in [TermRock source](https://github.com/tailrocks/termrock), re-exported through `view.rs` for manager-internal callers). Never hand-roll block creation, Paragraph scroll, or scrollbar rendering inline.
