...
| Brand header | <RepoFile path="crates/jackin-tui/src/components/brand_header.rs">crates/jackin-tui/src/components/brand_header.rs</RepoFile> | Brand pill row with the current surface label | Header chrome that needs the canonical jackin❯ brand mark |
| Button strip | <RepoFile path="crates/jackin-tui/src/components/button_strip.rs">crates/jackin-tui/src/components/button_strip.rs</RepoFile> | Shared centered button row with focused and disabled item styling | Dialogs and form footers with multiple inline actions |
| Confirm dialog | <RepoFile path="crates/jackin-tui/src/components/confirm_dialog.rs">crates/jackin-tui/src/components/confirm_dialog.rs</RepoFile> | Yes/No two-button confirm with the canonical layout (see Confirmation dialogs rule), plus generic structured detail rows and note rows for prompts that need extra context. Product-specific strings are assembled by the owning surface, not by `jackin-tui`. | Any destructive or irreversible action |
| Container info | <RepoFile path="crates/jackin-tui/src/components/container_info.rs">crates/jackin-tui/src/components/container_info.rs</RepoFile> | Shared read-only container/session information dialog with copyable rows, hyperlinks, hover state, and scroll | Debug info dialogs across console, launch, and capsule surfaces |
| Diff view | <RepoFile path="crates/jackin-tui/src/components/diff_view.rs">crates/jackin-tui/src/components/diff_view.rs</RepoFile> | Read-only side-by-side or single-pane text diff view backed by `similar::TextDiff` | Inspecting modified, added, deleted, or untracked file content |
| Filter input | <RepoFile path="crates/jackin-tui/src/components/filter_input.rs">crates/jackin-tui/src/components/filter_input.rs</RepoFile> | Canonical single-row `Filter:` input with placeholder and cursor styling | Picker dialogs that narrow a list by typed text |
| Hint bar | <RepoFile path="crates/jackin-tui/src/components/hint_bar.rs">crates/jackin-tui/src/components/hint_bar.rs</RepoFile> | Footer hint-bar renderer for grouped `HintSpan` shortcuts, including wrapped layout | Fixed footer rows that advertise active keyboard and mouse actions |
| Panel | <RepoFile path="crates/jackin-tui/src/components/panel.rs">crates/jackin-tui/src/components/panel.rs</RepoFile> | Shared bordered panel primitive with canonical focused, scrollable, and modal block styling | Any framed TUI panel or modal shell |
| Save / discard strip | <RepoFile path="crates/jackin-tui/src/components/save_discard_dialog.rs">crates/jackin-tui/src/components/save_discard_dialog.rs</RepoFile> | Save / Discard two-button strip, designed for form footers | Bottom of any editable form |
| Scrollable panel | <RepoFile path="crates/jackin-tui/src/components/scrollable_panel.rs">crates/jackin-tui/src/components/scrollable_panel.rs</RepoFile> | Shared scroll geometry, scrollable panels, line rendering, and vertical/horizontal scrollbar drawing | Any panel, dialog, or list body that can overflow |
| Select list | <RepoFile path="crates/jackin-tui/src/components/select_list.rs">crates/jackin-tui/src/components/select_list.rs</RepoFile> | Generic modal filter-picker over labelled string items plus full-width picker row rendering | Modal lists, sectioned pickers, and simple filterable selections |
| Status footer | <RepoFile path="crates/jackin-tui/src/components/status_footer.rs">crates/jackin-tui/src/components/status_footer.rs</RepoFile> | White bottom status footer with left status text, usage label, container chip, and debug/run-id chip layout | Launch, console, or capsule bottom chrome that reports current run state |
| Status popup | <RepoFile path="crates/jackin-tui/src/components/status_popup.rs">crates/jackin-tui/src/components/status_popup.rs</RepoFile> | Generic non-interactive modal status popup with a message and `Please wait` action row | Background work progress that blocks interaction until completion |
| Tab strip | <RepoFile path="crates/jackin-tui/src/components/tab_strip.rs">crates/jackin-tui/src/components/tab_strip.rs</RepoFile> | Shared Ratatui tab strip with active, inactive, focused, hover, and hit-test behavior | Settings/editor tabs and any equivalent tabbed TUI surface |
...
| Error popup | <RepoFile path="crates/jackin-tui/src/components/error_dialog.rs">crates/jackin-tui/src/components/error_dialog.rs</RepoFile> | Scrollable red-border error modal (see Error Surface rule) | All error display |

Layout and interaction primitives that support the catalogued widgets live in <RepoFile path="crates/jackin-tui/src/components/bottom_chrome.rs">bottom_chrome</RepoFile>, <RepoFile path="crates/jackin-tui/src/components/dialog_layout.rs">dialog_layout</RepoFile>, <RepoFile path="crates/jackin-tui/src/components/focus_owner.rs">focus_owner</RepoFile>, <RepoFile path="crates/jackin-tui/src/components/hover_tracker.rs">hover_tracker</RepoFile>, <RepoFile path="crates/jackin-tui/src/components/modal_backdrop.rs">modal_backdrop</RepoFile>, and <RepoFile path="crates/jackin-tui/src/components/modal_lifecycle.rs">modal_lifecycle</RepoFile>; surface-local decorative widgets such as launch rain in <RepoFile path="crates/jackin-launch-tui/src/tui/components/rain.rs">crates/jackin-launch-tui/src/tui/components/rain.rs</RepoFile> are intentionally not catalogued as shared components.
...
**Render primitives**: `modal_block` (<RepoFile path="crates/jackin-tui/src/components/panel.rs">crates/jackin-tui/src/components/panel.rs</RepoFile>), `breadcrumb_title` (<RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs">crates/jackin-console/src/tui/components/op_picker.rs</RepoFile>), `render_fatal` (<RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs">crates/jackin-console/src/tui/components/op_picker.rs</RepoFile>), `render_filter_input` (<RepoFile path="crates/jackin-tui/src/components/filter_input.rs">crates/jackin-tui/src/components/filter_input.rs</RepoFile>), `render_picker_list` (<RepoFile path="crates/jackin-tui/src/components/select_list.rs">crates/jackin-tui/src/components/select_list.rs</RepoFile>), and `render_scrollable_block_at` (<RepoFile path="crates/jackin-tui/src/components/scrollable_panel.rs">crates/jackin-tui/src/components/scrollable_panel.rs</RepoFile>) — use these building blocks when composing new modals rather than hand-rolling border + layout from scratch.

### One picker-list renderer, with full-width section dividers

Every modal list — the host console pickers and the in-container capsule menu/pickers — draws its rows through the shared picker renderer in `select_list` (<RepoFile path="crates/jackin-tui/src/components/select_list.rs">crates/jackin-tui/src/components/select_list.rs</RepoFile>). Full modal pickers use `render_picker_list` with `PickerRow` values; richer host pickers that own their own filter/state flow must still pass neutral, unselected `Line` rows to `render_picker_lines`. Callers must not pre-style selection, paint their own `▸` cursor, or stop the selected background at the text width. The shared renderer owns the selected-row gutter, full-width highlight, selection-follow viewport, and the rule that scrollbar gutter cells win over highlight cells.
