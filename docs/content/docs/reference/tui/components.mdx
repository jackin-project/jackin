...
| Brand header | [TermRock source](https://github.com/tailrocks/termrock) | Brand pill row with the current surface label | Header chrome that needs the canonical jackin❯ brand mark |
...
| Container info | [TermRock source](https://github.com/tailrocks/termrock) | Shared read-only container/session information dialog with copyable rows, hyperlinks, hover state, and scroll | Debug info dialogs across console, launch, and capsule surfaces |
...
| Status footer | [TermRock source](https://github.com/tailrocks/termrock) | White bottom status footer with left status text, usage label, container chip, and debug invocation-correlation chip layout | Launch, console, or capsule bottom chrome that reports current invocation state |
...
| Toast | [TermRock source](https://github.com/tailrocks/termrock) | Non-blocking, auto-expiring overlay for state feedback such as `Selection copied`, with caller-supplied reserved rows so it never replaces footer/action hints | Copy-success or similar transient feedback that must appear outside the hint/footer row |
