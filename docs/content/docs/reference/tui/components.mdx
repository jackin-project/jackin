...
| Container info | <RepoFile path="crates/jackin-console/src/tui/components/container_info_surface.rs">console copy</RepoFile> + <RepoFile path="crates/jackin-launch-tui/src/tui/components/container_info.rs">launch copy</RepoFile> + <RepoFile path="crates/jackin-capsule/src/tui/components/container_info_surface.rs">Capsule copy</RepoFile> | Current migration residual: three copies provide the same read-only information dialog. The [follow-through roadmap](/roadmap/shared-tui-extraction/) moves the neutral table/render/interaction body to TermRock while keeping product facts and row construction in jackin❯. | Debug info dialogs across console, launch, and Capsule surfaces |
...
Transient state feedback such as `Selection copied` uses the Toast contract. TermRock owns the canonical widget, while Capsule's remaining local placement/render implementation is tracked by the [Shared TUI Extraction Follow-through roadmap](/roadmap/shared-tui-extraction/). A toast is non-focusable, appears above the caller's reserved footer/status rows, and expires through the owning surface's state timer. Do not put copy-success or other state feedback in the hint/footer row: that row is reserved for actions currently available in the focused area.
