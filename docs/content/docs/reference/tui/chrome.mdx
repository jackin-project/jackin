...
| Host console (debug bar chip) | invocation ID (debug mode only surface) | — |
...
is available, `Invocation ID` is the first visible row on every surface.
Container ID, versions, role, agent, and target follow it in canonical order.

**Non-debug vs debug mode:**
- Non-debug: container ID, role, agent, workdir, version rows only.
- Debug mode: Invocation ID at the top, then all non-debug rows in canonical order. Debug mode does NOT produce a separate dialog — it enriches the same dialog with more rows. There is no separate "container info dialog" — only Debug info with a variable field set.
...
**Row and interaction contract:** `Invocation ID` is always the top row whenever available and is the opaque value used to correlate backend telemetry. Enter copies the first copyable row in canonical order, so it copies `Invocation ID` whenever present. No local telemetry-file path is shown. Every copyable row renders an explicit copy affordance, hover lift applies only to the copyable value/affordance cells, and click hit-testing follows the visible value under both horizontal and vertical scroll.
...
- Click → opens **Debug info** showing container ID, role, agent, workdir, and version. In debug mode, it also shows the invocation ID.

**Debug invocation ID chip** (shown only when `--debug` is active):
- Displays the invocation ID on a `DANGER_RED` background.
- Resting style: bold white text on DANGER_RED, OSC 22 hand pointer on hover.
- Click → opens the same **Debug info** dialog, enriched with the copyable invocation ID for backend correlation.

**Both chips open the same dialog:** The instance ID chip is always available and gives operational context. The invocation ID chip is debug-mode-only and gives correlation context. They are triggers for the same Debug info surface, not separate dialog types.
...
**The debug invocation ID is a compact red chip right-aligned on the status bar.** When `--debug` is active, the opaque invocation ID appears as a `DANGER_RED` background chip on the right side of the status bar. Only the chip itself gets the red background. The rest of the bar stays white.
...
This applies to the host console debug bar and any other surface that exposes an invocation ID in debug mode.
