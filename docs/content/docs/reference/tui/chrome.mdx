...
**Row and interaction contract:** `Run ID` is always the bare diagnostics id (`jk-run-…`), never the JSONL path. `Diagnostics log` is the full JSONL path and carries the `file://` hyperlink. Every copyable row renders an explicit copy affordance, hover lift applies only to the copyable value/affordance cells, and click hit-testing follows the visible value under both horizontal and vertical scroll. The capsule and launch surfaces may assemble different facts, but they do not own a second copy/hover/link implementation.

**Backdrop contract:** Opening Container info must not erase persistent chrome that was visible before the dialog opened. Status bars, status footers, and reserved bottom chrome remain owned by their normal renderers; the dialog paints only its own panel and any backdrop inside the content area.
