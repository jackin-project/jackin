...
- `Viewport::new(&lines, &theme).emphasis(…)` (console `scroll_block` adapter for call-shape compatibility) — passive scroll block; the console adapter only promotes `PanelEmphasis::Focused` when content actually overflows.
