...
- **Dialogs and modals always have focused borders (`Role::BorderFocused` / phosphor green) — they are always the active container when visible.** Exception: when dialogs are *stacked* (a child dialog opens over a parent), only the *topmost* dialog uses `PanelEmphasis::Focused`; every dialog beneath uses `PanelEmphasis::Normal` (`Role::Border`). The rule is "exactly one bright border per surface layer" — a stack of two green borders is a violation.
...
**Enforcement — use the canonical helpers, never inline border colors:** The correct border style is encoded in TermRock [Panel / Viewport](https://github.com/tailrocks/termrock) primitives:

- `Panel::new(&theme).title("…").emphasis(PanelEmphasis::Focused)` — titled active container (modal, picker, form, tab panel).
- `layout::render_dialog_shell(frame, area, title, PanelEmphasis::Focused, &theme)` — themed dialog shell that clears, paints the panel, and returns the inner body rect.
- `Panel::new(&theme).emphasis(PanelEmphasis::Normal)` — background container or non-top dialog in a stack.
- `Viewport::new(&lines, &theme).emphasis(…)` (console `scroll_block` adapter for call-shape compatibility) — maps the caller's `focused` flag to `PanelEmphasis::Focused` / `Normal` with no overflow gate. Passive list-detail surfaces clear focus in state when content fits (`list_pre_render_focus_plan` / `focused_block_still_scrollable`); forms and other interaction owners keep `focused = true` and paint green even when content fits.
- `Panel::inner(area)` — body rect inset by the panel chrome so content never touches the border.

Never construct `Block::default().borders(Borders::ALL).border_style(Style::default().fg(…))` for a modal — that silently bypasses semantic roles and produces a wrong focus indicator. TermRock panel/viewport tests own glyph and role-color regressions.
...
**Stack (one-bright-border rule).** A dialog opened on top of a background modal (e.g. a git-credential prompt that appears while a file browser is open) does not use a full backdrop. Instead it follows the one-bright-border rule: the background modal uses `PanelEmphasis::Normal` and the foreground dialog uses `PanelEmphasis::Focused`. Exactly one border is bright at any moment. Stack with `dialog_push` / `modal_parents` (see *Sub-dialogs push onto a stack* in the dialogs reference); dismiss with `dialog_pop_one` to return to the parent.
