...
**Implementation rule:** The dialog must become a single TermRock-owned neutral component instantiated by every surface, with product facts and row construction remaining in jackin❯. The current console, launch, and Capsule copies are an extraction residual tracked by the [Shared TUI Extraction Follow-through roadmap](/roadmap/shared-tui-extraction/). Any migration-time deviation in layout, padding, or border color between surfaces is a design violation.
...
**Implementation:** launch composes its footer with TermRock `StatusBar`, while console and Capsule currently use equivalent product-local `StatusFooter` implementations. Every screen must keep the chip present in `--debug` mode regardless of which surface the operator is on. The [Shared TUI Extraction Follow-through roadmap](/roadmap/shared-tui-extraction/) tracks converging the remaining copies on the canonical widget without changing the left context segment, usage headline, container chip, debug chip, or hit regions. The chip is clickable and opens the Debug info dialog.
