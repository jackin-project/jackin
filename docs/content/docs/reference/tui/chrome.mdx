...
### Status bar right group — usage, spend, container, run ID

The status bar's right side is a single ordered group (`StatusRightGroup` in <RepoFile path="crates/jackin-tui/src/components/status_footer.rs">crates/jackin-tui/src/components/status_footer.rs</RepoFile>): from left to right it reads `usage · spend · container · run_id`. Chunks are placed right-to-left under width pressure, so the **placement order is the survival priority**: `run_id` is placed first (always survives), then `spend`, then `container`, then `usage` last (drops first). This is deliberate — the percentage windows (`usage`, e.g. `Session 37% · Weekly 81%`) are the most compressible, while the monetary spend stays visible alongside the container identity.

**Spend chunk.** The focused account's monetary spend renders as its own chunk in compact money form (`$53/$300` — used over cap, whole units). It is sourced from the focused usage snapshot's `spend_status_label` (computed in <RepoFile path="crates/jackin-capsule/src/usage.rs">crates/jackin-capsule/src/usage.rs</RepoFile> from the `Spend`-slot quota bucket's structured `Money`), distinct from the percentage `usage` headline. The chunk is omitted entirely when the focused account has no monetary spend window (e.g. a subscription-only plan), rather than showing `$0`. Because it is placed before `container`, an account approaching its cap keeps its spend on screen even when the bar is narrow enough to drop the container name.
