...
The canonical RGB values live in the TermRock crate palette ([TermRock source](https://github.com/tailrocks/termrock)) so the host TUI and the in-container multiplexer cannot drift; [TermRock source](https://github.com/tailrocks/termrock) adapts them into ratatui `Color` constants that the rest of the host TUI imports. Never define local color constants.
