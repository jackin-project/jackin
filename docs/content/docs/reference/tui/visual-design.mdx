...
- **Capsule agent tabs reserve one status glyph cell** after the tab label: blocked `●` in red, working `◐` in amber, done `○`, idle `◆` in bright phosphor green, and unknown blank. Idle must be a full-cell visible mark, not a tiny separator dot, so it cannot be confused with usage/status text separators.
