...
call `Theme::default()` / `jackin_core::tui_theme` helpers such as `text_fg()`,
`accent_fg()`, `border_fg()`, and `text_strong()` so ordinary, strong, muted,
border, focus, danger, warning, tab, input, link, and scroll presentation stay
on TermRock's default phosphor preset.
