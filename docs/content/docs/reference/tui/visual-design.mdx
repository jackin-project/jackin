...
in `jackin-core`'s `host_colors.rs` and are adapted in `tui_theme.rs`. Never
...
|---|---|---|
| TermRock `Role` | `Text`, `TextStrong`, `TextMuted`, `Border`, `BorderFocused`, `Accent`, `Danger`, `Warning`, `Tab*`, `Link*`, `Scroll*` | Shared presentation via `Theme::default().style(role)` or `tui_theme` helpers |
...
**No inline `Color::Rgb(...)` literals outside `host_colors.rs` / `tui_theme.rs`.**
