...
API ([TermRock source](https://github.com/tailrocks/termrock)). jackin❯-specific
brand, rain, debug, and status tokens live once in `jackin-core`'s
`host_colors.rs`; its `tui_theme.rs` module adapts them to Ratatui colors for
every jackin❯ surface. Never define a second surface-local palette.

**All colors come from semantic TermRock roles or named jackin❯ tokens; no
inline `Color::Rgb(...)` literals outside `tui_theme.rs`.** Reusable component
semantics belong in TermRock. Product-specific colors belong in
`jackin-core/src/host_colors.rs` and receive their Ratatui adapter in
`jackin-core/src/tui_theme.rs`.
...
| `PHOSPHOR_DARK` | `Rgb(0, 80, 18)` | Separators and dark phosphor details |
| `BORDER_GRAY` | `Rgb(80, 80, 80)` | Inactive and background container borders |
