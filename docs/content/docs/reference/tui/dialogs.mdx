...
Completed drag selections remain visibly highlighted after mouse-up. Mouse-up copies the selected text and the bottom hint row switches to copied feedback, so the operator can see both what was copied and that the clipboard write happened. The persisted selection clears only on an explicit deselect action: a later ordinary click, typing input, or starting a new selection. A mouse-up that completes the copy is not itself a deselect action.
