...
`ContainerInfoState` (the shared "Debug info" dialog) is the reference implementation: long values (run-id and diagnostics-log paths) scroll horizontally instead of clipping, and its OSC 8 hyperlink overlay + click-to-copy hit-test follow the content under both scroll axes. The capsule rebuilds its dialog state every frame, so a scrollable capsule dialog persists its `DialogBodyScroll` on the `Dialog` enum variant (see `Dialog::ContainerInfo` / `Dialog::GitHubContext`) and threads it into the rebuilt state.
