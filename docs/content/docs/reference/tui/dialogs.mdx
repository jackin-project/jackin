...
**Universal dialog scroll rule:** When a dialog's content cannot fit in the dialog area after the preferred-size shrink (Defect 7), the dialog body must scroll using the shared scroll machinery — vertical scrollbar on the right border and `↑↓`/`J`/`K`/wheel to scroll. No dialog clips content silently. `ContainerInfoState` is the reference implementation: it exposes `scroll_y`, handles Up/Down/j/k in its key handler, and calls `render_vertical_scrollbar` when `total_rows > viewport`.
...
- **Mouse-click outside the box**: dismisses the dialog (same as Esc). This rule applies to **all** surfaces — host console, launch cockpit, and capsule — not only the capsule's dialog stack. Clicking *inside* the dialog body where there is no interactive element is a no-op (does not dismiss).
