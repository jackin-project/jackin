...
Modal sizes currently come from parallel product-local registries in <RepoFile path="crates/jackin-console/src/tui/components/modal_rects.rs">console</RepoFile> and <RepoFile path="crates/jackin-capsule/src/tui/components/modal_rects.rs">Capsule</RepoFile>. The [Shared TUI Extraction Follow-through roadmap](/roadmap/shared-tui-extraction/) tracks replacing their neutral sizing and placement logic with TermRock `DialogSpec`/`resolve_dialog` while keeping named product modes local. `rows` is the **total outer height** including both borders. Inner rows = `rows - 2`.
...
`DebugInfo::into_state()` is intended to be the only row-order and row-label source for the Debug info dialog. The current console, launch, and Capsule copies must remain behavior-identical until the [Shared TUI Extraction Follow-through roadmap](/roadmap/shared-tui-extraction/) establishes one jackin❯ product-data owner and one TermRock neutral renderer; no surface may independently fork the shell, row labels, copy behavior, hover styling, or scroll behavior.
...
The "Debug info" dialog must become one reusable neutral component, not a per-surface reimplementation. Today console, launch, and Capsule retain three copies of the state/render body; the [Shared TUI Extraction Follow-through roadmap](/roadmap/shared-tui-extraction/) tracks converging them on TermRock `DetailTable`/dialog primitives while keeping the product-owned `DebugInfo` facts and row construction in jackin❯. The look, row order, labels, and copy affordances must remain identical throughout the migration.
