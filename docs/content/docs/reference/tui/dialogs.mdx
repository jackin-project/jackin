...
Modal sizes come from the `modal_rects` registry in `jackin-tui`. `rows` is the **total outer height** including both borders. Inner rows = `rows - 2`.
...
6. `Agent`.
7. `Target`.
8. `Diagnostics log` — copyable and an OSC 8 `file://` hyperlink, present only when a local diagnostics file exists.
9. `Telemetry` — backend-only note, present when a run id exists but no local diagnostics file exists.

`Run ID` is the bare run id value, never a `.jsonl` path. `Diagnostics log` is the full JSONL path and is omitted when the active sink is backend-only. `Telemetry` is non-copyable explanatory text for that backend-only mode. Enter copies the first copyable row in the canonical order, so it copies `Run ID` whenever that row exists, and the dialog stays open so copied-row feedback can render. Mouse click copies the copyable value under the pointer; hover feedback applies only to copyable value cells and their copy affordance. Hints advertise `↵ copy value`, `Esc dismiss`, click copy, and only the scroll axes that actually overflow.
