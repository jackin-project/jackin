...
Shared host coordination dirs (already used by Capsule cross-instance refresh):

- `~/.jackin/data/daemon/usage-cooldowns` (`JACKIN_USAGE_COOLDOWN_DIR`)
- `~/.jackin/data/daemon/usage-snapshots` (`JACKIN_USAGE_SNAPSHOTS_DIR`)
- `~/.jackin/data/daemon/usage-locks` (`JACKIN_USAGE_LOCK_DIR`)
...
Developer ID + hardened runtime + notarize/staple (TableRock path). Homebrew cask or documented install. Uninstall leaves no root daemon. v1 does **not** require jackin daemon.
