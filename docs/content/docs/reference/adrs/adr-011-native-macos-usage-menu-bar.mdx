...
Immutable GitHub Release artifact `jackin-usage-menu-bar-<VERSION>-aarch64-apple-darwin.zip` (exactly `JackinUsageMenuBar.app`), produced only after Developer ID codesign (hardened runtime + secure timestamp), notarytool `Accepted`, staple, and Gatekeeper assessment. Supply-chain sidecars match CLI archives: `.sha256`, cosign `.bundle`, CycloneDX `.sbom.json`, plus a separate GitHub build-provenance attestation. Homebrew cask `jackin-usage-menu-bar` advances with the stable formula in one tap PR; the first cask is never auto-merged. Uninstall leaves no root daemon. v1 does **not** require the jackin❯ daemon.
