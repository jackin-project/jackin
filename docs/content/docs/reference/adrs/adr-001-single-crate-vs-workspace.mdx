...
**Status**: Accepted
**Current state**: Cargo workspace — 7 member crates under `crates/`, root `jackin` binary crate in `src/`. Total ~170 K lines of Rust.
**Date**: 2026-05-30
...
**Phase 1 — stay single-crate while total Rust LOC < 150 K.** All code lives under `src/`. No `crates/` directory. Fast `cargo build`, minimal <RepoFile path="Cargo.toml" /> overhead, no intra-workspace dependency graph to maintain.
...
**Phase 2 — workspace adopted (current state).** The codebase crossed ~170 K lines as surface-specific crates (`jackin-capsule`, `jackin-launch`, `jackin-console`, `jackin-tui`, `jackin-tui-lookbook`, `jackin-protocol`, `jackin-build-meta`) were extracted. The workspace root is <RepoFile path="Cargo.toml" /> with `members = [...]`; the root `jackin` crate remains the primary binary. The `crates/` directory is the canonical home for extracted crates.
