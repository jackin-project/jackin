...
- Incremental builds are faster per-crate: changing `jackin-capsule` does not rebuild unrelated product crates.
...
- New code that serves only one surface starts in that surface's crate; neutral TUI code shared across consumers belongs in TermRock, while product protocol code belongs in `jackin-protocol`.
