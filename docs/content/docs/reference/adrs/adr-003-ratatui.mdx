...
`ratatui` is pinned at `0.30` per-crate in each consuming crate — <RepoFile path="crates/jackin-tui/Cargo.toml" />, <RepoFile path="crates/jackin-capsule/Cargo.toml" />, <RepoFile path="crates/jackin-console/Cargo.toml" />, <RepoFile path="crates/jackin-launch/Cargo.toml" />, and <RepoFile path="crates/jackin-tui-lookbook/Cargo.toml" />:
