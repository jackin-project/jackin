...
`ratatui` is pinned at `0.30` in the workspace root <RepoFile path="Cargo.toml" />:
...
- Major `ratatui` version bumps require auditing widget API changes across all three surfaces. The workspace-level pin ensures all crates use the same version.
