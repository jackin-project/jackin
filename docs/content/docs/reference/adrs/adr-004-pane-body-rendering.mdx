...
**Status**: Accepted  
**Current state**: focused live pane-only updates encode jackin-term `GridPatch` rows directly through `SocketBackend::draw_grid_patch()`. Ratatui handles full redraws, scrollback views, chrome, dialogs, selection, and overlays through `PaneBodyWidget` plus the same `SocketBackend`.
**Date**: 2026-05-30  
...
- `PaneBodyWidget` is the custom cell widget for full redraws, scrollback views, and fallback dirty-patch frames.
- Focused live pane-only dirty patches use `SocketBackend::draw_grid_patch()` directly to avoid Ratatui's changed-cell allocation.
...
- tui-term is not a dependency. Revisit only if it supports jackin-term's owned model without losing dirty-patch rendering or typed passthrough semantics.
