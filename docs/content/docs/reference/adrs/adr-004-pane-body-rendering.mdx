...
**Status**: Accepted  
**Current state**: `SocketBackend` serialises `Buffer::diff` over the attach socket; pane body content is rendered from jackin-term `GridSnapshot`/`GridPatch` through `PaneBodyWidget`. Ratatui handles all chrome, dialogs, pane bodies, and overlays.
**Date**: 2026-05-30  
...
A thin `Widget` impl that blits jackin-term cells directly into the Ratatui `Buffer`. No third-party dependency beyond Ratatui itself. Fully compatible with `GridSnapshot` and `GridPatch`, using the same internal color/style conversion the socket backend already handles.
...
- tui-term will not be added as a dependency. Revisit only if it supports jackin-term's owned model without losing dirty-patch rendering or typed passthrough semantics.
