...
- <RepoFile path="crates/jackin-console/src/tui/components.rs" /> — promoted host-console visual components, including the file-browser state/input/render pieces under <RepoFile path="crates/jackin-console/src/tui/components/file_browser/mod.rs" />. File-browser Git URL lookup, directory navigation, and commit validation leave the component as typed outcomes and are executed by the root manager's file-browser adapter in <RepoFile path="src/console/manager/file_browser.rs" /> through <RepoFile path="crates/jackin-console/src/services/file_browser.rs" />. Browser launches leave input as typed `ConsoleInputOutcome::OpenUrl` values and are executed by the root run loop. `src/console/tui/widgets/` still contains the root-console op-picker transitional widget; the root-only auth credential adapter lives beside manager state in <RepoFile path="src/console/manager/auth_panel.rs" />.
