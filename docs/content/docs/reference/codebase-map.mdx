...
- <RepoFile path="crates/jackin-console/src/tui/components.rs" /> — promoted host-console visual components, including the file-browser state/input/render pieces under <RepoFile path="crates/jackin-console/src/tui/components/file_browser/mod.rs" />. File-browser Git URL lookup, browser-launch, directory navigation, and commit validation leave the component as typed outcomes and are executed by root-console input through <RepoFile path="crates/jackin-console/src/services/file_browser.rs" />. `src/console/tui/widgets/` still contains root-console transitional widgets that are not yet fully extracted.
