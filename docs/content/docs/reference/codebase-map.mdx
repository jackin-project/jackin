...
| <RepoFile path="src/net.rs" /> | Shared HTTP for binary acquisition — the reqwest text-GET client (`http_client` / `get_text` / `fetch_text`, with connect + request timeouts) and the fast-down parallel chunked `download_parallel` (work-stealing across `DOWNLOAD_CONCURRENCY` connections, mmap writes, overall `DOWNLOAD_TIMEOUT` + abort). |
