...
- <RepoFile path="src/console/services.rs" /> and `src/console/services/` — root-console side-effect adapters that still depend on the root application crate: browser launch, config persistence for role/workspace/settings saves, Docker-backed instance discovery and instance-refresh snapshots, mount inspection, 1Password probing/read validation, op-picker metadata loading, and workspace-save drift/cleanup workers. <RepoFile path="src/console/domain.rs" /> owns pure console product rules such as role-source candidate derivation, workspace-choice construction, and workspace-edit delta construction.
...
- <RepoFile path="src/console/preview.rs" /> — workspace preview and
  detail lines.
- `src/console/manager/` — the workspace-manager TUI subsystem. The message boundary lives in <RepoFile path="src/console/manager/message.rs" /> (`ManagerMessage` enum + `update_manager`); render functions are pure (immutable model) and live under `src/console/tui/render/`; input handlers under `src/console/manager/input/` translate terminal events into `ManagerMessage` variants and route external work through the console service adapters above. The manager model stores pending subscription receivers and render-safe snapshots, but instance refresh, mount inspection, browser opening, config persistence, and background worker creation are executed by service/effect code outside the model.
- Screen modules under <RepoFile path="crates/jackin-console/src/tui/screens.rs" />, including <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/model.rs" />, <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/update.rs" />, <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/view.rs" />, <RepoFile path="crates/jackin-console/src/tui/screens/editor/model.rs" />, <RepoFile path="crates/jackin-console/src/tui/screens/editor/update.rs" />, <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />, <RepoFile path="crates/jackin-console/src/tui/screens/settings/model.rs" />, <RepoFile path="crates/jackin-console/src/tui/screens/settings/update.rs" />, and <RepoFile path="crates/jackin-console/src/tui/screens/settings/view.rs" /> — extracted console screen model/helper code that no longer depends on the root application crate.
- <RepoFile path="crates/jackin-console/src/tui/components.rs" /> — promoted host-console visual components, including the file-browser state/input/render pieces under <RepoFile path="crates/jackin-console/src/tui/components/file_browser/mod.rs" />. File-browser Git URL lookup, directory navigation, and commit validation leave the component as typed outcomes and are executed by root-console input through <RepoFile path="crates/jackin-console/src/services/file_browser.rs" />. Browser launches leave input as typed `ConsoleInputOutcome::OpenUrl` values and are executed by the root run loop. `src/console/tui/widgets/` still contains root-console transitional widgets that are not yet fully extracted.
