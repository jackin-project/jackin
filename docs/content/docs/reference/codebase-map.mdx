...
- <RepoFile path="src/console/services.rs" /> and `src/console/services/` — root-console side-effect adapters that still depend on the root application crate: browser launch, config persistence for role/workspace/settings saves, Docker-backed instance discovery, 1Password probing/read validation, op-picker metadata loading, and workspace-save drift/cleanup workers. <RepoFile path="src/console/domain.rs" /> owns pure console product rules such as role-source candidate derivation and workspace-choice construction.
