...
- <RepoFile path="crates/jackin-console/src/tui/components.rs" /> — promoted host-console visual components, including the file-browser state/input/render pieces under <RepoFile path="crates/jackin-console/src/tui/components/file_browser/mod.rs" />. File-browser Git URL lookup and browser-launch requests leave the component as typed outcomes and are executed by root-console input through <RepoFile path="crates/jackin-console/src/services/file_browser.rs" />; filesystem and sandbox adapters still need the same effect/message treatment. `src/console/tui/widgets/` still contains root-console transitional widgets that are not yet fully extracted.
- <RepoFile path="crates/jackin-console/src/tui/effect.rs" /> — host-console typed TUI effect vocabulary. Root console manager code still executes those effects while top-level app/message/update files move into the surface crate.
