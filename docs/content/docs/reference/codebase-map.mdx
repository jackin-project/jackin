...
- <RepoFile path="src/console/services.rs" /> and `src/console/services/` — root-console side-effect adapters that still depend on the root application crate: browser launch, config persistence for role/workspace/settings saves, Docker-backed instance discovery and instance-refresh snapshots, mount inspection, 1Password probing/read validation, op-picker metadata loading/cache invalidation, role repository registration, agent resolution, and workspace-save drift/cleanup workers. <RepoFile path="src/console/domain.rs" /> owns pure console product rules such as role-source candidate derivation, launch provider derivation, workspace-choice construction, and workspace-edit delta construction. <RepoFile path="src/console/terminal.rs" /> binds the `jackin-console` terminal lifecycle to root crate host debug-buffering globals.
