...
- <RepoFile path="src/console/preview.rs" /> — workspace preview and
  detail lines.
- `src/console/manager/` — the workspace-manager TUI subsystem. The message boundary lives in <RepoFile path="src/console/manager/message.rs" /> (`ManagerMessage` enum + `update_manager`); render functions are pure (immutable model) and live under `src/console/manager/render/`; input handlers under `src/console/manager/input/` translate terminal events into `ManagerMessage` variants.
- `src/console/widgets/` — host-console adapters and still-local modal widgets. Shared tokens and promoted components live in <RepoFile path="crates/jackin-tui/src/components.rs" />.
