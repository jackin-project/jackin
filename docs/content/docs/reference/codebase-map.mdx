...
- <RepoFile path="src/console/preview.rs" /> — workspace preview and
  detail lines.
- `src/console/manager/` — the workspace-manager TUI subsystem. The message/update boundary lives in <RepoFile path="src/console/manager/message.rs" /> (`ManagerMessage` enum + `update_manager`), while <RepoFile path="src/console/manager/effects.rs" /> owns root-only effect execution and background polling. Render functions are pure (immutable model) and live under `src/console/tui/render/`; input handlers under `src/console/manager/input/` translate terminal events into `ManagerMessage` variants and route external work through the console service adapters above. The manager model stores pending subscription receivers and render-safe snapshots, but instance refresh, mount inspection, browser opening, settings persistence, auth-form `op` read validation, workspace-save writes, workspace-save drift checks, workspace-save cleanup workers, and other background worker creation are executed by service/effect code outside the model.
