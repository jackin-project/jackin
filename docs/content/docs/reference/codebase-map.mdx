...
- <RepoFile path="crates/jackin-console/src/run.rs" /> and <RepoFile path="crates/jackin-console/src/terminal.rs" /> — extracted event-loop shell helpers and terminal lifecycle abstractions used by the root integration layer.
...
- <RepoFile path="crates/jackin-console/src/widgets/mod.rs" /> — promoted host-console widgets. `src/console/tui/widgets/` now contains compatibility adapters and still-local widgets that are not yet fully extracted.
