...
- <RepoFile path="crates/jackin-console/src/tui/components.rs" /> — promoted host-console visual components, including the file-browser state/input/render pieces under <RepoFile path="crates/jackin-console/src/tui/components/file_browser/mod.rs" />. File-browser Git URL lookup, directory navigation, and commit validation leave the component as typed outcomes and are executed by the root file-browser service adapter in <RepoFile path="src/console/services/file_browser.rs" /> through <RepoFile path="crates/jackin-console/src/services/file_browser.rs" />. Browser launches leave input as typed `ConsoleInputOutcome::OpenUrl` values and are executed by the root TUI run loop in <RepoFile path="src/console/tui/run.rs" />. <RepoFile path="src/console/tui/auth_panel.rs" /> binds root credential types to the generic auth-panel component, and <RepoFile path="src/console/tui/op_picker/mod.rs" /> contains the root-console transitional op-picker binding until its root crate type dependencies move behind generic adapters.
