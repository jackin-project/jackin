...
2. **`crates/<surface-crate>/src/tui/`** — Everything TUI for that surface: Model (state structs), Update (message handlers, key dispatch), View (render functions, widgets), and event-loop wiring. For single-screen crates a flat layout is enough. For multi-screen crates (e.g. `jackin-console`), screens are sub-folders inside `tui/screens/`.

**Single-screen crate layout** (`jackin-capsule`, `jackin-launch`):
```
src/tui/
├── mod.rs          ← event-loop wiring
├── model.rs        ← Model (state structs)
├── update.rs       ← Update (message handlers, input dispatch)
└── view/           ← View (render fns, pure &Model reads)
    ├── statusbar.rs
    ├── dialog.rs
    └── ...
```
...
```
src/tui/
├── mod.rs          ← ConsoleState, event-loop wiring
├── screens/
...
**MUV contract inside `tui/`:**
- `model.rs` — pure data structs; no rendering, no I/O.
- `update.rs` — pure fn(Model + Message) → Effects; no rendering.
- `view.rs` / `view/` — pure `&Model` reads; no mutations, no I/O, no side effects.
- `mod.rs` / wiring — event loop that connects M + U + V; not M, U, or V itself.

**What belongs in `tui/`** (everything TUI):
- All three MUV layers for each screen
- Ratatui widget implementations
- Raw ANSI rendering functions
- Focus state, scroll state, modal state
- Event-loop wiring and terminal lifecycle

**What does NOT belong in `tui/`** (stays outside as business logic):
...
**Enforcement:** before adding any file, ask: "Would deleting this file affect workspace resolution, Docker, or config?" Yes → it does not go in `tui/`. No → it belongs in `tui/`.
