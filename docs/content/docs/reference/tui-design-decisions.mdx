...
Editor and settings save-footer base labels live with console-local footer hint components in <RepoFile path="crates/jackin-console/src/tui/components/footer_hints.rs" />. Root footer adapters may pass dirty/change facts and row modes, but they should not carry the `save workspace` or `save settings` copy.
