...
Console auth-form mode cycling, mode-to-next-focus planning, and full auth-form key/focus planning now live with the shared auth-panel component in <RepoFile path="crates/jackin-console/src/tui/components/auth_panel.rs" />. Editor and settings auth input keep root modal persistence, token-generation return paths, credential-source side trips, and credential commit/reset side effects, but share the same component-owned focus behavior.
