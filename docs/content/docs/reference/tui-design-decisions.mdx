...
Console top-level update plans for status-overlay open/dismiss, list-modal open/dismiss, inline-picker dismissal, inline provider follow-up, list focus, pre-render list-focus liveness, pre-render list-scroll reset, drag-state, and split-percent normalization live in <RepoFile path="crates/jackin-console/src/tui/update.rs" />. Focus helpers live in <RepoFile path="crates/jackin-console/src/tui/focus.rs" />, split/drag state primitives live in <RepoFile path="crates/jackin-console/src/tui/split.rs" />, mount-display and `op://` breadcrumb display models live in <RepoFile path="crates/jackin-console/src/tui/mount_display.rs" /> and <RepoFile path="crates/jackin-console/src/tui/op_breadcrumb.rs" />, and provider-picker state plus key/outcome planning lives with console-local components in <RepoFile path="crates/jackin-console/src/tui/components/provider_picker.rs" />. Root <RepoFile path="src/console/tui/message.rs" /> and list input/layout prep apply those plans to concrete root fields; they should not reintroduce generic modal, picker visibility, provider follow-up, provider picker key transitions, focus, scroll-reset, drag, or split rules inline.
