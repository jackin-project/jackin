...
| Layer | File | Owns |
|---|---|---|
| Model | `tui/app.rs`, screen `model.rs` | Visible terminal state: active screen, focus owner, modal stack, scroll offsets, hover/click state, picker state, animation state, render-safe snapshots |
...
| View | `tui/view.rs`, screen `view.rs` | Pure Ratatui/raw-ANSI frame composition from the current model |
...
| Components | `tui/components/` | Surface-local visual components not reusable enough for `jackin-tui` |
