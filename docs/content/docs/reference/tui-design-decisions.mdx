...
The transitional root op-picker adapter may own `operator_env` types, runner wiring, cache binding, and subscriptions, but user-visible picker copy and load-error state shaping belong in <RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" /> with the rest of the component. Root adapters pass concrete process errors or subscription-closed facts into component helpers; they do not construct recoverable/fatal picker error states inline.
