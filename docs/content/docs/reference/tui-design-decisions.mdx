...
**Render primitives**: `modal_block` (<RepoFile path="crates/jackin-tui/src/components/panel.rs" />), `breadcrumb_title` (<RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" />), `render_fatal` (<RepoFile path="src/console/tui/op_picker/render.rs" />), `render_filter_input` (<RepoFile path="crates/jackin-tui/src/components/filter_input.rs" />), and `render_scrollable_block_at` (<RepoFile path="crates/jackin-tui/src/components/scrollable_panel.rs" />) — use these building blocks when composing new modals rather than hand-rolling border + layout from scratch.
