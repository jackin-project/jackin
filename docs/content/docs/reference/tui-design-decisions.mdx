...
3. While the console migration is incomplete, also search the transitional root-console widget location before adding or moving anything: `rg 'struct .*State|fn render_' src/console/tui/widgets`.
...
Reusable TUI widgets live first in `crates/jackin-tui/src/components/`. Surface-local widgets should live under the owning surface's `src/tui/components/`. The root console still has transitional widgets in `src/console/tui/widgets/`; they remain valid current owners until migrated, but they are not the target structure for new canonical component work. Use this table to identify the right component before writing new code or moving an existing widget.
...
**Render primitives**: `modal_block` (<RepoFile path="crates/jackin-tui/src/components/panel.rs" />), `breadcrumb_title` (<RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" />), `render_fatal` (<RepoFile path="src/console/tui/widgets/op_picker/render.rs" />), `render_filter_input` (<RepoFile path="crates/jackin-tui/src/components/filter_input.rs" />), and `render_scrollable_block_at` (<RepoFile path="crates/jackin-tui/src/components/scrollable_panel.rs" />) — use these building blocks when composing new modals rather than hand-rolling border + layout from scratch.
