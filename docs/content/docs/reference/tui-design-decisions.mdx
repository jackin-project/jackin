...
Console top-level update plans for status-overlay open/dismiss, list-modal open/dismiss, and inline-picker dismissal live in <RepoFile path="crates/jackin-console/src/tui/update.rs" />. Root <RepoFile path="src/console/tui/message.rs" /> applies those plans to concrete root fields; it should not reintroduce generic modal or picker visibility rules inline.
