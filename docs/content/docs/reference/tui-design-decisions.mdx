...
Console settings-auth global GitHub env extraction also lives in <RepoFile path="src/console/domain.rs" />. Root settings state stores pending/original maps for UI dirty tracking and save previews; it does not derive global `[github.env]` defaults directly from `AppConfig`.
