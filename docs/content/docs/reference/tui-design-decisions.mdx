...
Hint rows are not hand-built. The vocabulary is the shared `HintSpan` enum in `jackin-tui` (`Key`, `Text`, `Sep`, `GroupSep`), and the styling rule is identical everywhere: `Key` is white + bold, `Text` is phosphor green (rendered with a leading space), `Sep` is a gray `" · "`, `GroupSep` is three spaces, and the row is centered. Because the host (ratatui) and the in-container multiplexer (raw ANSI) draw to different backends, there are exactly two renderers over the *same* spans — the host renderer in <RepoFile path="src/console/widgets/hints.rs" /> and the capsule renderer `render_hint_row` in <RepoFile path="crates/jackin-capsule/src/dialog.rs" /> — and no third. Define a screen's hint as a `&[HintSpan]` constant and pass it to the renderer; do not assemble styled `Span`s, raw separator strings, or per-surface colour choices inline. A new hand-rolled hint row (its own `Span::styled` key/label pairs, its own `" · "` literal, or its own dim/green choice) is a design-decision violation: route it through the shared span list + renderer instead. This is what lets the cockpit picker, the build-log overlay, and every capsule dialog footer read identically.
...
**All bordered scrollable content blocks must use `render_scrollable_block`** (defined in <RepoFile path="src/console/widgets/scrollable.rs" />, re-exported through `render/mod.rs` for manager-internal callers). Never hand-roll block creation, Paragraph scroll, or scrollbar rendering inline.
...
Input hit-testing, focusability, cursor-follow scrolling, selected-list windowing, scrollbar dragging, wheel/key max-offset math, right-padding content width, and scrollbar rendering must flow through `jackin_tui::scroll` in <RepoFile path="crates/jackin-tui/src/scroll.rs" />. That module is the source of truth for clamping, top-relative offsets, tail-relative offsets, full-cell thumb conversion, and `tui-scrollbar` metrics / interaction types. Console helpers in <RepoFile path="src/console/widgets/scrollable.rs" /> adapt that shared model to ratatui blocks (`viewport_width`, `viewport_height`, `render_selected_lines_in_area`, `render_lines_with_offset_in_area`, `horizontal_scrollbar_area`, `vertical_scrollbar_area`, `apply_horizontal_scroll_delta`, etc.) but must not grow a parallel metrics implementation. Do not reimplement `area.width - 2`, `area.height - 2`, `content - viewport`, cursor-follow viewport offsets, selected-list slicing, proportional thumb sizing, trailing scroll padding, scrollbar hit areas, track-position formulas, or wheel/key offset math in input handlers or widget renderers; drift between input geometry and renderer geometry is a scroll bug.
...
The canonical RGB values live in the `jackin-tui` crate palette (<RepoFile path="crates/jackin-tui/src/lib.rs" />) so the host TUI and the in-container multiplexer cannot drift; <RepoFile path="src/console/widgets/mod.rs" /> wraps them into ratatui `Color` constants that the rest of the host TUI imports. Never define local color constants.
...
| Error popup | <RepoFile path="src/console/widgets/error_popup.rs" /> | Scrollable red-border error modal (see Error Surface rule) | All error display |
| Panel rain | <RepoFile path="src/console/widgets/panel_rain.rs" /> | Background rain animation | Background decoration |

**Render primitives**: `modal_block`, `breadcrumb_title`, `render_fatal` (<RepoFile path="src/console/widgets/op_picker/render.rs" />), `render_filter_row` (<RepoFile path="src/console/widgets/mod.rs" />), and `render_scrollable_block` (<RepoFile path="src/console/widgets/scrollable.rs" />) — use these building blocks when composing new modals rather than hand-rolling border + layout from scratch.
