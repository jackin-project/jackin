...
Launch rich-terminal capability checks and the canonical rich-terminal requirement error now live with launch terminal helpers in <RepoFile path="crates/jackin-launch/src/tui/terminal.rs" />. Progress orchestration and rich renderer entry points call that helper so terminal-gating behavior and operator-facing copy cannot drift.
