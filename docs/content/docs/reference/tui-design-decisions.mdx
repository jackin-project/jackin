...
Console debug-bar chip geometry now lives with the event-loop shell helpers in <RepoFile path="crates/jackin-console/src/tui/run.rs" />. Root rendering and mouse hit-testing call the same helper so the clickable run-info chip cannot drift from the rendered chip width.
