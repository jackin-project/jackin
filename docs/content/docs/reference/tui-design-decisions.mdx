...
**Migration status:** Capsule `tui/` started (Phase 9). Console and launch surface migrations are Phase 10 in the [TUI architecture roadmap](/reference/roadmap/tui-architecture/). New TUI files must go into the final directory shape now regardless of migration status. Existing transitional files may remain until migrated, but do not add new canonical TUI code to old locations.
