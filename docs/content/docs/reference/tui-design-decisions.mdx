...
**Migration status:** Capsule, launch, and console all have thin `src/tui.rs` roots. Capsule visible model vocabulary now lives under `crates/jackin-capsule/src/tui/app.rs`: hover targets, pointer shape, visible pane layout snapshots, and drag-resize state. Capsule terminal input parsing lives under `crates/jackin-capsule/src/tui/input.rs`; visible redraw/update vocabulary lives under `crates/jackin-capsule/src/tui/update.rs`; attach-client effect and subscription vocabulary lives under `crates/jackin-capsule/src/tui/effect.rs` and `crates/jackin-capsule/src/tui/subscriptions.rs`; interactive attach-loop wiring lives under `crates/jackin-capsule/src/tui/run.rs`; attach-client raw-mode and reset handling lives under `crates/jackin-capsule/src/tui/terminal.rs`; full Ratatui frame composition plus pane chrome/redraw helpers live under `crates/jackin-capsule/src/tui/view.rs`; mouse text-selection state and highlight/copy helpers live under `crates/jackin-capsule/src/tui/selection.rs`. Capsule callers use the canonical `crates/jackin-capsule/src/tui/` module paths directly instead of flat root re-export aliases. Daemon/session/protocol authority stays outside `src/tui/`. Launch model/message/update/effect/run/view files live under `crates/jackin-launch/src/tui/`, and launch-local visual components live under `crates/jackin-launch/src/tui/components/`. Console effect types, subscription vocabulary, generic top-level message/update carriers, event-loop shell helpers, top-level header/footer view helpers, screen model/message/update/effect/view helpers, and extracted console-local visual components now live under `crates/jackin-console/src/tui/`; screen state uses `model.rs` under `crates/jackin-console/src/tui/screens/`, editor/settings tab label and environment-label composition uses each screen's `view.rs`, and workspaces disclosure rendering uses `crates/jackin-console/src/tui/screens/workspaces/view.rs`. Root-only console run-loop wiring now lives under <RepoFile path="src/console/tui/run.rs" />, debug-log naming helpers live under <RepoFile path="src/console/tui/debug.rs" />, root-only console model state now lives under <RepoFile path="src/console/tui/state.rs" />, launch-dispatch state mutation lives under <RepoFile path="src/console/tui/launch.rs" />, agent/role prompt flow lives under <RepoFile path="src/console/tui/prompts.rs" />, root-only console input dispatch now lives under <RepoFile path="src/console/tui/input/mod.rs" />, root-only product message/update now lives under <RepoFile path="src/console/tui/message.rs" />, and root-only typed effect requests live under <RepoFile path="src/console/tui/effect.rs" /> while their root crate dependencies move behind service/domain boundaries. Root-only footer hint composition lives under <RepoFile path="src/console/tui/render/footer.rs" /> and `src/console/tui/render/footer/`. Root-only effect execution/background polling stays outside the TUI boundary in <RepoFile path="src/console/effects.rs" />. The console file-browser component emits typed outcomes for filesystem-backed navigation, commit validation, and Git URL lookup; the root file-browser service adapter executes those requests through `crates/jackin-console/src/services/file_browser.rs`. Browser launches leave input as typed `ConsoleInputOutcome::OpenUrl` values and are executed by the root TUI run loop in <RepoFile path="src/console/tui/run.rs" /> through `src/console/services/browser.rs`. Manager mount-info, instance-refresh, settings-save, auth-form 1Password read validation, Claude token minting, role repository registration, workspace-save writes, workspace-save drift/cleanup workers, file-browser service requests, and op-picker metadata loads now run through effect/write/poll/adapter execution and service adapters rather than model methods or modal save handlers; pending worker structs and picker state store requests or receivers but do not spawn services themselves. Remaining transitional components may stay until migrated, but do not add new canonical TUI code to old locations.
...
Enforcement:
- Every widget in `src/console/tui/components/` must expose a `footer_items` function (or equivalent) and remove its internal hint row.
...
2. **Components have two canonical homes.** Cross-surface components live in `crates/jackin-tui/src/components/`. Surface-local components live in `crates/<surface-crate>/src/tui/components/`. Existing root-console components in `src/console/tui/components/` are transitional until migrated; use them when extending current behavior, but do not add new canonical component patterns there.
...
3. While the console migration is incomplete, also search the transitional root-console widget location before adding or moving anything: `rg 'struct .*State|fn render_' src/console/tui/components`.
...
Reusable TUI widgets live first in `crates/jackin-tui/src/components/`. Surface-local widgets should live under the owning surface's `src/tui/components/`. The root console still has transitional components in `src/console/tui/components/`; they remain valid current owners until migrated, but they are not the target structure for new canonical component work. Use this table to identify the right component before writing new code or moving an existing widget.
...
**Render primitives**: `modal_block` (<RepoFile path="crates/jackin-tui/src/components/panel.rs" />), `breadcrumb_title` (<RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" />), `render_fatal` (<RepoFile path="src/console/tui/components/op_picker/render.rs" />), `render_filter_input` (<RepoFile path="crates/jackin-tui/src/components/filter_input.rs" />), and `render_scrollable_block_at` (<RepoFile path="crates/jackin-tui/src/components/scrollable_panel.rs" />) — use these building blocks when composing new modals rather than hand-rolling border + layout from scratch.
