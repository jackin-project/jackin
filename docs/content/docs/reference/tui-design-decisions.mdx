...
Console op-picker pending-load requests now use the generic carrier in <RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" />; the root adapter still supplies root `operator_env` result types and executes `op` metadata loads outside the TUI boundary.
...
Enforcement:
- Every root-console modal widget such as `src/console/tui/components/op_picker/` must expose a `footer_items` function (or equivalent) and remove its internal hint row.
