...
Console workspace-list current-directory row, new-workspace row, instance row-label formatting, and live-pane default agent label now live with workspaces screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/view.rs" />. Root adapters supply expansion, active-instance, selection, hover, instance ID, role key, and optional agent facts.
