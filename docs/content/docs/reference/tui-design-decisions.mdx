...
Editor auth row adapters ask <RepoFile path="src/console/domain.rs" /> for effective-mode credential requirements and eligible role keys. The root TUI state maps those facts into crate-owned editor row planning; it does not own those product rules inline.
