...
Console op-picker account-load completion planning and pending-load requests now use crate-owned helpers/carriers in <RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" />; the root adapter still supplies root `operator_env` result types and executes `op` metadata loads outside the TUI boundary.
