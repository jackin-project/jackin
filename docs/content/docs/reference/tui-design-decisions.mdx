...
Root-modal widget dispatch now lives in <RepoFile path="src/console/tui/components/modal.rs" /> while <RepoFile path="src/console/tui/view/modal.rs" /> only aliases that adapter for the root frame view.
...
- `+ New workspace` — list stage (<RepoFile path="src/console/tui/view/list.rs" />)
