...
**All bordered scrollable content blocks must use `render_scrollable_block`** (defined in <RepoFile path="crates/jackin-tui/src/components/scrollable_panel.rs" />, re-exported through `render/mod.rs` for manager-internal callers). Never hand-roll block creation, Paragraph scroll, or scrollbar rendering inline.
...
- `+ New workspace` — list stage (<RepoFile path="src/console/tui/render/list.rs" />)
- `+ Add role` — workspace editor Roles tab (<RepoFile path="src/console/tui/render/editor.rs" />)
- `+ Add mount`, `+ Add environment variable`, `+ Add {role} environment variable` — settings/editor mount + env tabs (<RepoFile path="src/console/tui/render/settings.rs" />)
