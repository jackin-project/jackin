...
The host console uses a Ratatui-style Model/Update/View separation. This is a **hard requirement** for every new TUI path: it is what makes the update layer testable, the render layer pure, and background I/O non-blocking.

**Messages and central update.** Every state change that could be driven by a test goes through `ManagerMessage` and `update_manager` instead of mutating `ManagerState` inline in the event handler. Event handlers translate terminal input into messages; `update_manager` applies the message and returns `UpdateResult` carrying a dirty flag. Adding a new input path means adding a `ManagerMessage` variant and a match arm in `update_manager`.

**Pure view.** Render functions take immutable model state (`&ManagerState`, `&EditorState`, `&SettingsState`). The only mutations allowed before `render` are geometry-prep calls (`prepare_for_render`) that run immediately before the draw and cache content geometry into the state. Nothing that runs inside `terminal.draw(...)` may write to the model, spawn background tasks, or perform I/O.

**Background I/O through channels.** Long-running work (instance refresh, role loading, 1Password lookups) runs on background threads that communicate with the event loop through channels. The event loop polls those channels on each tick and routes the results through `ManagerMessage`. No blocking disk, socket, or subprocess call runs on the event-loop thread.

**Async event loop.** The console event loop uses `crossterm::event::EventStream` + `tokio::select!` instead of `event::poll(50ms)`. This yields to the Tokio reactor between events so background tasks can advance. An `animation_tick` interval drives periodic redraws for spinners and other animated components.

**Component library.** Repeated visual patterns live in `crates/jackin-tui/src/components/`. New screens compose existing components; they do not hand-roll a second panel, picker, confirm dialog, or hint bar. Every `jackin-tui` component has a lookbook story in `crates/jackin-tui-lookbook/` visible in the terminal browser (`cargo run -p jackin-tui-lookbook -- --terminal`) and a generated SVG in the docs site that must never drift from the real widget.

---

## TUI source code location — mandatory directory convention

All code for the terminal user interface (Model, Update, View, and event-loop wiring) lives in a single `tui/` directory per surface crate. Everything NOT in `tui/` is business logic — workspace resolution, Docker commands, config writes, network I/O, data models that have no visual representation.
...
1. **`crates/jackin-tui/`** — Shared component library. Panels, pickers, scrollable blocks, hint bars, confirm dialogs, tab strips. Nothing surface-specific. Consumed by every other TUI crate.

2. **`crates/<surface-crate>/src/`** — Everything TUI for that surface: Model (state structs), Update (message handlers, key dispatch), View (render functions, widgets), and event-loop wiring. `tui/` is the View layer inside that surface crate. For single-screen crates one root `tui/` is enough. For multi-screen crates (e.g. `jackin-console`), screens are sub-folders at the crate root, each with its own `tui/`.

**Single-screen crate layout** (`jackin-capsule`, `jackin-launch`):
```
src/
├── run.rs          ← event-loop wiring
├── state.rs        ← Model (state structs)
├── update.rs       ← Update (message handlers, input dispatch)
...
```
src/
├── run.rs          ← ConsoleState/event-loop wiring
├── state.rs        ← top-level console model
...
│   ├── update.rs
│   └── tui/
└── widgets/        ← surface-specific sub-components
```

**MUV contract inside each surface crate:**
- `state.rs` — pure data structs; no rendering, no I/O.
- `update.rs` — pure fn(Model + Message) → Effects; no rendering.
- `tui/` — pure `&State` reads; no mutations, no I/O, no side effects.
- `run.rs` / wiring — event loop that connects M + U + V; not M, U, or V itself.

**What belongs in `tui/`** (the View layer):
- Ratatui widget implementations
- Raw ANSI rendering functions
- Pure frame composition and geometry helpers
- View-only hover/click hitbox descriptions

**What does NOT belong in `tui/`**:
- Model state (`state.rs`), including focus state, scroll state, and modal state
- Update logic (`update.rs`), including message handlers and input dispatch
- Event-loop wiring and terminal lifecycle (`run.rs`, `terminal.rs`)
- Workspace resolution, role loading, Docker commands
- Network and I/O calls
- Config file writes and reads
- Data models that have no visual representation (e.g. `session.rs` PTY management in the capsule)

**Enforcement:** before adding any file, ask: "Does this file mutate state, dispatch input, manage terminal lifecycle, or touch workspace resolution, Docker, or config?" Yes → it does not go in `tui/`. No → it belongs in `tui/`.

**Migration status:** Capsule `tui/` started (Phase 9). Console and launch surface migrations are Phase 10. New files must go into the correct directory now regardless of migration status.
...
Enforcement:
- Every widget in `src/console/tui/widgets/` must expose a `footer_items` function (or equivalent) and remove its internal hint row.
...
Equivalently: the filter starts at `box_row + 1`, the spacer at `box_row + 2`, and the first list row at `box_row + 3`. The natural box height is `items + 4` (top border + filter + spacer + items + bottom border). Both the host pickers (<RepoFile path="crates/jackin-console/src/widgets/role_picker.rs" /> and <RepoFile path="src/console/tui/widgets/op_picker/render.rs" />) and the in-multiplexer pickers (<RepoFile path="crates/jackin-capsule/src/tui/dialog.rs" />) honour this layout — an operator's eye must read identically across surfaces.
...
1. **`Vec<Dialog>` stack** (canonical, used by `crates/jackin-capsule/src/tui/dialog.rs`). `dialog_top()` is "is a dialog open"; entries land via `dialog_push`; back-nav arms call `dialog_pop_one`; terminal-action arms call `dialog_clear`.
...
2. **Components live in `crates/jackin-console/src/widgets/` first, with temporary compatibility adapters in `src/console/tui/widgets/`.** Before writing any new TUI state machine or renderer, `grep` both directories. If a widget handles the same visual pattern (drill-down picker, text input box, 2-button choice, yes/no confirm, filter list), extend that widget rather than creating a parallel one.
...
1. Run `grep -r 'struct.*State' crates/jackin-console/src/widgets/ src/console/tui/widgets/` and `ls crates/jackin-console/src/widgets src/console/tui/widgets`. Read the matches. Is there a widget that already owns the pattern?
2. Run `grep -r 'fn render_' crates/jackin-console/src/widgets/ src/console/tui/widgets/` for the render-level equivalent.
3. If yes — extend the existing widget. Add a mode enum, a new bool flag, or a new variant. Do not create a second module.
4. If no — propose the design first; get explicit approval; then implement. The first implementation goes into `crates/jackin-console/src/widgets/` so the next caller can find it.

### Why this rule exists — canonical lesson

A `token_store_picker` widget once duplicated the Account → Vault → Item navigation state machine from <RepoFile path="src/console/tui/widgets/op_picker/mod.rs" /> — a 1067-line copy of closely parallel code where every `op_picker` bug fix to navigation, filtering, R-key refresh, or section display had to be manually reproduced or the two implementations drifted. That copy has since been deleted and `op_picker` was extended instead: an opt-in `OpPickerMode::Create` turns on the `+ New item`, `+ New field`, and `+ New section` creation sentinels and the naming sub-stages, and the picker commits an `OpPickerSelection` enum (`Existing` / `NewItem` / `EditItemField`). One widget now serves both the browse and create call sites. This is the positive reference for the rule: when a new call site needs more, **extend the shared widget with a mode**, never fork it into a second drill-down. The remaining Claude-token work is tracked in the [Workspace Claude Token Setup](/reference/roadmap/workspace-claude-token-setup/) roadmap item.

---

## Reusable Component Catalog

Shared TUI widgets live in `crates/jackin-tui/src/components/`, `crates/jackin-console/src/widgets/`, and the remaining console-local `src/console/tui/widgets/` modules. Use this table to identify the right component before writing new code.

| Component | Module | What it provides | When to use |
|---|---|---|---|
| Op picker (1Password drill-down) | <RepoFile path="src/console/tui/widgets/op_picker/mod.rs" /> | Account → Vault → Item → Field 4-stage drill-down with filter, R-key refresh, collapsible section headers, background loading, and `op://` reference commit. An opt-in `OpPickerMode::Create` adds `+ New item` / `+ New field` / `+ New section` creation sentinels and naming sub-stages, committing an `OpPickerSelection` enum | Any flow that needs to browse, pick, or create a 1Password field reference |
| Role picker | <RepoFile path="crates/jackin-console/src/widgets/role_picker.rs" /> | Filter list of available roles with type-to-narrow and Enter commit | Any flow that needs to select a role from the known set |
...
| Scope picker | <RepoFile path="crates/jackin-console/src/widgets/scope_picker.rs" /> | Two-button horizontal strip: `All roles` / `Specific role` | Any "apply to everything vs apply to one" choice |
| Source picker | <RepoFile path="crates/jackin-console/src/widgets/source_picker.rs" /> | Two-button horizontal strip: `Plain text` / `1Password` | Choosing whether a value is literal or an `op://` reference |
| Agent choice | <RepoFile path="crates/jackin-console/src/widgets/agent_choice.rs" /> | Two-button agent selector used in the Auth-tab `+ Add` flow | Picking one agent from a small fixed set |
| Mount destination choice | <RepoFile path="crates/jackin-console/src/widgets/mount_dst_choice.rs" /> | Two-button strip for mount destination choices | Mount configuration flows |
...
| File browser | <RepoFile path="crates/jackin-console/src/widgets/file_browser/mod.rs" /> | Host filesystem navigation | Any flow needing to pick a local file or directory |
| GitHub picker | <RepoFile path="crates/jackin-console/src/widgets/github_picker.rs" /> | GitHub org/repo drill-down | Any flow needing to select a GitHub repository |
...
**Render primitives**: `modal_block`, `breadcrumb_title`, `render_fatal` (<RepoFile path="src/console/tui/widgets/op_picker/render.rs" />), `render_filter_row` (<RepoFile path="src/console/tui/widgets/mod.rs" />), and `render_scrollable_block` (<RepoFile path="crates/jackin-tui/src/components/scrollable_panel.rs" />) — use these building blocks when composing new modals rather than hand-rolling border + layout from scratch.
...
- `+ Add mount`, `+ Add environment variable`, `+ Add {role} environment variable` — settings/editor mount + env tabs (<RepoFile path="src/console/tui/render/global_mounts.rs" />)
