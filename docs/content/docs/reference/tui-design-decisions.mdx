...
**Migration status:** Capsule, launch, and console all have thin `src/tui.rs` roots. Launch top-level composition lives in `crates/jackin-launch/src/tui/view.rs`, and launch-local visual components live under `crates/jackin-launch/src/tui/components/`. Console screen model/update helpers now live under `crates/jackin-console/src/tui/screens/` using `model.rs` for screen state, and extracted console-local visual components live under `crates/jackin-console/src/tui/components/`. Remaining transitional widgets may stay until migrated, but do not add new canonical TUI code to old locations.
...
| File browser | <RepoFile path="crates/jackin-console/src/tui/components/file_browser/mod.rs" /> + <RepoFile path="crates/jackin-console/src/services/file_browser.rs" /> | Host filesystem navigation UI plus non-TUI directory/sandbox/Git URL adapters | Any flow needing to pick a local file or directory |
