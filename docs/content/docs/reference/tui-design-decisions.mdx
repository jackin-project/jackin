...
Console workspace-list current-directory row, new-workspace row, instance row-label formatting, live-pane default agent label, picker sidebar title formatting, and role-global-mount title formatting now live with workspaces screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/view.rs" />. Root adapters supply expansion, active-instance, selection, hover, instance ID, role key, optional agent, workspace-name, role-name, and role-label facts.
