...
3. While the console migration is incomplete, also search the transitional console widget locations before adding or moving anything: `rg 'struct .*State|fn render_' crates/jackin-console/src/widgets src/console/tui/widgets`.
...
Reusable TUI widgets live first in `crates/jackin-tui/src/components/`. Surface-local widgets should live under the owning surface's `src/tui/components/`. The console still has transitional widgets in `crates/jackin-console/src/widgets/` and `src/console/tui/widgets/`; they remain valid current owners until migrated, but they are not the target structure for new canonical component work. Use this table to identify the right component before writing new code or moving an existing widget.
