...
Console settings-environment value-edit labels now live with settings screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/settings/view.rs" />. Root settings input supplies selected key/current value and keeps environment mutation side effects.
