...
Console auth-form mode cycling and mode-to-next-focus planning now live with the shared auth-panel component in <RepoFile path="crates/jackin-console/src/tui/components/auth_panel.rs" />. Editor and settings auth input keep root modal persistence, token-generation return paths, and credential commit side effects, but share the same component-owned mode-row behavior.
