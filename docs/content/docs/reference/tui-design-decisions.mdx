...
Console GitHub auth mode changes preserve existing `[github.env]` maps through <RepoFile path="src/console/domain.rs" />. Root auth-tab input applies the selected mode to concrete workspace or role config, but the product rule that mode changes must not clobber staged `GH_TOKEN` values stays outside TUI input handlers.
