...
Console workspace instance action/status acceptance planning and preview-pane focused key routing now live with workspaces screen update helpers in <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/update.rs" />. Root list input maps concrete root action/status values into crate-owned facts before selecting a container and supplies preview pane counts before dispatching root messages or focused reconnect actions.
