...
The transitional root op-picker adapter may own `operator_env` types, runner wiring, cache binding, and subscriptions, but user-visible picker copy such as recoverable worker errors belongs in <RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" /> with the rest of the component.
