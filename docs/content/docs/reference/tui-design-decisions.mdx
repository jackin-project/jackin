...
Capsule dialog state construction defaults for rename-tab, split-direction, close-target, confirm-action, container-info, and GitHub-context dialogs now also live under <RepoFile path="crates/jackin-capsule/src/tui/components/dialog.rs" />; daemon code supplies runtime facts and executes terminal actions.
