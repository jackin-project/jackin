...
Console settings-auth global GitHub env extraction, source lookup, credential-env commit routing, and credential-env clear routing also live in <RepoFile path="src/console/domain.rs" />. Root settings state stores pending/original maps for UI dirty tracking, form mounting, and save previews; it does not decide inline whether a credential comes from `[github.env]` or normal environment maps.
