...
All code for a terminal user interface lives under one `src/tui/` directory per surface crate and follows The Elm Architecture: Model, Message, Update, Effects/Subscriptions, View, and event-loop wiring. Everything outside `src/tui/` is non-visual product code: workspace resolution, Docker commands, config writes, network I/O, credential handling, capsule protocol/session authority, and data models that have no terminal-interaction role.
...
2. **`crates/<surface-crate>/src/tui/`** — The whole TUI application layer for that surface: Model, Message, Update, Effects, Subscriptions, View, local components, terminal adapter, and event-loop wiring. Single-screen crates stay flat. Multi-screen crates add `screens/<screen>/` under `src/tui/`.
...
**Migration status:** Capsule `tui/` started (Phase 9). Console and launch surface migrations are Phase 10. New files must go into the final directory shape now regardless of migration status.
