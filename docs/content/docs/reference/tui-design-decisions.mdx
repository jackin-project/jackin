...
**Migration status:** Capsule, launch, and console all have thin `src/tui.rs` roots. Capsule visible model vocabulary now lives under `crates/jackin-capsule/src/tui/app.rs`: mux mode, hover targets, pointer shape, visible agent state and protocol-to-visible state mapping, visible pane layout snapshots and derivation, drag-resize state, mux-mode derivation, chrome-hover priority, pointer-shape derivation, cursor-visibility derivation, and visible tab-pane classification. Capsule visible pane tree/tab geometry, content-row and content-rect derivation, split-spawn sizing, main-view hint row reservations, and pane-local mouse coordinate mapping now live under `crates/jackin-capsule/src/tui/layout.rs`. Capsule terminal input parsing, tab double-click timing, escape-time defaults, SGR passive-motion vocabulary, mouse protocol gating/encoding, plus key-binding data/parsing live under `crates/jackin-capsule/src/tui/input.rs`, while the env/logging resolver for those bindings lives outside the TUI boundary in `crates/jackin-capsule/src/services/input_bindings.rs`; semantic action vocabulary plus parsed-input, prefix-command, command-palette route planning, confirmed-action route planning, palette-toggle routing, pane-motion, mouse-release, status-bar-click, and branch-context-bar-click action mapping live under `crates/jackin-capsule/src/tui/message.rs`; visible redraw/update vocabulary, hover redraw policy, dialog action redraw planning, dialog copy-feedback timing, selection-start redraw planning, pane-data redraw planning, partial-frame invalidation policy, and drag-resize ratio math live under `crates/jackin-capsule/src/tui/update.rs`; attach-client effect and subscription vocabulary plus capsule tick intervals live under `crates/jackin-capsule/src/tui/effect.rs` and `crates/jackin-capsule/src/tui/subscriptions.rs`; interactive attach-loop wiring lives under `crates/jackin-capsule/src/tui/run.rs`; attach-client raw-mode, reset handling, terminal size normalization, and OSC 22 pointer-shape encoding live under `crates/jackin-capsule/src/tui/terminal.rs`; full Ratatui frame composition plus raw-ANSI status bar, pane chrome, pane body, dialog overlay, dialog footer, bottom chrome, selection-highlight render helpers, and spawn-failure visible message/banner formatting live under `crates/jackin-capsule/src/tui/view.rs`; status-bar prefix-mode mapping and supplied visible identity-label storage live under `crates/jackin-capsule/src/tui/components/status_bar.rs`, and command-palette close-label derivation lives under `crates/jackin-capsule/src/tui/components/dialog.rs`; mouse text-selection state, start, motion, and copy helpers live under `crates/jackin-capsule/src/tui/selection.rs`. Capsule callers use the canonical `crates/jackin-capsule/src/tui/` module paths directly instead of flat root re-export aliases. Pure pull-request render snapshots live under <RepoFile path="crates/jackin-capsule/src/pull_request.rs" />, while daemon/session/protocol authority stays outside `src/tui/` and maps protocol state into visible frame data before calling TUI composition helpers. Launch model/message/update/effect/run/view files live under `crates/jackin-launch/src/tui/`, including semantic prompt context lines that the TUI maps to styled Ratatui lines, and launch-local visual components live under `crates/jackin-launch/src/tui/components/`; the docker-build log sink remains outside the TUI in <RepoFile path="crates/jackin-launch/src/build_log.rs" />, and the launch run loop snapshots it into `LaunchView` so the build-log overlay renders from model state. Launch hyperlink-overlay bytes are derived in `crates/jackin-launch/src/tui/view.rs` and emitted only by `crates/jackin-launch/src/tui/run.rs` after drawing; launch standalone prompt context styling, rich-dialog-required fallback wording, and role-trust confirm dialog state are also constructed inside `crates/jackin-launch/src/tui/run.rs`, keeping `crates/jackin-launch/src/progress.rs` focused on orchestration. Console generic top-level app, create-prelude, modal vocabulary, and manager-stage model, auth UI vocabulary, surface auth-kind ordering, credential-requirement policy, Claude token-generate eligibility, effect types, subscription vocabulary, generic pending-worker carriers, generic top-level message/update carriers, term-width horizontal scroll planning, generic selection and unclamped scroll planning, debug key-name formatting, event-loop shell helpers, split/mouse geometry helpers, generic sidebar input facts, seam hit-region helpers, mouse-wheel step policy, list body and visual-row hit-testing helpers, scrollbar drag offset helpers, tab-strip hit-testing, tabbed content-area geometry, top-level header/footer view helpers, footer-height calculation, screen model/message/update/effect/view helpers, editor-secret key lookup/mutation plus secret row unmask-target and label-free action/Enter planning, settings stage state, settings global-mount state, settings-auth state, settings-env state, settings-env key lookup/mutation, row action/Enter planning, settings-env mask toggling/delete-row mutation helpers, settings modal vocabulary, and extracted console-local visual components now live under `crates/jackin-console/src/tui/`; screen state uses `model.rs` under `crates/jackin-console/src/tui/screens/`, editor/settings tab label and environment-label composition uses each screen's `view.rs`, editor secret delete/role-trust/isolated-save confirm state, editor name/secret value/new-secret value/secret key/role-load/mount-destination text-input state, editor secret new-key/empty-key labels and duplicate-key derivation, secret source-picker/scope-picker, role-picker default/launch confirm labels, and mount-destination choice state, editor frame/body layout, General/Mounts/Roles/Secrets/Auth-tab line composition, row-width primitives, and viewport scroll clamping live in `crates/jackin-console/src/tui/screens/editor/view.rs`, shared editor/settings auth-source display vocabulary and display derivation from supplied values live in `crates/jackin-console/src/tui/components/editor_rows.rs`, and shared auth source-picker plus credential text-input state helpers live in `crates/jackin-console/src/tui/components/auth_panel.rs`, workspace/global mount row rendering lives in `crates/jackin-console/src/tui/components/mount_rows.rs`, mount-row, workspace-list, create-prelude, destructive-confirm, editor/settings row, secret/env footer hint blocks, editor/settings contextual row-mode planning, workspace-list footer-mode planning, modal footer mode vocabulary, and workspace/settings save-preview line rendering live in `crates/jackin-console/src/tui/components/footer_hints.rs` and `crates/jackin-console/src/tui/components/save_preview.rs`, and workspace delete and instance purge confirm state/planning live in `crates/jackin-console/src/tui/screens/workspaces/update.rs`; create-prelude destination/name text-input, mount-destination choice, workdir-picker state helpers, plus workspaces disclosure, list-name row/block/scrollbar rendering, instance-detail pane rendering, running-instance summary, instance session empty/error wording, new-workspace description pane rendering, provider/role/agent picker sidebar rendering plus launch role-picker confirm label, and General/Mounts/Global-mounts/Environments/Roles subpanel rendering use `crates/jackin-console/src/tui/screens/workspaces/view.rs`. Root-only console app binding now lives under <RepoFile path="src/console/tui/app.rs" />, root-only console auth-panel binding now lives under <RepoFile path="src/console/tui/components/auth_panel.rs" /> for editor and settings auth display rows, root-only editor tab-strip plus General/Mounts/Roles/Secrets/Auth tab-body display mapping now lives under <RepoFile path="src/console/tui/components/editor.rs" />, root-only settings tab-strip/tab-body/global-mount/environment/trust display mapping now lives under <RepoFile path="src/console/tui/components/settings.rs" />, root-only `EnvValue` secret-display mapping lives under <RepoFile path="src/console/tui/components/env_value.rs" />, and root-only workspace-list body/name/instance/picker/mount/environment/role detail/sidebar display mapping now lives under <RepoFile path="src/console/tui/components/workspace_list.rs" />. Root-only console run-loop wiring now lives under <RepoFile path="src/console/tui/run.rs" />, root-state debug location labels live in <RepoFile path="src/console/tui/debug.rs" />, root-only console model state with concrete manager-stage and settings-state aliases, config-backed settings construction, create-prelude completion helpers, and root-bound editor auth/secret and settings environment row adapters now live under <RepoFile path="src/console/tui/state.rs" />, launch-dispatch state mutation lives under <RepoFile path="src/console/tui/launch.rs" />, agent/role prompt flow with crate-owned role-resolution popup wording lives under <RepoFile path="src/console/tui/prompts.rs" />, root-only console input dispatch now lives under <RepoFile path="src/console/tui/input/mod.rs" />, with concrete `InputOutcome` aliases backed by generic input, modal, prompt outcome, and mount-info refresh result vocabulary in <RepoFile path="crates/jackin-console/src/tui/message.rs" />, root-only product update logic and concrete `ManagerMessage` alias now live under <RepoFile path="src/console/tui/message.rs" />, and root-only effect aliases live under <RepoFile path="src/console/tui/effect.rs" />; the typed manager, file-browser, and workspace-save effect vocabulary lives under <RepoFile path="crates/jackin-console/src/tui/effect.rs" /> while root crate dependencies move behind service/domain boundaries. Root pre-render cache preparation now lives under <RepoFile path="src/console/tui/layout/prepare.rs" />, keeping footer-height, scroll-clamp, terminal-size, and confirm-save modal geometry mutation outside the render module. Root-only auth-kind/config-mode derivation, shared mount draft construction, synthetic current-directory mount construction, global-mount naming and scope parsing, workspace-save prospective mount merging, and save-preview validation message text live outside the TUI boundary in <RepoFile path="src/console/domain.rs" />. Root-only footer hint composition lives under <RepoFile path="src/console/tui/components/footer.rs" /> and `src/console/tui/components/footer/` as adapters that map root state to crate-owned footer facts, editor/settings context modes, footer hint modes, and item builders; workspace-list and editor/settings contextual footer-mode planning are crate-owned. Root-only auth-panel display mapping lives under <RepoFile path="src/console/tui/components/auth_panel.rs" /> as an adapter between root auth rows/config facts and crate-owned editor/settings row display values. Root-only save-preview code lives under <RepoFile path="src/console/tui/components/save_preview.rs" /> as snapshot mapping into the crate-owned save-preview component. Root-only effect execution/background polling stays outside the TUI boundary in <RepoFile path="src/console/effects.rs" />. The console file-browser component emits typed outcomes for filesystem-backed navigation, commit validation, and Git URL lookup; Git URL lookup is requested as `ManagerEffect::ResolveFileBrowserGitUrl`, navigation and commit validation are requested as `ManagerEffect::ApplyFileBrowserOutcome`, and the root file-browser service adapter executes file-browser requests through `crates/jackin-console/src/services/file_browser.rs`. Browser launches and workspace deletes now queue `ManagerEffect::OpenUrl` and `ManagerEffect::RemoveWorkspace` from input/update and execute outside the TUI boundary. Manager mount-info, instance-refresh, settings-save, auth-form 1Password read validation, Claude token minting, role repository registration, trusted-role persistence, workspace-save drift gates, isolation-record scans, cleanup workers, workspace writes, create-prelude file-browser opens/reopens, editor add-mount browser opens, global mount browser opens, browser launches, workspace deletes, file-browser service requests, and op-picker metadata loads now run through effect/poll/adapter execution and service adapters rather than model methods or modal save handlers; op-picker display metadata structs, naming text-input state helpers, query/filter helpers, render trait and renderer, selected-index routing, naming-stage input routing, filter-reset selection routing, pane-refresh planning, account-stage navigation planning, vault-stage navigation planning, item-stage navigation planning, field-back navigation planning, field-stage commit routing, field-label commit routing, existing-field commit routing, section-stage navigation planning, section-collapse routing, naming-stage transition planning, naming text-input state construction, field-row derivation, and cache type live in <RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" />, while root `operator_env` keeps compatibility aliases for the CLI runner and root picker state implements the crate-owned render trait; agent resolution returns typed picker choices before prompt state opens the inline picker, and role-resolution, no-eligible-launch, and workspace-list instance and instance-refresh index prompt status/error wording lives in console-local popup helpers, token-generate visible status wording plus in-place instance-action busy/failure wording lives in <RepoFile path="crates/jackin-console/src/tui/run.rs" /> and token-generate request take semantics live on the manager model. Settings frame layout, settings inline error-message wording, settings error-popup title, sensitive-path abort wording, global-mount confirm/scope-picker/role-picker default/launch confirm labels/text-input state and add-cancel error wording, settings-env source/scope-picker/delete-confirm state plus role-picker default/launch confirm labels, settings-env key/value text-input construction, plus General/Mounts/Environment/Auth/Trust-tab line composition now also live with settings screen view helpers. The remaining transitional op-picker component may stay until migrated, but do not add new canonical TUI code to old locations.

Capsule focus-change, drag-resize, and selection-change redraw planning also lives with visible update vocabulary in <RepoFile path="crates/jackin-capsule/src/tui/update.rs" />. The daemon still owns concrete focus, drag, and selection state mutation, but it asks the TUI update boundary for the visible invalidation reasons.

Capsule palette-route redraw planning also lives with visible update vocabulary in <RepoFile path="crates/jackin-capsule/src/tui/update.rs" />. The daemon still executes palette commands, while TUI update owns the route-to-redraw reason for terminal actions.

Capsule direct-action frame planning also lives with visible update vocabulary in <RepoFile path="crates/jackin-capsule/src/tui/update.rs" />. The daemon still applies concrete tab, pane, dialog, zoom, and detach mutations, while TUI update owns the full-frame versus overlay-frame reasons for those direct actions.

Capsule wheel scrollback, unsafe partial fallback, and pane-cache-miss redraw reasons also live with visible update vocabulary in <RepoFile path="crates/jackin-capsule/src/tui/update.rs" />. The daemon/compositor still owns concrete scrollback, fallback, and cache facts, while TUI update owns the visible invalidation reasons.

Capsule first-attach, resize, session-exit, status-change, dialog-change, and explicit redraw reasons also live with visible update vocabulary in <RepoFile path="crates/jackin-capsule/src/tui/update.rs" />. The daemon still owns attach, resize, session, status-worker, and prefix-mode facts, but it should request named TUI update helpers instead of constructing those full-redraw reasons inline.

Console settings global-mount missing-GitHub-URL, scope text fallback, and add/edit validation wording now lives with settings screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/settings/view.rs" />; root global-mount input supplies only mount-info URL availability, selected row, add-draft presence, and optional configured role-scope facts.

Console editor Roles-tab status, role-row, and load-row width planning now lives with editor screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />; root editor geometry supplies only role counts and names before applying the crate-owned width rules.

Console editor Auth-tab row width planning now lives with editor screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />; root editor geometry maps root auth rows to the same crate-owned display rows used by rendering, then applies the crate-owned width rules.

Console editor Secrets-tab row width planning now lives with editor screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />; root editor geometry supplies secret values, unmask state, role registry membership, and role counts through the same closures used by rendering.

Console editor secret value-edit current-value fallback now lives with editor screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />. Root editor input supplies optional persisted values only.
...
Console top-level update plans for status-overlay open/dismiss, list-modal open/dismiss, inline-picker dismissal, inline-picker shell key policy, inline provider follow-up, list focus, pre-render list-focus liveness, pre-render list-scroll reset, drag-state, and split-percent normalization live in <RepoFile path="crates/jackin-console/src/tui/update.rs" />. Focus helpers live in <RepoFile path="crates/jackin-console/src/tui/focus.rs" />, split/drag state primitives live in <RepoFile path="crates/jackin-console/src/tui/split.rs" />, mount-display and `op://` breadcrumb display models live in <RepoFile path="crates/jackin-console/src/tui/mount_display.rs" /> and <RepoFile path="crates/jackin-console/src/tui/op_breadcrumb.rs" />, and provider-picker state, terminal-key mapping, and outcome planning lives with console-local components in <RepoFile path="crates/jackin-console/src/tui/components/provider_picker.rs" />. Root <RepoFile path="src/console/tui/message.rs" /> and list input/layout prep apply those plans to concrete root fields; they should not reintroduce generic modal, picker visibility, inline-picker shell key policy, provider follow-up, provider picker key mapping and transitions, focus, scroll-reset, drag, or split rules inline.

Workspace save-preview mount summaries are now formatted from crate-owned preview rows in <RepoFile path="crates/jackin-console/src/tui/components/save_preview.rs" />. Root save-preview mapping supplies shortened source/destination paths, mode, isolation, and mount-info label facts instead of preformatted summary strings.

Create-workspace save-preview fallback naming also lives with the console-local save-preview component in <RepoFile path="crates/jackin-console/src/tui/components/save_preview.rs" />. Root save-preview adapters pass only the optional pending workspace name; they do not hard-code the visible `"(unnamed)"` create-mode fallback.
...
Console editor/settings file-browser URL opens return typed modal outcomes and are queued as `ManagerEffect::OpenUrl` by input dispatch. Modal handlers must not side-channel browser requests through mutable URL slots.

Console op-picker selected label fallback, account/vault/item/field load-completion planning, selected account/vault/item cache-key derivation, concealed-first field-load sorting policy, and pending-load requests now use crate-owned helpers/carriers in <RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" />; the root adapter still supplies root `operator_env` result types and executes `op` metadata loads outside the TUI boundary.

Console op-picker commit selection shaping now also lives in <RepoFile path="crates/jackin-console/src/tui/components/op_picker.rs" />. Root code may provide concrete `operator_env` reference and field-target values, but it must use the crate-owned helpers for the `Existing`, `NewItem`, and `EditItemField` picker selection shapes.
...
Console launch prompt agent-choice lookup calls the non-TUI agent service boundary in <RepoFile path="src/console/services/agents.rs" /> from the root run loop. Prompt helpers keep terminal drawing and state transitions only; Docker-backed role/agent resolution stays outside `src/console/tui/`.

Console GitHub auth mode changes preserve existing `[github.env]` maps through <RepoFile path="src/console/domain.rs" />. Editor Auth-tab commit/reset mutation for workspace and role auth layers also lives there: root auth input commits the shared auth form, then delegates mode-block updates, Github env insertion, and Zai ignore cleanup.

Console settings-auth global GitHub env extraction, source lookup, credential-env commit routing, and credential-env clear routing also live in <RepoFile path="src/console/domain.rs" />. Generic settings state stores pending/original maps for UI dirty tracking, form mounting, and save previews; its root binding does not decide inline whether a credential comes from `[github.env]` or normal environment maps.

Editor auth row/form adapters ask <RepoFile path="src/console/domain.rs" /> for effective-mode credential requirements, eligible role keys, explicit workspace modes, display source-layer lookup, and workspace/role mode-plus-credential derivation. The root TUI state maps those facts into crate-owned editor row planning and shared auth forms; it does not own those product rules inline.

Launch rich-terminal capability checks, terminal-size-to-frame-area fallback, and the canonical rich-terminal requirement error now live with launch terminal helpers in <RepoFile path="crates/jackin-launch/src/tui/terminal.rs" />. Progress orchestration, rich renderer entry points, render ticks, and cockpit input subscriptions call those helpers so terminal-gating behavior, build-log scroll geometry, and operator-facing copy cannot drift.
...
Workspace-list root rendering and pre-render scroll preparation both use the crate-owned split-column helper in <RepoFile path="crates/jackin-console/src/tui/list_geometry.rs" />. Root adapters supply selection and pane content facts; the reusable list/preview column allocation lives with console TUI geometry.
...
Settings tab content-height rules for Mounts, Auth, Environment, and Trust tabs live with the settings screen view helpers; root layout code supplies row counts and error-state facts, and Auth height consumes the same detail-row-count helper as settings input focus.

Settings environment add-cancel wording lives with settings screen view helpers. Root input supplies the cancellation state transition and uses the crate-owned message when the modal chain closes.
...
Console debug-bar chip geometry and run-id empty fallback now live with the event-loop shell helpers in <RepoFile path="crates/jackin-console/src/tui/run.rs" />. Root rendering and mouse hit-testing call the same helper so the clickable run-info chip cannot drift from the rendered chip width or no-run fallback.

Console create-workspace prelude destination/name default derivation now lives with workspaces screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/view.rs" />. Root state stores the concrete path facts, but visible default strings for destination/name inputs and mount-destination choices come from the screen helper.
...
Console workspace-list current-directory row, new-workspace row, instance row-label formatting, instance purge-confirm label formatting, list-width labels, live-pane default agent label, picker sidebar title formatting, global-mount title formatting, and role-global-mount title formatting now live with workspaces screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/view.rs" />. Root adapters and layout code supply expansion, active-instance, selection, hover, instance ID, container base, role key, optional agent, workspace-name, role-name, and role-label facts.

Console editor tab-bar focus planning, field-selection row/scroll planning, mount-row direct selection focus planning, horizontal-scroll focus/offset planning, and mouse scroll-focus routing now lives with editor screen update helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/update.rs" />. Root update/mouse applies the selected row, vertical scroll offset, horizontal scroll offset, workspace-mount focus flag, and tab-content focus flag to concrete editor state.

Console editor stage state now lives in <RepoFile path="crates/jackin-console/src/tui/screens/editor/model.rs" />. The root crate binds it through a concrete alias and keeps only workspace-schema dirty/count helpers plus per-mount isolation cycling because those still depend on root config and isolation types.

Console editor workspace-name fallback derivation now lives with editor screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />. Root editor input and root auth-panel state pass only edit/create mode, optional pending name, and caller-specific create fallback before opening rename input or resolving the workspace key used by auth rows.

Console settings tab-bar focus planning, tab-move focus planning, direct tab-select planning, auth-row selection planning, Auth-tab detail row-count planning, global-mount selection/scroll planning, environment-row selection/scroll planning, trust-row selection/scroll planning, trust-row direct selection focus planning, horizontal-scroll offset planning, mouse scroll-focus routing, and auth-kind entry/clear focus planning now live with settings screen update helpers in <RepoFile path="crates/jackin-console/src/tui/screens/settings/update.rs" />. Root update/mouse applies those plans to concrete settings state and does not decide inline whether the credential-source row is present.
...
Console settings-environment value-edit/current-value fallback and new-key labels now live with settings screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/settings/view.rs" />. Root settings input supplies selected scope/key/optional persisted value and keeps environment mutation side effects.

Console workspace instance action/status acceptance planning, list-selection movement/direct-select clearing plans, selected-tree disclosure planning, preview-pane focus/key routing and cursor movement, workspace-list mouse scroll-focus routing, workspace-list unclamped scroll offset planning, and destructive-confirm outcome planning now live with workspaces screen update helpers in <RepoFile path="crates/jackin-console/src/tui/screens/workspaces/update.rs" />. Root list input maps concrete root action/status values into crate-owned facts before selecting a container, supplies mouse-region facts before applying list-name and mount-scroll focus, and supplies preview pane counts before dispatching root messages or focused reconnect actions; root update keeps only preview focus/cursor, tree expansion state, concrete picker fields, and concrete scroll-slot storage, and root confirm handlers keep only root effects/actions after consuming the crate-owned commit/return/continue plan.

Console auth-panel title formatting, auth-form mode cycling, mode-to-next-focus planning, and full auth-form key/focus planning now live with the shared auth-panel component in <RepoFile path="crates/jackin-console/src/tui/components/auth_panel.rs" />; auth-form target kind access lives with <RepoFile path="crates/jackin-console/src/tui/screens/settings/model.rs" />. Editor and settings auth input keep root modal persistence, token-generation return paths, credential-source side trips, and credential commit/reset side effects, but share the same component-owned title and focus behavior.

Auth form literal-buffer derivation lives with the shared auth-panel component in <RepoFile path="crates/jackin-console/src/tui/components/auth_panel.rs" />. Root editor/settings adapters may store the buffer for modal round-trips, but they do not inspect credential variants to compute it.
...
Capsule visible agent-label formatting for pane/tab chrome and shell session launch labels now live with visible multiplexer model helpers in <RepoFile path="crates/jackin-capsule/src/tui/app.rs" />. Daemon/session code supplies agent slug and provider label facts.
...
Capsule dialog state construction defaults for rename-tab, split-direction, close-target, confirm-action, container-info, provider-picker, and GitHub-context dialogs now also live under <RepoFile path="crates/jackin-capsule/src/tui/components/dialog.rs" />; container-info diagnostics, provider choices, and GitHub-context branch/PR loading status are TUI-owned display snapshots, while daemon code supplies runtime facts and executes terminal actions. Capsule attach-spawn, spawn-capacity, and spawn-failure agent-label wording lives with spawn-failure visible formatting in <RepoFile path="crates/jackin-capsule/src/tui/view.rs" />; daemon code still enforces the limits, performs PTY/session allocation, and sends the banner frame.
...
Console role-load error text is assembled through <RepoFile path="crates/jackin-console/src/tui/components/error_popup.rs" />. Root state may classify root-only repository and validation error types, but popup wording belongs to the console TUI component boundary.
...
Background-work status popups follow the same split: <RepoFile path="src/console/effects.rs" /> starts role registration, workspace drift checks, and isolated-state cleanup, while <RepoFile path="crates/jackin-console/src/tui/components/status_popup.rs" /> owns the visible status title/message construction.
...
Test-only helpers that need command runners, service calls, or config persistence belong with effect/service adapters, not input handlers. Role-input tests may use fake runners through <RepoFile path="src/console/effects.rs" />, but <RepoFile path="src/console/tui/input/editor.rs" /> must stay limited to input-to-outcome routing and modal state transitions.
...
Editor save prompts and save-blocking error text that depend only on visible editor facts live with editor screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />. Root save input and the effect executor still own drift detection, preserved-state cleanup, and workspace writes; they pass render-safe facts such as affected container names into the screen helpers.

Editor General-tab row vocabulary and content-width planning also live with editor screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />. Root layout code may shorten paths and pass current booleans, but it must not duplicate the row labels or enabled/disabled wording used by rendering.

Editor header title wording lives with editor screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/editor/view.rs" />. Root rendering supplies only whether the editor is creating or editing and, for edit mode, the workspace name.

Settings header title wording lives with settings screen view helpers in <RepoFile path="crates/jackin-console/src/tui/screens/settings/view.rs" />. Root rendering should ask that helper for the screen title rather than carrying settings-screen copy.

Workspace-list header title wording lives with top-level console view helpers in <RepoFile path="crates/jackin-console/src/tui/view.rs" />. Root frame rendering should ask that helper for the workspace screen title rather than carrying workspace-screen copy.
...
Generated-token source-picker label construction lives with the auth-panel component in <RepoFile path="crates/jackin-console/src/tui/components/auth_panel.rs" />. Editor and settings input code decides when token generation starts, but visible source-picker state comes from the component helper.
