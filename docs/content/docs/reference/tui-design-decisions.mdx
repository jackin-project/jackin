...
Console global quit/Q-intercept policy and confirm-state construction now live in <RepoFile path="crates/jackin-console/src/tui/run.rs" />; the root run loop supplies `QuitInterceptState` facts from root stage/modal state.
