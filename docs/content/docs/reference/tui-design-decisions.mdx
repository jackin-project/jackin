...
2. **Components live in `src/console/tui/widgets/`.** Before writing any new TUI state machine or renderer, `grep` the directory. If a widget handles the same visual pattern (drill-down picker, text input box, 2-button choice, yes/no confirm, filter list), extend that widget rather than creating a parallel one.
...
1. Run `grep -r 'struct.*State' src/console/tui/widgets/` and `ls src/console/tui/widgets/`. Read the matches. Is there a widget that already owns the pattern?
2. Run `grep -r 'fn render_' src/console/tui/widgets/` for the render-level equivalent.
3. If yes — extend the existing widget. Add a mode enum, a new bool flag, or a new variant. Do not create a second module.
4. If no — propose the design first; get explicit approval; then implement. The first implementation goes into `src/console/tui/widgets/` so the next caller can find it.
...
| Scope picker | <RepoFile path="src/console/tui/widgets/scope_picker.rs" /> | Two-button horizontal strip: `All roles` / `Specific role` | Any "apply to everything vs apply to one" choice |
| Source picker | <RepoFile path="src/console/tui/widgets/source_picker.rs" /> | Two-button horizontal strip: `Plain text` / `1Password` | Choosing whether a value is literal or an `op://` reference |
...
| Mount destination choice | <RepoFile path="src/console/tui/widgets/mount_dst_choice.rs" /> | Two-button strip for mount destination choices | Mount configuration flows |
...
| GitHub picker | <RepoFile path="src/console/tui/widgets/github_picker.rs" /> | GitHub org/repo drill-down | Any flow needing to select a GitHub repository |
