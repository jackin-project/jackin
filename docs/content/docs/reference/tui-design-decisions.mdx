...
| Scope picker | `crates/jackin-console/src/widgets/scope_picker.rs` today; migrating to `crates/jackin-console/src/tui/components/scope_picker.rs` | Two-button horizontal strip: `All roles` / `Specific role` | Any "apply to everything vs apply to one" choice |
| Source picker | `crates/jackin-console/src/widgets/source_picker.rs` today; migrating to `crates/jackin-console/src/tui/components/source_picker.rs` | Two-button horizontal strip: `Plain text` / `1Password` | Choosing whether a value is literal or an `op://` reference |
