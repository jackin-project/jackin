...
| `--keep-sidecar-container` | With `--sidecar-container`, keep the warmed jackin-owned sidecar container, network, and cert volume after readiness succeeds. A later fresh launch can adopt those ready resources once, then normal instance cleanup owns them. |
...
- with `--sidecar-container`, a `docker:dind` container, network, and cert volume through the same readiness path used by fresh launches, removed before the command returns unless `--keep-sidecar-container` is passed, in which case a later fresh launch may adopt them as one-shot warm resources;
...
The broader instant-launch roadmap still tracks daemon-maintained prewarm surfaces, launch-time adoption of persistent warm runtime resources, and plan-gated background refresh. See [Instant Launch Architecture](/reference/roadmap/instant-launch-architecture/).
