...
If a cached agent binary exists but has lost its executable bit, jackin' repairs that jackin-owned cache file, records `agent_binary_cache_repaired` diagnostics, and keeps prewarm on the local-cache path instead of redownloading the binary.
