...
| `--sidecar` | Pull the `docker:dind` sidecar image if Docker does not already have it locally. This does not create a sidecar container, network, or cert volume. |
...
- with `--sidecar` or `--image`, the `docker:dind` image used to start per-instance sidecars;
...
The broader instant-launch roadmap still tracks plugin bundle artifacts, warmed sidecar containers, and plan-gated background refresh. See [Instant Launch Architecture](/reference/roadmap/instant-launch-architecture/).
