...
| `--keep-sidecar-container` | With `--sidecar-container`, keep the warmed jackin-owned sidecar container, network, and cert volume after readiness succeeds. This is an explicit persistent warm-resource opt-in. |
...
- with `--sidecar-container`, a `docker:dind` container, network, and cert volume through the same readiness path used by fresh launches, removed before the command returns unless `--keep-sidecar-container` is passed;
