...
| `compare` | Compare stage timings, nested timings, and build-context sizes across two or more runs. |
...
`compare` accepts two or more run IDs or direct `.jsonl` paths. It prints each run's observed timeline, event count, cache hit/miss count, then ranks broad stages and nested timings by the slowest duration observed across all supplied runs. It also compares maximum build-context bytes and file counts so cold/warm/restart runs show whether Docker context creation stayed in the foreground path. Use it to prove whether a launch change moved work out of the foreground path instead of only improving one isolated run.
