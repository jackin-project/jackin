...
`compare` accepts two or more run IDs or direct `.jsonl` paths. It prints each run's startup span with a delta and slowdown or speedup ratio against the fastest supplied run by default; pass `--baseline first` when the first argument is the cold or pre-change run and every following run should be compared to that baseline. It also prints observed timeline, event count, cache hit/miss count, then ranks broad stages and nested timings by the slowest duration observed across all supplied runs. It compares the selected launch plan, maximum build-context bytes/file counts, slowest parsed Docker build steps, and the first cache decision per run, so cold/warm/restart runs show whether attach/start/create/build plans, Docker context creation, build-step work, or image rebuilding stayed in the foreground path. Use it to prove whether a launch change moved work out of the foreground path instead of only improving one isolated run.
