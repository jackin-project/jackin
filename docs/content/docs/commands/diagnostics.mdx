...
| `compare` | Compare stage timings, nested timings, build-context sizes, and cache decisions across two or more runs. |
...
`compare` accepts two or more run IDs or direct `.jsonl` paths. It prints each run's observed timeline, event count, cache hit/miss count, then ranks broad stages and nested timings by the slowest duration observed across all supplied runs. It also compares maximum build-context bytes/file counts and the first cache decision per run, so cold/warm/restart runs show whether Docker context creation or image rebuilding stayed in the foreground path. Use it to prove whether a launch change moved work out of the foreground path instead of only improving one isolated run.
