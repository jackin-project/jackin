...
`compare` accepts two or more run IDs or direct `.jsonl` paths. It prints each run's startup span, observed timeline, event count, cache hit/miss count, then ranks broad stages and nested timings by the slowest duration observed across all supplied runs. It also compares the selected launch plan, maximum build-context bytes/file counts, slowest parsed Docker build steps, and the first cache decision per run, so cold/warm/restart runs show whether attach/start/create/build plans, Docker context creation, build-step work, or image rebuilding stayed in the foreground path. Use it to prove whether a launch change moved work out of the foreground path instead of only improving one isolated run.
