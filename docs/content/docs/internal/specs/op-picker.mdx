...
Behavioral invariant contract for the 1Password picker state machine spread across `src/console/tui/op_picker/` (root-console adapter) and <RepoFile path="crates/jackin-console/src/tui/components/op_picker/mod.rs" /> (surface-local render/state helpers).
...
| INV-2 | No secret values in the picker path — `RawOpField` has no `value` field; serde drops it silently | `grep value op_picker/mod.rs`; exhaustive destructure test at `operator_env.rs` |
