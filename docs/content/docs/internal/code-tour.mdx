...
Entry: `main.rs` → `app::run` → `runtime::load_role` → `runtime::launch::load_agent` → `load_agent_with`.

The `load_agent_with` pipeline runs five phases: resolve source + trust, resolve/claim instance, resolve launch inputs, build + prepare state, launch + finalize. See the [launch.rs behavioral spec](/internal/specs/runtime-launch/) for the invariants.
