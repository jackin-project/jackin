...
Per the solo-maintainer staffing model in <RepoFile path="AGENTS.md">AGENTS.md</RepoFile>, there is no second human to notice a `RUSTSEC` advisory, a copyleft license sneaking in through a transitive bump, or a regression in an untested branch. Automated CI checks are the substitute, exactly as multi-agent review substitutes for the missing second reviewer elsewhere. Every tool below installs through `mise` (per the mise-only CI rule in `.github/AGENTS.md` (removed in #956; consolidated into root AGENTS.md)) and, where it gates, folds into the `ci-required` aggregator.
...
The planned CI integration is whole-branch, not diff-only: <RepoFile path=".github/workflows/ci-pr.yml">.github/workflows/ci-pr.yml</RepoFile> would run two separate required jobs (the old `docs.yml` workflow was consolidated into the generated PR workflow before this integration landed):
...
| `miri` | **Adopt (advisory)** | Hygiene job `miri` runs independent pure-crate invocations (`jackin-core`, `jackin-config`, `jackin-manifest`) with per-crate PASS/FAIL summary lines — no `\|\|` fallback masking. Advisory; not a merge gate. |
...
- **Project-consistent: install via `mise`, run the binary directly** — jackin❯ mise-only CI tool-install rule in `.github/AGENTS.md` (removed in #956; consolidated into root AGENTS.md) requires every CI tool to come from <RepoFile path="mise.toml">mise.toml</RepoFile>, like the existing `cargo:cargo-nextest` and `cargo:cargo-zigbuild` entries.

**Recommendation: the mise path**, to honour the project's hard rule and keep one tool-install mechanism. The `cargo-deny-action` is noted as the upstream-canonical alternative, explicitly *not* adopted because it bypasses `mise`. A new `deny` unit in <RepoFile path=".github/workflows/ci-pr.yml">.github/workflows/ci-pr.yml</RepoFile>, alongside the existing per-crate units:

```yaml
  deny:
    # gated by the affected-crate selection in the plan job
...
The `deny`, Codebook spell-check, and (informational) coverage steps slot into CI behind the existing aggregators. Codebook would join <RepoFile path=".github/workflows/ci-pr.yml">.github/workflows/ci-pr.yml</RepoFile> through the required aggregator as `spell-check-docs` and `spell-check-source`. Coverage is non-gating and does not join a required aggregator.
