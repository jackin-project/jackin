...
- `cargo nextest run -p jackin-usage -p jackin-usage-ffi --locked` — exact CI
  command for the `rust-jackin-usage` / `rust-jackin-usage-ffi` units —
  <RepoFile path=".github/workflows/ci-pr.yml" /> (`group-rust` dispatches
  `ci-unit-rust.yml`; per-crate commands in <RepoFile path=".github/ci/project.toml" />)
  (confidence: HIGH)
...
- `cargo xtask desktop bindings-check`
  — desktop-cadence step, <RepoFile path=".github/workflows/desktop-cadence.yml" />
  (via `mise run desktop-ci`); requires
...
- `cargo xtask desktop build --version "$JACKIN_APP_VERSION" --build "$JACKIN_APP_BUILD"`
  ...
  native/dist/JackinDesktop.app [zip]` — desktop-cadence steps
  (`mise run desktop-ci`, via `desktop-merge`/`desktop-scheduled`);
  operator wrappers `mise run desktop-build|desktop-verify|desktop-run|desktop`
  — <RepoFile path="mise.toml" /> (confidence: HIGH)
- The merge cadence also exercises the built app via UI tests
  (`mise run desktop-test-ui`, after `desktop-ci`)
  (confidence: HIGH)
...
- `cargo clippy --workspace --all-targets --all-features --locked -- -D warnings`
  and `cargo fmt` — workspace baseline, `crates/CLAUDE.md` (removed in #956; consolidated into root AGENTS.md) (lint section);
