...
`jackin` is a Matrix-inspired local CLI for spawning isolated Claude Code agents. Each loaded agent runs in its own Docker container with a dedicated Docker-in-Docker sidecar and its own persisted Claude state.
