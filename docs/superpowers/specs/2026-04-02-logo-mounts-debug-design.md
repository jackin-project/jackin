...
Agent repos may include a `logo.txt` file at the repo root. During `jackin load`, after the Matrix intro and before the config table, jackin reads and displays it in `PHOSPHOR_GREEN` with 2-space indent per line.
