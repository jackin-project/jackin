...
1. **Canvas rain** behind everything — faithful port of `src/tui.rs`'s `digital_rain` algorithm (same ASCII char pool, same age-based color gradient from `WHITE` through `PHOSPHOR_DARK`, same 35ms frame rate). Canvas opacity 0.32 with a radial vignette overlay.
...
- Rain algorithm source: `src/tui.rs` (functions `digital_rain`, `age_to_color`, `should_mutate`, constants `RAIN_CHARS`, `PHOSPHOR_GREEN`, `PHOSPHOR_DIM`, `PHOSPHOR_DARK`)
