...
```
docs/src/components/landing/rainEngine.test.ts:14:test('ageToColor returns PHOSPHOR_GREEN for age 3-5', () => {
docs/src/components/landing/rainEngine.ts:10:  if (age <= 5)   return 'rgb(0,255,65)';      // PHOSPHOR_GREEN
docs/src/components/landing/rainEngine.ts:12:  if (age <= 16)  return 'rgb(0,140,30)';      // PHOSPHOR_DIM
docs/src/components/landing/rainEngine.ts:13:  if (age <= 24)  return 'rgb(0,80,18)';       // PHOSPHOR_DARK
```
...
- Line 10: `      // PHOSPHOR_GREEN` → `      // PHOSPHOR_BRIGHT`
- Line 12: `      // PHOSPHOR_DIM`   → `      // PHOSPHOR_MID`
- Line 13: `      // PHOSPHOR_DARK`  → `      // PHOSPHOR_DEEP`
...
// PHOSPHOR_GREEN/DIM/DARK comment labels in rainEngine.ts (lines 10/12/13)
...
Replaces "CLI..." in both Formula/jackin.rb and
...
- Replace `"CLI..."` in both formula `desc` lines with neutral copy.
...
Plus possibly franchise prose in commit bodies (`ae271b0` body mentions "phosphor green").
...
For commit bodies, replace any "phosphor green" / "I am the Architect" prose with neutral equivalents (e.g., "phosphor green").
...
for repo in jackin homebrew-tap jackin-agent-smith jackin-the-architect; do
  ...
  grep -ri --include='*.md' --include='*.toml' --include='*.rb' --include='*.ts' --include='*.tsx' --include='*.rs' \
    -E 'matrix-inspired|i am the architect.*matrix|agent smith personality|PHOSPHOR_GREEN|PHOSPHOR_DIM|PHOSPHOR_DARK' \
