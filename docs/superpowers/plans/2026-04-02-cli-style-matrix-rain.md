...
**Goal:** Add phosphor-themed CLI help styling with rain/jack banner and rewrite digital rain with per-cell age tracking for cinematic visuals.
...
```bash
git add src/cli.rs
git commit -m "feat: add phosphor-themed CLI help styling with rain/jack banner"
```
