...
- [x] Fix `ErrorDialog` spacing in the shared component, not at one caller.
- [x] Update lookbook stories/SVGs for changed shared dialog or panel output.
...
| done | TUI / Error dialog | `Load role failed` had two blank rows before `OK` | Shared `ErrorDialog` owns one content-to-action spacer | `DLG-5`; shared render test; `error-default.svg` |
...
**`CAP-2`** *(deferred: `capsule-scrollback-debug-rerun`)* — Vertical scrollback is non-monotonic / flickers. Confirmed mechanisms: `input_dispatch.rs:363,419` (`scroll_by` on `filled>0`); `compositor.rs:161-162` (`filled=0` for alt-screen); `session.rs:846-850,694` (`feed_pty` → `scroll_to_live` resets offset); `compositor.rs:584` (`append_cursor_state` keyed on `scrollback_offset!=0`). Telemetry first is done: `compositor.rs` emits `pane scroll frame:` through `cdebug!`, logging pane id, focused state, agent/label, `alt_screen`, mouse mode, content rows, actual vs reported scrollback, actual vs reported offset, viewport rows/cols, screen rows/cols, visible start row, thumb `(start,len)`, cursor row/col, and cursor visibility from the same frame that renders panes and appends cursor state. Verified with `cargo nextest run -p jackin-capsule`, `cargo clippy -p jackin-capsule --all-targets --all-features --locked -- -D warnings`, and `cargo fmt --check`.

**Deferred follow-up:** operator must rerun the capsule scrollback flicker repro with `--debug` and provide the run id/log containing `pane scroll frame:` lines. Remaining files/behaviors: `crates/jackin-capsule/src/daemon/input_dispatch.rs` wheel route, `crates/jackin-capsule/src/session.rs` scrollback clamp/reset semantics, `crates/jackin-capsule/src/daemon/compositor.rs` pane scrollbar reporting/rendered slice/cursor gate, and tests in `crates/jackin-capsule/src/daemon/tests.rs`. Behavior change should wait for that evidence. Then normalize the wheel path: decode SGR buttons to typed direction/axis; ignore horizontal wheel for vertical scrollback unless a real horizontal path exists; coalesce wheel events per frame into one signed delta; apply through one shared tail-scroll helper and clamp once; keep slice + thumb + hint + cursor on that one post-clamp offset; do not reset the operator's view on live PTY output unless content is invalidated. Tests: same-direction burst → monotonic top row + offset; interleaved PTY while scrolled → no flicker to tail.

**`CAP-3`** *(done)* — Capsule pane scrollbar thumb geometry already uses `jackin_tui::scroll::tail_vertical_thumb`, the shared tail-scroll helper. The remaining custom part is accepted and documented in `apply_pane_scrollbar`: capsule paints the thumb directly into terminal-cell border cells because the body is `PaneBodyWidget`, not a line-based `render_scrollable_block` surface. This keeps geometry shared while making the PTY-cell paint boundary explicit. Verified with `cargo nextest run -p jackin-capsule`, `cargo clippy -p jackin-capsule --all-targets --all-features --locked -- -D warnings`, and `cargo fmt --check`.
...
**`DLG-5`** — done. Shared `ErrorDialog` now sizes the body slot from `estimated_message_rows`, capped by available dialog space, instead of giving all leftover rows to the message body. The five-slot layout remains leading spacer, message body, one spacer, `OK`, trailing spacer. `render_single_line_message_has_one_blank_row_before_ok` locates the message and `OK` rows and asserts exactly one blank row between them; `docs/public/tui-lookbook/error-default.svg` was regenerated from the shared story. Verified with `cargo fmt --check`, `cargo clippy -p jackin-tui --all-targets --all-features --locked -- -D warnings`, `cargo nextest run -p jackin-tui`, `cargo run -p jackin-tui-lookbook -- docs/public/tui-lookbook`, and `cargo run -p jackin-tui-lookbook -- --check docs/public/tui-lookbook`.
...
**`RMP-5`** *(done)* — Original audit note is stale on this branch. The current capsule path is documented as Ratatui-first, not a broad parallel ANSI renderer: `architecture.mdx` says status bar, pane borders/bodies, dialogs, selection, and scrollbars paint into one Ratatui `Buffer`; ADR-004 records `PaneBodyWidget` / `SocketBackend` as the accepted pane-body boundary. Remaining raw output is explicitly scoped to the attach-tail adapters (cursor visibility/position, OSC title, bottom chrome) plus focused live `GridPatch` updates through `SocketBackend::draw_grid_patch()` to avoid Ratatui diff allocation. Evidence: `cd docs && bun run build` passes; `cargo nextest run -p jackin-capsule frame_plans_keep_diff_tier_reasons_out_of_full_redraws pending_status_change_uses_no_clear_diff_frame unchanged_diff_frame_suppresses_cached_raw_bottom_chrome` passes.

**`RMP-6`** *(done)* — Stale acceptance notes are now reconciled against branch reality: Defect 48 records the formerly red `fmt`, Debug-info test, DCO, docs-link timeout, lookbook drift, and mise-pin failures with command/run evidence; Defect 50 records the verified 58-arm dispatch table; Defect 52 records the remaining live performance proof gaps instead of claiming them complete. `agent_binary.rs` already carries a central resolver exception, `multiplexer_utils.rs` carries the single provider-key dispatch exception, and `agent-runtime-trait.mdx` now mirrors those reasons instead of saying they still need reconciliation. Verified with `cd docs && bun run build`.
