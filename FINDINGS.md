...
`crates/jackin/src/console` is not a small entrypoint shim today. It is the largest remaining part of the host console implementation: 80 Rust files and 34,350 lines, versus 143 Rust files and 56,137 lines in `crates/jackin-console/src`.
...
|---|---:|---:|---|
| `crates/jackin/src/console` total | 80 | 34,350 | Remaining root console implementation |
...
| `tui/` | 65 | 31,604 | Remaining TUI state, input, update, rendering adapters, run loop, tests |
...
Editor tab and mount-row hover target planning now lives in `jackin-console/src/tui/screens/editor/update.rs`; root supplies active tab, modal state, content area, scroll offset, and mounts before mutating the concrete hover target.

Editor Auth-row mouse hit planning now lives in `jackin-console/src/tui/screens/editor/update.rs`; root supplies active tab, modal state, content area, scroll offset, and Auth rows before focusing the concrete row.

Settings tab and Trust-row hover target planning now lives in `jackin-console/src/tui/screens/settings/update.rs`; root supplies modal-open facts, active tab, content area, scroll offset, and row count before mutating the concrete hover target.
...
Settings tab move/select, General selection/toggle, environment role expansion, global-mount/environment/trust selection, Trust row-select/toggle, and settings horizontal-scroll plan application now lives with the shared settings models in `jackin-console/src/tui/screens/settings/model.rs`; root message handling constructs crate-owned plans and delegates mutation to the owner model.
...
Editor auth-kind, tab move/select, field-selection, mount-row selection, General toggles, selected-mount readonly toggles, role expansion, secret mask toggles, and horizontal-scroll plan application now lives on the shared editor state in `jackin-console/src/tui/screens/editor/model.rs`; root message handling constructs plans and delegates mutation to the owner model.
