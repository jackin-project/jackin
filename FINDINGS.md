...
`crates/jackin/src/console` is not a small entrypoint shim today. It is the largest remaining part of the host console implementation: 80 Rust files and 34,362 lines, versus 143 Rust files and 56,020 lines in `crates/jackin-console/src`.
...
|---|---:|---:|---|
| `crates/jackin/src/console` total | 80 | 34,362 | Remaining root console implementation |
...
| `tui/` | 65 | 31,616 | Remaining TUI state, input, update, rendering adapters, run loop, tests |
...
Settings tab move/select, global-mount/environment/trust selection, Trust row-select, and settings horizontal-scroll plan application now lives with the shared settings models in `jackin-console/src/tui/screens/settings/model.rs`; root message handling constructs crate-owned plans and delegates mutation to the owner model.
