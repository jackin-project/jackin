...
`crates/jackin/src/console` is not a small entrypoint shim today. It is the largest remaining part of the host console implementation: 80 Rust files and 34,361 lines, versus 143 Rust files and 56,069 lines in `crates/jackin-console/src`.
...
|---|---:|---:|---|
| `crates/jackin/src/console` total | 80 | 34,361 | Remaining root console implementation |
...
| `tui/` | 65 | 31,615 | Remaining TUI state, input, update, rendering adapters, run loop, tests |
...
Editor auth-kind, tab move/select, field-selection, mount-row selection, and horizontal-scroll plan application now lives on the shared editor state in `jackin-console/src/tui/screens/editor/model.rs`; root message handling constructs plans and delegates mutation to the owner model.
