...
`crates/jackin/src/console` is not a small entrypoint shim today. It is the largest remaining part of the host console implementation: 80 Rust files and 34,414 lines, versus 143 Rust files and 56,640 lines in `crates/jackin-console/src`.
...
|---|---:|---:|---|
| `crates/jackin/src/console` total | 80 | 34,414 | Remaining root console implementation |
...
| `tui/` | 65 | 31,668 | Remaining TUI state, input, update, rendering adapters, run loop, tests |
...
Launch-provider commit dismissal now delegates to the existing inline-picker dismissal plan application in `jackin-console/src/tui/update.rs`; root no longer mutates that picker slot directly on commit.
