...
`crates/jackin/src/console` is not a small entrypoint shim today. It is the largest remaining part of the host console implementation: 80 Rust files and 34,651 lines, versus 143 Rust files and 52,517 lines in `crates/jackin-console/src`.
...
|---|---:|---:|---|
| `crates/jackin/src/console` total | 80 | 34,651 | Remaining root console implementation |
...
| `tui/` | 65 | 31,905 | Remaining TUI state, input, update, rendering adapters, run loop, tests |
...
Mouse-wheel modal scroll target classification now lives in `jackin-console/src/tui/update.rs`; root maps concrete modal variants into the crate-owned scroll-target vocabulary before mutating picker/file-browser state.
