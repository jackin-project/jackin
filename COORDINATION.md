...
| Laris | 2026-06-07 | Split oversized CI `check` job and fix current CI failures | `COORDINATION.md`; `.github/workflows/ci.yml`; `docs/lychee.toml` | Complete and pushed through head `8c915a5b`. Laris replaced the monolithic Rust `check` job with parallel cargo-prefixed jobs: global `cargo check all features`, global `cargo clippy`, `cargo dependency policy`, global `cargo check default`, single full-suite `cargo nextest`, `cargo fuzz`, matrixed `cargo bench build`, plus `cargo audit`, `cargo msrv check`, and static-name `cargo build validator` behind the stable `ci-required` aggregate gate. Every Cargo job restores Rust toolchain, Cargo registry/git/source cache, per-job branch-scoped Cargo `target/` cache, and GitHub-backed `sccache`; compile-heavy stable-toolchain jobs wait for global `cargo check all features` as the warm gate. CI run `27097866798` attempt 2 passed and verified sequential same-branch cache reuse: Rust toolchain, Cargo registry, and branch target caches restored on primary keys; `cargo check all features` and `cargo nextest` showed 100% Rust sccache hit rates with 0 misses; inspected Cargo commands had no `Downloading crates` output. The construct image build is split into `construct E2E image`, uploaded as `construct-trixie-image`, then downloaded/loaded by `cargo nextest`; attempt 2 verified the artifact handoff and image load. Validator skipped title is now static `cargo build validator`, avoiding `${{ matrix.target }}` leakage. Docs run `27095850514` failed `docs-link-check` on a `biggo.com` timeout, so Laris excluded that flaky external URL in lychee config rather than editing Angela-owned roadmap content. |
