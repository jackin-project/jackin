...
| Laris | 2026-06-07 | Split oversized CI `check` job and fix current CI failures | `COORDINATION.md`; `.github/workflows/ci.yml`; `docs/lychee.toml` | Active. Laris replaced the monolithic Rust `check` job with parallel cargo-prefixed jobs: global `cargo check all features`, global `cargo clippy`, `cargo dependency policy`, global `cargo check default`, single full-suite `cargo nextest`, `cargo fuzz`, matrixed `cargo bench build`, plus `cargo audit`, `cargo msrv check`, and `cargo build validator` behind the stable `ci-required` aggregate gate. Every Cargo job restores Cargo registry/git cache; compile-heavy jobs use `mold`, GitHub-backed `sccache`, `sccache --show-stats`, and per-job Cargo `target/` caches so repeat runs can reuse Cargo build artifacts in addition to rustc object cache. Current local edit follows the ChainArgos warm-sccache pattern by making the existing global `cargo check all features` job the warm gate for downstream compile-heavy stable-toolchain lanes (`cargo clippy`, `cargo check default`, `cargo nextest`, `cargo fuzz`, and `cargo bench build`) instead of adding a duplicate warm job. `cargo msrv check` stays independent because it uses a different Rust version. Docs run `27095850514` failed `docs-link-check` on a `biggo.com` timeout, so Laris excluded that flaky external URL in lychee config rather than editing Angela-owned roadmap content. Prior head `762ee0e8` passed all GitHub checks; current local edit is only the warm-sccache scheduling change. |
