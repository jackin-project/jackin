...
| Laris | 2026-06-07 | Split oversized CI `check` job and fix current CI failures | `COORDINATION.md`; `.github/workflows/ci.yml`; `docs/lychee.toml` | Active. Laris replaced the monolithic Rust `check` job with parallel cargo-prefixed jobs: global `cargo check all features`, global `cargo clippy`, `cargo dependency policy`, global `cargo check default`, single full-suite `cargo nextest`, `cargo fuzz`, matrixed `cargo bench build`, plus `cargo audit`, `cargo msrv check`, and `cargo build validator` behind the stable `ci-required` aggregate gate. Every Cargo job restores Cargo registry/git cache; compile-heavy jobs use `mold`, GitHub-backed `sccache`, `sccache --show-stats`, and now per-job Cargo `target/` caches so repeat runs can reuse Cargo build artifacts in addition to rustc object cache. Docs run `27095850514` failed `docs-link-check` on a `biggo.com` timeout, so Laris excluded that flaky external URL in lychee config rather than editing Angela-owned roadmap content. Prior head `6379e85e` passed all GitHub checks; current local edit adds per-job Cargo build-artifact caches for the dependency rebuild/download issue. |
