...
| Laris | 2026-06-07 | Split oversized CI `check` job and fix current CI failures | `COORDINATION.md`; `.github/workflows/ci.yml`; `docs/lychee.toml` | Active. Laris replaced the monolithic Rust `check` job with parallel `fmt`, global `check-all-features`, global `clippy`, `dependency-policy`, global `check-default`, single full-suite `test`, `fuzz`, matrixed `bench-build`, plus existing `audit`, `msrv`, and `build-validator` behind the stable `ci-required` aggregate gate. Every Cargo job restores Cargo registry/git cache; compile-heavy jobs use `mold` and GitHub-backed `sccache`, with `sccache --show-stats` emitted for verification. Docs run `27095850514` failed `docs-link-check` on a `biggo.com` timeout, so Laris excluded that flaky external URL in lychee config rather than editing Angela-owned roadmap content. Prior head `d25d6e54` passed all GitHub checks; current local edit restores `check-default` to one global workspace job per operator request. |
