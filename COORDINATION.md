...
| Laris | 2026-06-07 | Split oversized CI `check` job | `COORDINATION.md`; `.github/workflows/ci.yml` | Implemented locally; pending GitHub CI after push. Laris replaced the monolithic Rust `check` job with parallel `fmt`, package-matrix `check-all-features`, package-matrix `clippy`, `dependency-policy`, package-matrix `check-default`, four-way partitioned `test`, `fuzz`, matrixed `bench-build`, plus existing `audit`, `msrv`, and `build-validator` behind the stable `ci-required` aggregate gate. Compile-heavy jobs use the existing registry cache pattern, `mold`, and GitHub-backed `sccache`; `msrv` now does too. Local validation: workflow YAML parses, `git diff --check` passes, and the package matrix covers all 17 workspace crates. |
