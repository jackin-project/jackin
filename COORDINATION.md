...
| Laris | 2026-06-07 | Split oversized CI `check` job and fix current CI failures | `COORDINATION.md`; `.github/workflows/ci.yml`; `docs/lychee.toml` | Active. Laris replaced the monolithic Rust `check` job with parallel cargo-prefixed jobs: global `cargo check all features`, global `cargo clippy`, `cargo dependency policy`, global `cargo check default`, single full-suite `cargo nextest`, `cargo fuzz`, matrixed `cargo bench build`, plus `cargo audit`, `cargo msrv check`, and static-name `cargo build validator` behind the stable `ci-required` aggregate gate. Every Cargo job restores Cargo registry/git cache; compile-heavy jobs use `mold`, GitHub-backed `sccache`, `sccache --show-stats`, and per-job Cargo `target/` caches so repeat runs can reuse Cargo build artifacts in addition to rustc object cache. Laris made the existing global `cargo check all features` job the warm gate for downstream compile-heavy stable-toolchain lanes (`cargo clippy`, `cargo check default`, `cargo nextest`, `cargo fuzz`, and `cargo bench build`) instead of adding a duplicate warm job; CI run `27096945020` confirmed that ordering and passed. Cargo `target/` cache keys are scoped per branch (`github.head_ref || github.ref_name`) and manifest/toolchain shape, with fallback restore prefixes for the pre-branch cache keys so the branch can seed from already-green runs. Registry caches include `~/.cargo/registry/src` in addition to tarballs, index, and git db. Rust toolchain caching (`~/.rustup/toolchains`, `~/.rustup/update-hashes`) is in every Cargo job so sequential branch runs do not repeatedly download the pinned Rust components; `cargo msrv check` uses an MSRV-version key because it uses a different Rust version. Current local edit splits the construct E2E image build out of `cargo nextest` into a separate `construct E2E image` job that uploads a Docker tar artifact for nextest to load when construct inputs changed, and removes the skipped matrix-expression leak from the validator job title. Docs run `27095850514` failed `docs-link-check` on a `biggo.com` timeout, so Laris excluded that flaky external URL in lychee config rather than editing Angela-owned roadmap content. |
