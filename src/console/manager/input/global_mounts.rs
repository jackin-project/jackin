...
use crate::console::manager::auth_panel::{AuthForm, CredentialInput};
    ...
    match &mut modal {
        ...
        SettingsAuthModal::SourcePicker { state } => {
                ...
                match outcome {
                    ...
                    ModalOutcome::Commit(SourceChoice::Op) => {
                        ...
                        auth.modal = Some(SettingsAuthModal::OpPicker {
                            state: Box::new(
                                crate::console::tui::widgets::op_picker::OpPickerState::new_create_with_cache(
                                    op_cache,
            ...
            match outcome {
                ...
                ModalOutcome::Commit(SourceChoice::Op) => {
                    auth.modal = Some(SettingsAuthModal::OpPicker {
                        state: Box::new(
                            crate::console::tui::widgets::op_picker::OpPickerState::new_with_cache(
                                op_cache,
                            ),
                        ),
                    });
        ...
        SettingsAuthModal::OpPicker { state } => {
            ...
            match outcome {
                ...
                ModalOutcome::Commit(
                    crate::console::tui::widgets::op_picker::OpPickerSelection::NewItem { .. }
                    | crate::console::tui::widgets::op_picker::OpPickerSelection::EditItemField { .. },
                ) => unreachable!("settings-auth browse OpPicker runs in Browse mode"),
                ModalOutcome::Commit(
                    crate::console::tui::widgets::op_picker::OpPickerSelection::Existing(op_ref),
                ) => {
...
fn handle_settings_token_generate_pick(
    auth: &mut super::super::state::SettingsAuthState,
    pending_token_generate: &mut Option<super::super::state::PendingTokenGenerate>,
    outcome: ModalOutcome<crate::console::tui::widgets::op_picker::OpPickerSelection>,
    modal: SettingsAuthModal<'static>,
) {
    use crate::console::tui::widgets::op_picker::OpPickerSelection;
...
fn open_settings_env_picker_modal(
    ...
    settings.env.modal = Some(SettingsEnvModal::OpPicker {
        state: Box::new(
            crate::console::tui::widgets::op_picker::OpPickerState::new_with_cache(op_cache),
        ),
    });
}
