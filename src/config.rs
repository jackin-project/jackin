    ...
    pub fn validate_mounts(mounts: &[(String, MountConfig)]) -> anyhow::Result<Vec<MountConfig>> {
        ...
        for (name, mount) in mounts {
            ...
            if !mount.dst.starts_with('/') {
                anyhow::bail!("mount {:?} destination must be an absolute path: {}", name, mount.dst);
            }
            if !seen_dst.insert(mount.dst.clone()) {
                anyhow::bail!("duplicate mount destination {:?} in mount {:?}", mount.dst, name);
            }
...
"#;
        ...
        assert!(resolved.iter().any(|(_, m)| m.dst == "/home/claude/.gradle/caches"));
        assert!(resolved.iter().any(|(_, m)| m.dst == "/secrets" && m.readonly));
        assert!(resolved.iter().any(|(_, m)| m.dst == "/config" && !m.readonly));
    }
...
"#;
    ...
    fn validate_mounts_rejects_missing_src() {
        let mounts = vec![("test-mount".to_string(), MountConfig {
    ...
    fn validate_mounts_rejects_duplicate_dst() {
        ...
        let mounts = vec![
            ("mount-a".to_string(), MountConfig { src: src.clone(), dst: "/data".to_string(), readonly: false }),
            ("mount-b".to_string(), MountConfig { src, dst: "/data".to_string(), readonly: true }),
        ];
        let err = AppConfig::validate_mounts(&mounts).unwrap_err();
    ...
    fn validate_mounts_expands_tilde_in_src() {
        ...
        let mounts = vec![("home-mount".to_string(), MountConfig {
            src: "~".to_string(),
