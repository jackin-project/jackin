...
async fn purge_container_filesystem(
    ...
    if crate::instance::is_apple_container_instance(&paths.data_dir.join(container_name)) {
        // Purge is the safe path (eject is the force path): refuse while the VM
        // still exists so the state dir survives and the operator ejects first,
        // mirroring the Docker `ensure_role_resources_absent_for_purge` guard.
        // An already-removed VM is fine — purging a torn instance is not blocked.
        super::apple_container::ensure_absent_for_purge(container_name).await?;
    } else {
...
mod tests {
    ...
    async fn purge_container_state_refuses_when_role_container_exists() {
        ...
        let docker = FakeDockerClient {
            inspect_queue: std::cell::RefCell::new(VecDeque::from([
                crate::docker_client::ContainerState::Stopped {
                    exit_code: 0,
                    oom_killed: false,
                },
            ])),
    ...
    async fn prune_images_counts_rmi_in_use_error_as_skipped_not_failed() {
        ...
        let docker = FakeDockerClient {
            ...
            remove_image_queue: std::cell::RefCell::new(VecDeque::from([
                crate::docker_client::RemoveImageOutcome::InUse,
            ])),
            ..Default::default()
        };
    ...
    async fn prune_images_removes_images_not_in_use() {
        let docker = FakeDockerClient {
            ...
            list_containers_queue: std::cell::RefCell::new(VecDeque::from([vec![]])),
            remove_image_queue: std::cell::RefCell::new(VecDeque::from([
                crate::docker_client::RemoveImageOutcome::Removed,
            ])),
            ..Default::default()
        };
    ...
    async fn prune_images_skips_when_image_disappears_between_list_and_rmi() {
        ...
        let docker = FakeDockerClient {
            ...
            list_containers_queue: std::cell::RefCell::new(VecDeque::from([vec![]])),
            remove_image_queue: std::cell::RefCell::new(VecDeque::from([
                crate::docker_client::RemoveImageOutcome::NotFound,
            ])),
            ..Default::default()
        };
