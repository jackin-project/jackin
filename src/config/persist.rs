...
"#,
        ...
        assert!(global.contains(r#"version = "v1alpha3""#), "{global}");
        ...
        assert!(workspace.contains(r#"version = "v1alpha3""#), "{workspace}");
        assert!(
...
"#;
    ...
    fn load_or_init_dual_migrates_legacy_config_with_legacy_workspaces() {
        ...
        // `version = "v1alpha3"` on the global file AND on each split
...
"#,
        ...
        assert_eq!(global_parsed["version"].as_str().unwrap(), "v1alpha3");
        ...
        assert_eq!(prod_parsed["version"].as_str().unwrap(), "v1alpha3");
    ...
    fn load_workspace_files_migrates_legacy_split_file_in_place() {
        ...
        // tests — every other workspace fixture uses `version = "v1alpha3"`.
        let temp = tempdir().unwrap();
        let paths = JackinPaths::for_tests(temp.path());
        paths.ensure_base_dirs().unwrap();
        std::fs::create_dir_all(&paths.workspaces_dir).unwrap();
