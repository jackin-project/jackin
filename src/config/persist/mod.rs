...
//!
//! The atomic write helpers and workspace file-stem validation now live in
//! `jackin_config::persist`; this module re-exports them and owns the
//! `AppConfig`-level load logic that still depends on the binary crate.
...
pub(crate) use jackin_config::{atomic_write, validate_workspace_file_stem};
