...
impl AppleContainerApi for AppleContainerClient {
    async fn run_container(&self, name: &str, spec: &AppleContainerSpec) -> Result<()> {
        ...
        crate::debug_log!("apple-container", "container_run name={name} ok");
        Ok(())
    }
    ...
    async fn stop_container(&self, name: &str) -> Result<()> {
        crate::debug_log!("apple-container", "container_state action=stop name={name}");
    ...
    async fn remove_container(&self, name: &str) -> Result<()> {
        crate::debug_log!("apple-container", "container_state action=rm name={name}");
        let output = tokio::process::Command::new("container")
            .args(["rm", name])
            .output()
            .await?;
