...
//! etc.), and `FakeDockerClient` are re-exported from `jackin-core` so all
//! consumer crates depend on the trait, not the bollard implementation.
//!
...
fn cached_context_endpoint() -> Option<DockerContextEndpoint> {
    ...
    if let Some(cached) = CACHE.get() {
        crate::debug_log!("docker", "context endpoint cache hit host={}", cached.host);
