...
Requirements: Git 2.54+ (config-based hooks; Apple Git 2.54 meets the
floor exactly) and `mise` on Git's runtime PATH — true in terminals with
mise shims, otherwise the hook fails closed with `mise: command not found`
(use a terminal or add the shims directory to the GUI client's PATH). The
first hook run builds `jackin-xtask` (one-time ~1-2 min); later runs reuse
the cache. A global hk pre-commit hook is rejected because it would bypass
the repository-owned snapshot boundary.
...
`git stash push --include-untracked --keep-index`, validates the staged
snapshot only, then restores — byte-identical when green. If restoration
fails, the stash is retained; inspect `git status` / `git stash list` and
recover it before changing the worktree.
...
(skips green on a clean tree). Do not install hk globally for this
repository: a global hk hook is rejected by `mise run hooks-install`
because it does not use the repository-owned snapshot boundary.
