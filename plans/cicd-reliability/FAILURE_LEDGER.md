...
- C4 CLOSED (cargo window) 2026-09-21 by #1024 (1daf30b4, parent + builder/reviewer child slices): same signature recurred on ee1da0c6 main (run 35581113129/job 106274079830: 600.09s silence inside `boltffi pack apple` → watchdog SIGKILL; green baseline 9ee50f6a silent 401-408s in same phase; manual dispatch re-run 35581117100 on identical tree went SUCCESS = transient confirmed, attempt-1 preserved). Forensics: no cargo cache on macOS lane (VELNOR_CACHE_CARGO_* empty) → full desktop-release (thin-LTO) staticlib rebuild every run; boltffi captures cargo output (381-line proof: only 4 summary lines in CI); 1.5× headroom consumed by host variance. Fix: streaming `cargo build --profile desktop-release --target aarch64-apple-darwin -p jackin-usage-ffi` pre-build in `build_xcframework` (desktop.rs:725-737 + FFI_CRATE const) ahead of pack — PR run showed 381 Compiling lines and boltffi phase 401s→14s (fingerprints matched, incremental; proven locally: warm pack #2 = 5.9s vs #1 = 4m7s rebuild, target/aarch64-apple-darwin/ confirms --target). Rejected: dumb heartbeat (masks true hangs), per-unit threshold raise (timeout inflation, §14-forbidden), boltffi -v (efficacy unverifiable, sources unavailable). RESIDUAL (same class, separate slice): pack's bindings-generation window still silent 452s boltffi-internal (1.33× headroom) — needs boltffi-internals research (CLI sources unavailable: crates.io blocked, registry lacks boltffi-cli).
