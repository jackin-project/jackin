...
`1c8b99077a5fe82cbfd19e6b5803a0a2db67d8ba`, audited in the separate worktree
`/private/tmp/jackin-pr1002-evidence` on branch `audit/pr1002-evidence`.
...
Evidence runs at the exact target head:

- `AUDIT-F`: `cargo nextest run -p jackin-config -p jackin-env -p jackin-protocol -p jackin-usage --all-features` — **1055 passed, 1 skipped**, exit 0.
- `AUDIT-R`: `cargo nextest run -p jackin-runtime -p jackin-console -p jackin-capsule -p jackin-instance -p jackin-core -p jackin --all-features` — **3826 passed, 1 skipped, 1 timed out**, exit 100. The timeout was `crates/jackin-console/src/tui/view/png_baselines/tests.rs::png_baselines_screens_match` after 360 seconds; it is recorded as failed, not silently converted to pass.
- Focused follow-up: `cargo nextest run -p jackin-console --all-features -j1 -E 'test(png_baselines_screens_match)'` — **1 passed**, exit 0, 153.035 seconds. This does not erase the failed aggregate `AUDIT-R` gate; it shows the timeout is not reproduced in the isolated test.
...
| F14 | Antigravity pools, CLI JSON, identity, old command, 401, availability-only | `crates/jackin-usage/src/usage/antigravity/tests.rs::summary_pools_map_exact_bucket_ids`, `agy_version_gate_rejects_pre_json`, `availability_only_never_becomes_quota`, `gated_snapshot_reports_unsupported_without_running_usage`; AUDIT-F pass | fixture_verified | unsupported | Headless/old-command limitations are explicit; no GUI account proof. |
...
| Antigravity / Google | fixture_verified | not_run | unsupported | `gated_snapshot_reports_unsupported_without_running_usage` proves the headless/old-command state; GUI/keyring support remains unverified. |
