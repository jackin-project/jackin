...
- Capsule runtime setup now routes captured, null-output, and optional synchronous commands through one `process.command` owner. Isolation Git inspection and the host `ps` keep-awake liveness probe use the same shape. The shared runtime owner now covers every direct runtime subprocess: sync/async completion for `jackin-exec`, notification dispatch, Apple-container commands and parallel Git pulls, plus lifecycle guards for streamed Docker attach-proxy and custom-timeout snapshot children. Raw spawn calls exist only inside the owner; spawn, wait, nonzero, timeout, and I/O failures complete it truthfully, and the unused raw-child Apple attach API is gone. Failure paths no longer retain stderr, program/argument material, PID, repository/container/image names, mounts, environment values, focus IDs, or exit details in error text or generic telemetry. Focused matrices prove normal and child-handle outcomes plus privacy; production Git-pull and liveness exporter tests prove joined-thread context and PID/output privacy. Continue the wider all-crate subprocess inventory and the remaining HTTP, storage, stream, launch-stage, and metric boundaries.
