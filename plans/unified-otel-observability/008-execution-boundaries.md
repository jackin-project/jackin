...
- Capsule runtime setup now routes captured, null-output, and optional synchronous commands through one `process.command` owner. Isolation Git inspection uses the same shape. A shared host owner now covers both `ps` keep-awake liveness and desktop URL/file open/reveal commands, with raw calls confined to the owner and failure text excluding program, arguments, URL/path, PID, output, and exit detail. The shared runtime owner covers every direct runtime subprocess: sync/async completion for `jackin-exec`, notification dispatch, Apple-container commands and parallel Git pulls, plus lifecycle guards for streamed Docker attach-proxy and custom-timeout snapshot children. Raw spawn calls exist only inside the owner; spawn, wait, nonzero, timeout, and I/O failures complete it truthfully, and the unused raw-child Apple attach API is gone. Focused matrices prove normal and child-handle outcomes plus privacy; production Git-pull, liveness, and desktop exporter tests prove joined-thread context and operator-data privacy. Continue the wider all-crate subprocess inventory and the remaining HTTP, storage, stream, launch-stage, and metric boundaries.
