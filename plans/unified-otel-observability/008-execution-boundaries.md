...
- Capsule runtime setup now routes captured, null-output, and optional synchronous commands through one `process.command` owner. Isolation Git inspection uses the same shape. A shared host owner covers both `ps` keep-awake liveness and desktop URL/file open/reveal commands. The shared runtime owner covers every direct runtime subprocess, including lifecycle guards for streamed Docker attach-proxy and custom-timeout snapshot children. A shared image owner now covers GitHub-token lookup plus downloaded agent and Capsule verification with bounded timeouts and explicit closed identities even when executables live at private temporary paths. Raw process APIs are confined to owners in all three crates; spawn, wait, nonzero, timeout, and I/O failures complete truthfully while failure text excludes programs, arguments, URLs/paths, PIDs, output/token material, versions, and exit detail. Focused matrices prove normal and child-handle outcomes plus privacy. Continue the wider all-crate subprocess inventory and the remaining HTTP, storage, stream, launch-stage, and metric boundaries.
