...
- Capsule runtime setup now routes captured, null-output, and optional synchronous commands through one `process.command` owner. Isolation Git inspection uses the same shape. The shared runtime sync/async owner now covers `jackin-exec` credential resolution, host notification dispatch, and every Apple-container exec/list/lifecycle/readiness/version/attach command; the unused raw-child Apple attach API is gone. Spawn failures are sanitized at the owner, and Apple failure paths no longer retain stderr, program/argument material, container/image names, mounts, environment values, focus IDs, or exit details in error text or generic telemetry. A focused matrix proves success, nonzero, spawn, and timeout outcomes plus stdout/stderr/program/argument privacy. Continue migrating Git-pull, snapshot, Docker attach, and host-liveness commands through the runtime owner.
