...
- Capsule runtime setup routes captured, null-output, and optional synchronous commands through one `process.command` owner. A new shared Capsule owner now also covers arbitrary `jackin-exec` commands as `configured_command`, dirty-exit Git run/capture, and firewall tool probes; their spawn/nonzero/timeout outcomes are bounded and command, args, cwd, stderr, and exit detail are absent from error chains and exporter output. Isolation Git inspection uses the same shape. Shared host, runtime, and image owners cover their complete audited inventories, including streamed/custom-timeout children and downloaded temporary binaries with explicit closed identities. Focused matrices prove normal and child-handle outcomes plus privacy. Complete the four remaining Capsule spawned/timeout boundaries, then continue the wider all-crate subprocess inventory and the remaining HTTP, storage, stream, launch-stage, and metric boundaries.
