...
- Attach handshake, detach, focus, single-frame clipboard image, and chunked clipboard transfer operations all use contextual envelopes and bounded registered RPC methods; legacy uncontextualized control frames are rejected as recovered degradation rather than traced. Server response guards travel beside the encoded response and complete only after the socket write. Response enqueue now fails the guard immediately without stranding ownership, and invalid handshake/control correlation creates one local SERVER failure only after extraction rejects it, emits one typed error, performs no product side effect, and covers the rejection write. Exporter-backed tests prove invalid handshake/control shape, no detach side effect, and closed-output delivery failure.
