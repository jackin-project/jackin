...
Current syntax-shaped production invocation census (macro name followed by `(`, excluding `tests.rs` and `jackin-xtask`) after the isolation, instance, host, image-fallback, launch-TUI, usage-collector, oppicker, PTY-session, Capsule daemon/attach/client/clipboard/context/firewall/compositor/context-management/control/file-export/input-dispatch/pane-layout/ports/resource-metrics/session-lifecycle/runtime-setup/process-child, Apple-container/Git-pull/snapshot/attach processes, input-binding/in-container-client, role-source resolution, auth input, global-settings auth, editor role input, manager fallback, runtime attention/cleanup/exit-claim, trust/image-version reuse, PR context, and recovered-degradation migration passes: 27 `telemetry_info!` and 137 `telemetry_debug!` sites. The prior 179 total incorrectly included Rustdoc mentions, a test fixture string, and test-only calls; those are not production invocations. Role-source parsing and resolution now rely on typed return values without exporting raw selectors, role keys, repository URLs, trust state, or raw errors. Expected unavailable auth actions are silent, broken auth/editor invariants emit bodyless typed errors, and selection/cleanup/exit-claim/credential-probe/PR fallback recovery emits bodyless WARN. The generic macro machinery and these 164 production sites remain open; `telemetry_trace!`, `telemetry_warn!`, and `telemetry_error!` invocations are zero.
