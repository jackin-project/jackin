...
- Standard `gen_ai.client.token.usage` is generated from the registry as a `{token}` histogram with required bounded provider, `chat` operation, and input/output token-type dimensions. Changed token-monitor totals record only positive per-poll deltas; separately reported Anthropic/Amp/Kimi cache counts are included in input while Codex's already-inclusive input total is not double-counted. Unknown OpenCode providers are not guessed, and no session/model/account identity is exported. Remaining: governed auth-provision events and privacy negatives, plus a schema-valid complete Capsule `app.crash` event (UUID, session and bounded exception fields) and final flush.
