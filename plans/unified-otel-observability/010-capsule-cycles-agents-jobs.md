...
- Capsule launch config now carries the same resolved closed per-agent auth mode used by host provisioning, validates complete allowlisted `sync|api_key|oauth_token|ignore` values, and injects only the selected bounded mode into runtime setup through `JACKIN_AUTH_MODE`; no credential provenance or value crosses this carrier. Remaining: make materialization mode-aware and emit its outcome, then define the separate Capsule-safe OTLP endpoint/authentication carrier contract. Prove classified OTLP credentials are Capsule-safe, host headers/client keys are never copied, unclassified auth reports a coverage gap, and `network.mode=none` injects nothing and opens no egress.
