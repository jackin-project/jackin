...
- Prewarm scheduling/attempt paths emit count, active, and duration metrics keyed only by `job.type`, outcome, and stable error type. The registry rejects `job.id` as a metric dimension. Connection state is owned by the shared connection operation guard; standard process CPU/memory plus Tokio worker/alive/global-queue instruments use cheap snapshots. A registry-description mismatch previously caused second-line governance to reject the entire metric batch; the memory observable now sources its exact generated unit/description, and host/daemon/Capsule real-OTLP tests prove all runtime/process metric names plus successful traces/logs/metrics delivery.
- Image jobs now preserve one producer/job identity while opening one linked consumer per target; sidecar lock/live-state dedup classifies `outcome=skip`, and failure/error outcomes are typed. Helper and real-OTLP conformance tests prove shared job identity, per-attempt fan-out, links, and the skip/failure/error/timeout/panic/abort matrix. Capsule production-path tests prove quiet passes emit no span while substantive agent-status work emits one autonomous `background.cycle` root with the bounded `agent_status` name and no `job.id`. Remaining Plan 010 work is the integrated workspace/strict-lint gate.
- Standard `gen_ai.client.token.usage` is generated from the registry as a `{token}` histogram with required bounded provider, `chat` operation, and input/output token-type dimensions. Changed token-monitor totals record only positive per-poll deltas; separately reported Anthropic/Amp/Kimi cache counts are included in input while Codex's already-inclusive input total is not double-counted. Unknown OpenCode providers are not guessed, and no session/model/account identity is exported. Host and Capsule auth provisioning now emit generated bounded `auth.provision` outcomes without credential-derived data. Capsule panic handling calls the shared governed `app.crash` emitter while the exporter/session guard is active, then performs emergency exporter shutdown; `crash_event_exports_complete_bounded_private_shape` proves a UUID crash ID, active session ID, `panic` exception type, redacted/capped exception message, and no spurious outcome/error fields.
- Capsule launch config now carries the same resolved closed per-agent auth mode used by host provisioning, validates complete allowlisted `sync|api_key|oauth_token|ignore` values, and injects only the selected bounded mode into runtime setup through `JACKIN_AUTH_MODE`; no credential provenance or value crosses this carrier. Capsule materialization enforces that mode, removes stale file/directory credentials for environment and ignore modes, classifies unavailable material without exposing values or paths, and emits exactly one generated `auth.provision` terminal event at the setup owner; focused tests cover bounded outcomes, exactly-once export, and privacy. Capsule OTLP export also fails closed unless network, endpoint classification, and explicitly Capsule-safe auth all permit it. Production-shape helper tests prove disabled export injects no OTLP/trace context and adds no firewall host, while enabled export uses only the dedicated safe header carrier and never copies client-key or host-only header configuration.
- Capsule Multiplexer focus ownership now routes real pane changes, tab-bar entry/exit, command-palette open/close, and modal transitions through `WidgetFocusTracker`. It exports paired bounded widget focus events and duration metrics using only stable widget IDs; the old synthetic pane-focus action span is removed. An exporter-backed production-path test proves the full transition sequence, zero focus spans, and no role/workspace/runtime identity leakage.
...
- [x] Producer/consumer link + shared `job.id` test passes; cycles carry no `job.id`
- [x] Agent-state event fires only on change; `reported` has no source id
