...
- PTY exit events now classify wait failures as `io_error` and signal/non-zero exits as `process_exit_nonzero` without formatting the underlying error or PTY content. Remaining: propagate operator cancellation into the live exit path and add exporter-backed paired spawn/exit privacy coverage.
