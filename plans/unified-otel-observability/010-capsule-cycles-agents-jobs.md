...
- Standard `gen_ai.client.token.usage` is generated from the registry as a `{token}` histogram with required bounded provider, `chat` operation, and input/output token-type dimensions. Changed token-monitor totals record only positive per-poll deltas; separately reported Anthropic/Amp/Kimi cache counts are included in input while Codex's already-inclusive input total is not double-counted. Unknown OpenCode providers are not guessed, and no session/model/account identity is exported. Host auth provisioning now emits one generated `auth.provision` outcome per attempted agent at the join owner, covering success, skip, unavailable material, I/O failure, and panic with bounded agent/mode/source/outcome/error fields and no body or credential-derived values. Remaining: propagate the bounded auth mode into Capsule materialization and prove its privacy, plus a schema-valid complete Capsule `app.crash` event (UUID, session and bounded exception fields) and final flush.
