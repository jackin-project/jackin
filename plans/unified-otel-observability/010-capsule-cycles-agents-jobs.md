...
- Capsule launch config now carries the same resolved closed per-agent auth mode used by host provisioning, validates complete allowlisted `sync|api_key|oauth_token|ignore` values, and injects only the selected bounded mode into runtime setup through `JACKIN_AUTH_MODE`; no credential provenance or value crosses this carrier. Capsule materialization enforces that mode, removes stale file/directory credentials for environment and ignore modes, classifies unavailable material without exposing values or paths, and emits exactly one generated `auth.provision` terminal event at the setup owner; focused tests cover bounded outcomes, exactly-once export, and privacy. Remaining: define the separate Capsule-safe OTLP endpoint/authentication carrier contract. Prove classified OTLP credentials are Capsule-safe, host headers/client keys are never copied, unclassified auth reports a coverage gap, and `network.mode=none` injects nothing and opens no egress.
