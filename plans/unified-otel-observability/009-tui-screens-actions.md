...
- Host and launch mappings are now compile-time exhaustive: host reducer messages and launch cockpit/build-log actions are explicitly classified as bounded semantic actions or non-semantic mechanics, so new variants cannot silently inherit wildcard fallback. Launch exit/dialog actions retain their root through the immediate render; complete the remaining host transition/effect/render causality and exporter-backed keyboard/mouse equivalence.
