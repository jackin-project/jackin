...
- Host and launch mappings are now compile-time exhaustive: host reducer messages and launch cockpit/build-log actions are explicitly classified as bounded semantic actions or non-semantic mechanics, so new variants cannot silently inherit wildcard fallback. Host transitions and host/launch action-triggered render children retain the bounded action parent. The sliding-window jank monitor and complete privacy-safe crash event with shutdown flush are implemented and tested. Complete exporter-backed keyboard/mouse equivalence and integrated host/launch/Capsule proof.
