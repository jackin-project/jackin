...
| P07 | Same identity across clients, no duplicate quota; exact OpenRouter model persists | S7 re-probe PASS @9c48c928 (product-identical to c2e25bfe; clean worktree /tmp/jackin-p07-clean): identity 1 row + 0 unresolved via real CLI (/tmp/jackin-p07-id/usage.json), 2-key no-profile control 2 rows (/tmp/jackin-p07-neg), caps==1 in usage disc_ 31/31 + broker 56/56; model pins byte-exact CLI write + load/save round-trip (/tmp/jackin-p07-model); plan JSON top-level + per-instance pins byte-exact via real load --dry-run (/tmp/jackin-p07-plan/dry*.json), no-drift sweep runtime dry_run 19/19 + CLI wire load_cmd 11/11 |
