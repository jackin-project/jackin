...
- Migrated consumers: runtime (exec_host, apple container, attach, git pull, snapshot, host_daemon), capsule (exec, exit_assess, firewall, runtime_setup, git/util probes), xtask (`cmd` shim + all call sites), isolation git_inspect, build-meta git SHA, root-crate preflight, pr-trailers, host desktop openers, instance gh/keychain probes, e2e profile_matrix.
