...
The retired roadmap's "OTLP LogRecord wire contract" required every registered event definition to carry: top-level name; default severity; one stable body intent; required and optional attributes with native types; allowed outcomes; privacy class; cardinality class; sink eligibility; error-fingerprint inputs; and the owning crate — with registration failing closed on unknown keys, missing required keys, prohibited spellings, or a body that violates its redaction/size policy. At the time of this plan, none of that existed: event names were derived at runtime with `kind.replace('_', ".")` (a shape the contract explicitly prohibited), outcomes/categories/components were computed by substring sniffing, the failure-grouping key shipped as `error_type` (prohibited spelling — the contract requires `error.type`), the stage dimension shipped as bare `stage` instead of `jackin.stage`, expected shutdown shipped the unregistered outcome `expected_shutdown` instead of `expected_close`, and the typed facade silently discarded every attribute callers passed it. This plan built the registry and fixed the canonical spellings; plans 002–009 validate against it.
