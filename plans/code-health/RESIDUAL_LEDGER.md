...
| R-038-env-console-tail | plan 038 long tail | **DEFER** (partial) | +058–064: doctor, roles, env resolve, console services, ConfigEditor, AppConfig CRUD, isolation list/drift, auth_error traces (`Option<&WorkspaceName>`), token_setup revoke + expiry helpers. **materialize_workspace stays `&str`**: IsolationRecord.workspace dual-semantics — saved config stem **or** ad-hoc path label (`CurrentDir`/`Path` loads use workdir as label; `WorkspaceName::parse` rejects `/`). Remaining: TUI display labels, host CLI status/prewarm/context string params, `TokenSetupScope` still holds `String`, editor `workspace_doc_mut`. | Next: TUI display labels or TokenSetupScope newtype |
...
| R-thiserror-mid-tranches | matrix Phase 2 | **DEFER** (partial) | 037 core+env; **065** instance; **066** isolation; **067** docker; **068** image (`ImageError`). Remaining measured: config ~66 bail/anyhow sites. | Next crate plan: jackin-config |
