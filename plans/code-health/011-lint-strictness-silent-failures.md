...
2. reads `suppression-budget.toml` (create it: header comment + `[[crate]]` entries `{ name = "jackin-console", bare_allow = <measured> }` for every crate with a nonzero count, generated via a `--print-budget` flag mirroring `lint.rs:40-48`);
3. enforces shrink-only semantics exactly like the file-size gate (`lint.rs:220-244`): count above budget → fail naming the crate and the delta; count below budget → fail telling the executor to shrink the row; row for a crate now at zero → fail telling them to delete it. Failure text must state the fix and the rerun command (`cargo xtask lint suppressions --print-budget`), matching the bar set by `test_layout.rs:277-281`.
