...
6. **Duplicate helper families** (bounded heuristic — the roadmap names this section; a real similarity pass is out of scope): group non-test `fn` names across crates by normalized name (strip `_impl`/`try_`/`with_` affixes); report names defined in ≥3 crates with their locations. Noise is acceptable in a report-only lane; a `--min-crates <n>` knob defaults to 3.
7. **Advisory tool output**: one summary line per advisory signal already computable locally — currently the count of `#[expect]`s whose lint no longer fires is NOT locally computable, so this section starts with just the suppression-scanner's "bare vs reasoned" ratio and a placeholder note naming the scheduled lanes (plan 035) whose artifacts later feed it. An honest thin section beats omitting the roadmap-named heading.
...
With `--format json`, emit one JSON object (serde_json, already a workspace dep used by xtask) with keys `largest_production_files`, `largest_test_files`, `untested_large_modules`, `suppressions` (with `by_lint`, `by_crate`, `bare_by_crate`), `pub_surface`, `agent_docs`, `duplicate_helpers`, `advisory`, and `verification_map` (every `cargo metadata` workspace member → its narrowest command, `cargo nextest run -p <crate>` unless an exception row applies — this is the machine-checkable half of roadmap Phase 6 item 2; a member missing from the map is a bug in the generator). No prose on stdout in json mode. This is the first machine-readable xtask lane (the roadmap's "diagnostics are prompts" principle); keep the shape flat and stable.
...
With `--write-baseline`, write `code-health-baseline.toml` at the repo root: a dated header comment (`# Generated by cargo xtask health --write-baseline. Phase 0 baseline; Phase 7's ratchet engine consumes these floors.`) and the aggregate numbers only (totals per budget family: suppression counts per lint per crate, bare counts per crate, doc byte totals per file, pub-item counts per crate, largest-file counts). No per-finding detail. Suite wall time — the fifth roadmap-named baseline family — is CI-side only (nextest junit artifacts, plan 013); write a commented pointer row (`# suite-wall-time: seeded by plan 013 from CI junit artifacts, not locally computable`) so the omission is recorded in the file itself. Run it and commit the generated file.
