...
if [[ "$*" == *"release view"* && "$*" == *"--json assets"* ]]; then
  cat <<'NAMES'
jackin-desktop-1.2.3-aarch64-apple-darwin.zip
jackin-desktop-1.2.3-aarch64-apple-darwin.zip.sha256
jackin-desktop-1.2.3-aarch64-apple-darwin.zip.bundle
jackin-desktop-1.2.3-aarch64-apple-darwin.zip.sbom.json
NAMES
  exit 0
fi
if [[ "$*" == *"release view"* ]]; then
...
if [[ "$url" == *Casks/jackin-desktop.rb* ]]; then
  cat <<'CASK'
  version "1.2.3"
  sha256 "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
  url "https://github.com/jackin-project/jackin/releases/download/v1.2.3/jackin-desktop-1.2.3-aarch64-apple-darwin.zip"
CASK
  exit 0
fi
exit 1
...
#!/usr/bin/env bash
if [[ "$*" == *"release view"* && "$*" == *"--json assets"* ]]; then
  echo 'jackin-desktop-9.9.9-aarch64-apple-darwin.zip'
  exit 0
fi
if [[ "$*" == *"release view"* ]]; then
  exit 0
