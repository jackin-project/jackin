...
3. **The home-mount must be `none` for jackin'.** Default `rw` `$HOME` mapping is the antithesis of explicit-mounts-only. Any machine-based backend must set home-mount `none` and add explicit mounts — **but machines currently document no extra-volume mechanism**, which is the single blocker for machine-as-role-environment today. Until machines grow `-v`, `container run` remains the only shape that satisfies jackin's mount contract. Track upstream.
...
- **OrbStack**: now competing with `container machine` head-on (persistent Linux machines on Mac); without a per-VM-kernel answer it trends toward "best Docker engine + best shared-kernel machines", which keeps it as jackin's *default Docker backend* engine and nothing more.
