...
| `agent/` | AI-agent dispatch (Claude / Codex). `mod.rs` (`Agent` enum + slug parsing), `profile.rs` (per-agent install block + required env, consumed by `derived_image` and `runtime/launch::verify_required_agent_env`) |
