...
- [ ] P1.9 Manual smoke matrix executed (`--debug`) — `BLOCKED(operator)`: needs an interactive Docker-capable host terminal. Recipe: `cargo run --bin jackin -- console --debug`; stream Codex; wheel up 3 pages → view holds still; wheel down to bottom (wheel only) → input box + live footer return; type while scrolled → snap; focus swap while scrolled → no cursor in history. Partial run 2026-06-10 (run id `jk-run-1138ee`): surfaced D17 (codex composer background band missing — fixed in-PR by the OSC 10/11 color report) and confirmed the pane scrollbar correct (`scrollback_actual=0, thumb=None` at startup; thumb appears only once a row is actually evicted, matching the host terminal's own scrollback behavior).
...
| D17 | Agents render with no color theming: codex's composer loses its background band (present on the host terminal, absent in the capsule) | OSC 10/11 default-color queries fell into the unhandled-OSC drop arm — never answered, never forwarded. Codex gates all background SGR on the OSC 11 reply: a PTY probe shows 0 `48;2` sequences when the query goes silent vs 111 when answered. Found in operator smoke of PR 1; fixed by answering OSC 10/11 from grid state seeded with the attach client's palette (queried from the host terminal pre-Hello) | `grid.rs::handle_osc` catch-all |
